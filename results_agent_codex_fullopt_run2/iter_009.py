"""Penalty-only custom Adam step-size sweep.

HYPOTHESIS: The penalty-only custom Adam basin is close to the run best, so
using different initial step sizes across the three starts may cross the last
small gap without adding more restarts.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    solve_low = make_solver(160.0, 4200, 1800, 0.1, 0.2, 2.8, 2.4)
    solve_mid = make_solver(220.0, 4200, 1800, 0.1, 0.2, 3.0, 2.5)
    solve_high = make_solver(280.0, 4200, 1800, 0.1, 0.2, 3.4, 2.8)
    ...
    for init_x, init_y, solver in (
        (starts[0][0], starts[0][1], solve_low),
        (starts[1][0], starts[1][1], solve_mid),
        (starts[2][0], starts[2][1], solve_high),
    ):
