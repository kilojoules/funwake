"""Exact-size BO grid with wind-frame row-band wake-relief finish.

HYPOTHESIS: The current run's surplus-grid/prune basin is plateauing near
5584 GWh. Single and pair perturbation finishes mostly tie the same local
layout, suggesting the remaining opportunity may require coherent movement of
several turbines that define a wake row. Move crosswind and downwind bands in
the dominant wind frame, then reproject and directly score the resulting
layouts.
...
single-turbine sparse finish, and row/band-level wind-frame move ensemble.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    cand_x = jnp.stack([best_x, polish_x, single_x, double_x, row_x, cascade_x])
    cand_y = jnp.stack([best_y, polish_y, single_y, double_y, row_y, cascade_y])
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
