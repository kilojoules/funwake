"""Exact-size BO grid with wake-anchor projected finish.
...
stronger basin. The last affine/ripple finishes stayed in the same local
basin, so this attempt perturbs only the weakest wake-affected turbines before
the projected Adam polish to test asymmetric basin exits.
...
a wake-anchor refinement seeded by the lowest weighted-power turbines.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def turbine_value(x, y):
        p = sim(x, y, ws_amb=ws, wd_amb=wd, ti_amb=None).power()[:, :n_target]
        return jnp.sum(p * weights[:, None], axis=0)

    def wake_anchor_refine(seed_x, seed_y):
        values = turbine_value(seed_x, seed_y)
        worst = jnp.argsort(values)[:10]
        core = jnp.zeros((n_target,)).at[worst].set(1.0)
        ...
        for down, cross, spread in [
            ...
            along = px * ux + py * uy
            across = px * vx + py * vy
            span = jnp.maximum(jnp.std(across), min_spacing)
            taper = 0.65 + 0.35 * jnp.cos(jnp.clip(across / span, -1.0, 1.0) * jnp.pi)
            mask = core * taper
            sx = seed_x + mask * (down * ux + cross * vx + spread * along * vx)
            sy = seed_y + mask * (down * uy + cross * vy + spread * along * vy)
            starts.append(project(sx, sy, 18, 0.012, 1.0012, 0.12))

        x = jnp.stack([s[0] for s in starts])
        y = jnp.stack([s[1] for s in starts])
        lrs = jnp.array([0.55, 0.65, 0.75, 0.85, 0.60, 0.70, 0.80, 0.90])[:, None]
        mx = jnp.zeros_like(x)
        my = jnp.zeros_like(y)
        vx_acc = jnp.zeros_like(x)
        vy_acc = jnp.zeros_like(y)
        ...
        def step(carry, t):
            ...
            px = px - lrs * ax_hat / (jnp.sqrt(bx_hat) + 1e-8)
            py = py - lrs * ay_hat / (jnp.sqrt(by_hat) + 1e-8)
            px, py = project_batch(px, py, 3, 0.010, 1.0010, 0.11)
            cur_scores = jax.vmap(aep_obj)(px, py)
            cur_feas = jax.vmap(feasible_strict)(px, py)
            better = cur_feas & (cur_scores < bs)
            bx_best = jnp.where(better[:, None], px, bx_best)
    ...
    wake_x, wake_y = wake_anchor_refine(polish_x, polish_y)
    cand_x = jnp.stack([best_x, polish_x, wake_x])
    cand_y = jnp.stack([best_y, polish_y, wake_y])
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
