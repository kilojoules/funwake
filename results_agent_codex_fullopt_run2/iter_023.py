"""Genetic surplus-grid search with augmented-Lagrangian pruning.

HYPOTHESIS: Annealing spent too many evaluations in weak isolated basins; a
small genetic_algorithm can preserve high-performing dense lattice traits while
crossover and mutation explore coupled offsets, shear, angle, and spacing.
AXIS: genetic_algorithm crossover and mutation population search over
wind-aligned surplus-grid parameters plus augmented-Lagrangian NAdam refinement.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    spread = np.array([0.050, 0.050, 0.115, 0.175, 0.175, 0.115, 0.060])
    history_pos = []
    history_val = []
    ...
    while len(population) < population_size:
        if len(population) < 12:
            base = anchors[len(population) % len(anchors)]
            population.append(base + rng.normal(0.0, spread * 0.65, size=7))
        else:
            population.append(mean + rng.normal(0.0, spread, size=7))
    ...
    for generation in range(8):
        ...
        mutation_scale = spread * (0.95 - 0.075 * generation)
        mutation_rate = np.array([0.70, 0.70, 0.58, 0.52, 0.52, 0.58, 0.48])
        ...
        while len(next_population) < population_size:
            if rng.random() < 0.18:
                ...
                alpha = rng.uniform(-0.15, 1.15, size=7)
