"""Exact-size BO grid with cascaded single/pair wake-relief finish.
...
found by sparse single-turbine moves, so use the single move as a discrete
basin jump and then apply the pair-coupled wake-relief search from both seeds.
...
a cascaded single-turbine then two-turbine wind-frame move ensemble focused on
likely wake pairs.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    cascade_x, cascade_y = pair_wake_finish(single_x, single_y)
    cand_x = jnp.stack([best_x, polish_x, single_x, pair_x, cascade_x])
    cand_y = jnp.stack([best_y, polish_y, single_y, pair_y, cascade_y])
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
