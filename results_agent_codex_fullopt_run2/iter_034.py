"""CMA surplus-grid search with marginal surplus pruning.

HYPOTHESIS: The incumbent surplus-grid path is strong, but individual-power
pruning can keep turbines whose wake cost exceeds their standalone production;
short marginal-removal pruning on the strongest surplus layouts should choose a
better active subset before the known-good final AL NAdam ensemble.
AXIS: cmaes covariance-adaptation search over surplus-grid parameters plus
marginal surplus pruning and a per-candidate final AL NAdam learning-rate
ensemble.
