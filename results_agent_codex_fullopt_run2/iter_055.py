"""Exact-size BO grid with double-single plus pair wake-relief finish.

HYPOTHESIS: The current run's surplus-grid/prune basin is plateauing near
5584 GWh. The pair-only finish tied the basin but lost the last few hundredths
found by sparse single-turbine moves. Applying that sparse move twice can make
two independent discrete basin jumps before the pair-coupled wake-relief search
tests whether coupled perturbations help.

AXIS: BO-tuned exact-size grid plus staged AL-NAdam halving, with a final
learning-rate ensemble, projection ensemble, strict projected Adam polish, and
a double sparse single-turbine cascade followed by a two-turbine wind-frame
move ensemble focused on likely wake pairs.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def pair_wake_finish(seed_x, seed_y):
        ux = jnp.cos(dominant_wd)
        uy = jnp.sin(dominant_wd)
        vx = -uy
        vy = ux

        ii, jj = jnp.triu_indices(n_target, k=1)
        du = (seed_x[jj] - seed_x[ii]) * ux + (seed_y[jj] - seed_y[ii]) * uy
        dv = (seed_x[jj] - seed_x[ii]) * vx + (seed_y[jj] - seed_y[ii]) * vy
        pair_priority = jnp.exp(-(dv / (0.55 * min_spacing)) ** 2) * jnp.exp(
            -jnp.abs(du) / (2.4 * min_spacing)
        )
        pair_priority = jnp.where(jnp.abs(du) > 0.25 * min_spacing, pair_priority, 0.0)
        top = jnp.argsort(pair_priority)[-64:]
        pair_i = ii[top]
        pair_j = jj[top]
        ...
        idx_i = jnp.repeat(pair_i, base_shifts.shape[0])
        idx_j = jnp.repeat(pair_j, base_shifts.shape[0])
        shifts = jnp.tile(base_shifts, (pair_i.shape[0], 1))

        def transform_pair(idx_a, idx_b, shift):
            du_a, dv_a, du_b, dv_b = shift
            mask_a = (jnp.arange(n_target) == idx_a).astype(seed_x.dtype)
            mask_b = (jnp.arange(n_target) == idx_b).astype(seed_x.dtype)
            return (
                seed_x
                + mask_a * (du_a * ux + dv_a * vx)
                + mask_b * (du_b * ux + dv_b * vx),
                seed_y
                + mask_a * (du_a * uy + dv_a * vy)
                + mask_b * (du_b * uy + dv_b * vy),
            )

        raw_x, raw_y = jax.vmap(transform_pair)(idx_i, idx_j, shifts)
        rep_x, rep_y = project_batch(raw_x, raw_y, 12, 0.008, 1.0008, 0.10)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
        local_feas = jax.vmap(feasible_strict)(local_x, local_y)
    ...
    pair_x, pair_y = pair_wake_finish(polish_x, polish_y)
    cascade_x, cascade_y = pair_wake_finish(double_x, double_y)
    cand_x = jnp.stack([best_x, polish_x, single_x, double_x, pair_x, cascade_x])
    cand_y = jnp.stack([best_y, polish_y, single_y, double_y, pair_y, cascade_y])
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
