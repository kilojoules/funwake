"""CMA surplus-grid search with final AL learning-rate ensemble.

HYPOTHESIS: The incumbent CMA/prune path is strong, but the final basin may be
sensitive to polish aggressiveness; duplicating the best pruned layouts across
several augmented-Lagrangian NAdam learning rates can capture a better feasible
endpoint without changing the reliable upstream search.
AXIS: cmaes covariance-adaptation search over surplus-grid parameters plus
contribution pruning and a per-candidate final AL NAdam learning-rate ensemble.
