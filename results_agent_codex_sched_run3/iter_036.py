"""HYPOTHESIS: Replacing inverse-LR penalty coupling with an explicit
quadratic penalty ramp may preserve the strong warmup/cosine basin search
while avoiding the very large late penalty wall that can over-repair layouts.
AXIS: alpha_quadratic_ramp on alpha0-gated warmup/cosine repair.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    real_tail = jnp.clip((t - 0.86) / 0.14, 0.0, 1.0)
    final = jnp.clip((t - 0.94) / 0.06, 0.0, 1.0)
    alpha = alpha0 * (1.0 + 5.6 * t * t)
    alpha_real = alpha * (2.25 + 2.4 * t + 17.0 * late * late
                          + 165.0 * real_tail * real_tail
                          + 875.0 * final * final)
