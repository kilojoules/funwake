"""HYPOTHESIS: The incumbent train path may be using LR bumps partly as
penalty-relief windows because alpha is inversely coupled to the bumped LR.
Keeping the same mobility pulses but coupling alpha mostly to a smoother LR
envelope should preserve feasibility pressure while still allowing the basin
shift from the validated dual-window schedule.
AXIS: train_scale_constraint_preserving_bump_coupling_with_hard_rowp_dispatch.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    alpha_lr = 0.82 * lr_real + 0.18 * real_base
    alpha_real = 2.879478 * alpha0 * real_init / jnp.maximum(alpha_lr, 1e-10)
