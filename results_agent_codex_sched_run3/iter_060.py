"""HYPOTHESIS: The incumbent LR/alpha envelope is brittle, but the first
moment memory may be adjustable without moving the validated penalty path.
A short low-beta1 entry phase should make early steps more responsive, then
hand back to the proven train envelope and ROWP dispatch.
AXIS: train_scale_short_entry_momentum_reset_on_incumbent_envelope.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    entry = jnp.clip((0.18 - t) / 0.18, 0.0, 1.0)
    entry = entry * entry * (3.0 - 2.0 * entry)
    beta1_real = 0.239994 - 0.105 * entry
