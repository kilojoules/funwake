"""HYPOTHESIS: The exact train branch is strong but may be stuck behind the
timing of its two mobility windows. Keep the same alpha0 routing, but shift
the train-scale mid window earlier and make the late window narrower so the
final constraint phase has more quiet settling time.
AXIS: train_scale_phase_shifted_dual_windows_with_existing_repair_routes.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    real_mid = jnp.exp(-0.5 * ((t - 0.526) / 0.105) ** 2)
    real_late = jnp.exp(-0.5 * ((t - 0.895) / 0.088) ** 2)
    lr_real = jnp.maximum(real_base + 0.438 * real_init * real_mid
                          + 0.145 * real_init * real_late, 1e-10)
