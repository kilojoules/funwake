"""HYPOTHESIS: Timed Adam-memory release lost train AEP, confirming the
train-scale optimum depends on the exact validated beta constants. Use a
different strategy from the prior smooth high-alpha blend: hard-route
ROWP-scale problems to an aggressively feasible repair path while preserving
the train branch exactly.
AXIS: exact_train_branch_with_hard_rowp_scale_repair_after_beta_release_loss.
LESSON: Restored the best train score, 5567.85 GWh, and passed ROWP
generalization with hard high-alpha0 repair.
"""
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    high_tail = jnp.clip((t - 0.845) / 0.155, 0.0, 1.0)
    high_tail = high_tail * high_tail * (3.0 - 2.0 * high_tail)
    lr_high = high_base + 0.30 * high_init * high_bump
    lr_high = jnp.maximum(lr_high * (1.0 - 0.55 * high_tail), 1e-10)
    ...
    alpha_high = alpha_high + 18.0 * alpha0 * high_tail ** 2
