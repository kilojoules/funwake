"""HYPOTHESIS: The best train branch may need a basin change before the
validated mid/late mobility windows rather than a terminal polish. Add a small
early objective kick while leaving the incumbent alpha coupling and ROWP
dispatch unchanged.
AXIS: train_scale_early_basin_kick_before_incumbent_dual_windows.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    real_early = jnp.exp(-0.5 * ((t - 0.285) / 0.075) ** 2)
    real_mid = jnp.exp(-0.5 * ((t - 0.546162) / 0.120347) ** 2)
    real_late = jnp.exp(-0.5 * ((t - 0.923544) / 0.112811) ** 2)
    lr_real = real_base + 0.105 * real_init * real_early
    lr_real = jnp.maximum(lr_real + 0.458205 * real_init * real_mid
