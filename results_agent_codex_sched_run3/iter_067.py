"""HYPOTHESIS: The validated LR path may be using its Gaussian bumps as both
mobility and temporary penalty relief. Keep LR unchanged, but couple train
alpha to a slightly smoother denominator so the bumps are less constraint-soft
without changing the actual step-size path.
AXIS: exact_train_lr_with_partial_bump_decoupled_penalty.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    alpha_denom = jnp.maximum(0.92 * lr_real + 0.08 * real_base, 1e-10)
    alpha_real = 2.879478 * alpha0 * real_init / alpha_denom
