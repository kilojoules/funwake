"""HYPOTHESIS: The broad smooth alpha0 gates in iter_114 blended the train farm
away from the validated dual-window path. Keep the different smooth-gate idea,
but narrow the transition bands so normal train-scale alpha0 values remain on
the proven path while only near-threshold held-out cases are blended.
AXIS: narrow_smooth_alpha0_gated_dual_window
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    low_u = jnp.clip((1.00e-4 - alpha0) / 0.25e-4, 0.0, 1.0)
    high_u = jnp.clip((alpha0 - 5.00e-4) / 1.00e-4, 0.0, 1.0)
