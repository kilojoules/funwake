"""BO-seeded genetic lattice pool with AL-NAdam polishing.

HYPOTHESIS: The previous best basin is robust, but direct centroid and swarm
add-ons did not improve it. A small genetic algorithm over the compact lattice
parameter space can combine high-scoring spacing, phase, shear, and rotation
genes from BO elites while mutation preserves enough diversity to find a nearby
basin before the expensive AL-NAdam stages.

AXIS: Explicit genetic_algorithm crossover plus mutation population over
BO-seeded lattice parameters, staged augmented_lagrangian AL-NAdam halving,
projection ensemble, strict projected Adam polish, single-turbine sparse
finish, row/band-level wind-frame moves, a nesterov_momentum projected finish,
and scipy_basin_hopping affine polish.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    pop_size = 18 if n_target <= 55 else 14
    n_generations = 7 if n_target <= 55 else 5
    elite_count = 6
    key, subkey = jax.random.split(key)
    population = jax.random.uniform(subkey, (pop_size, 7))
    seed_idx = jnp.argsort(y_raw)[-elite_count:][::-1]
    population = population.at[:elite_count].set(x_raw[seed_idx])
    fitness = score_params_batch(population)

    for gen in range(n_generations):
        elite_idx = jnp.argsort(fitness)[-elite_count:][::-1]
        elites = population[elite_idx]
        elite_fit = fitness[elite_idx]
        key, sub1, sub2, sub3, sub4 = jax.random.split(key, 5)
        p1_idx = jax.random.randint(sub1, (pop_size - elite_count,), 0, elite_count)
        p2_idx = jax.random.randint(sub2, (pop_size - elite_count,), 0, elite_count)
        parent1 = elites[p1_idx]
        parent2 = elites[p2_idx]
        blend = jax.random.uniform(sub3, parent1.shape)
        crossover_children = blend * parent1 + (1.0 - blend) * parent2
        mutation_scale = 0.18 * (0.78 ** gen)
        mutation = mutation_scale * jax.random.normal(sub4, crossover_children.shape)
        mutated = jnp.clip(crossover_children + mutation, 0.0, 1.0)
        population = jnp.vstack([elites, mutated])
        fitness = jnp.concatenate([elite_fit, score_params_batch(mutated)])

    x_raw = jnp.vstack([x_raw, population])
    y_raw = jnp.concatenate([y_raw, fitness])
    top_params = x_raw[jnp.argsort(y_raw)[-12:][::-1]]
    ...
    if n_target < 40:
        def quick_project(x, y):
            for _ in range(80):
                ...
                force = jnp.maximum(0.0, min_spacing * 1.01 - dist)
                x = x + jnp.sum(force * dx / dist, axis=1) * 0.30
                y = y + jnp.sum(force * dy / dist, axis=1) * 0.30
            return x, y

        center_x = jnp.mean(boundary[:, 0])
        center_y = jnp.mean(boundary[:, 1])
        manual_params = jnp.array(
            [
                [1.05, 1.28, -dominant_wd, center_x, center_y, 0.00, 1.00],
                [1.10, 1.34, -dominant_wd, center_x, center_y, 0.18, 1.00],
                [1.16, 1.30, jnp.pi / 2.0 - dominant_wd, center_x, center_y, -0.14, 1.04],
                [1.24, 1.24, -dominant_wd + 0.18, center_x, center_y, 0.00, 1.10],
            ]
        )
        small_layouts = [quick_project(*generate_grid(scale_params(p))) for p in top_params[:4]]
        small_layouts.extend([quick_project(*generate_grid(p)) for p in manual_params])
        ...
        def small_feasible(x, y):
            ...
            dy = y[:, None] - y[None, :]
            dist = jnp.sqrt(dx**2 + dy**2 + jnp.eye(n_target) * 1e18)
            return (jnp.max(sdf) <= 1e-3) & (jnp.min(dist) >= min_spacing * 0.999)

        def small_violation(x, y):
            sdf = polygon_sdf(x, y, boundary)
            dx = x[:, None] - x[None, :]
            dy = y[:, None] - y[None, :]
            dist = jnp.sqrt(dx**2 + dy**2 + jnp.eye(n_target) * 1e18)
        ...
        violations = jax.vmap(small_violation)(sx, sy)
        idx = jnp.argmin(jnp.where(feas, vals, 1e6 + violations))
