"""Exact-size BO grid with edge-ring breathing finish.

HYPOTHESIS: The sparse trust-constr path was fast but gave up too much AEP.
The strongest basin still comes from BO-seeded AL-NAdam, while the last
weak-wake fan dilation again tied the 5584.45 GWh plateau. A different escape
should avoid another wind-frame sector warp and instead perturb turbines near
the polygon edge: breathe selected weak/contact edge arcs along the signed
distance normal and tangent, project back, and keep only strict feasible AEP
improvements.
...
nesterov_momentum projected finish followed by boundary-ring breathing and
sector/contact cleanup.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def void_reinsert_finish(seed_x, seed_y):
        ...
        span_x = x_max - x_min
        span_y = y_max - y_min
        u_center = center[0] * ux + center[1] * uy
        v_center = center[0] * vx + center[1] * vy
        u_span = jnp.sqrt(span_x * span_x + span_y * span_y) * 0.62
        v_span = jnp.sqrt(span_x * span_x + span_y * span_y) * 0.62
        uu, vv = jnp.meshgrid(
            jnp.linspace(u_center - u_span, u_center + u_span, 18),
            jnp.linspace(v_center - v_span, v_center + v_span, 18),
        )
        cand_u = uu.ravel()
        cand_v = vv.ravel()
        cand_x = cand_u * ux + cand_v * vx
        cand_y = cand_u * uy + cand_v * vy
        sdf = polygon_sdf(cand_x, cand_y, boundary)
        dx = cand_x[:, None] - seed_x[None, :]
        dy = cand_y[:, None] - seed_y[None, :]
        nearest = jnp.min(jnp.sqrt(dx**2 + dy**2 + 1e-6), axis=1)
        edge_bonus = jnp.minimum(-sdf, min_spacing * 2.0)
        void_score = nearest + 0.15 * edge_bonus
        void_score = jnp.where(sdf < -0.05 * min_spacing, void_score, -1e12)
        site_idx = jnp.argsort(void_score)[-20:]
        site_x = cand_x[site_idx]
        site_y = cand_y[site_idx]

        pair_idx = jnp.repeat(weak, site_x.shape[0])
        pair_site = jnp.tile(jnp.arange(site_x.shape[0]), weak.shape[0])

        def transform_reinsert(idx, site):
            mask = (jnp.arange(n_target) == idx).astype(seed_x.dtype)
            return (
                seed_x + mask * (site_x[site] - seed_x[idx]),
                seed_y + mask * (site_y[site] - seed_y[idx]),
            )

        raw_x, raw_y = jax.vmap(transform_reinsert)(pair_idx, pair_site)
        rep_x, rep_y = project_batch(raw_x, raw_y, 12, 0.010, 1.0010, 0.12)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
        local_feas = jax.vmap(feasible_strict)(local_x, local_y)
    ...
    def lane_phase_finish(seed_x, seed_y):
        ux = jnp.cos(dominant_wd)
        uy = jnp.sin(dominant_wd)
        vx = -uy
        vy = ux
        ...
        v = seed_x * vx + seed_y * vy
        u_min = jnp.min(u)
        u_span = jnp.maximum(jnp.max(u) - u_min, 1.0)
        v_edges = jnp.quantile(v, jnp.linspace(0.0, 1.0, 7))

        lane_specs = []
        for li in range(6):
            lane_specs.append([v_edges[li] - 1.0, v_edges[li + 1] + 1.0])
        lanes = jnp.array(lane_specs)
        phase_specs = min_spacing * jnp.array(
            [
                [0.10, 0.00, 1.0, 0.00, 0.00],
                [-0.10, 0.00, 1.0, 0.55, 0.00],
                [0.16, 0.03, 1.5, 0.15, 0.04],
                [-0.16, -0.03, 1.5, 0.70, -0.04],
                [0.22, 0.06, 2.0, 0.30, 0.08],
                [-0.22, -0.06, 2.0, 0.85, -0.08],
                [0.08, 0.12, 2.5, 0.45, 0.04],
                [-0.08, -0.12, 2.5, 0.95, -0.04],
                [0.26, -0.04, 1.0, 0.25, 0.10],
                [-0.26, 0.04, 1.0, 0.75, -0.10],
            ]
        )
        lane_batch = jnp.repeat(lanes, phase_specs.shape[0], axis=0)
        shifts = jnp.tile(phase_specs, (lanes.shape[0], 1))

        def transform_lane(lane, shift):
            v_lo, v_hi = lane
            amp_u, amp_v, freq_scaled, phase_scaled, tilt_scaled = shift
            freq = freq_scaled / min_spacing
            phase = phase_scaled / min_spacing * 2.0 * jnp.pi
            tilt = tilt_scaled / min_spacing
            mask = ((v >= v_lo) & (v <= v_hi)).astype(seed_x.dtype)
            t = (u - u_min) / u_span
            wave = jnp.sin((freq * jnp.pi) * t + phase)
            cwave = jnp.cos((freq * jnp.pi) * t + phase)
            v_mid = 0.5 * (v_lo + v_hi)
            v_width = jnp.maximum(v_hi - v_lo, 1.0)
            v_rel = (v - v_mid) / v_width
            du = amp_u * (0.45 * wave + 0.55 * jnp.sign(wave)) + tilt * min_spacing * v_rel
            dv = amp_v * (0.45 * cwave + 0.55 * jnp.sign(cwave))
            return (
                seed_x + mask * (du * ux + dv * vx),
                seed_y + mask * (du * uy + dv * vy),
            )

        raw_x, raw_y = jax.vmap(transform_lane)(lane_batch, shifts)
        rep_x, rep_y = project_batch(raw_x, raw_y, 12, 0.010, 1.0010, 0.12)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
    ...
    def weak_wake_fan_finish(seed_x, seed_y):
        power = sim(seed_x, seed_y, ws_amb=ws, wd_amb=wd, ti_amb=None).power()[
            :, :n_target
        ]
        turbine_value = jnp.sum(power * weights[:, None], axis=0)
        low_cut = jnp.quantile(turbine_value, 0.38)
        dx = seed_x[:, None] - seed_x[None, :]
        dy = seed_y[:, None] - seed_y[None, :]
        dist = jnp.sqrt(dx**2 + dy**2 + jnp.eye(n_target) * 1e9)
        contact = jnp.min(dist, axis=1) < min_spacing * 1.045
        ux = jnp.cos(dominant_wd)
        uy = jnp.sin(dominant_wd)
        vx = -uy
        vy = ux
        ...
        sectors = jnp.array(sector_specs)
        fan_specs = min_spacing * jnp.array(
            [
                [0.10, 0.00, 0.10],
                [-0.10, 0.00, 0.10],
                [0.18, 0.00, 0.14],
        ...
        sector_batch = jnp.repeat(sectors, fan_specs.shape[0], axis=0)
        shifts = jnp.tile(fan_specs, (sectors.shape[0], 1))

        def transform_fan(sector, shift):
            u_lo, u_hi, v_lo, v_hi = sector
            du_shift, dv_shift, fan = shift
            in_sector = (u >= u_lo) & (u <= u_hi) & (v >= v_lo) & (v <= v_hi)
            weak = (turbine_value <= low_cut) | contact
            focused = in_sector & weak
            mask_bool = jnp.where(jnp.sum(focused) > 0, focused, in_sector)
            mask = mask_bool.astype(seed_x.dtype)
            v_mid = 0.5 * (v_lo + v_hi)
            v_width = jnp.maximum(v_hi - v_lo, 1.0)
            fan_shift = fan * (v - v_mid) / v_width
            return (
                seed_x + mask * (du_shift * ux + (dv_shift + fan_shift) * vx),
                seed_y + mask * (du_shift * uy + (dv_shift + fan_shift) * vy),
            )

        raw_x, raw_y = jax.vmap(transform_fan)(sector_batch, shifts)
        rep_x, rep_y = project_batch(raw_x, raw_y, 10, 0.010, 1.0010, 0.12)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
    ...
    def edge_ring_breath_finish(seed_x, seed_y):
        power = sim(seed_x, seed_y, ws_amb=ws, wd_amb=wd, ti_amb=None).power()[
            :, :n_target
        ]
        turbine_value = jnp.sum(power * weights[:, None], axis=0)
        low_cut = jnp.quantile(turbine_value, 0.40)
        sdf = polygon_sdf(seed_x, seed_y, boundary)
        edge_cut = jnp.quantile(sdf, 0.58)
        grad_b = jax.vmap(
            jax.grad(
                lambda px, py: polygon_sdf(jnp.array([px]), jnp.array([py]), boundary)[0],
                argnums=(0, 1),
            )
        )(seed_x, seed_y)
        nx = grad_b[0]
        ny = grad_b[1]
        norm = jnp.sqrt(nx * nx + ny * ny + 1e-9)
        nx = nx / norm
        ny = ny / norm
        tx = -ny
        ty = nx
        ...
        dy = seed_y[:, None] - seed_y[None, :]
        dist = jnp.sqrt(dx**2 + dy**2 + jnp.eye(n_target) * 1e9)
        contact = jnp.min(dist, axis=1) < min_spacing * 1.035
        cx = jnp.mean(boundary[:, 0])
        cy = jnp.mean(boundary[:, 1])
        ang = jnp.arctan2(seed_y - cy, seed_x - cx)
        arc_edges = jnp.linspace(-jnp.pi, jnp.pi, 9)
        arc_specs = []
        for ai in range(8):
            arc_specs.append([arc_edges[ai] - 0.02, arc_edges[ai + 1] + 0.02])
        arcs = jnp.array(arc_specs)
        move_specs = min_spacing * jnp.array(
            [
                [-0.18, 0.00],
                [-0.12, 0.10],
        ...
        arc_batch = jnp.repeat(arcs, move_specs.shape[0], axis=0)
        shifts = jnp.tile(move_specs, (arcs.shape[0], 1))

        def transform_arc(arc, shift):
            lo, hi = arc
            dn, dt = shift
            in_arc = (ang >= lo) & (ang <= hi)
            focused = in_arc & ((sdf >= edge_cut) | (turbine_value <= low_cut) | contact)
            mask_bool = jnp.where(jnp.sum(focused) > 0, focused, in_arc)
            mask = mask_bool.astype(seed_x.dtype)
            return seed_x + mask * (dn * nx + dt * tx), seed_y + mask * (dn * ny + dt * ty)

        raw_x, raw_y = jax.vmap(transform_arc)(arc_batch, shifts)
        rep_x, rep_y = project_batch(raw_x, raw_y, 12, 0.010, 1.0010, 0.12)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
    ...
    edge_x, edge_y = edge_ring_breath_finish(nesterov_x, nesterov_y)
    edge2_x, edge2_y = edge_ring_breath_finish(edge_x, edge_y)
    sector_x, sector_y = sector_contact_finish(edge2_x, edge2_y)
    cand_x = jnp.stack(
        [
            best_x,
            polish_x,
