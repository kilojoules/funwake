"""Exact-size BO grid with boundary-ring redistribution finish.

HYPOTHESIS: The sparse trust-constr path was fast but gave up too much AEP.
The strongest basin still comes from BO-seeded AL-NAdam, but individual
turbine, smooth-warp, sector, and reinsert moves all return the same plateau.
A different escape should test boundary-ring redistribution: weak interior
turbines are nudged toward the nearest boundary normal and slight tangential
offsets before projection. That changes the outer/interior density split
instead of another wake-chain or weak-pair permutation.
...
nesterov_momentum projected finish followed by boundary-ring redistribution,
coordinated weak-pair anti-symmetric moves, wind-rose wake-chain decoupling,
and one sector/contact cleanup.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def wake_chain_finish(seed_x, seed_y):
        power = sim(seed_x, seed_y, ws_amb=ws, wd_amb=wd, ti_amb=None).power()[
            :, :n_target
        ]
        turbine_value = jnp.sum(power * weights[:, None], axis=0)
        value_cut = jnp.quantile(turbine_value, 0.58)

        pair_i, pair_j = jnp.triu_indices(n_target, k=1)
        dx = seed_x[pair_i] - seed_x[pair_j]
        dy = seed_y[pair_i] - seed_y[pair_j]
        wd_rad_local = jnp.deg2rad(wd)
        wx = jnp.cos(wd_rad_local)
        wy = jnp.sin(wd_rad_local)
        cx = -wy
        cy = wx
        along = wx[:, None] * dx[None, :] + wy[:, None] * dy[None, :]
        cross = cx[:, None] * dx[None, :] + cy[:, None] * dy[None, :]
        near_wake = jnp.exp(-(cross / (0.70 * min_spacing)) ** 2) * jnp.exp(
            -jnp.abs(along) / (3.6 * min_spacing)
        )
        wind_pressure = (weights * ws**3)[:, None] * near_wake
        pair_score = jnp.sum(wind_pressure, axis=0) / (
            jnp.minimum(turbine_value[pair_i], turbine_value[pair_j]) + 1e-9
        )

        n_pairs_top = min(12, int(n_target * (n_target - 1) // 2))
        top = jnp.argsort(pair_score)[-n_pairs_top:]
        move_idx = jnp.where(
            turbine_value[pair_i[top]] <= turbine_value[pair_j[top]],
            pair_i[top],
            pair_j[top],
        )
        direction_idx = jnp.argmax(wind_pressure[:, top], axis=0)
        move_specs = min_spacing * jnp.array(
            [
                [0.00, 0.11],
                [0.00, -0.11],
        ...
        idxs = jnp.repeat(move_idx, move_specs.shape[0])
        dirs = jnp.repeat(direction_idx, move_specs.shape[0])
        shifts = jnp.tile(move_specs, (n_pairs_top, 1))

        def transform_chain(idx, direction, shift):
            du_shift, dv_shift = shift
            theta = wd_rad_local[direction]
            ux = jnp.cos(theta)
            uy = jnp.sin(theta)
            vx = -uy
            vy = ux
            ...
            anchor_v = v[idx]
            tube = (
                (jnp.abs(v - anchor_v) <= 0.72 * min_spacing)
                & (jnp.abs(u - anchor_u) <= 2.3 * min_spacing)
                & (turbine_value <= value_cut)
            )
            single = jnp.arange(n_target) == idx
            mask_bool = tube | single
            mask = mask_bool.astype(seed_x.dtype)
            return (
                seed_x + mask * (du_shift * ux + dv_shift * vx),
                seed_y + mask * (du_shift * uy + dv_shift * vy),
        ...
        rep_x, rep_y = jax.vmap(transform_chain)(idxs, dirs, shifts)
        rep_x, rep_y = project_batch(rep_x, rep_y, 12, 0.010, 1.0010, 0.11)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
        local_feas = jax.vmap(feasible_strict)(local_x, local_y)
    ...
    def weak_pair_finish(seed_x, seed_y):
        power = sim(seed_x, seed_y, ws_amb=ws, wd_amb=wd, ti_amb=None).power()[
            :, :n_target
        ]
        turbine_value = jnp.sum(power * weights[:, None], axis=0)
        low_cut = jnp.quantile(turbine_value, 0.55)
        sdf = polygon_sdf(seed_x, seed_y, boundary)
        edge_cut = jnp.quantile(sdf, 0.70)
        weak = (turbine_value <= low_cut) | (sdf >= edge_cut)

        pair_i, pair_j = jnp.triu_indices(n_target, k=1)
        dx = seed_x[pair_i] - seed_x[pair_j]
        dy = seed_y[pair_i] - seed_y[pair_j]
        dist = jnp.sqrt(dx**2 + dy**2 + 1e-6)
        weak_pair = weak[pair_i].astype(seed_x.dtype) + weak[pair_j].astype(seed_x.dtype)
        pair_score = (
            5.0 * weak_pair
            + 0.7 * min_spacing / (dist + 0.1 * min_spacing)
            - 0.00015 * (turbine_value[pair_i] + turbine_value[pair_j])
        )

        n_pairs_top = min(16, int(n_target * (n_target - 1) // 2))
        top = jnp.argsort(pair_score)[-n_pairs_top:]
        top_i = pair_i[top]
        top_j = pair_j[top]
        move_specs = min_spacing * jnp.array(
        ...
        idx_i = jnp.repeat(top_i, move_specs.shape[0])
        idx_j = jnp.repeat(top_j, move_specs.shape[0])
        shifts = jnp.tile(move_specs, (n_pairs_top, 1))
        ux = jnp.cos(dominant_wd)
        uy = jnp.sin(dominant_wd)
        vx = -uy
        ...
        def transform_pair(i, j, shift):
            du_shift, dv_shift = shift
            mask_i = (jnp.arange(n_target) == i).astype(seed_x.dtype)
            mask_j = (jnp.arange(n_target) == j).astype(seed_x.dtype)
            low_i = (turbine_value[i] <= turbine_value[j]).astype(seed_x.dtype)
            low_j = 1.0 - low_i
            ix_shift = du_shift * ux + dv_shift * vx
            iy_shift = du_shift * uy + dv_shift * vy
            jx_shift = -0.70 * ix_shift
            jy_shift = -0.70 * iy_shift
            move_x = mask_i * (low_i * ix_shift + low_j * jx_shift)
            move_y = mask_i * (low_i * iy_shift + low_j * jy_shift)
            move_x = move_x + mask_j * (low_j * ix_shift + low_i * jx_shift)
            move_y = move_y + mask_j * (low_j * iy_shift + low_i * jy_shift)
            return seed_x + move_x, seed_y + move_y

        raw_x, raw_y = jax.vmap(transform_pair)(idx_i, idx_j, shifts)
        rep_x, rep_y = project_batch(raw_x, raw_y, 14, 0.010, 1.0010, 0.12)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
        local_feas = jax.vmap(feasible_strict)(local_x, local_y)
    ...
    def boundary_ring_finish(seed_x, seed_y):
        power = sim(seed_x, seed_y, ws_amb=ws, wd_amb=wd, ti_amb=None).power()[
            :, :n_target
        ]
        turbine_value = jnp.sum(power * weights[:, None], axis=0)
        ...
        norm = jnp.sqrt(grad_b[0] ** 2 + grad_b[1] ** 2 + 1e-9)
        nx = grad_b[0] / norm
        ny = grad_b[1] / norm
        tx = -ny
        ty = nx

        interior = jnp.maximum(0.0, -sdf) / (min_spacing + 1e-9)
        low_value = (jnp.max(turbine_value) - turbine_value) / (
            jnp.max(turbine_value) - jnp.min(turbine_value) + 1e-9
        )
        ring_score = 2.5 * low_value + 0.8 * interior
        n_top = min(18, int(n_target))
        top = jnp.argsort(ring_score)[-n_top:]
        move_specs = min_spacing * jnp.array(
            [
                [0.10, 0.00],
                [0.16, 0.00],
        ...
        idxs = jnp.repeat(top, move_specs.shape[0])
        shifts = jnp.tile(move_specs, (n_top, 1))

        def transform_ring(idx, shift):
            normal_shift, tangent_shift = shift
            mask = (jnp.arange(n_target) == idx).astype(seed_x.dtype)
            move_x = normal_shift * nx[idx] + tangent_shift * tx[idx]
            move_y = normal_shift * ny[idx] + tangent_shift * ty[idx]
            return seed_x + mask * move_x, seed_y + mask * move_y

        raw_x, raw_y = jax.vmap(transform_ring)(idxs, shifts)
        rep_x, rep_y = project_batch(raw_x, raw_y, 14, 0.010, 1.0010, 0.12)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
    ...
    ring_x, ring_y = boundary_ring_finish(nesterov_x, nesterov_y)
    pair_x, pair_y = weak_pair_finish(ring_x, ring_y)
    sector_from_pair_x, sector_from_pair_y = sector_contact_finish(pair_x, pair_y)
    pair_sector_x, pair_sector_y = weak_pair_finish(
        sector_from_pair_x, sector_from_pair_y
    )
    ring_sector_x, ring_sector_y = boundary_ring_finish(pair_sector_x, pair_sector_y)
    wake_x, wake_y = wake_chain_finish(pair_x, pair_y)
    sector_x, sector_y = sector_contact_finish(wake_x, wake_y)
    cand_x = jnp.stack(
        [
            best_x,
            polish_x,
