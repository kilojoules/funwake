"""Dual-seed BO grid with wake-chain decoupling finish.
...
A different upstream search should merge the known strong BO seed with an
independent compact BO seed before AL-NAdam, then use the wake-chain finish.
This tests whether the plateau is caused by single-seed basin bias rather than
only by local finishing moves.

AXIS: Dual BO-tuned exact-size grids plus staged AL-NAdam halving, with a final
...
nesterov_momentum projected finish followed by wind-rose wake-chain decoupling
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def run_bo(seed, n_initial, n_rounds, n_candidates):
        key = jax.random.PRNGKey(seed)
        key, subkey = jax.random.split(key)
        x_raw = jax.random.uniform(subkey, (n_initial, 7))
        y_raw = jnp.array([grid_score(scale_params(p)) for p in x_raw])
        for _ in range(n_rounds):
            key, subkey = jax.random.split(key)
            cand = jax.random.uniform(subkey, (n_candidates, 7))
    ...
    x_a, y_a = run_bo(42, 12, 18, 800)
    x_b, y_b = run_bo(2027, 10, 12, 640)
    x_raw = jnp.vstack([x_a, x_b])
    y_raw = jnp.append(y_a, y_b)
    ...
    wake_x, wake_y = wake_chain_finish(nesterov_x, nesterov_y)
