"""Exact-size BO grid with weak-void reinsert finish.
...
lane-phase shear pass again tied the 5584.45 GWh plateau. A different escape
should test sparse topology changes instead of another smooth warp: identify
low-output turbines, sample under-occupied feasible voids across the boundary,
reinsert one weak turbine into each promising void, then project and keep only
strictly feasible AEP improvements.
...
nesterov_momentum projected finish followed by weak-turbine void reinsertion
and sector/contact cleanup.

LESSON: Scored 5584.45 GWh feasible in 129.4s; weak-void reinsertion
generalized feasibly in 115.0s but did not escape the 5584.45 plateau. Note
run_optimizer defaults to 60s locally; use --timeout 180 for these long fullopt
scripts.
"""
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    void_x, void_y = void_reinsert_finish(nesterov_x, nesterov_y)
    sector_x, sector_y = sector_contact_finish(void_x, void_y)
