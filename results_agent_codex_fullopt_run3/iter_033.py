"""Exact-size BO grid with weak-wake fan finish.
...
lane-phase topology warp again tied the 5584.45 GWh plateau. A different
escape should avoid coherent lane waves and instead reshape only the weak or
contact-heavy turbines within wind-frame sectors: push them in small streamwise
steps while fanning them crosswind around each local sector center, then project
back and keep only strict feasible AEP improvements.
...
nesterov_momentum projected finish followed by weak-wake fan dilation and
sector/contact cleanup.

LESSON: Scored 5584.45 GWh feasible in 141.3s; held-out ROWP generalized
feasibly in 135.5s. Weak-wake fan dilation again tied the plateau and did not
beat the existing 5584.45 GWh best.
"""
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    fan_x, fan_y = weak_wake_fan_finish(nesterov_x, nesterov_y)
    fan2_x, fan2_y = weak_wake_fan_finish(fan_x, fan_y)
    sector_x, sector_y = sector_contact_finish(fan2_x, fan2_y)
    cand_x = jnp.stack(
        [
            best_x,
            polish_x,
