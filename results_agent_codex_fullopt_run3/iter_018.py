"""Exact-size BO grid with wake-weighted relocation finish.

HYPOTHESIS: The centroid and basin-hopping variants kept returning the same
5584 GWh basin or degraded it. The remaining gains may come from a few
under-producing turbines rather than another global lattice seed, so target the
lowest weighted-power turbines with larger wind-frame relocations and keep the
best feasible projected layout.
...
nesterov_momentum projected finish followed by wake-weighted coordinate
relocation of low-producing turbines.

LESSON: Scored 5584.45 GWh feasible, tied current best; targeted relocation
did not escape the dominant BO/AL-NAdam basin but generalized feasibly.
"""
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def wake_weighted_relocation_finish(seed_x, seed_y):
        power = sim(seed_x, seed_y, ws_amb=ws, wd_amb=wd, ti_amb=None).power()[
            :, :n_target
        ]
        turbine_value = jnp.sum(power * weights[:, None], axis=0)
        n_sel = min(6, n_target)
        selected = jnp.argsort(turbine_value)[:n_sel]
        ...
        idxs = jnp.repeat(selected, move_specs.shape[0])
        shifts = jnp.tile(move_specs, (n_sel, 1))
        ...
        raw_x, raw_y = jax.vmap(transform_one)(idxs, shifts)
        rep_x, rep_y = project_batch(raw_x, raw_y, 8, 0.010, 1.0010, 0.12)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
        local_feas = jax.vmap(feasible_strict)(local_x, local_y)
    ...
    reloc_x, reloc_y = wake_weighted_relocation_finish(nesterov_x, nesterov_y)
    cand_x = jnp.stack(
        [best_x, polish_x, single_x, double_x, row_x, cascade_x, nesterov_x, reloc_x]
    )
    cand_y = jnp.stack(
        [best_y, polish_y, single_y, double_y, row_y, cascade_y, nesterov_y, reloc_y]
    )
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
