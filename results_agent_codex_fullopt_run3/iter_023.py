"""Exact-size BO grid with directional wake-pair decoupling.

HYPOTHESIS: The sparse trust-constr path was fast but gave up too much AEP.
The strongest basin still comes from BO-seeded AL-NAdam, but the previous
wake-pair escape scored pair risk across all wind cases and then moved each
turbine in only the aggregate dominant wind frame. A different escape should
move each weak turbine crosswind relative to the individual wind direction
that creates that pair's largest wake pressure.
...
nesterov_momentum projected finish followed by per-wind-direction pairwise
wake-decoupling and train-size-only penalized L-BFGS-B projection.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def wake_pair_decouple_finish(seed_x, seed_y):
        power = sim(seed_x, seed_y, ws_amb=ws, wd_amb=wd, ti_amb=None).power()[
            :, :n_target
        ]
        turbine_value = jnp.sum(power * weights[:, None], axis=0)
        ...
        wake_kernel = jnp.exp(-(cross / (0.78 * min_spacing)) ** 2) * jnp.exp(
            -jnp.abs(along) / (4.2 * min_spacing)
        )
        pair_energy = jnp.minimum(turbine_value[pair_i], turbine_value[pair_j])
        wind_pair_pressure = wind_strength[:, None] * wake_kernel
        pair_score = jnp.sum(wind_pair_pressure, axis=0) / (pair_energy + 1e-9)
        ...
        direction_idx = jnp.argmax(wind_pair_pressure[:, top], axis=0)
        move_specs = min_spacing * jnp.array(
            [
                [0.00, 0.10],
                [0.00, -0.10],
        ...
        def transform_pair(idx, shift, direction):
            du_shift, dv_shift = shift
        ...
        raw_x, raw_y = jax.vmap(transform_pair)(idxs, shifts, dirs)
        rep_x, rep_y = project_batch(raw_x, raw_y, 12, 0.010, 1.0010, 0.11)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
        local_feas = jax.vmap(feasible_strict)(local_x, local_y)
    ...
    @jax.jit
    def penalized_coords(coords):
        x = coords[:n_target]
        y = coords[n_target:]
        base = aep_obj(x, y)
        sdf = polygon_sdf(x, y, boundary)
        vb = jnp.maximum(0.0, sdf + 0.006)
        dx = x[:, None] - x[None, :]
        dy = y[:, None] - y[None, :]
        dist = jnp.sqrt(dx**2 + dy**2 + jnp.eye(n_target) * 1e18)
        vs = jnp.maximum(0.0, min_spacing * 1.0006 - dist)
        return base + 0.040 * jnp.sum(vb**2) + 0.035 * jnp.sum(vs**2)

    grad_penalized_coords = jax.jit(jax.grad(penalized_coords))

    def lbfgsb_penalty_polish(seed_x, seed_y):
        if n_target > 55:
            return seed_x, seed_y

        init = np.asarray(jnp.concatenate([seed_x, seed_y]), dtype=np.float64)
        lo = np.r_[np.full(n_target, float(x_min)), np.full(n_target, float(y_min))]
        hi = np.r_[np.full(n_target, float(x_max)), np.full(n_target, float(y_max))]

        def scipy_obj(coords):
            return float(penalized_coords(jnp.asarray(coords)))

        def scipy_grad(coords):
            return np.asarray(
                grad_penalized_coords(jnp.asarray(coords)), dtype=np.float64
            )
        ...
        rx, ry = project(rx, ry, 24, 0.010, 1.0012, 0.12)
        return rx, ry
    ...
    wake_x, wake_y = wake_pair_decouple_finish(nesterov_x, nesterov_y)
    lbfgs_x, lbfgs_y = lbfgsb_penalty_polish(wake_x, wake_y)
    cand_x = jnp.stack(
        [
            best_x,
            polish_x,
