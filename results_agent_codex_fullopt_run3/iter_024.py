"""Exact-size BO grid with low-output turbine reinsertion finish.

HYPOTHESIS: The sparse trust-constr path was fast but gave up too much AEP.
The strongest basin still comes from BO-seeded AL-NAdam, but recent pair,
sector, and L-BFGS finishes keep returning the same 5584.45 GWh layout. A
different escape should temporarily remove the weakest turbines and test
whole-farm feasible reinsertions into open pockets, because this can cross
larger layout permutations than local displacement moves.
...
nesterov_momentum projected finish followed by low-output turbine drop/reinsert
search and one sector/contact cleanup.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def reinsert_finish(seed_x, seed_y):
        power = sim(seed_x, seed_y, ws_amb=ws, wd_amb=wd, ti_amb=None).power()[
            :, :n_target
        ]
        turbine_value = jnp.sum(power * weights[:, None], axis=0)
        low_count = min(8, n_target)
        per_turbine = 22

        span_x = x_max - x_min
        span_y = y_max - y_min
        xs = jnp.linspace(x_min + 0.025 * span_x, x_max - 0.025 * span_x, 27)
        ys = jnp.linspace(y_min + 0.025 * span_y, y_max - 0.025 * span_y, 31)
        gx, gy = jnp.meshgrid(xs, ys)
        px = gx.ravel()
        py = gy.ravel()
        sdf = polygon_sdf(px, py, boundary)
        inside = sdf <= -0.010 * min_spacing
        ...
        cxw = -wy
        cyw = wx
        base_idx = jnp.arange(n_target)
        weak_idxs = jnp.argsort(turbine_value)[:low_count]

        cand_layouts_x = []
        cand_layouts_y = []
        for k in range(low_count):
            drop_idx = weak_idxs[k]
            dx = px[:, None] - seed_x[None, :]
            dy = py[:, None] - seed_y[None, :]
            dist = jnp.sqrt(dx**2 + dy**2 + 1e-6)
            dist = jnp.where(base_idx[None, :] == drop_idx, 1e9, dist)
            min_dist = jnp.min(dist, axis=1)
            feasible_reinsert = inside & (min_dist >= min_spacing * 1.004)

            along = wx[:, None, None] * dx[None, :, :] + wy[:, None, None] * dy[
                None, :, :
            ]
            cross = cxw[:, None, None] * dx[None, :, :] + cyw[:, None, None] * dy[
                None, :, :
            ]
            wake_kernel = jnp.exp(-(cross / (0.82 * min_spacing)) ** 2) * jnp.exp(
                -jnp.abs(along) / (4.4 * min_spacing)
            )
            wake_kernel = jnp.where(
                base_idx[None, None, :] == drop_idx, 0.0, wake_kernel
            )
            wake_risk = jnp.sum(wind_strength[:, None, None] * wake_kernel, axis=(0, 2))

            clear_norm = min_dist / (jnp.max(min_dist) + 1e-9)
            edge_norm = -sdf / (jnp.max(-sdf) + 1e-9)
            risk_norm = wake_risk / (jnp.max(wake_risk) + 1e-9)
            score = clear_norm + 0.20 * edge_norm - 0.75 * risk_norm
            score = jnp.where(feasible_reinsert, score, -1e12)
            top = jnp.argsort(score)[-per_turbine:]

            tx = px[top]
            ty = py[top]
            mask = base_idx[None, :] == drop_idx
            cand_layouts_x.append(jnp.where(mask, tx[:, None], seed_x[None, :]))
            cand_layouts_y.append(jnp.where(mask, ty[:, None], seed_y[None, :]))

        rep_x = jnp.concatenate(cand_layouts_x, axis=0)
        rep_y = jnp.concatenate(cand_layouts_y, axis=0)
        rep_x, rep_y = project_batch(rep_x, rep_y, 8, 0.008, 1.0008, 0.10)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
        local_feas = jax.vmap(feasible_strict)(local_x, local_y)
    ...
    reinsert_x, reinsert_y = reinsert_finish(nesterov_x, nesterov_y)
    sector_x, sector_y = sector_contact_finish(reinsert_x, reinsert_y)
    cand_x = jnp.stack(
        [
            best_x,
            polish_x,
