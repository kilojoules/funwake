"""Exact-size BO grid with basin-hopping affine wake-relief finish.

HYPOTHESIS: The current polished BO-grid basin is strong but may still have
small affine escape routes after row/band and Nesterov finishes. A bounded
scipy_basin_hopping pass around the final layout can test those local basin
escapes without replacing the known competitive refinement path.
