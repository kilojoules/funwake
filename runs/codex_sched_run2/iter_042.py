"""Tiny anti-phase alpha dip on the best noisy backbone.

HYPOTHESIS: The best feasible noisy schedule may be slightly over-penalized
during mid-run basin selection; a very small alpha dip aligned with the LR
noise window could release objective movement, while extra late repair should
preserve feasibility.
AXIS: alpha_anti_phase_dip paired with the current best lr_noise_injection
backbone.
LESSON: pending
"""
import jax
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    alpha = alpha * (1.0 - 0.005 * jnp.exp(-0.5 * ((t - 0.42) / 0.18) ** 2))
