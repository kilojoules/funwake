"""Incumbent LR path with annealed Adam memory.

HYPOTHESIS: The high-floor noisy exponential schedule has the best basin entry,
but a fixed low Adam memory may overreact during the noisy middle window and
under-react during final constraint repair. A modest beta2 lift during basin
selection followed by a late collapse should smooth exploration, then sharpen
the last repair phase without repeating the alpha-bump strategy.
AXIS: adam_high_beta2-to-low anneal on proven noisy exponential envelope.
LESSON: pending
"""
import jax
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    beta1 = 0.11 + 0.035 * noise_gate - 0.065 * late
    beta1 = jnp.maximum(beta1, 0.045)
    beta2 = 0.20 + 0.12 * noise_gate - 0.105 * late
    beta2 = jnp.maximum(beta2, 0.085)
