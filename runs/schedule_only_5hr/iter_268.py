"""Iter 268: Milder asymmetric cosine t^0.85 + 7x alpha + 4x quad.

t^0.85 is closer to linear cosine but still stays high a bit longer.
Less aggressive than t^0.7, might be the sweet spot for feasibility.
"""
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    cosine_lr = lr_min + (lr_init - lr_min) * 0.5 * (1.0 + jnp.cos(jnp.pi * cosine_t ** 0.85))
