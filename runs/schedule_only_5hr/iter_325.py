"""Iter 325: iter_322 with wider feas bursts (0.025 and 0.02).

More sustained feasibility correction periods.
"""
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    feas1 = 0.10 * lr_init * jnp.exp(-0.5 * ((t - 0.84) / 0.025) ** 2)
    feas2 = 0.07 * lr_init * jnp.exp(-0.5 * ((t - 0.92) / 0.02) ** 2)
    lr = lr_base + bump1 + bump2 + feas1 + feas2

    alpha_base = 5.0 * alpha0 * lr_init / jnp.maximum(lr, 1e-10)
    late = jnp.maximum(t - 0.5, 0.0) / 0.5
    alpha_extra = 3.0 * alpha0 * late ** 2
    fb1 = 12.0 * alpha0 * jnp.exp(-0.5 * ((t - 0.84) / 0.025) ** 2)
    fb2 = 22.0 * alpha0 * jnp.exp(-0.5 * ((t - 0.92) / 0.02) ** 2)
