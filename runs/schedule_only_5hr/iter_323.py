"""Iter 323: iter_317 with reduced alpha_base=4.0 to allow more AEP opt.

Slightly less constraint pressure in early/mid optimization.
"""
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    feas1 = 0.12 * lr_init * jnp.exp(-0.5 * ((t - 0.85) / 0.02) ** 2)
    feas2 = 0.08 * lr_init * jnp.exp(-0.5 * ((t - 0.93) / 0.015) ** 2)
    lr = lr_base + bump1 + bump2 + feas1 + feas2

    alpha_base = 4.0 * alpha0 * lr_init / jnp.maximum(lr, 1e-10)
    late = jnp.maximum(t - 0.5, 0.0) / 0.5
    alpha_extra = 3.0 * alpha0 * late ** 2
    fb1 = 18.0 * alpha0 * jnp.exp(-0.5 * ((t - 0.85) / 0.02) ** 2)
    fb2 = 30.0 * alpha0 * jnp.exp(-0.5 * ((t - 0.93) / 0.015) ** 2)
