"""Surplus-grid simulated annealing with augmented-Lagrangian pruning.

HYPOTHESIS: A Metropolis annealing schedule over the compact surplus-grid
parameter manifold can escape the repeated dense-lattice local basin found by
CMA/PSO/BO while still feeding the proven surplus prune and NAdam polish.
AXIS: simulated_annealing search over wind-aligned surplus-grid parameters
plus augmented-Lagrangian NAdam refinement and contribution-based pruning.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    anchors = np.array(
        [
            ...
            [0.055, 0.095, 0.44, 0.47, 0.54, 0.40, 0.34],
            [0.095, 0.055, 0.56, 0.53, 0.46, 0.60, 0.56],
            [0.13, 0.075, 0.50, 0.40, 0.60, 0.50, 0.42],
            [0.06, 0.06, 0.36, 0.55, 0.50, 0.66, 0.62],
            [0.11, 0.12, 0.64, 0.45, 0.48, 0.34, 0.30],
        ],
        dtype=float,
    )
    base_step = np.array([0.055, 0.055, 0.070, 0.115, 0.115, 0.090, 0.060])
    history_pos = []
    history_val = []

    # Three short annealing chains keep runtime close to prior successful runs.
    for chain in range(3):
        current = reflect_unit(anchors[chain] + rng.normal(0.0, 0.018, size=7))
        current_val = eval_unit(current)
        best = current.copy()
        ...
        for step in range(34):
            frac = step / 33.0
            temp = 9.0 * (0.18 / 9.0) ** frac
            scale = base_step * (1.35 - 0.85 * frac)

            if step % 9 == 8:
                proposal = anchors[(chain + step // 9 + 1) % len(anchors)].copy()
                proposal += rng.normal(0.0, scale * 0.7, size=7)
            else:
                proposal = current.copy()
                if rng.random() < 0.42:
                    dims = np.array([0, 1, 2, 5, 6])
                elif rng.random() < 0.72:
                    dims = np.array([2, 3, 4, 5])
                else:
                    dims = np.arange(7)
                proposal[dims] += rng.normal(0.0, scale[dims], size=len(dims))

            proposal = reflect_unit(proposal)
            proposal_val = eval_unit(proposal)
            delta = proposal_val - current_val
            accept = delta >= 0.0 or rng.random() < np.exp(delta / max(temp, 1e-6))
            if accept:
                current = proposal
            ...
            history_pos.append(proposal.copy())
            history_val.append(proposal_val)

        # A small quench around each chain's best point improves exploitative
        # coverage without turning this into another CMA-style population search.
        for scale_factor in [0.36, 0.24, 0.16, 0.10]:
            for _ in range(4):
                proposal = reflect_unit(
                    best + rng.normal(0.0, base_step * scale_factor, size=7)
                )
                proposal_val = eval_unit(proposal)
                if proposal_val > best_val:
                    best = proposal.copy()
    ...
    for anchor in anchors[3:]:
        value = eval_unit(anchor)
        history_pos.append(anchor.copy())
