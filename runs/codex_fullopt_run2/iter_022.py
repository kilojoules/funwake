"""Dual-annealed surplus grid with augmented-Lagrangian pruning.

HYPOTHESIS: The hand-rolled annealer was too diffuse; SciPy dual_annealing
should spend more evaluations near useful dense-lattice basins while retaining
temperature-driven escapes before surplus pruning and NAdam polishing.
AXIS: scipy dual_annealing simulated_annealing over wind-aligned surplus-grid
parameters plus augmented-Lagrangian NAdam refinement.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    jitter = np.array([0.035, 0.035, 0.055, 0.095, 0.095, 0.070, 0.050])
    history_pos = []
    history_val = []

    def objective_unit(z):
    ...
    for anchor in anchors:
        objective_unit(anchor)
    for i in range(18):
        objective_unit(anchors[i % len(anchors)] + rng.normal(0.0, jitter, size=7))

    bounds_unit = [(0.0, 1.0)] * 7
    for i, x0 in enumerate([anchors[0], anchors[1], anchors[2]]):
        dual_annealing(
            objective_unit,
            bounds_unit,
            maxiter=26,
