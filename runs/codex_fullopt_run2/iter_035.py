"""CMA surplus-grid search with surplus single-swap subset repair.

HYPOTHESIS: The best surplus-grid layouts may only need one or two active
membership changes after contribution pruning; scoring focused inactive-for-
active swaps should find subsets greedy removal misses without spending the
whole budget on combinatorics.
AXIS: cmaes covariance-adaptation search over surplus-grid parameters plus
bounded single-swap subset repair before the known-good final AL NAdam
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def prune_swap_variants(x, y, n_keep):
        r = sim(x, y, ws_amb=ws, wd_amb=wd, ti_amb=None)
        p = r.power()
        indiv_aep = jnp.sum(p * weights[:, None], axis=0)
        by_indiv = np.array(jnp.argsort(indiv_aep))
        base_idx = by_indiv[-n_target:].astype(np.int32)
        inactive_idx = by_indiv[: n_total - n_target].astype(np.int32)
        active_worst = base_idx[np.argsort(np.array(indiv_aep[base_idx]))[:12]]
        inactive_best = inactive_idx[np.argsort(np.array(indiv_aep[inactive_idx]))[::-1]]

        candidates = [base_idx]
        for drop_idx in active_worst:
            loc = int(np.where(base_idx == drop_idx)[0][0])
            for add_idx in inactive_best:
                cand = base_idx.copy()
                cand[loc] = add_idx
                candidates.append(cand)

        cand_idx = jnp.asarray(np.array(candidates, dtype=np.int32))
        cand_x = x[cand_idx]
        cand_y = y[cand_idx]
        scores = jax.vmap(aep_obj_any)(cand_x, cand_y)
        chosen = cand_idx[jnp.argsort(scores)[:n_keep]]
        return [(x[idx], y[idx]) for idx in chosen]

    pruned = []
    for i in range(4):
        pruned.extend(prune_swap_variants(cur_x[i], cur_y[i], 2))
