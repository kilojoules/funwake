"""Broad skopt-style surplus-grid Bayesian search with NAdam pruning.

HYPOTHESIS: Optimizing a small surplus layout before pruning lets the search
select a better active subset than fixed-n_target lattice restarts. Unlike the
failed narrow trust-region BO, a broad skopt-like GP expected-improvement scan
should preserve exploration of separated lattice basins before polishing.
AXIS: skopt BayesianOptimization-style broad GP search over surplus-grid
parameters plus feasibility-multiplier NAdam refinement with surplus pruning.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    x_raw = jnp.vstack([anchors, jax.random.uniform(subkey, (n_init, 7))])
    ...
    for i in range(20):
        key, subkey = jax.random.split(key)
        global_cand = jax.random.uniform(subkey, (720, 7))
        best = x_raw[jnp.argmax(y_val)]
        key, subkey = jax.random.split(key)
        local_noise = jax.random.normal(subkey, (360, 7))
        local_scale = jnp.array([0.09, 0.09, 0.18, 0.22, 0.22, 0.18, 0.09])
        local_radius = jnp.maximum(0.05, 0.22 * (0.94**i))
        local_cand = jnp.clip(best + local_noise * local_scale * local_radius / 0.22, 0.0, 1.0)
        x_cand = jnp.vstack([global_cand, local_cand, anchors])
        mu, std = gp_predict(x_cand, x_raw, y_val, length=jnp.maximum(0.14, 0.31 * (0.98**i)))
