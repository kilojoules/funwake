"""Exact-size BO grid with row-band wind-frame finish.
...
stronger basin. The latest wake-anchor pass did not beat the local basin; a
middle-scale row-band shift can move coherent wake rows without disturbing the
whole farm or only one turbine at a time.
...
a direct scored row-band shift ensemble in the dominant wind frame.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def row_band_finish(seed_x, seed_y):
        ...
        def transform_one(spec):
            center, du_shift, dv_shift, bend = spec
            cx = jnp.mean(seed_x)
            cy = jnp.mean(seed_y)
            px = seed_x - cx
            py = seed_y - cy
            ...
            norm_cross = cross / span
            mask = jnp.exp(-0.5 * ((norm_cross - center) / 0.22) ** 2)
            mask = jnp.where(jnp.abs(norm_cross - center) < 0.62, mask, 0.0)
            down = down + mask * (du_shift + bend * cross)
            cross = cross + mask * dv_shift
            return (
                cx + down * ux + cross * vx,
                cy + down * uy + cross * vy,
            )
        ...
        rep_x, rep_y = project_batch(raw_x, raw_y, 14, 0.009, 1.0009, 0.10)
    ...
    cand_x = jnp.stack([best_x, polish_x, row_x])
    cand_y = jnp.stack([best_y, polish_y, row_y])
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
