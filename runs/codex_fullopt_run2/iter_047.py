"""Exact-size BO grid with wind-ripple seeded projected Adam.
...
stronger basin. The projected-AEP-gradient finish improved the local basin,
but it only seeds from global affine variants. Adding a few dominant-wind
ripple/shear starts before the gradient polish may escape row wake alignments
that pure affine starts leave untouched.

AXIS: BO-tuned exact-size grid plus staged AL-NAdam halving, with a final
learning-rate ensemble, projection ensemble, and strict projected Adam polish
seeded by both affine and wind-frame ripple starts.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def projected_adam_polish(seed_x, seed_y):
        ...
        def wind_ripple_transform(spec):
            du_shift, dv_shift, down_scale, cross_scale, shear, ripple, phase = spec
            cx = jnp.mean(seed_x)
            cy = jnp.mean(seed_y)
            px = seed_x - cx
            py = seed_y - cy
            ...
            return (
                cx + down * ux + cross * vx + du_shift * ux + dv_shift * vx,
                cy + down * uy + cross * vy + du_shift * uy + dv_shift * vy,
            )

        for spec in [
            (18.0, 0.0, 1.002, 0.999, 0.000, 10.0, 0.0),
            (-18.0, 0.0, 0.998, 1.001, 0.000, -10.0, 0.0),
            (0.0, 18.0, 1.000, 1.000, 0.004, 14.0, jnp.pi / 2.0),
            (0.0, -18.0, 1.000, 1.000, -0.004, -14.0, jnp.pi / 2.0),
        ]:
            sx, sy = wind_ripple_transform(spec)
            starts.append(project(sx, sy, 14, 0.010, 1.0010, 0.11))

        x = jnp.stack([s[0] for s in starts])
        y = jnp.stack([s[1] for s in starts])
        lrs = jnp.array(
            [0.8, 1.0, 1.2, 1.5, 1.8, 0.9, 1.1, 1.4, 0.95, 1.25, 1.55, 1.75]
        )[:, None]
        mx = jnp.zeros_like(x)
        my = jnp.zeros_like(y)
        vx_acc = jnp.zeros_like(x)
    ...
    cand_x = jnp.stack([best_x, polish_x])
    cand_y = jnp.stack([best_y, polish_y])
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
