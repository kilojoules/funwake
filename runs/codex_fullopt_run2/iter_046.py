"""Exact-size BO grid with wind-frame affine/ripple finish.

HYPOTHESIS: The current run's surplus-grid/prune basin is plateauing near
5577 GWh, and iter_038 showed that a direct exact-size BO lattice can find a
stronger basin. The last projected-AEP-gradient finish is a local optimizer;
a deterministic wind-frame post-pass can test wake-alignment relief moves that
act on rows and the whole farm instead of following the local gradient.
...
a final feasible selection over wind-frame shift/stretch/shear/ripple repairs.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def wind_frame_finish(seed_x, seed_y):
        ux = jnp.cos(dominant_wd)
        uy = jnp.sin(dominant_wd)
        vx = -uy
        vy = ux

        def transform_one(spec):
            du_shift, dv_shift, rot, down_scale, cross_scale, shear, ripple, phase = spec
            cx = jnp.mean(seed_x)
            cy = jnp.mean(seed_y)
            px = seed_x - cx
            py = seed_y - cy
            down = px * ux + py * uy
            cross = px * vx + py * vy
            span = jnp.maximum(jnp.std(cross), min_spacing)
            down = down * down_scale + shear * cross
            down = down + ripple * jnp.sin((cross / span) * jnp.pi * 2.0 + phase)
            cross = cross * cross_scale
            tx = down * ux + cross * vx
            ty = down * uy + cross * vy
            rx = tx * jnp.cos(rot) - ty * jnp.sin(rot)
            ry = tx * jnp.sin(rot) + ty * jnp.cos(rot)
            return (
                rx + cx + du_shift * ux + dv_shift * vx,
                ry + cy + du_shift * uy + dv_shift * vy,
            )

        z = 0.0
        specs = jnp.array(
            [
                [z, z, z, 1.000, 1.000, z, z, z],
                [25.0, z, z, 1.000, 1.000, z, z, z],
                [-25.0, z, z, 1.000, 1.000, z, z, z],
                [z, 25.0, z, 1.000, 1.000, z, z, z],
                [z, -25.0, z, 1.000, 1.000, z, z, z],
                [z, z, 0.005, 1.000, 1.000, z, z, z],
                [z, z, -0.005, 1.000, 1.000, z, z, z],
                [z, z, z, 1.002, 0.999, z, z, z],
                [z, z, z, 0.998, 1.001, z, z, z],
                [z, z, z, 1.004, 0.998, z, z, z],
                [z, z, z, 0.996, 1.002, z, z, z],
                [z, z, z, 1.000, 1.000, 0.004, z, z],
                [z, z, z, 1.000, 1.000, -0.004, z, z],
                [z, z, z, 1.000, 1.000, z, 10.0, z],
                [z, z, z, 1.000, 1.000, z, -10.0, z],
                [z, z, z, 1.000, 1.000, z, 14.0, jnp.pi / 2.0],
                [z, z, z, 1.000, 1.000, z, -14.0, jnp.pi / 2.0],
            ]
        )
        raw_x, raw_y = jax.vmap(transform_one)(specs)
    ...
    wind_x, wind_y = wind_frame_finish(polish_x, polish_y)
    cand_x = jnp.stack([best_x, polish_x, wind_x])
    cand_y = jnp.stack([best_y, polish_y, wind_y])
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
