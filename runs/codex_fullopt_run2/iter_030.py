"""CMA surplus-grid search with wake-diversity pruning.

HYPOTHESIS: The best CMA basin may be losing value at the discrete prune step;
selecting active turbines with a wake-diversity penalty can preserve a subset
that polishes better than contribution-only pruning.
AXIS: cmaes covariance-adaptation search over surplus-grid parameters plus
wake_exposure subset pruning before augmented-Lagrangian NAdam refinement.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def prune_variants(x, y):
        r = sim(x, y, ws_amb=ws, wd_amb=wd, ti_amb=None)
        p = r.power()
        indiv_aep = jnp.sum(p * weights[:, None], axis=0)
        indiv_z = (indiv_aep - jnp.mean(indiv_aep)) / (jnp.std(indiv_aep) + 1e-9)

        wind_x = jnp.cos(dominant_wd)
        wind_y = jnp.sin(dominant_wd)
        along = x * wind_x + y * wind_y
        cross = -x * wind_y + y * wind_x
        da = along[:, None] - along[None, :]
        dc = cross[:, None] - cross[None, :]
        dist = jnp.sqrt((x[:, None] - x[None, :]) ** 2 + (y[:, None] - y[None, :]) ** 2 + 1e-6)
        mask = 1.0 - jnp.eye(n_total)

        wake_shadow = jnp.exp(-(dc / (1.65 * min_spacing)) ** 2)
        wake_shadow = wake_shadow * jnp.exp(-jnp.maximum(0.0, da) / (5.0 * min_spacing))
        wake_shadow = jnp.sum(mask * wake_shadow, axis=1)
        wake_z = (wake_shadow - jnp.mean(wake_shadow)) / (jnp.std(wake_shadow) + 1e-9)

        crowd = jnp.sum(mask * jnp.exp(-(dist / (1.45 * min_spacing)) ** 2), axis=1)
        crowd_z = (crowd - jnp.mean(crowd)) / (jnp.std(crowd) + 1e-9)

        variants = []
        for wake_w, crowd_w in [
            (0.0, 0.0),
            (0.035, 0.000),
            (0.070, 0.000),
            (0.050, 0.025),
            ...
            score = indiv_z - wake_w * wake_z - crowd_w * crowd_z
            idx = jnp.argsort(score)[-n_target:]
            variants.append((x[idx], y[idx]))
        return variants

    pruned = []
    for i in range(len(cur_x)):
        pruned.extend(prune_variants(cur_x[i], cur_y[i]))
