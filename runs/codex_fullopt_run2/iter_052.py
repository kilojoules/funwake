"""Exact-size BO grid with single-turbine wake-relief finish.
...
stronger basin. The row-band pass tied the incumbent, so this probes sparse
one-turbine perturbations in the wind frame to cross active spacing/boundary
nonsmoothness that projected Adam may not cross.
...
a direct scored single-turbine wind-frame move ensemble.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    cand_x = jnp.stack([best_x, polish_x, single_x])
    cand_y = jnp.stack([best_y, polish_y, single_y])
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
