"""Exact-size BO grid with final coordinate-nudge search.
...
stronger basin. Projection and affine repairs operate globally; a strictly
feasible one-turbine nudge search can exploit local wake gaps that the smooth
penalty optimizer leaves after repair.

AXIS: BO-tuned exact-size grid plus staged AL-NAdam halving, with a final
learning-rate ensemble, projection ensemble, and AEP-scored coordinate pattern
search over single-turbine moves.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def nudge_search(x, y, step):
        angles = dominant_wd + jnp.array(
            [
                0.0,
                0.5 * jnp.pi,
                jnp.pi,
        ...
        move_x = step * jnp.cos(angles)
        move_y = step * jnp.sin(angles)
        dirs = len(angles)
        total = n_target * dirs
        turbine_idx = jnp.repeat(jnp.arange(n_target), dirs)
        dxs = jnp.tile(move_x, n_target)
        dys = jnp.tile(move_y, n_target)
        cand_x = jnp.repeat(x[None, :], total + 1, axis=0)
        cand_y = jnp.repeat(y[None, :], total + 1, axis=0)
        rows = jnp.arange(1, total + 1)
        cand_x = cand_x.at[rows, turbine_idx].add(dxs)
        cand_y = cand_y.at[rows, turbine_idx].add(dys)
        scores = jax.vmap(aep_obj)(cand_x, cand_y)
        feas = jax.vmap(feasible_strict)(cand_x, cand_y)
        feas = feas.at[0].set(True)
        penalized = jnp.where(feas, scores, scores + 1e9)
        idx = jnp.argmin(penalized)
        return cand_x[idx], cand_y[idx]

    for step in (60.0, 35.0, 20.0, 10.0):
        best_x, best_y = nudge_search(best_x, best_y, step)
