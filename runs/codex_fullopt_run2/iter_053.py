"""Exact-size BO grid with pair wake-relief finish.

HYPOTHESIS: The current run's surplus-grid/prune basin is plateauing near
5584 GWh, and the last single-turbine perturbation only improved the incumbent
by a few hundredths of a GWh. Wake losses are pair-coupled, so moving aligned
pairs apart in the dominant wind frame may cross local spacing/boundary
nonsmoothness that a one-turbine finish cannot.
...
a direct scored two-turbine wind-frame move ensemble focused on likely wake
pairs.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def pair_wake_finish(seed_x, seed_y):
        ...
        top = jnp.argsort(pair_priority)[-80:]
        pair_i = ii[top]
        pair_j = jj[top]
    ...
    cand_x = jnp.stack([best_x, polish_x, pair_x])
    cand_y = jnp.stack([best_y, polish_y, pair_y])
    scores = jax.vmap(aep_obj)(cand_x, cand_y)
