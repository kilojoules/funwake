"""Exact-size BO grid with no-surplus staged AL-NAdam.

HYPOTHESIS: The current run's surplus-grid/prune basin is plateauing near
5577 GWh. A direct exact-size BO lattice with staged augmented-Lagrangian NAdam
may recover a stronger basin by optimizing only the final turbine count instead
of selecting from surplus turbines after a different wake pattern has formed.

AXIS: BO-tuned multi-parameter exact grid plus staged AL-NAdam halving, with
direct projection and no surplus-pruning or TopFarm warm-start polish.
