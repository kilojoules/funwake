"""HYPOTHESIS: A qualitative explore/refine switch may improve the validated
dual-window schedule: slightly looser early penalties can aid wake
rearrangement, then a restored/tightened repair phase preserves feasibility.
AXIS: schedule_two_phase on alpha0-gated dual-window schedule.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    phase_lr = jnp.where(t < 0.34, 1.035, 0.995)
    lr_real = lr_real * phase_lr

    late = jnp.maximum(t - 0.5, 0.0) / 0.5
    alpha_real = 2.879478 * alpha0 * real_init / lr_real
    alpha_real = alpha_real + 16.850946 * alpha0 * late ** 2
    phase_alpha = jnp.where(t < 0.34, 0.925, 1.018)
    alpha_real = alpha_real * phase_alpha
    final_repair = jnp.clip((t - 0.88) / 0.12, 0.0, 1.0)
    final_repair = final_repair * final_repair * (3.0 - 2.0 * final_repair)
    alpha_real = alpha_real + 3.5 * alpha0 * final_repair ** 2
    ...
    alpha_repair = alpha_repair + 40.00 * alpha0 * tail ** 2
