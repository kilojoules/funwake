"""HYPOTHESIS: Current ROWP validation requires the stronger normal-scale
terminal repair from the anti-phase family; keep the shallow mid relaxation
and late boundary tail because cleaner terminal-only repair became infeasible.
AXIS: final_rowp_safe_anti_phase_tail_repair.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    tail_real = jnp.clip((t - 0.82) / 0.18, 0.0, 1.0)
    tail_real = tail_real * tail_real * (3.0 - 2.0 * tail_real)
    alpha_real = 2.879478 * alpha0 * real_init / lr_real
    alpha_real = alpha_real + 16.850946 * alpha0 * late ** 2
    alpha_real = alpha_real + 18.0 * alpha0 * tail_real ** 2
    alpha_real = alpha_real * (1.0 - 0.075 * jnp.exp(-0.5 * ((t - 0.585) / 0.075) ** 2))
