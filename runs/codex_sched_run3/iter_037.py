"""HYPOTHESIS: The best gated LR/alpha schedule may not need medium Adam
memory; TopFarm-low betas could make constraint corrections more local while
preserving the same feasible terminal repair path.
AXIS: adam_topfarm_low on alpha0-gated warmup/cosine repair.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    beta1 = jnp.where(low_scale, 0.245, 0.1)
    beta2_repair = 0.60 - 0.05 * repair_mid + 0.03 * tail
    beta2 = jnp.where(low_scale, beta2_repair, 0.2)
