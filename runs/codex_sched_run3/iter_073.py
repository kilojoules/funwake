"""HYPOTHESIS: The last attempts changed only alpha0 routing while preserving
the train branch. Try a different train-scale mechanism: keep the validated
dual-window LR/alpha envelope, but release Adam memory during the two mobility
windows and rebuild averaging during terminal constraint lock.
AXIS: train_scale_timed_adam_memory_release_with_existing_scale_repairs.
LESSON: Timed beta release remained feasible but dropped train AEP to 5561.73,
so the exact constant train-scale betas are part of the strong basin.
"""
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    memory_release = jnp.maximum(real_mid, 0.75 * real_late)
    terminal = jnp.clip((t - 0.900) / 0.100, 0.0, 1.0)
    terminal = terminal * terminal * (3.0 - 2.0 * terminal)
    beta1_real = 0.255 - 0.070 * memory_release - 0.055 * terminal
    beta2_real = 0.690 - 0.115 * memory_release + 0.045 * terminal
    ...
    beta1 = jnp.where(low_scale, 0.245, jnp.where(high_scale, 0.300, beta1_real))
    beta2 = jnp.where(low_scale, beta2_repair, jnp.where(high_scale, 0.500, beta2_real))
