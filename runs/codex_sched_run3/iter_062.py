"""HYPOTHESIS: The validated dual-window LR/alpha trajectory is brittle to
direct path edits, but the final settling phase may tolerate a variance-memory
ramp. Keep the train and scale-dispatch envelopes intact while increasing
beta2 late so Adam takes smoother terminal steps without changing the basin.
AXIS: late_beta2_smoothing_on_validated_three_regime_dispatch.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    settle = jnp.clip((t - 0.72) / 0.28, 0.0, 1.0)
    settle = settle * settle * (3.0 - 2.0 * settle)
    beta2_real = 0.635963 + 0.105 * settle
