"""HYPOTHESIS: The train branch may be freezing before the final wake-layout
polish; a narrow terminal mobility shelf plus a low-memory Adam reset should
let feasible layouts make local AEP improvements without changing the
alpha0-gated ROWP repair behavior from the incumbent.
AXIS: terminal_low_memory_polish_on_three_regime_schedule.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    real_polish = jnp.exp(-0.5 * ((t - 0.968) / 0.026) ** 2)
    lr_real = real_base + 0.458205 * real_init * real_mid
    lr_real = lr_real + 0.165784 * real_init * real_late
    lr_real = jnp.maximum(lr_real + 0.030 * real_init * real_polish, 1e-10)
    ...
    high_gate = jnp.clip((alpha0 - 4.70e-4) / 1.40e-4, 0.0, 1.0)
    high_gate = high_gate * high_gate * (3.0 - 2.0 * high_gate)

    lr_mid_high = (1.0 - high_gate) * lr_real + high_gate * lr_high
    alpha_mid_high = (1.0 - high_gate) * alpha_real + high_gate * alpha_high

    polish_gate = jnp.clip((t - 0.905) / 0.075, 0.0, 1.0)
    polish_gate = polish_gate * polish_gate * (3.0 - 2.0 * polish_gate)
    beta1_real = 0.239994 - 0.075 * polish_gate
    beta2_real = 0.635963 - 0.155 * polish_gate
    beta1_mid_high = (1.0 - high_gate) * beta1_real + high_gate * 0.300
    beta2_mid_high = (1.0 - high_gate) * beta2_real + high_gate * 0.500

    lr = jnp.where(low_scale, lr_repair, lr_mid_high)
    alpha = jnp.where(low_scale, alpha_repair, alpha_mid_high)
    beta1 = jnp.where(low_scale, 0.245, beta1_mid_high)
    beta2 = jnp.where(low_scale, beta2_repair, beta2_mid_high)
