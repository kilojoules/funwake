"""HYPOTHESIS: The train-scale optimum is brittle, so keep that branch
unchanged and change only the ROWP/high-alpha route. A simpler cosine mobility
pulse with a terminal alpha lock may generalize as well as the prior plateau
branch while avoiding its extra late reheating.
AXIS: exact_train_band_with_high_alpha_cosine_pulse_terminal_lock.
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    high_bump = jnp.exp(-0.5 * ((t - 0.680) / 0.058) ** 2)
    high_tail = jnp.clip((t - 0.875) / 0.125, 0.0, 1.0)
    high_tail = high_tail * high_tail * (3.0 - 2.0 * high_tail)
    lr_high = high_base + 0.28 * high_init * high_bump
    lr_high = jnp.maximum(lr_high * (1.0 - 0.62 * high_tail), 1e-10)
    alpha_high = 5.05 * alpha0 * high_init / lr_high
    alpha_high = alpha_high + 3.25 * alpha0 * late ** 2
    alpha_high = alpha_high + 1.05 * alpha0 * high_bump
    alpha_high = alpha_high + 22.00 * alpha0 * high_tail ** 2
    beta1_high = 0.300 - 0.055 * high_tail
    beta2_high = 0.500 + 0.075 * high_tail
