"""Exact-size BO grid with wake-pair plus quasi-Newton penalty polish.
...
wake relocation and sector/contact blocks did not escape it. A different
escape should hand the best wake-pair layout to a very short penalized
L-BFGS-B pass, because the dominant basin may still have a small continuous
KKT-style improvement that first-order projected steps keep missing.
...
nesterov_momentum projected finish followed by pairwise wake-decoupling and
train-size-only penalized L-BFGS-B projection.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def wake_pair_decouple_finish(seed_x, seed_y):
        ...
        pair_score = jnp.sum(wind_strength[:, None] * wake_kernel, axis=0) / (
            pair_energy + 1e-9
        )
        ...
        raw_x, raw_y = jax.vmap(transform_pair)(idxs, shifts)
