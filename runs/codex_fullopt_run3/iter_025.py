"""Exact-size BO grid with smooth wind-frame warp finish.
...
turbine, wake-pair, sector, and reinsert moves all return the same plateau.
A different escape should test smooth row-scale warps in the dominant wind
frame, because small sinusoidal/shear curvature can change many wake alignments
without the hard discontinuities of block moves.
...
nesterov_momentum projected finish followed by smooth wind-frame polynomial and
sinusoidal warp search plus one sector/contact cleanup.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def smooth_warp_finish(seed_x, seed_y):
        ux = jnp.cos(dominant_wd)
        uy = jnp.sin(dominant_wd)
        vx = -uy
        vy = ux
        ...
        u0 = u - jnp.mean(u)
        v0 = v - jnp.mean(v)
        u0 = u0 / (jnp.max(jnp.abs(u0)) + 1e-9)
        v0 = v0 / (jnp.max(jnp.abs(v0)) + 1e-9)
        modes = jnp.stack(
            [
                u0,
                v0,
                ...
                v0**2 - jnp.mean(v0**2),
                jnp.sin(jnp.pi * u0),
                jnp.sin(2.0 * jnp.pi * u0),
                jnp.sin(jnp.pi * v0),
                jnp.sin(2.0 * jnp.pi * v0),
            ]
        )
        modes = modes - jnp.mean(modes, axis=1)[:, None]
        modes = modes / (jnp.max(jnp.abs(modes), axis=1)[:, None] + 1e-9)
        amps = min_spacing * jnp.array([-0.18, -0.12, -0.07, 0.07, 0.12, 0.18])

        warped_x = []
        warped_y = []
        for mode_i in range(modes.shape[0]):
            mode = modes[mode_i]
            for dir_x, dir_y in [(ux, uy), (vx, vy)]:
                for amp_i in range(amps.shape[0]):
                    shift = amps[amp_i] * mode
                    warped_x.append(seed_x + shift * dir_x)
                    warped_y.append(seed_y + shift * dir_y)

        rep_x = jnp.stack(warped_x)
        rep_y = jnp.stack(warped_y)
        rep_x, rep_y = project_batch(rep_x, rep_y, 10, 0.008, 1.0008, 0.10)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
        local_feas = jax.vmap(feasible_strict)(local_x, local_y)
    ...
    warp_x, warp_y = smooth_warp_finish(nesterov_x, nesterov_y)
    sector_x, sector_y = sector_contact_finish(warp_x, warp_y)
    cand_x = jnp.stack(
        [
            best_x,
            polish_x,
