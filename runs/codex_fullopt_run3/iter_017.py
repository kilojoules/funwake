"""BayesianOptimization-style GP/EI grid with additive centroid starts.

HYPOTHESIS: The explicit genetic algorithm was feasible but pulled the
polisher into a weaker basin. A more exploitative BayesianOptimization-style
GP/EI search over the compact lattice parameters should preserve the proven
BO basin while centroid starts provide limited non-lattice diversity.

AXIS: BayesianOptimization-style internal GP expected-improvement search over
exact-size lattice parameters plus additive init_kmeans centroid layouts,
staged AL-NAdam halving, projection ensemble, strict projected Adam polish,
single-turbine sparse finish, row/band-level wind-frame moves, a
nesterov_momentum projected finish, and scipy_basin_hopping affine polish.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
        ...
        def quick_project(x, y):
            ...
            for _ in range(36):
                for e in range(n_edges):
                    ...
                    push = jnp.maximum(0.0, 0.02 - clearance)
                    x = x + push * nx
                    y = y + push * ny
                dx = x[:, None] - x[None, :]
                dy = y[:, None] - y[None, :]
    ...
    def kmeans_project(x, y, n_steps=34):
        n_edges = boundary.shape[0]
        for _ in range(n_steps):
            ...
            overlap = jnp.maximum(0.0, min_spacing * 1.001 - dist)
            x = x + jnp.sum(overlap * dx / dist, axis=1) * 0.48
            y = y + jnp.sum(overlap * dy / dist, axis=1) * 0.48
        return x, y

    def kmeans_layout(theta_off):
        n_grid = int(np.sqrt(n_target)) + 25
        gx, gy = jnp.meshgrid(
            jnp.linspace(x_min, x_max, n_grid),
            jnp.linspace(y_min, y_max, n_grid),
        )
        px = gx.ravel()
        py = gy.ravel()
        sdf = polygon_sdf(px, py, boundary)
        inside = (sdf < -min_spacing * 0.06).astype(px.dtype)
        theta = dominant_wd + theta_off
        ux = jnp.cos(theta)
        uy = jnp.sin(theta)
        vx = -uy
        ...
        u = (px - center_x) * ux + (py - center_y) * uy
        v = (px - center_x) * vx + (py - center_y) * vy
        v_scale = jnp.std(v) + 1e-6
        u_scale = jnp.std(u) + 1e-6
        edge_pull = jnp.clip(-sdf / (min_spacing + 1e-6), 0.0, 2.5)
        weights_k = inside * (
            1.0
            + 0.26 * jnp.abs(v) / v_scale
            + 0.10 * jnp.abs(u) / u_scale
            + 0.09 * edge_pull
        )
        init_score = weights_k + 0.03 * ((u / u_scale) ** 2 + (v / v_scale) ** 2)
        idx = jnp.argsort(init_score)[-n_target:]
        cx = px[idx]
        cy = py[idx]
        for _ in range(7):
            d2 = (px[:, None] - cx[None, :]) ** 2 + (py[:, None] - cy[None, :]) ** 2
            nearest = jnp.argmin(d2, axis=1)
            membership = (nearest[:, None] == jnp.arange(n_target)[None, :]).astype(px.dtype)
            w = membership * weights_k[:, None]
            denom = jnp.sum(w, axis=0) + 1e-9
            nx = jnp.sum(px[:, None] * w, axis=0) / denom
            ny = jnp.sum(py[:, None] * w, axis=0) / denom
            active = denom > 1e-6
            cx = jnp.where(active, nx, cx)
            cy = jnp.where(active, ny, cy)
        return kmeans_project(cx, cy)

    layouts = [generate_grid(scale_params(p)) for p in top_params]
    layouts.extend([kmeans_layout(0.0), kmeans_layout(jnp.pi / 7.0)])
