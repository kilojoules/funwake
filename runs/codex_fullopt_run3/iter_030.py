"""Exact-size BO grid with lane-phase shear finish.
...
boundary-ring redistribution again tied the 5584.45 GWh plateau. A different
escape should perturb whole wind-frame lanes with alternating phase/shear
offsets, because that changes wake alignment across many rows while preserving
local spacing better than isolated boundary or weak-turbine moves.
...
nesterov_momentum projected finish followed by wind-frame lane-phase shear
perturbations and sector/contact cleanup.

LESSON: Scored 5584.45 GWh feasible, tied the current best; lane-phase shear
generalized feasibly but did not escape the dominant BO/AL-NAdam plateau.
"""
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def lane_phase_finish(seed_x, seed_y):
        ...
        v = seed_x * vx + seed_y * vy
        v_lo = jnp.min(v)
        v_span = jnp.maximum(jnp.max(v) - v_lo, 1.0)
        v_norm = (v - v_lo) / v_span
        v_edges = jnp.quantile(v, jnp.array([0.2, 0.4, 0.6, 0.8]))
        lane_id = jnp.sum(v[:, None] > v_edges[None, :], axis=1)
        alt = jnp.where((lane_id % 2) == 0, 1.0, -1.0)

        specs = min_spacing * jnp.array(
            [
                [1.0, 0.00, 0.10, 0.00, 0.05],
                [1.0, 0.25, 0.10, 0.00, -0.05],
                [1.0, 0.50, 0.14, 0.00, 0.06],
                [1.5, 0.00, 0.10, 0.04, 0.04],
                [1.5, 0.33, 0.12, -0.04, -0.04],
                [2.0, 0.00, 0.08, 0.05, 0.05],
                [2.0, 0.25, 0.10, -0.05, -0.05],
                [0.5, 0.00, 0.16, 0.00, 0.08],
                [0.5, 0.50, 0.16, 0.00, -0.08],
                [1.0, 0.12, 0.00, 0.08, 0.06],
                [1.0, 0.62, 0.00, -0.08, -0.06],
                [1.5, 0.18, 0.06, 0.06, 0.08],
                [1.5, 0.68, -0.06, -0.06, -0.08],
                [2.5, 0.10, 0.07, 0.04, 0.04],
            ]
        )

        def transform_lane(spec):
            freq, phase_len, amp_u, amp_v, alt_amp = spec
            phase = phase_len / min_spacing
            angle = 2.0 * jnp.pi * (freq * v_norm + phase)
            du = amp_u * jnp.sin(angle) + alt_amp * alt
            dv = amp_v * jnp.cos(angle)
            return seed_x + du * ux + dv * vx, seed_y + du * uy + dv * vy

        raw_x, raw_y = jax.vmap(transform_lane)(specs)
        rep_x, rep_y = project_batch(raw_x, raw_y, 16, 0.010, 1.0010, 0.11)
        local_x = jnp.concatenate([seed_x[None, :], rep_x], axis=0)
        local_y = jnp.concatenate([seed_y[None, :], rep_y], axis=0)
        local_scores = jax.vmap(aep_obj)(local_x, local_y)
        local_feas = jax.vmap(feasible_strict)(local_x, local_y)
    ...
    sector_x, sector_y = sector_contact_finish(lane_x, lane_y)
