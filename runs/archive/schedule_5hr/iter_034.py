"""Vanilla SGD fori_loop, 3-phase penalty, generous step clipping.

12000 iterations. By phase 3 (iter 10000+), LR is ~0.01 so enforcement
barely moves turbines but still fixes violations. Step clip at 3x
min_spacing only prevents extreme jumps.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    initial_lr = 250.0
    lr_decay_rate = 0.999
    max_step = 3.0 * min_spacing  # generous clip for robustness

    # 3-phase penalty schedule
    phase1_end = 4000   # feasibility
    phase2_end = 10000  # AEP exploration
    # phase 3: 10000-12000 enforcement (LR ~ 0.01, very gentle)
    ...
    def sgd_step(i, state):
        ...
        alpha = jnp.where(in_p1, alpha_s1, jnp.where(in_p2, alpha_s2, alpha_s3))
        ks_rho = jnp.where(in_p1, ks_rho_s1, jnp.where(in_p2, ks_rho_s2, ks_rho_s3))
