"""ADAM with mid-optimization random perturbation (basin hopping).

Key insight: all approaches converge to ~5566 GWh basin. To break out,
we need a mechanism to jump between basins.

Strategy:
1. ADAM Phase 1 (8000 iters): full feasibility→exploration→partial enforcement
2. Random perturbation: shift each turbine by random ±min_spacing/3
3. ADAM Phase 2 (5000 iters): re-explore from perturbed solution, then enforce
4. topfarm polish

The perturbation breaks out of the local optimum found in Phase 1.
Phase 2 re-optimizes from the perturbed positions.

Two separate fori_loops with perturbation between them.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # === Phase 1: ADAM 8000 iters (feasibility→exploration→partial enforcement) ===
    p1_phase1_end = 1500   # feasibility
    p1_phase2_end = 6000   # exploration
    p1_total = 8000        # partial enforcement

    def adam_step_p1(i, state):
        x, y, m_x, m_y, v_x, v_y = state

        at_restart = (i == p1_phase1_end) | (i == p1_phase2_end)
        ...
        in_p1 = i < p1_phase1_end
        in_p2 = (i >= p1_phase1_end) & (i < p1_phase2_end)

        p1_lr = 200.0 * (0.9993 ** i)
        p2_lr = 150.0 * (0.99973 ** (i - p1_phase1_end))
        p3_lr = 80.0 * (0.99944 ** (i - p1_phase2_end))
        lr = jnp.where(in_p1, p1_lr, jnp.where(in_p2, p2_lr, p3_lr))

        alpha = jnp.where(in_p1, 200.0,
                jnp.where(in_p2, 2.0,
                5.0 + 295.0 * 0.5 * (1.0 - jnp.cos(
                    jnp.pi * (i - p1_phase2_end) / (p1_total - p1_phase2_end)))))
        ...
        step_in_phase = jnp.where(
            in_p1, i + 1,
            jnp.where(in_p2, i - p1_phase1_end + 1,
                      i - p1_phase2_end + 1)).astype(jnp.float64)
    ...
    x1, y1, _, _, _, _ = jax.lax.fori_loop(0, p1_total, adam_step_p1, init_state)

    # === Random perturbation ===
    key = jax.random.PRNGKey(7)
    perturb_scale = min_spacing / 3.0
    k1, k2 = jax.random.split(key)
    dx = jax.random.uniform(k1, (n_target,), minval=-perturb_scale, maxval=perturb_scale)
    dy = jax.random.uniform(k2, (n_target,), minval=-perturb_scale, maxval=perturb_scale)
    x_perturbed = x1 + dx
    y_perturbed = y1 + dy

    # === Phase 2: ADAM 5000 iters (re-explore + enforce) ===
    p2_explore_end = 2500
    p2_total = 5000

    def adam_step_p2(i, state):
        x, y, m_x, m_y, v_x, v_y = state

        at_restart = (i == p2_explore_end)
        m_x = jnp.where(at_restart, 0.0, m_x)
        m_y = jnp.where(at_restart, 0.0, m_y)
        v_x = jnp.where(at_restart, 1e-8, v_x)
        v_y = jnp.where(at_restart, 1e-8, v_y)

        in_explore = i < p2_explore_end

        explore_lr = 100.0 * (0.9993 ** i)
        enforce_lr = 60.0 * (0.99944 ** (i - p2_explore_end))
        lr = jnp.where(in_explore, explore_lr, enforce_lr)

        alpha = jnp.where(in_explore, 3.0,
                5.0 + 595.0 * 0.5 * (1.0 - jnp.cos(
                    jnp.pi * (i - p2_explore_end) / (p2_total - p2_explore_end))))
        ...
        step_in_phase = jnp.where(
            in_explore, i + 1,
            i - p2_explore_end + 1).astype(jnp.float64)
    ...
    init_state2 = (x_perturbed, y_perturbed,
                   jnp.zeros_like(init_x), jnp.zeros_like(init_y),
                   jnp.zeros_like(init_x), jnp.zeros_like(init_y))

    explored_x, explored_y, _, _, _, _ = jax.lax.fori_loop(
        0, p2_total, adam_step_p2, init_state2)
