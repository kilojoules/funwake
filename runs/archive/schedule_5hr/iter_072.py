"""Hybrid: momentum restarts + decoupled low-spacing exploration.

Lesson from iter_071 (5543.65): zero spacing collapses turbines too much.
Lesson from iter_058 (5566.35): momentum restarts help escape local minima.
Lesson from iter_067 (~5566): sw=0.1 exploration with bw=20 is a sweet spot.

This combines all three insights:
- Custom ADAM with momentum restarts (from iter_058)
- Decoupled sp/bp with sp=0.1 during exploration (from iter_067)
- Higher boundary weight during exploration bp=20 (from iter_067)
- Extended exploration phase with higher LR

Phase structure (3 phases, momentum restarts at boundaries):
- Phase 1 (0-2000): Feasibility - sp=200, bp=200, LR=200 decay
  [MOMENTUM RESET at 2000]
- Phase 2 (2000-9000): Long exploration - sp=0.1, bp=20, LR=200 constant then decay
  [MOMENTUM RESET at 9000]
- Phase 3 (9000-13000): Enforcement - sp cosine 5->600, bp cosine 20->600, LR=50 decay
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    p2_end = 9000   # long exploration (7000 iters!)
    # p3: 9000-13000 enforcement
    ...
    def adam_step(i, state):
        ...
        # Phase 1: LR 200 decaying
        p1_lr = 200.0 * (0.9993 ** i)

        # Phase 2: LR 200 constant for first 3000, then decay to 30
        p2_step = i - p1_end
        p2_const = 3000
        p2_lr = jnp.where(p2_step < p2_const,
                          200.0,
                          200.0 * (0.99952 ** (p2_step - p2_const)))

        # Phase 3: LR 50 decaying to 3
        p3_lr = 50.0 * (0.99930 ** (i - p2_end))
        ...
        alpha_bp = jnp.where(in_p1, 200.0,
                   jnp.where(in_p2, 20.0,
                   # Phase 3: cosine ramp 20 -> 600
                   20.0 + 580.0 * 0.5 * (1.0 - jnp.cos(
                       jnp.pi * (i - p2_end) / (total_iters - p2_end)))))
