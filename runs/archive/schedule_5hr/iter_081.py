"""ADAM with stochastic gradient noise for escaping local optima.

Key insight: iter_058 and iter_068 both converge to exactly 5566.35
despite using different methods (custom ADAM vs 3-stage topfarm).
This suggests they're stuck in the same local optimum.

Strategy: Inject small random noise into gradients during exploration
phase (Langevin dynamics). This helps the optimizer explore beyond
the local basin that traps deterministic ADAM.

Schedule:
- Phase 1 (0-1500): alpha=200, no noise (feasibility push)
- Phase 2 (1500-10000): alpha=1, gradient noise (long noisy exploration)
- Phase 3 (10000-13000): alpha cosine ramp 1→700, no noise (enforcement)
+ Topfarm polish
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # === Custom ADAM with gradient noise ===
    ...
    p2_end = 10000   # exploration with noise
    ...
    def adam_step(i, state):
        ...
        rng_key, noise_key_x, noise_key_y = jax.random.split(rng_key, 3)
        ...
        p3_alpha = 1.0 + 699.0 * 0.5 * (1.0 - jnp.cos(jnp.pi * p3_prog))

        alpha = jnp.where(in_p1, 200.0, jnp.where(in_p2, 1.0, p3_alpha))

        g_x, g_y = grad_fn(x, y, alpha, alpha)

        # Inject gradient noise during exploration phase
        noise_scale = jnp.where(in_p2, lr * 0.08, 0.0)
        g_x = g_x + jax.random.normal(noise_key_x, shape=x.shape) * noise_scale
        g_y = g_y + jax.random.normal(noise_key_y, shape=y.shape) * noise_scale
        ...
        # Phase-relative bias correction
