"""3-stage topfarm: maximize constant-LR exploration.

Insight from topfarm internals: alpha scales as lr0/lr, so penalty
INCREASES as LR decays. To keep exploration truly free, maximize
the constant_lr portion of Stage 2.

Changes from iter_068:
- Stage 2: sw=0.01 (vs 0.1), const_lr=4500 (vs 3500), max_iter=4000 (vs 5000)
  → 4500 iters at truly constant low penalty (vs 3500)
  → Total 8500 iters (same), but 53% at constant LR (vs 41%)
- Stage 1: trimmed to save time
- Stage 3: slightly fewer iters
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # Stage 2: Maximized constant-LR exploration
    s2 = SGDSettings(
        learning_rate=250.0,
        max_iter=4000, additional_constant_lr_iterations=4500,
        tol=1e-6, beta1=0.1, beta2=0.2,
        spacing_weight=0.01, boundary_weight=15.0,
