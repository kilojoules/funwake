"""ADAM + gradient noise injection (Langevin-style exploration).

Base: iter_058 schedule (proven 5566.35 GWh).
New: During phase 2 (exploration), add scaled Gaussian noise to gradients.
This creates stochastic gradient Langevin dynamics, which:
- Helps escape shallow local optima
- Explores more of the energy landscape
- Noise magnitude decays over the phase for smooth convergence

Also tries: slightly longer phase 2 (6500 iters) and lower alpha_explore=1.5.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    p1_end = 2000
    p2_end = 8500   # slightly longer exploration
    # Phase 3: 8500-13000 (4500 iters)

    # Noise parameters
    noise_scale = 0.3  # fraction of gradient magnitude
    ...
    def adam_step(i, state):
        ...
        alpha = jnp.where(in_p1, 200.0,
                jnp.where(in_p2, 1.5,  # slightly lower than iter_058's 2.0
                5.0 + 595.0 * 0.5 * (1.0 - jnp.cos(
                    jnp.pi * (i - p2_end) / (total_iters - p2_end)))))

        g_x, g_y = grad_fn(x, y, alpha, alpha)

        # === Gradient noise injection during exploration (phase 2) ===
        k1, k2, rng_key = jax.random.split(rng_key, 3)
        noise_x = jax.random.normal(k1, shape=x.shape)
        noise_y = jax.random.normal(k2, shape=y.shape)

        # Scale noise relative to gradient magnitude, decay over phase 2
        grad_mag = jnp.sqrt(jnp.mean(g_x**2 + g_y**2) + 1e-12)
        # Noise decays linearly through phase 2
        phase2_progress = jnp.clip((i - p1_end) / (p2_end - p1_end), 0.0, 1.0)
        noise_factor = noise_scale * grad_mag * (1.0 - phase2_progress)
        # Only inject noise during phase 2
        noise_active = in_p2.astype(jnp.float64)

        g_x = g_x + noise_active * noise_factor * noise_x
        g_y = g_y + noise_active * noise_factor * noise_y
