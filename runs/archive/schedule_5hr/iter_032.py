"""Vanilla SGD + fori_loop with 3-phase schedule and per-turbine step clipping.

Phase 1: aggressive feasibility (alpha=250)
Phase 2: AEP exploration (alpha=3)
Phase 3: re-enforce constraints (alpha=100) without destroying AEP gains

Per-turbine step clipping prevents NaN on extreme geometries.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def sgd_step(i, state):
        ...
        ks_rho = jnp.where(in_p1, ks_rho_1, jnp.where(in_p2, ks_rho_2, ks_rho_3))

        gx, gy = grad_fn(x, y, alpha_sp, alpha_bp, ks_rho)

        # Compute step and clip per-turbine magnitude
        step_x = lr * gx
        step_y = lr * gy
