"""Custom ADAM with aggressive exploration schedule + topfarm polish.

Key changes from iter_059 (5549.2):
- Constant high momentum (0.9/0.999) throughout - proven in iter_047
- Higher LR (200 vs 150)
- More aggressive exploration: alpha drops to 0.5 (near zero penalty)
- Longer exploration phase (55%), shorter enforcement (rely on polish)
- 12000 custom iters (matching iter_047)
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # === Custom ADAM with cosine penalty schedule ===
    def penalized_grad(x, y, alpha):
        def total(x, y):
            obj = objective(x, y)
            sp = spacing_penalty(x, y, min_spacing)
            bp = boundary_penalty(x, y, boundary)
            return obj + alpha * sp + alpha * bp
        return jax.grad(total, argnums=(0, 1))(x, y)
    ...
    # Penalty schedule: cosine high→low→high
    phase1_end = 2000
    phase2_end = 7500
    ...
    def adam_step(i, state):
        ...
        p2_prog = (i - phase1_end) / (phase2_end - phase1_end)
        p2_alpha = alpha_low + (alpha_high - alpha_low) * 0.5 * (1.0 + jnp.cos(jnp.pi * p2_prog))

        # Phase 3: cosine ramp
        p3_prog = (i - phase2_end) / (total_iters - phase2_end)
        p3_alpha = alpha_low + (alpha_final - alpha_low) * 0.5 * (1.0 - jnp.cos(jnp.pi * p3_prog))

        alpha = jnp.where(in_p1, alpha_high,
                          jnp.where(in_p2, p2_alpha, p3_alpha))

        gx, gy = penalized_grad(x, y, alpha)
        ...
        return (x - sx * scale, y - sy * scale, mx, my, vx, vy)
