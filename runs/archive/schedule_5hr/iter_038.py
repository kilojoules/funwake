"""Single-start 3-stage with 60-degree offset grid + moderate enforcement.

Uses iter_017's proven feasible settings for stages 1-2, with stage 3
enforcement at sw=200 (between feasible sw=300 and infeasible sw=100).
60-degree offset grid explores a different local basin than the
standard perpendicular grid used by all previous attempts.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # Stage 1: Feasibility push (iter_017 settings)
    ...
    # Stage 3: Moderate enforcement (200 = midpoint between 300 and 100)
    ...
    # --- 60-degree offset grid initialization ---
    wd_rad = jnp.deg2rad(wd)
    dominant = jnp.arctan2(
        jnp.sum(weights * jnp.sin(wd_rad)),
        jnp.sum(weights * jnp.cos(wd_rad)))
    angle = dominant + jnp.pi / 3  # 60-degree offset instead of 90-degree

    cos_a, sin_a = jnp.cos(angle), jnp.sin(angle)
    cx_bnd, cy_bnd = jnp.mean(boundary[:, 0]), jnp.mean(boundary[:, 1])
    translated = boundary - jnp.array([cx_bnd, cy_bnd])
    ...
    orig_pts = (inv_rot @ rot_pts.T).T + jnp.array([cx_bnd, cy_bnd])
