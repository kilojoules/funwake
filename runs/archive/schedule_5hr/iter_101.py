"""iter_058 with gradually increasing Phase 2 alpha.

One targeted modification: Phase 2 alpha ramps from 1→5 (instead of
constant 2). As LR decays during exploration, constraints tighten
gradually rather than holding constant then jumping. This avoids the
hard transition at the phase 2→3 boundary.

Also: Phase 2 starting LR boosted to 180 (from 150) for more aggressive
initial exploration. Everything else identical to iter_058.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # === Custom ADAM with gradually increasing Phase 2 alpha ===
    ...
    def adam_step(i, state):
        ...
        # Same Phase 1 and 3 LR as iter_058
        p1_lr = 200.0 * (0.9993 ** i)
        # Phase 2: slightly higher starting LR (180 vs 150)
        p2_lr = 180.0 * (0.99970 ** (i - p1_end))  # ~180→30 over 6000
        p3_lr = 80.0 * (0.99944 ** (i - p2_end))
        lr = jnp.where(in_p1, p1_lr, jnp.where(in_p2, p2_lr, p3_lr))

        # Phase 2: gradual alpha increase 1→5 (instead of constant 2)
        p2_progress = (i - p1_end) / (p2_end - p1_end)
        p2_alpha = 1.0 + 4.0 * p2_progress  # linear 1→5

        alpha = jnp.where(in_p1, 200.0,
                jnp.where(in_p2, p2_alpha,
