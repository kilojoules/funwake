"""2-stage topfarm: skip feasibility, maximize explore + enforce time.

The feasibility stage (sw=200, 1500 iters) just pushes grid turbines
to satisfy constraints - but the wind-aware grid init is already mostly
feasible. By skipping it, we get ~1500 more iterations for exploration.

Also try gamma_min_factor=0.1 in exploration (LR stays higher longer)
and sw=0.3 for near-zero spacing penalty during exploration.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # Stage 1: Long exploration with high LR retention
    ...
    # 2-stage pipeline: explore → enforce
