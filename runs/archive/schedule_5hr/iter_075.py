"""iter_058 + mid-enforcement momentum restart + higher alpha.

iter_058 got 5566.35. This makes two targeted changes:
1. Additional momentum restart mid-enforcement (i=10500)
   to escape any stale momentum from exploration->enforcement transition
2. Higher final enforcement alpha: 800 (vs 600)
3. Slightly extended run: 14000 iters (vs 13000)

Everything else matches iter_058 exactly.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # --- Wind-aware grid initialization (matching iter_058) ---
    ...
    p3_mid = 10500  # mid-enforcement restart
    # p3: 8000-14000 enforcement

    def adam_step(i, state):
        x, y, m_x, m_y, v_x, v_y = state

        # === Momentum warm restarts at phase boundaries + mid-enforcement ===
        at_restart = (i == p1_end) | (i == p2_end) | (i == p3_mid)
        ...
        alpha = jnp.where(in_p1, 200.0,
                jnp.where(in_p2, 2.0,
                # Phase 3: cosine ramp 5 -> 800 (vs 600 in iter_058)
                5.0 + 795.0 * 0.5 * (1.0 - jnp.cos(
        ...
        # Reset bias correction at mid-enforcement restart too
        in_p3a = (i >= p2_end) & (i < p3_mid)
        step_in_phase = jnp.where(
            in_p1, i + 1,
            jnp.where(in_p2, i - p1_end + 1,
            jnp.where(in_p3a, i - p2_end + 1,
                       i - p3_mid + 1))).astype(jnp.float64)
