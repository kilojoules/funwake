"""Custom ADAM optimizer with jax.lax.fori_loop and 3-phase penalty scheduling.

Key differences from previous attempts (iter_010-025 all used topfarm_sgd_solve):
- Custom loop instead of topfarm_sgd_solve: 12000 iters in single JIT
- ADAM with standard momentum (beta1=0.9, beta2=0.999)
- 3-phase penalty: feasibility -> AEP exploration -> re-enforcement
- Inspired by v4 best (+59.3 GWh) which used vanilla SGD with fori_loop
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    final_x, final_y, _, _, _, _ = jax.lax.fori_loop(
        0, total_iterations, adam_step, init_state)
