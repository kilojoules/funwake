"""Custom SGD: iter_032 approach + decaying-LR enforcement phase.

iter_032 hit 5600 but infeasible (LR too low for enforcement).
iter_050 used constant LR=30 enforcement but over-enforced (-8 GWh).

This uses a decaying LR for enforcement that starts at 15 and decays
to 1, giving strong initial push to fix violations then fading to
preserve AEP. Alpha=200 in enforcement.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # phase 3: 9500-13000 enforcement with decaying LR
    ...
    enforce_lr_start = 15.0
    enforce_lr_end = 1.0
    enforce_iters = total_iterations - phase2_end  # 3500
    ...
    def sgd_step(i, state):
        ...
        # Exponential decay during phases 1-2
        decayed_lr = initial_lr * (lr_decay_rate ** i)
        # Linear decay during enforcement: 15 → 1
        enforce_progress = (i - phase2_end) / enforce_iters
        enforce_lr = enforce_lr_start + (enforce_lr_end - enforce_lr_start) * enforce_progress
