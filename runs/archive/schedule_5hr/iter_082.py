"""Max-dispersion initialization + proven ADAM schedule.

All previous attempts use uniform subsampling from the grid (every k-th
point). This wastes spatial diversity. Max-dispersion (farthest-point
sampling) maximizes the minimum distance between selected points, giving
much better coverage of the polygon.

Initialization: Farthest-point sampling from wind-aligned grid
ADAM schedule: iter_058 pattern (proven 5566.35), 3-phase with momentum resets
Polish: topfarm_sgd_solve
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    if len(ix) >= n_target:
        # === Farthest-point sampling (max-dispersion) ===
        # Use numpy for this sequential algorithm (runs at trace time)
        pts_x = np.array(ix)
        pts_y = np.array(iy)
        n_pts = len(pts_x)
        ...
        min_dist_sq[first_idx] = -1.0  # mark as selected
    ...
    # === Custom ADAM with iter_058 schedule (proven 5566.35) ===
