"""iter_058 schedule scaled up to 18000 iters (from 13000).

Current best (iter_058, 5566.35 GWh) uses 13000 ADAM iters + 1800 polish
and runs in ~38s. Budget is 60s. This leaves ~20s unused.

By scaling to 18000 ADAM iters we get:
- Phase 1: 0-3000 (feasibility, was 0-2000)
- Phase 2: 3000-12000 (exploration, 9000 iters, was 6000)
- Phase 3: 12000-18000 (enforcement, 6000 iters, was 5000)

More exploration AND enforcement time. Same penalty/LR patterns.
Expected runtime: ~48s + 5s polish = ~53s.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # === Custom ADAM - iter_058 schedule scaled to 18000 ===
    ...
    p1_end = 3000   # was 2000
    p2_end = 12000  # was 8000
    ...
    def adam_step(i, state):
        ...
        # Same LR decay rates as iter_058 (adjusted for longer phases)
        p1_lr = 200.0 * (0.9995 ** i)       # 200→~45 over 3000 iters
        p2_lr = 150.0 * (0.99982 ** (i - p1_end))  # 150→~28 over 9000 iters
        p3_lr = 80.0 * (0.99952 ** (i - p2_end))   # 80→~5.4 over 6000 iters
