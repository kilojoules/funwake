"""4-stage topfarm with smooth penalty transitions.

The 3-stage approach (iter_068) jumps from sw=0.05 to sw=300 between
stages 2-3, potentially destroying good positions. This 4-stage adds
a transition stage with moderate penalties.

Stage 1 (1000+500): Feasibility (sw=200, bw=200)
Stage 2 (4500+3500): Exploration (sw=0.05, bw=15, LR=250)
Stage 3 (2000+1000): Transition (sw=30, bw=60, LR=100)
Stage 4 (2000+1000): Strong enforcement (sw=300, bw=300, LR=40)
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # Stage 1: Quick feasibility
    s1 = SGDSettings(
        learning_rate=200.0,
        max_iter=1000, additional_constant_lr_iterations=500,
    ...
    s2 = SGDSettings(
        learning_rate=250.0,
        max_iter=4500, additional_constant_lr_iterations=3500,
        tol=1e-6, beta1=0.1, beta2=0.2,
        spacing_weight=0.05, boundary_weight=15.0,
        ks_rho=50.0, gamma_min_factor=0.01,
    )

    # Stage 3: Gentle transition
    s3 = SGDSettings(
        learning_rate=100.0,
        max_iter=2000, additional_constant_lr_iterations=1000,
        tol=1e-6, beta1=0.1, beta2=0.2,
        spacing_weight=30.0, boundary_weight=60.0,
        ks_rho=100.0, gamma_min_factor=0.01,
    )

    # Stage 4: Strong enforcement
    s4 = SGDSettings(
        learning_rate=40.0,
        max_iter=2000, additional_constant_lr_iterations=1000,
        tol=1e-6, beta1=0.1, beta2=0.2,
