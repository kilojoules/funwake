"""2+1 stage: internal alpha ramp + brief enforcement polish.

iter_088 scored 5563.55 (+22.83) but failed stressed boundary test.
Fix: add brief enforcement stage 3 for robust feasibility.
Also increase bw from 25→35 in stage 2.

Stage 1 (1200 total): Quick feasibility push
Stage 2 (10000 total): Combined explore+enforce via internal alpha ramp
Stage 3 (3000 total): Strong enforcement polish
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    s2 = SGDSettings(
        ...
        tol=1e-6, beta1=0.1, beta2=0.2,
        spacing_weight=8.0, boundary_weight=35.0,
        ks_rho=80.0, gamma_min_factor=0.01,
    )

    # Stage 3: Strong enforcement polish
    s3 = SGDSettings(
        learning_rate=50.0,
