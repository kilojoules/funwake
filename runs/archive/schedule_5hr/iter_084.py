"""Hexagonal grid initialization + proven iter_058 ADAM schedule.

Key change: hexagonal grid instead of rectangular. Hex packing is 15%
denser and produces different candidate points, leading to a different
initial layout after subsampling. This puts the optimizer in a different
basin of attraction.

Row spacing: min_spacing * sqrt(3)/2 (hex optimal)
Column spacing: min_spacing
Every other row offset by min_spacing/2

ADAM schedule: exact copy of iter_058 (5566.35 proven).
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # --- Wind-aware HEXAGONAL grid initialization ---
    ...
    # Hexagonal grid: row spacing = min_spacing * sqrt(3)/2
    row_spacing = min_spacing * np.sqrt(3) / 2
    col_spacing = min_spacing

    n_rows = int(np.ceil((float(ry_max) - float(ry_min)) / row_spacing))
    n_cols = int(np.ceil((float(rx_max) - float(rx_min)) / col_spacing))

    hex_pts = []
    for row in range(n_rows):
        y_val = float(ry_min) + row_spacing / 2 + row * row_spacing
        x_offset = col_spacing / 2 if row % 2 == 0 else col_spacing
        for col in range(n_cols):
            x_val = float(rx_min) + x_offset + col * col_spacing
            if x_val < float(rx_max) and y_val < float(ry_max):
                hex_pts.append([x_val, y_val])

    if len(hex_pts) > 0:
        rot_pts = jnp.array(hex_pts)
    else:
        # Fallback to rectangular
        gx, gy = jnp.meshgrid(
            jnp.linspace(rx_min + min_spacing / 2, rx_max - min_spacing / 2, n_cols),
            jnp.linspace(ry_min + min_spacing / 2, ry_max - min_spacing / 2, n_rows))
