"""Custom ADAM with separate boundary/spacing penalties.

Key insight: during exploration, boundary violations waste turbines
(outside = zero power), but spacing violations just reduce wake-free
power slightly. So we should keep boundary_alpha high but drop
spacing_alpha to near-zero during exploration.

This lets turbines cluster in high-wind corridors (close together)
while staying inside the polygon, then enforcement pushes them apart.

Phase structure (13000 iters, same as best iter_058):
  Phase 1 (0-2000): Feasibility push
    boundary_alpha=200, spacing_alpha=200, LR exp 200->50
  Phase 2 (2000-8000): Split-penalty exploration
    boundary_alpha=15, spacing_alpha=0.3, LR exp 150->15
  Phase 3 (8000-13000): Strong enforcement (both)
    boundary_alpha cosine 10->600, spacing_alpha cosine 3->600, LR exp 60->3

+ 1200 iter topfarm polish.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def adam_step(i, state):
        ...
        p2_lr = 150.0 * (0.99965 ** (i - p1_end))
        p3_lr = 60.0 * (0.99940 ** (i - p2_end))
        lr = jnp.where(in_p1, p1_lr, jnp.where(in_p2, p2_lr, p3_lr))

        # Separate boundary and spacing penalty weights
        # Phase 1: both high for feasibility establishment
        # Phase 2: boundary moderate (keep in-bounds), spacing near-zero (allow clustering)
        # Phase 3: both ramp up for enforcement
        p3_prog = (i - p2_end) / (total_iters - p2_end)

        alpha_bp = jnp.where(in_p1, 200.0,
                   jnp.where(in_p2, 15.0,
                   10.0 + 590.0 * 0.5 * (1.0 - jnp.cos(jnp.pi * p3_prog))))

        alpha_sp = jnp.where(in_p1, 200.0,
                   jnp.where(in_p2, 0.3,
                   3.0 + 597.0 * 0.5 * (1.0 - jnp.cos(jnp.pi * p3_prog))))
