"""Hybrid of iter_058 + iter_068: best of both 5566.35 approaches.

iter_058: Custom ADAM with momentum resets, per-phase LR, alpha=2 both
iter_068: 3-stage topfarm with asymmetric penalties (sw=0.1, bw=20)

This combines:
- iter_058's custom ADAM with momentum resets (more flexible scheduling)
- iter_068's asymmetric Phase 2 penalties (spacing=0.5, boundary=20)
  Allows turbines to get close (violating spacing) while staying in-bounds
- Slightly higher enforcement ramp (alpha_sp→800, alpha_bp→500)
- Longer exploration (9000 iters vs 6000)
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # --- Wind-aware grid initialization (proven best) ---
    ...
    # === Custom ADAM: asymmetric penalties + momentum resets ===
    ...
    p2_end = 10000   # long asymmetric exploration
    # p3: 10000-13000 enforcement
    ...
    def adam_step(i, state):
        ...
        # Per-phase LR (from iter_058)
        p1_lr = 200.0 * (0.9993 ** i)
        p2_lr = 180.0 * (0.99975 ** (i - p1_end))
        p3_lr = 60.0 * (0.9994 ** (i - p2_end))
        lr = jnp.where(in_p1, p1_lr, jnp.where(in_p2, p2_lr, p3_lr))

        # ASYMMETRIC penalty schedule (key innovation)
        # Phase 1: High for both (feasibility push)
        # Phase 2: Very low spacing, moderate boundary (from iter_068)
        # Phase 3: Cosine ramp to high for both
        p3_prog = (i - p2_end) / (total_iters - p2_end)
        p3_sp = 0.5 + 799.5 * 0.5 * (1.0 - jnp.cos(jnp.pi * p3_prog))
        p3_bp = 20.0 + 480.0 * 0.5 * (1.0 - jnp.cos(jnp.pi * p3_prog))

        alpha_sp = jnp.where(in_p1, 200.0, jnp.where(in_p2, 0.5, p3_sp))
        alpha_bp = jnp.where(in_p1, 200.0, jnp.where(in_p2, 20.0, p3_bp))
