"""Tuned iter_058 schedule with lower exploration alpha.

iter_058 is the best at 5566.35 GWh. This variant pushes exploration harder:
- Phase 2 alpha lowered from 2.0 to 0.5 (more freedom to slide past neighbors)
- Phase 2 LR increased: 200→20 (more aggressive exploration)
- Phase 2 lengthened to 7000 iters (was 6000)
- Phase 3 shortened to 4000 iters (was 5000) to fit budget
- Final alpha raised to 700 (was 600) for stricter enforcement
- Same wind-aligned grid + uniform subsampling init (proven reliable)
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    # --- Wind-aware grid initialization (proven reliable) ---
    ...
    # === Custom ADAM with tuned 3-phase schedule ===
    ...
    p1_end = 2000
    p2_end = 9000   # longer exploration: 7000 iters (was 6000)
    # Phase 3: 9000-13000 (4000 iters)
    ...
    def adam_step(i, state):
        ...
        # Per-phase LR (exponential decay)
        p1_lr = 200.0 * (0.9993 ** i)
        p2_lr = 200.0 * (0.99967 ** (i - p1_end))  # 200→~20 over 7000 iters
        p3_lr = 80.0 * (0.99908 ** (i - p2_end))    # 80→~2 over 4000 iters
        lr = jnp.where(in_p1, p1_lr, jnp.where(in_p2, p2_lr, p3_lr))

        # Penalty schedule: key change is very low alpha in phase 2
        alpha = jnp.where(in_p1, 200.0,
                jnp.where(in_p2, 0.5,  # was 2.0 - allow more freedom
