"""Vanilla SGD + fori_loop replicating v4 best, with per-element grad clipping.

2-phase penalty (feasibility then AEP), 12000 iterations, ks_rho passed
to penalty functions. Per-element gradient clipping for robustness.
"""
import jax
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def sgd_step(i, state):
        ...
        gx = jnp.clip(gx, -1.0, 1.0)
        gy = jnp.clip(gy, -1.0, 1.0)
