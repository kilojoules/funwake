"""HYPOTHESIS: The alpha0 gate improved train score and low-scale stress tests,
but ROWP's larger alpha0 needs a separate feasibility-robust path. Use three
scale regimes: low-alpha repair for synthetic tight tests, mid-alpha validated
dual-window for the train farm, and high-alpha single-bump counter-ramp for
ROWP-scale generalization.
AXIS: three_regime_alpha0_gated_schedule
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    # Mid-scale train path: validated warmup + dual mobility windows.
    ...
    beta1 = jnp.where(low_scale, 0.245, jnp.where(high_scale, 0.3, 0.239994))
    beta2 = jnp.where(low_scale, beta2_repair, jnp.where(high_scale, 0.5, 0.635963))
