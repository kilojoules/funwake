"""Affine wake-column recovery with pressure-balanced column shear.

HYPOTHESIS: The last layout is locally spacing-active, but columns aligned to
the dominant wind still contain power-imbalanced turbines. A small zero-mean
shear within each wake column may recover exposure while preserving the contact
graph better than another contact-pair tweak.

AXIS: incumbent affine recovery, active-pair release, wake-column group polish,
then train-size-gated pressure-balanced column shears before the existing
contact-clique and fine polish.
