"""Affine recovery with contact-clique edge polish.

HYPOTHESIS: The strongest basin is constrained by a contact graph of active
spacing and boundary constraints. Rotating active pairs and sliding small
contact cliques along feasible edge/tangent directions may expose improvements
that wake-column translations cannot reach.

AXIS: incumbent affine recovery and active-pair release followed by deterministic
contact-pair rotations, clique translations, and boundary-edge slides.
    ...
    def contact_edge_clique_refine(x0, y0, obj0):
        ...
        for step in (0.018, 0.011, 0.006):
            ...
            for idx in np.argsort(-gnorm)[: min(8, n_target)]:
                ...
                if idx not in seeds:
                    seeds.append(idx)
            edge_order = np.argsort(nearest_bnd)[: min(8, n_target)]
