"""Exact-grid AL-NAdam with guarded topfarm polish.

HYPOTHESIS: TopFarm warm-start polish can slightly improve the strong exact-grid
AL basin, but previous evidence shows it can also degrade AEP.  Keep the full
incumbent AL path and accept the polish only when it is feasible and improves
the measured objective.

AXIS: incumbent exact-grid AL-NAdam plus guarded topfarm acceptance.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    sdf_ok = jnp.max(polygon_sdf(sx, sy, boundary)) <= 1e-3
    dx = sx[:, None] - sx[None, :]
    dy = sy[:, None] - sy[None, :]
    dist = jnp.sqrt(dx**2 + dy**2 + 1e-6)
    mask = jnp.triu(jnp.ones((n_target, n_target)), k=1)
    spacing_ok = jnp.min(jnp.where(mask > 0, dist, 1e12)) >= min_spacing * 0.999
    improved = aep_obj(sx, sy) < aep_obj(px, py)

    if bool(sdf_ok & spacing_ok & improved):
        return sx, sy
    return px, py
