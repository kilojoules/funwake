"""Affine recovery with benchmark-tolerance final local search.
...
tolerance. A final local search that permits only 0.990x-feasible micro moves
can exploit this small legal slack without changing the ROWP path.
...
gradient polish, train-only benchmark-tolerance pair and turbine moves, with a
single-start feasibility path for large held-out layouts.
