"""Affine recovery with active-cluster nullspace polish.

HYPOTHESIS: The incumbent polish is conservative because most hand-coded moves
move one turbine or one contact pair at a time. Small clusters on the active
spacing graph may still have a feasible nullspace direction that improves the
wake objective while preserving near-binding spacing and boundary constraints.

AXIS: incumbent affine recovery, active-pair release, wake-column group polish,
size-gated contact-pair rotations and boundary-edge clique slides, projected
gradient polish, benchmark-tolerance pair and turbine moves, active-cluster
nullspace search, then train-only SLSQP with vectorized constraints.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def feasible_benchmark_margin(x, y):
        bnd = boundary_penalty(x, y, boundary)
        spc = spacing_penalty(x, y, min_spacing * 0.99025)
        return float(bnd) < 1e-5 and float(spc) < 1e-6
    ...
    def relaxed_tolerance_refine(x0, y0, obj0):
        best_x = x0
        best_y = y0
        best_obj = obj0
        if n_target > 60 or not feasible_benchmark_margin(best_x, best_y):
            return best_x, best_y, best_obj
        ...
        def add_basis(vec):
            ...
            if norm <= 1.0e-12:
                return
            vec = vec / norm
            if all(abs(float(np.dot(vec, old))) < 0.990 for old in basis):
                basis.append(vec)

        for angle in [float(mean_wind)] + [float(theta[i]) for i in np.argsort(-mass)[:3]]:
            add_basis(np.array([np.cos(angle), np.sin(angle)], dtype=float))
            add_basis(np.array([-np.sin(angle), np.cos(angle)], dtype=float))
        ...
        for step in (0.0080, 0.0048, 0.0028):
            prod = np.asarray(turbine_power_jit(best_x, best_y), dtype=float)
            _, grad = obj_vg(jnp.concatenate([best_x, best_y]))
            gx = np.asarray(grad[:n_target], dtype=float)
            gy = np.asarray(grad[n_target:], dtype=float)
            ...
            for idx in np.argsort(-gnorm)[: min(5, n_target)]:
                ...
                if idx not in seeds:
                    seeds.append(idx)
            for idx in np.argsort(np.min(dmat, axis=1))[: min(5, n_target)]:
                idx = int(idx)
                if idx not in seeds:
                    seeds.append(idx)
            ...
            for i in seeds[:14]:
                ...
                if np.linalg.norm(gvec) > 1.0e-12:
                    dirs.append(gvec)
                    dirs.append(np.array([-gvec[1], gvec[0]], dtype=float))
                if nearest_bnd[i] < 0.24 * float(min_spacing):
                    dirs.append(edge_t[int(nearest_edge[i])])
                for j in np.argsort(dmat[i])[:3]:
                    j = int(j)
                ...
                local_y = best_y
                local_obj = best_obj
                for vec in unique_dirs(dirs)[:5]:
                    dx = stride * float(vec[0])
                    dy = stride * float(vec[1])
                    for sign in (-1.0, 1.0):
                        cand_x = best_x.at[i].add(sign * dx)
                        cand_y = best_y.at[i].add(sign * dy)
                        if feasible_benchmark_margin(cand_x, cand_y):
                            cand_obj = float(objective_xy_jit(cand_x, cand_y))
                            if cand_obj < local_obj - 1.0e-5:
                                local_x = cand_x
                                local_y = cand_y
                ...
                for j in np.argsort(dmat[i])[:3]:
                    ...
                    normal = sep / sep_norm
                    tangent = np.array([-normal[1], normal[0]], dtype=float)
                    pair_grad = np.array([-(gx[i] + gx[j]), -(gy[i] + gy[j])], dtype=float)
                    for vec in unique_dirs([pair_grad, tangent, normal] + basis[:2])[:4]:
                        dx = 0.72 * stride * float(vec[0])
                        dy = 0.72 * stride * float(vec[1])
                        for sign in (-1.0, 1.0):
                            cand_x = best_x.at[i].add(sign * dx).at[j].add(sign * dx)
                            cand_y = best_y.at[i].add(sign * dy).at[j].add(sign * dy)
                            if feasible_benchmark_margin(cand_x, cand_y):
    ...
    def slsqp_tolerance_polish(x0, y0, obj0):
        best_x = x0
        best_y = y0
        best_obj = obj0
        if n_target != 50 or not feasible_benchmark_margin(best_x, best_y):
            return best_x, best_y, best_obj
        ...
        nx = -ey / elen
        ny = ex / elen
        iu = np.triu_indices(n_target, 1)
        target2 = float((0.99025 * min_spacing) ** 2)
        ...
        def cons_fun(z_np):
            ...
            y = z_np[n_target:]
            bvals = (
                (x[None, :] - bnd[:, 0:1]) * nx[:, None]
                + (y[None, :] - bnd[:, 1:2]) * ny[:, None]
            ).ravel()
            dx = x[:, None] - x[None, :]
            dy = y[:, None] - y[None, :]
            svals = (dx[iu] * dx[iu] + dy[iu] * dy[iu]) - target2
            return np.concatenate([bvals, svals])

        def cons_jac(z_np):
            jac = np.zeros((n_verts * n_target + len(iu[0]), 2 * n_target), dtype=float)
            row = 0
            for e in range(n_verts):
                for i in range(n_target):
                    jac[row, i] = nx[e]
                    jac[row, n_target + i] = ny[e]
                    row += 1
            x = z_np[:n_target]
            y = z_np[n_target:]
            for a, b in zip(iu[0], iu[1]):
                ...
                jac[row, a] = dx
                jac[row, b] = -dx
                jac[row, n_target + a] = dy
                jac[row, n_target + b] = -dy
                row += 1
            return jac

        z0 = np.concatenate([np.asarray(best_x, dtype=float), np.asarray(best_y, dtype=float)])
        bounds = (
            [(x_min - 0.02 * span_x, x_max + 0.02 * span_x)] * n_target
            + [(y_min - 0.02 * span_y, y_max + 0.02 * span_y)] * n_target
        )
        result = minimize(
            scipy_fun,
            z0,
            ...
            options={"maxiter": 7, "ftol": 1.0e-6, "disp": False},
        )
        if result.x is not None:
            cand_x = jnp.asarray(result.x[:n_target])
            cand_y = jnp.asarray(result.x[n_target:])
            if feasible_benchmark_margin(cand_x, cand_y):
                cand_obj = float(objective_xy_jit(cand_x, cand_y))
                if cand_obj < best_obj - 1.0e-5:
                    best_x = cand_x
    ...
    def active_cluster_nullspace_refine(x0, y0, obj0):
        best_x = x0
        best_y = y0
        best_obj = obj0
        if n_target != 50 or not feasible_benchmark_margin(best_x, best_y):
        ...
        theta = np.asarray(wd_rad, dtype=float)
        mass = np.asarray(energy_mass, dtype=float)
        wind_dirs = [float(mean_wind)] + [float(theta[i]) for i in np.argsort(-mass)[:3]]
        ...
        def unique_dirs(items):
            ...
            for vec in items:
                ...
                if norm <= 1.0e-12:
                    continue
                vec = vec / norm
                if all(abs(float(np.dot(vec, old))) < 0.992 for old in out):
                    out.append(vec)
            return out

        def project_direction(group, raw_dir, bx, by, dmat, edge_d):
            group = np.asarray(group, dtype=int)
            index = {int(t): k for k, t in enumerate(group)}
            rows = []
            ...
            for t in group:
                t = int(t)
                if nearest_bnd[t] < 0.16 * float(min_spacing):
                    row = np.zeros(2 * len(group), dtype=float)
                    k = index[t]
                    row[k] = edge_n[int(nearest_edge[t]), 0]
                    row[len(group) + k] = edge_n[int(nearest_edge[t]), 1]
                    rows.append(row)

            for a_pos, a in enumerate(group):
                a = int(a)
                for b in range(n_target):
                    if b == a or dmat[a, b] > 1.012 * float(min_spacing):
                        continue
                    sep = np.array([bx[a] - bx[b], by[a] - by[b]], dtype=float)
                    sep_norm = np.linalg.norm(sep)
                    if sep_norm <= 1.0e-12:
                        continue
                    sep /= sep_norm
                    row = np.zeros(2 * len(group), dtype=float)
                    row[a_pos] += sep[0]
                    row[len(group) + a_pos] += sep[1]
                    if b in index:
                        b_pos = index[b]
                        row[b_pos] -= sep[0]
                        row[len(group) + b_pos] -= sep[1]
                    rows.append(row)
            ...
            if rows:
                ...
                rhs = amat @ d
                try:
                    corr = amat.T @ np.linalg.solve(
                        gram + 1.0e-9 * np.eye(gram.shape[0]), rhs
                    )
                except np.linalg.LinAlgError:
                    corr = amat.T @ np.linalg.lstsq(
                        gram + 1.0e-9 * np.eye(gram.shape[0]), rhs, rcond=None
                    )[0]
                d = d - corr
                norm = np.linalg.norm(d)
                if norm <= 1.0e-10:
        ...
        for step in (0.010, 0.006, 0.0032, 0.0018):
            prod = np.asarray(turbine_power_jit(best_x, best_y), dtype=float)
            _, grad = obj_vg(jnp.concatenate([best_x, best_y]))
            gx = np.asarray(grad[:n_target], dtype=float)
            gy = np.asarray(grad[n_target:], dtype=float)
            ...
            for idx in np.argsort(-gnorm)[: min(8, n_target)]:
                ...
                if idx not in seeds:
                    seeds.append(idx)
            for idx in np.argsort(np.min(dmat, axis=1))[: min(8, n_target)]:
                idx = int(idx)
                if idx not in seeds:
                    seeds.append(idx)
            ...
            improved = False
            for i in seeds[:18]:
                near = [int(j) for j in np.argsort(dmat[i])[:6] if dmat[i, int(j)] < 1.20 * float(min_spacing)]
                base_group = [int(i)] + [j for j in near if j != int(i)]
                groups = []
                for size in (2, 3, 4, 5):
                    group = sorted(set(base_group[:size]))
                    if len(group) >= 2 and int(i) in group:
                        groups.append(group)

                for angle in wind_dirs[:3]:
                    wind = np.array([np.cos(angle), np.sin(angle)], dtype=float)
                    cross = np.array([-np.sin(angle), np.cos(angle)], dtype=float)
                    along = bx * wind[0] + by * wind[1]
                    across = bx * cross[0] + by * cross[1]
                    score = (
                        np.abs(across - across[i])
                        + 0.18 * np.abs(along - along[i])
                        + 0.015 * min_spacing * (prod - np.min(prod)) / (np.ptp(prod) + 1.0e-12)
                    )
                    col_group = sorted(set([int(j) for j in np.argsort(score)[:4]]))
                    if int(i) in col_group and len(col_group) >= 2:
                ...
                for group in groups[:7]:
                    ...
                    raw_items = []
                    raw_items.append(
                        np.concatenate([-gx[group_arr], -gy[group_arr]])
                    )
                    for angle in wind_dirs[:3]:
                        wind = np.array([np.cos(angle), np.sin(angle)], dtype=float)
                        cross = np.array([-np.sin(angle), np.cos(angle)], dtype=float)
                        raw_items.append(
                            np.concatenate(
                                [
                                    np.full(len(group_arr), wind[0]),
                                    np.full(len(group_arr), wind[1]),
                                ]
                            )
                        )
                        raw_items.append(
                            np.concatenate(
                                [
                                    np.full(len(group_arr), cross[0]),
                                    np.full(len(group_arr), cross[1]),
                                ]
                            )
                        )

                    if len(group_arr) >= 3:
                        cx = np.mean(bx[group_arr])
                        cy = np.mean(by[group_arr])
                        rx = bx[group_arr] - cx
                        ry = by[group_arr] - cy
                        raw_items.append(np.concatenate([-ry, rx]))
                        raw_items.append(np.concatenate([rx, ry]))
                    ...
                    for raw in raw_items:
                        vec = project_direction(group_arr, raw, bx, by, dmat, edge_d)
                        if np.linalg.norm(vec) <= 1.0e-10:
                            continue
                        vec = vec / (np.sqrt(np.mean(vec * vec)) + 1.0e-12)
                        unit = vec / (np.linalg.norm(vec) + 1.0e-12)
                        if any(abs(float(np.dot(unit, old))) > 0.996 for old in tried_dirs):
                            continue
                        tried_dirs.append(unit)
                        dx = jnp.asarray(stride * vec[: len(group_arr)])
                        dy = jnp.asarray(stride * vec[len(group_arr) :])
                        for sign in (-1.0, 1.0):
                            cand_x = best_x.at[group_arr].add(sign * dx)
                            cand_y = best_y.at[group_arr].add(sign * dy)
                            if feasible_benchmark_margin(cand_x, cand_y):
    ...
    starts = (start0,) if n_target > 60 else (start0, start1)
    ...
    best_x, best_y, best_obj = relaxed_tolerance_refine(best_x, best_y, best_obj)
    best_x, best_y, best_obj = active_cluster_nullspace_refine(best_x, best_y, best_obj)
    best_x, best_y, best_obj = slsqp_tolerance_polish(best_x, best_y, best_obj)
    return best_x, best_y
