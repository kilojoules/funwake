"""Augmented-Lagrangian polish of the strongest hex/Nesterov basin.

HYPOTHESIS: The best layout is spacing-active, so preserving the strongest
wind-aware hex/Nesterov basin and adding a short constraint_augmented_lagrangian
polish may recover objective movement along the feasible boundary that pure
quadratic penalty and trust-constr polishing did not exploit.

AXIS: constraint_augmented_lagrangian polish after wind-aware hexagonal
lattice starts and nesterov_momentum.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def raw_constraint_z(z, spacing_scale):
        x = z[:n_target]
        y = z[n_target:]
        return boundary_penalty(x, y, boundary) + spacing_penalty(
            x, y, min_spacing * spacing_scale
        )

    def aug_loss_z(z, lambda_k, rho, spacing_scale):
        c = raw_constraint_z(z, spacing_scale)
        return objective_z(z) + lambda_k * c + 0.5 * rho * c * c

    obj_vg = jax.jit(jax.value_and_grad(objective_z))
    con_vg = jax.jit(jax.value_and_grad(constraint_z))
    aug_vg = jax.jit(jax.value_and_grad(aug_loss_z, argnums=0))
    raw_constraint_jit = jax.jit(raw_constraint_z)
    ...
    def aug_run(z0):
        ...
        def body(carry, _):
            z, v, lambda_k, rho, lr = carry
            _, g = aug_vg(z, lambda_k, rho, 0.990)
            rms = jnp.sqrt(jnp.mean(g * g)) + 1e-12
            v = 0.22 * v - lr * g / rms
            z = z + v
            z = jnp.concatenate(
                [
                    jnp.clip(z[:n_target], x_min - 0.06 * span_x, x_max + 0.06 * span_x),
                    jnp.clip(z[n_target:], y_min - 0.06 * span_y, y_max + 0.06 * span_y),
                ]
            )
            c = raw_constraint_jit(z, 0.990)
            lambda_k += rho * c
            lambda_k = jnp.maximum(lambda_k, 0.0)
            rho = jnp.minimum(rho * 1.004, 220.0)
            lr = lr * 0.996
            return (z, v, lambda_k, rho, lr), None

        (z, _, _, _, _), _ = jax.lax.scan(body, (z0, v0, lambda0, rho0, lr0), jnp.arange(520))
        return z
    ...
    z3 = aug_run(jnp.concatenate([best_x, best_y]))
    best_x, best_y, best_obj = consider(best_x, best_y, best_obj, z3)
    z4 = polish_run(jnp.concatenate([best_x, best_y]), 0.990)
    best_x, best_y, best_obj = consider(best_x, best_y, best_obj, z4)
