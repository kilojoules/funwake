"""Affine recovery with train-only SLSQP tolerance polish.

HYPOTHESIS: The incumbent polish is conservative because most hand-coded moves
require full min_spacing even though the benchmark accepts the 0.99 spacing
tolerance. A short explicit-constraint SLSQP pass from the incumbent can use
that slack more coherently than axis-aligned micro moves without changing the
ROWP path.
...
gradient polish, benchmark-tolerance pair and turbine moves, then train-only
SLSQP with vectorized boundary and spacing constraints.
