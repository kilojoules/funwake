"""Mixed-frame exact-grid AL-NAdam ensemble.

HYPOTHESIS: The best exact-grid basin is strong, but the single dominant-wind
frame may exclude a nearby orientation family.  Keeping the incumbent
dominant-frame BO starts while adding a compact absolute-angle BO tournament
can expose a different basin without losing the known 5583 GWh path.

AXIS: incumbent BO exact grid plus absolute-orientation BO starts, followed by
the same staged augmented-Lagrangian NAdam halving and projection.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    @jax.jit
    def generate_dominant_grid(params):
    ...
    @jax.jit
    def generate_absolute_grid(params):
        theta, sx, sy, ox, oy, shear, aspect = params
        sy_actual = sy * aspect
        n_side = int(np.sqrt(n_target)) + 15
        ii, jj = jnp.meshgrid(
            jnp.arange(n_side) - n_side // 2, jnp.arange(n_side) - n_side // 2
    ...
    dom_low = jnp.array([1.02, 1.02, -jnp.pi / 4, x_min, y_min, -0.6, 0.8])
    dom_high = jnp.array([5.0, 5.0, jnp.pi / 4, x_max, y_max, 0.6, 1.3])
    abs_low = jnp.array([-jnp.pi, 1.02, 1.02, x_min, y_min, -0.6, 0.8])
    abs_high = jnp.array([jnp.pi, 5.0, 5.0, x_max, y_max, 0.6, 1.3])
    ...
    def bo_layouts(generate_grid, bounds_low, bounds_high, seed, n_init, n_iter, n_cand, n_keep):
        def scale_params(p):
        ...
        for _ in range(n_iter):
            key, subkey = jax.random.split(key)
            cand = jax.random.uniform(subkey, (n_cand, 7))
        ...
        top_params = x_raw[jnp.argsort(y_raw)[-n_keep:][::-1]]
        return [generate_grid(scale_params(p)) for p in top_params]
    ...
    layouts = bo_layouts(
        generate_dominant_grid, dom_low, dom_high, 42, 12, 18, 800, 8
    )
    layouts += bo_layouts(
        generate_absolute_grid, abs_low, abs_high, 131, 10, 10, 600, 4
    )
