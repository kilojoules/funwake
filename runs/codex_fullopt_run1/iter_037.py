"""Final pressure-balanced column shear on the incumbent basin.

HYPOTHESIS: Attempt 36 showed pressure-balanced column shears are not a good
pre-contact perturbation because they send the later contact polish to a weaker
endpoint. Applying the same move family only after all incumbent polishing can
test the idea without degrading the established layout.

AXIS: incumbent affine recovery, active-pair release, wake-column group polish,
size-gated contact-clique and fine polish, then final train-size-gated
pressure-balanced column shears accepted only against the final objective.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def pressure_column_shear_refine(x0, y0, obj0):
        best_x = x0
        best_y = y0
        best_obj = obj0
        ...
        for idx in np.argsort(-mass)[:2]:
            angle = float(theta[idx])
            if all(abs(np.cos(angle - old)) < 0.990 for old in angles):
                angles.append(angle)

        def unique_dirs(items, max_dot=0.994):
            out = []
            for vec in items:
                vec = np.asarray(vec, dtype=float)
                norm = np.linalg.norm(vec)
        ...
        for step in (0.009, 0.005):
            ...
            seeds = []
            for idx in np.argsort(prod)[: min(6, n_target)]:
                seeds.append(int(idx))
            for idx in np.argsort(-gnorm)[: min(4, n_target)]:
                idx = int(idx)
                if idx not in seeds:
                    seeds.append(idx)
            ...
            for angle in angles:
                ...
                along = bx * wind[0] + by * wind[1]
                across = bx * cross[0] + by * cross[1]
                grad_cross = -(gx * cross[0] + gy * cross[1])
                grad_wind = -(gx * wind[0] + gy * wind[1])
                ...
                for i in seeds:
                    ...
                    for size in (3, 4):
                        group = np.asarray(near[:size], dtype=int)
                        if len(group) != size or i not in group:
                            continue
                        ...
                        centered_along = along[group] - np.mean(along[group])
                        denom_along = np.max(np.abs(centered_along)) + 1.0e-12
                        rank_w = centered_along / denom_along

                        p_w = np.mean(prod[group]) - prod[group]
                        p_w = p_w - np.mean(p_w)
                        p_w = p_w / (np.max(np.abs(p_w)) + 1.0e-12)

                        gc_w = grad_cross[group] - np.mean(grad_cross[group])
                        gc_w = gc_w / (np.max(np.abs(gc_w)) + 1.0e-12)
                        gw_w = grad_wind[group] - np.mean(grad_wind[group])
                        gw_w = gw_w / (np.max(np.abs(gw_w)) + 1.0e-12)
                        ...
                        patterns = (rank_w, p_w, 0.6 * p_w + 0.4 * gc_w, gw_w)

                        for weights_pat in patterns:
                            weights_pat = np.asarray(weights_pat, dtype=float)
                            weights_pat = weights_pat - np.mean(weights_pat)
                            scale = np.max(np.abs(weights_pat)) + 1.0e-12
                            weights_pat = weights_pat / scale
                            if np.max(np.abs(weights_pat)) < 1.0e-9:
                                continue
                            for vec in dirs[:2]:
                                offsets_x = jnp.asarray(stride * weights_pat * float(vec[0]))
                                offsets_y = jnp.asarray(stride * weights_pat * float(vec[1]))
                                for sign in (-1.0, 1.0):
                                    cand_x = best_x.at[group].add(sign * offsets_x)
                                    cand_y = best_y.at[group].add(sign * offsets_y)
                                    if feasible_exact(cand_x, cand_y):
                                        cand_obj = float(objective_xy_jit(cand_x, cand_y))
                                        if cand_obj < local_obj - 1.0e-5:
                                            local_x = cand_x
    ...
    best_x, best_y, best_obj = pressure_column_shear_refine(best_x, best_y, best_obj)
