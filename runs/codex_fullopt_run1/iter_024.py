"""Targeted turbine-level coordinate search after incumbent recovery.

HYPOTHESIS: The repeated 5560.8 GWh basin is close to a constrained local
optimum, but a few low-producing turbines may still admit small feasible
one-at-a-time moves that the smooth penalty trajectory does not take.

AXIS: greedy coordinate_search over selected low-power turbines and weighted
wind/crosswind directions after the established nesterov_momentum recovery.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def coordinate_refine(x0, y0, obj0):
        best_x = x0
        best_y = y0
        best_obj = obj0
    ...
    best_x, best_y, best_obj = consider(best_x, best_y, best_obj, z)
    best_x, best_y, best_obj = coordinate_refine(best_x, best_y, best_obj)
