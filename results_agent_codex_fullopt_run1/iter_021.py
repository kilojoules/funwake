"""BayesianOptimization-style surrogate search over lattice seed parameters.

HYPOTHESIS: Bayesian optimization can spend a small number of expensive seed
evaluations on a GP surrogate over coupled hex-lattice parameters, selecting a
different basin than blind population search before the proven momentum polish.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    bounds = np.array(
        [
            ...
            [0.991, 1.018],   # spacing multiplier
            [0.00, 1.00],     # x phase
            [0.00, 1.00],     # y phase
            [-0.020, 0.020],  # shear
            [-0.95, 0.95],    # ranking-angle offset
            [-0.55, 0.55],    # edge/interior ranking bias
        ],
        dtype=float,
    )
    ...
    def hexagonal_lattice(params):
        ...
        margin = max(0.0, min(0.055, 0.010 + 0.010 * abs(radial_bias))) * min_spacing
        ...
        rank = (
            ...
            + 0.055 * min_spacing * jnp.sin((px + 0.39 * py) / (2.65 * min_spacing))
        )
