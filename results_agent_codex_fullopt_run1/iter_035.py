"""Affine wake-column recovery with contact-clique polish.

HYPOTHESIS: The wake-column polish is still the strongest basin opener on the
50-turbine training farm, but the contact-clique pass is too expensive on the
larger held-out farm. Applying it only on smaller layouts should preserve the
train gain while keeping ROWP inside the generalization timeout.

AXIS: incumbent affine recovery, active-pair release, wake-column group polish,
then size-gated contact-pair rotations and boundary-edge clique slides.
