"""Latin-hypercube wind-aware hex starts with proven Nesterov polish.

HYPOTHESIS: Deterministic init_latin_hypercube sampling of hex-lattice
orientation, phase, spacing, and shear may cover a basin missed by the
hand-picked starts and the raw-AEP differential-evolution seed, while the
same full Nesterov polish preserves the known strong incumbent basin.

AXIS: init_latin_hypercube over wind-aware hexagonal lattice parameters plus
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    sampler = qmc.LatinHypercube(d=4, seed=13013)
    lhs = sampler.random(2)
    starts = [
        hexagonal_lattice(base_angle, 1.000, 0.24, 0.000),
        hexagonal_lattice(
            base_angle - 0.42 + 0.84 * lhs[0, 0],
            0.994 + 0.020 * lhs[0, 1],
            lhs[0, 2],
            -0.018 + 0.036 * lhs[0, 3],
        ),
        hexagonal_lattice(
            base_angle - 0.42 + 0.84 * lhs[1, 0],
            0.994 + 0.020 * lhs[1, 1],
            lhs[1, 2],
            -0.018 + 0.036 * lhs[1, 3],
        ),
