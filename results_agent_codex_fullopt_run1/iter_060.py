"""Exact-size BO grid with no-TopFarm AL-NAdam recovery.

HYPOTHESIS: The last guarded TopFarm polish did not beat the underlying
custom exact-grid basin, and independent random TopFarm restarts score below
baseline. Returning to the no-TopFarm augmented-Lagrangian path should recover
the strongest basin without accepting penalty-solver drift.

AXIS: BO-tuned multi-parameter exact grid plus staged AL-NAdam halving, with
direct projection and no TopFarm warm-start polish.
