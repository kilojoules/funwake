"""Tiny genetic_algorithm over hex-lattice parameters with incumbent retention.

HYPOTHESIS: a small genetic_algorithm can combine useful orientation, phase,
spacing, and shear traits from several feasible lattice seeds through crossover
and mutation, while retaining the two exact best-known starts should recover
the 5560.8 GWh basin if the evolved seed is not better.

AXIS: deterministic crossover and mutation search for one extra hex-lattice
seed plus the two
incumbent wind-aware hex starts and nesterov_momentum local penalty polish.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def genetic_seed():
        lo = np.array([-0.42, 0.995, 0.00, -0.018], dtype=float)
        hi = np.array([0.42, 1.012, 1.00, 0.018], dtype=float)
        population = np.array(
            [
                [0.16, 1.006, 0.58, 0.015],
                [0.00, 1.000, 0.24, 0.000],
                [-0.18, 1.002, 0.82, -0.010],
                [0.31, 0.997, 0.12, 0.009],
                [-0.34, 1.010, 0.46, 0.004],
            ],
            dtype=float,
        )
        values = np.array([genetic_value(p) for p in population], dtype=float)

        for generation in range(4):
            order = np.argsort(values)
            elites = population[order[:3]].copy()
            children = [elites[0], elites[1]]
            for k in range(4):
                a = elites[k % 3]
                b = elites[(k + 1) % 3]
                mix = 0.35 + 0.10 * ((generation + k) % 4)
                child = mix * a + (1.0 - mix) * b
                mutation = np.array(
                    [
                        0.055 * np.sin(1.7 * (generation + 1) * (k + 1)),
                        0.0018 * np.cos(1.1 * (generation + 2) * (k + 1)),
                        0.120 * np.sin(0.9 * (generation + 3) * (k + 1)),
                        0.0045 * np.cos(1.3 * (generation + 4) * (k + 1)),
                    ],
                    dtype=float,
                )
                child = np.minimum(np.maximum(child + mutation, lo), hi)
    ...
    genetic_run = make_runner(88.0, 0.0042, 900, 2600, 0.35)
    ...
    start0 = hexagonal_lattice(base_angle, 1.000, 0.24, 0.000)
    start1 = hexagonal_lattice(base_angle + 0.16, 1.006, 0.58, 0.015)
    start2 = hexagonal_lattice(
        base_angle + float(genetic_params[0]),
        float(genetic_params[1]),
        float(genetic_params[2]),
        float(genetic_params[3]),
    )
    ...
    z = genetic_run(jnp.concatenate([start2[0], start2[1]]), 1.0)
    best_x, best_y, best_obj = consider(best_x, best_y, best_obj, z)
