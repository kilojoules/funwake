"""Energy-weighted exact BO grid with staged augmented-Lagrangian NAdam.

HYPOTHESIS: The best exact-grid basin may be slightly mis-oriented because the
previous dominant wind angle used frequency only; weighting by wind-speed cubed
should align the lattice to the energy-carrying directions before the same
staged AL-NAdam search.

AXIS: energy-weighted BO exact grid plus staged AL-NAdam halving.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    wind_energy = weights * ws**3
    vx = jnp.sum(jnp.cos(wd_rad) * wind_energy)
    vy = jnp.sum(jnp.sin(wd_rad) * wind_energy)
