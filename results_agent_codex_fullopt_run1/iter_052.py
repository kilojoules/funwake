"""Exact-grid BO with local parameter exploit and staged AL-NAdam.

HYPOTHESIS: The best exact-grid basin is sensitive to the seven lattice
parameters, and the global BO pass may stop near the good basin without
exploiting it.  A cheap local perturbation search around the best BO lattice
parameters should give AL-NAdam a slightly better starting basin.

AXIS: BO exact grid plus local normalized-parameter exploit before AL-NAdam.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    top_raw = x_raw[jnp.argsort(y_raw)[-8:][::-1]]
    noise_scale = jnp.array([0.025, 0.025, 0.035, 0.04, 0.04, 0.035, 0.025])
    local = [top_raw]
    for scale in (1.0, 1.8, 3.0):
        key, subkey = jax.random.split(key)
        offsets = jax.random.normal(subkey, (top_raw.shape[0], 4, 7)) * noise_scale * scale
        cand = jnp.clip(top_raw[:, None, :] + offsets, 0.0, 1.0).reshape((-1, 7))
        local.append(cand)
    local_raw = jnp.vstack(local)
    local_y = jnp.array([grid_score(scale_params(p)) for p in local_raw])
    top_params = local_raw[jnp.argsort(local_y)[-10:][::-1]]
