"""Exact-size BO grid with projected gradient fan polish.

HYPOTHESIS: The surplus basin is good but pruning is fragile; optimizing an
exact n_target lattice with shear/aspect parameters can avoid the pruning loss
while a final projected gradient/affine fan can escape the last few meters of
AL-NAdam stalling near active constraints.

AXIS: BO-tuned exact grid, staged AL-NAdam halving, then projected
constraint-aware gradient and affine line-search polish.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    @jax.jit
    def turbine_value(x, y):
        r = sim(x, y, ws_amb=ws, wd_amb=wd, ti_amb=None)
        return jnp.sum(r.power()[:, :n_target] * weights[:, None], axis=0)
    ...
    grad_aep = jax.jit(jax.grad(aep_value, argnums=(0, 1)))

    def select_best(cand_x, cand_y):
        bx = jnp.stack(cand_x)
        by = jnp.stack(cand_y)
        vals = jax.vmap(aep_value)(bx, by)
    ...
    def fan_polish(x, y):
        ...
        wind_v = jnp.array([-jnp.sin(dominant_wd), jnp.cos(dominant_wd)])

        for _ in range(2):
            gx, gy = grad_aep(x, y)
            gnorm = jnp.sqrt(gx * gx + gy * gy) + 1e-12
            ux = gx / gnorm
            uy = gy / gnorm

            indiv = turbine_value(x, y)
            low = jnp.argsort(indiv)[: min(14, n_target)]
            mask_low = jnp.zeros((n_target,)).at[low].set(1.0)

            cx = jnp.mean(x)
            cy = jnp.mean(y)
            rx = x - cx
            ry = y - cy
            along = rx * wind_u[0] + ry * wind_u[1]
            cross = rx * wind_v[0] + ry * wind_v[1]
            along_z = along / (jnp.std(along) + 1e-9)
            cross_z = cross / (jnp.std(cross) + 1e-9)

            cand_x = [x]
            cand_y = [y]
            for frac in (0.003, 0.006, 0.010, 0.015, -0.003, -0.006):
                step = frac * min_spacing
                px, py = project(x + step * ux, y + step * uy)
                cand_x.append(px)
                cand_y.append(py)
                px, py = project(x + step * mask_low * ux, y + step * mask_low * uy)
                cand_x.append(px)
                cand_y.append(py)

            for frac in (0.004, 0.008, -0.004):
                step = frac * min_spacing
                px, py = project(x + step * cross_z * wind_v[0], y + step * cross_z * wind_v[1])
                cand_x.append(px)
                cand_y.append(py)
                px, py = project(x + step * along_z * wind_u[0], y + step * along_z * wind_u[1])
                cand_x.append(px)
                cand_y.append(py)
            ...
            for su, sv, sh in (
                ...
                na = along * (1.0 + su) + sh * cross
                nc = cross * (1.0 + sv)
                tx = cx + na * wind_u[0] + nc * wind_v[0]
                ty = cy + na * wind_u[1] + nc * wind_v[1]
                px, py = project(tx, ty)
                cand_x.append(px)
                cand_y.append(py)

            x, y = select_best(cand_x, cand_y)

        return project(x, y)

    return fan_polish(cur_x[0], cur_y[0])
