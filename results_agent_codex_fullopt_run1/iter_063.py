"""Exact-size BO grid with deterministic projected perturbation polish.

HYPOTHESIS: The BO exact-grid AL-NAdam path repeatedly reaches the same strong
5583 GWh basin, but the final projection may leave tiny wake-alignment losses.
A deterministic perturb-and-project tournament around the incumbent can test
nearby feasible layouts without another broad initialization search.

AXIS: incumbent BO exact grid plus staged AL-NAdam halving, followed by a
compact local tournament of radial, crosswind, and rank-wave nudges.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def local_polish(x, y):
        center = jnp.mean(boundary, axis=0)
        rel_x = x - center[0]
        rel_y = y - center[1]
        rad_norm = jnp.sqrt(rel_x**2 + rel_y**2 + 1e-9)
        ux = rel_x / rad_norm
        uy = rel_y / rad_norm
        wx = jnp.cos(dominant_wd)
        wy = jnp.sin(dominant_wd)
        cx_dir = -wy
        cy_dir = wx
        rank = (x - jnp.mean(x)) * wx + (y - jnp.mean(y)) * wy
        rank = rank / (jnp.std(rank) + 1e-6)
        wave1 = jnp.sin(1.7 * rank)
        wave2 = jnp.cos(2.3 * rank)
        edge = jnp.tanh((rad_norm - jnp.mean(rad_norm)) / (jnp.std(rad_norm) + 1e-6))
        ...
        xs = []
        ys = []
        for radial_amp, cross_amp, edge_amp in variant_offsets:
            dx = min_spacing * (
                radial_amp * wave2 * ux
                + cross_amp * wave1 * cx_dir
                + edge_amp * edge * ux
            )
            dy = min_spacing * (
                radial_amp * wave2 * uy
                + cross_amp * wave1 * cy_dir
                + edge_amp * edge * uy
            )
            px, py = project(x + dx, y + dy)
            xs.append(px)
            ys.append(py)
        cand_x = jnp.stack(xs)
        cand_y = jnp.stack(ys)
        best = jnp.argmin(jax.vmap(aep_obj)(cand_x, cand_y))
        return cand_x[best], cand_y[best]

    return local_polish(cur_x[0], cur_y[0])
