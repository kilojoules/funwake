"""Simulated-annealing escape from the best hexagonal Nesterov basin.

HYPOTHESIS: The best layout appears locally spacing-active; a few feasible
simulated_annealing shakes around that layout may cross small wake-interaction
ridges that deterministic Nesterov and trust-constr polish both leave intact.

AXIS: simulated_annealing with constraint projection, followed by short
nesterov_momentum recovery from init_hexagonal wind_aware starts.
...
def optimize(sim, n_target, boundary, min_spacing, wd, ws, weights):
    ...
    def project_spacing(x, y, target):
        for _ in range(6):
            dx = x[:, None] - x[None, :]
            dy = y[:, None] - y[None, :]
            dist = jnp.sqrt(dx * dx + dy * dy + jnp.eye(n_target) * 1e12)
            viol = jnp.maximum(0.0, target - dist)
            x = x + 0.48 * jnp.sum(viol * dx / (dist + 1e-9), axis=1)
            y = y + 0.48 * jnp.sum(viol * dy / (dist + 1e-9), axis=1)
            x, y = project_boundary(x, y, 1.5)
        return x, y

    def repair(x, y):
        x, y = project_boundary(x, y, 1.5)
        return project_spacing(x, y, min_spacing * 0.992)
    ...
    escape_run = make_runner(46.0, 0.020, 180, 1120, 0.30)
    polish_run = make_runner(19.0, 0.08, 100, 760, 0.16)
    ...
    def annealed_escape(best_x, best_y, best_obj):
        key = jax.random.PRNGKey(20260426)
        current_x = best_x
        current_y = best_y
        current_obj = best_obj
        for k in range(3):
            temp = (0.26 - 0.055 * k) * min_spacing
            key, kx, ky = jax.random.split(key, 3)
            cand_x = current_x + temp * jax.random.normal(kx, (n_target,))
            cand_y = current_y + temp * jax.random.normal(ky, (n_target,))
            cand_x = jnp.clip(cand_x, x_min - 0.02 * span_x, x_max + 0.02 * span_x)
            cand_y = jnp.clip(cand_y, y_min - 0.02 * span_y, y_max + 0.02 * span_y)
            cand_x, cand_y = repair(cand_x, cand_y)
            if feasible(cand_x, cand_y):
                cand_obj = float(objective_xy(cand_x, cand_y))
                accept = cand_obj < current_obj
                uphill = cand_obj - current_obj
                threshold = 0.80 / (1.0 + k)
                if (not accept) and uphill < threshold:
                    accept = True
                if accept:
                    current_x, current_y, current_obj = cand_x, cand_y, cand_obj

        z = escape_run(jnp.concatenate([current_x, current_y]), 0.998)
    ...
    best_x, best_y, best_obj = annealed_escape(best_x, best_y, best_obj)

    z2 = polish_run(jnp.concatenate([best_x, best_y]), 0.9908)
