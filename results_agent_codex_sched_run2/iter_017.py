"""Dual-bump schedule with later settling and softer late alpha.

HYPOTHESIS: Iteration 16's later settling improved AEP and feasibility; the
late penalty multiplier may now be stronger than needed, so a softer alpha
boost could keep layouts feasible while recovering more objective value.
AXIS: alpha_linear_ramp/late alpha softening on the iter_016 dual-bump schedule.
LESSON: pending
"""
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    alpha = alpha * (1.0 + 5.0 * late + 38.0 * late * late)
