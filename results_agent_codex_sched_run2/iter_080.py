"""Incumbent basin with a tiny deterministic alpha relief.

HYPOTHESIS: The previous positive alpha bump reduced train AEP, suggesting
mid-run constraint pressure may already be slightly too high. A tiny alpha
relief during the noisy search window may let wake-improving moves persist,
while the unchanged late repair envelope restores feasibility.
AXIS: small mid-run alpha anti-bump on proven noisy LR backbone.
LESSON: pending
"""
import jax
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    relief = jnp.exp(-0.5 * ((t - 0.46) / 0.11) ** 2)
    alpha = alpha * (1.0 - 0.008 * noise_gate * relief)
