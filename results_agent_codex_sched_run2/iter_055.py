"""Conservative scheduled Adam memory with stronger final repair.

HYPOTHESIS: A large late beta increase made boundary repair too sluggish on
the stressed polygon. Keep the dynamic-beta idea, but make the memory ramp
small and pay for it with a slightly stronger end-phase alpha multiplier.
AXIS: dynamic Adam betas plus final alpha repair on the best noisy backbone.
LESSON: pending
"""
import jax
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    alpha = alpha * (1.0 + 8.5 * late + 66.0 * late * late)

    beta1 = 0.08 + 0.06 * late
    beta2 = 0.18 + 0.10 * late
