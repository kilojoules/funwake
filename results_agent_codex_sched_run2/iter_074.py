"""Incumbent basin with a small deterministic alpha bump.

HYPOTHESIS: The high-AEP basin is sensitive to the LR perturbation, so keep
that entry path and test whether a tiny mid-run constraint bump can stabilize
feasibility without materially changing the objective trajectory.
AXIS: small mid-run alpha bump on the proven noisy LR backbone.
LESSON: pending
"""
import jax
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    bump = jnp.exp(-0.5 * ((t - 0.46) / 0.11) ** 2)
    alpha = alpha * (1.0 + 0.006 * noise_gate * bump)
