"""Quadratic alpha ramp on the best noisy LR path.

HYPOTHESIS: Replacing the best schedule's linear alpha growth with a convex
quadratic ramp can reduce mid-run penalty drag during wake-basin selection
while ending with stronger feasibility repair.
AXIS: alpha_quadratic_ramp on the current best lr_noise_injection backbone.
LESSON: pending
"""
import jax
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    alpha = alpha0 * (1.0 + 0.70 * t * t)
    alpha = 20.0 * alpha * lr0 / jnp.maximum(lr_base, 1e-10)
