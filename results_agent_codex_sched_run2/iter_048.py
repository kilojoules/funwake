"""Second alternate LR-noise seed on the best schedule.

HYPOTHESIS: Nearby deterministic LR-noise seeds can sample slightly different
layout basins while preserving the best-known alpha and LR envelope.
AXIS: lr_noise_injection seed sweep on the best alpha_linear_ramp backbone.
LESSON: pending
"""
import jax
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    key = jax.random.fold_in(jax.random.PRNGKey(20020), step.astype(jnp.uint32))
    ...
    lr = lr_base
    lr += lr_base * (0.08 * noise_gate) * lr_noise
