"""Slower exponential cooling on the incumbent noisy envelope.

HYPOTHESIS: The previous floor sweep was mostly inactive because the
exponential tail remains above the clamp. Reducing the decay exponent should
create a real late learning-rate increase while preserving the same noise and
constraint repair path that finds the current best basin.
AXIS: lr_exponential_decay exponent sweep on proven noisy envelope.
LESSON: pending
"""
import jax
...
def schedule_fn(step, total_steps, lr0, alpha0):
    ...
    lr_base = jnp.maximum(lr_start * jnp.exp(-7.80 * t * t), lr_floor)
