"""Merge LUMI 500-multistart shard JSONs.

Usage:
    pixi run python experiments/H_codex_six_runs/merge_lumi_500_shards.py
"""
import glob
import json
import os
...
def main():
    merged = {}
    for shard in sorted(glob.glob(os.path.join(HERE, "matrix_lumi_500_shard*.json"))):
        d = json.load(open(shard))
        merged.update(d)
        print(f"  merged {os.path.basename(shard)}: {len(d)} entries (running total {len(merged)})")
    out = os.path.join(HERE, "matrix_lumi_500_results.json")
    ...
    out2 = os.path.join(HERE, "matrix_lumi_500_bestof.json")
