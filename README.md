...
bash setup.sh          # clone pixwake + compute the baselines (one time)
...
(500 multi-start SGD). Timeout is 60 s per run.
...
pixi run python agent_cli.py \
    ...
    --wind-csv /path/to/wind_timeseries.csv \
    --hot-start results/seed_schedule.py \
    --time-budget 18000            # 5 hours
```

`--wind-csv` is required and no sample ships in the repo; supply your own
wind resource. Keep `--schedule-only` (see the warning above).
