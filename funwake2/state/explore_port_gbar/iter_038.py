"""Schedule: phase-transition the Adam moments (beta1/beta2 ramp) exactly in
sync with the alpha-engagement window instead of on an independently-tuned
beta_p0 (isolated single-lever change).

Diagnosis vs. the parent: beta_p0 = 0.50 - 0.10*scale_pos is its own,
separately-fitted onset for the beta1/beta2 ramp, while relax_p1 = 0.45 +
0.10*extremity + 0.08*scale_pos is the onset at which the constraint penalty
(alpha) actually starts ramping toward full strength. For large-N farms
(scale_pos > 0, e.g. dei_n80_omnidir with 80 turbines and many constraint
pairs) these two onsets diverge in opposite directions: beta_p0 moves EARLIER
while relax_p1 moves LATER, so Adam's moments start averaging toward the
low-momentum/high-adaptivity regime well before the penalty has begun
engaging. During the (extended, for large N) exploration hold this
prematurely damps the very momentum that would otherwise help the optimizer
escape local traps in a densely-packed, many-pair layout with noisy
constraint gradients -- consistent with dei_n80_omnidir being the flattest
farm in the fitness set (+0.0005%).

This applies the previously-untried "phase-transition Adam moments with the
alpha phase" idea from the prior-art menu: beta1/beta2 now ramp starting at
the SAME onset as alpha's own ramp (relax_p1), so momentum only starts
smoothing once the constraint penalty itself starts engaging, restoring
coherent exploration on large-N farms without touching lr, alpha, relax,
spike, or hold_frac at all. _BETA_P0 / _BETA_P0_SCALE_SPAN are removed since
beta_p0 is now derived, not independently fit.

All other machinery (self-contained inverse-time product lr decay to
gamma_min, short linear warmup, density-extremity relax/spike/hold, absolute-N
scale on hold/spike/relax_p1, and the alpha terminal denominator floor) is
...
# Adam moments: native early -> mildly averaged late. Onset is now derived
# from relax_p1 (the alpha-engagement onset) below, not fit independently.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Phase-transition the Adam moments in sync with alpha's own engagement
    # onset (relax_p1) rather than an independently-tuned beta_p0, so
    # momentum only starts averaging once the constraint penalty itself
    # starts ramping -- keeps exploration coherent through the (possibly
    # extended) large-N hold instead of damping it early.
    r = _ramp(p, relax_p1, _BETA_P1)
