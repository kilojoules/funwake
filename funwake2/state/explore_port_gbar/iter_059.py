"""Schedule: add an explicit multiplicative cap on the terminal alpha value
for the largest-N farms, on top of the parent's denominator-floor bound
(isolated single-lever change).

Diagnosis vs. the parent: the parent already bounds alpha's denominator via
`alpha_denom_floor = gamma_min * (1 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos)`,
which prevents alpha from diverging all the way to native's `alpha0/gamma_min`
ceiling. But dei_n80_omnidir (n=80, by far the largest farm in the fitness
set) is still flat (+0.0005%), meaning the residual terminal alpha value --
`alpha0*diam*relax*spike / alpha_denom_floor` -- is still large enough to
keep late-stage optimization constraint-dominated on this many-pair farm.
The denominator floor bounds the *rate of blow-up as lr_base->gamma_min* but
doesn't independently cap the *shaped numerator* (relax*spike, which still
grows toward `1 + spike0` as progress -> 1). This change adds a second,
independent lever: an explicit ceiling on alpha itself, expressed as a
multiple of alpha's "natural" scale at lr0 (`alpha0*diam/lr0`, i.e. what
alpha would be with an unshaped denominator equal to the initial
exploration lr). The cap is gated strictly to scale_pos > 0.5 (only
n_turbines well above _N_REF=30, e.g. n80's scale_pos ~= 0.7) so dei_n50
(scale_pos ~= 0.37) and the small/mid farms (parque_n10/n20, scale_pos = 0)
are completely unaffected and match the parent exactly -- isolating the
change to the one farm the diagnosis identifies as still under-served,
consistent with the prior-art "bounded alpha plateau" hypothesis (frontier
section, alpha row) that the parent only partially implemented.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0, the parent's alpha
denominator floor) is unchanged.
"""
...
# NEW: additionally cap alpha's absolute value (independent of the
# denominator floor above) for the largest-N farms only, so the shaped
# numerator (relax*spike) cannot itself keep alpha constraint-dominated
# late in the run on very crowded layouts.
_ALPHA_CAP_MULT = 20.0
_ALPHA_CAP_SCALE_THRESH = 0.5
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # NEW: independent absolute cap on alpha, only for the largest-N farms
    # (scale_pos above threshold), expressed relative to alpha's natural
    # scale at the initial exploration lr -- prevents the shaped numerator
    # (relax*spike, which still ramps toward 1+spike0) from keeping alpha
    # constraint-dominated late in a very crowded layout even after the
    # denominator floor above has already bounded the 1/lr_base blow-up.
    if scale_pos > _ALPHA_CAP_SCALE_THRESH:
        alpha_natural = alpha0 * diam / lr0
        alpha_cap = alpha_natural * (1.0 + spike0) * _ALPHA_CAP_MULT
        alpha = jnp.minimum(alpha, alpha_cap)
