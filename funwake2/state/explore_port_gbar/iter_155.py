...
N~=60-100+ band (scale_pos in [0.5, 0.78], effectively only dei_n80_omnidir)
via `high_n_ramp`. That gate leaves a whole band un-served: dei_n50 and
dei_n50_uniform sit at scale_pos~=0.37 -- well above the sparse parque farms
(scale_pos=0) but below the high-N gate's onset (0.5) -- so they only get the
small *linear* scale_pos terms, none of the targeted relief that has been
raising the score for the n80 band across recent generations.

dei_n50_uniform is consistently the single worst-scoring farm (+0.014%,
roughly half of dei_n50's own +0.028%), even though it shares identical
N/D/min_spacing (hence identical `size`/`scale_pos`) with dei_n50 -- the two
farms differ only in wind rose (omnidirectional-ish vs directional), which
this schedule cannot see directly, but the *shape* of the schedule (hold
length, relax delay) can still be adapted for the N~50 band as a group,
consistent with the mechanism that already helped n80.

This variant adds a second, independent gated bump `mid_n_ramp` centered on
scale_pos in [0.2, 0.45] (so N~40-50) that (a) grants a bit more exploration
hold and (b) delays constraint engagement a bit further -- mirroring what
already worked for the n80 band, but with smaller spans so any residual
overlap onto the high-N band (or the unseen ROWP test farm, N=74,
scale_pos~=0.65) stays a mild nudge rather than a restructuring. The bump is
suppressed by `(1 - high_n_ramp)` so it fades out once the high-N gate takes
over, avoiding double-counting on n80. Parque farms (scale_pos=0) and every
other mechanism are untouched.
"""
...
# Second, independent gate centered on the N~=40-50 band (scale_pos in
# [0.2, 0.45]) -- targets dei_n50 / dei_n50_uniform, which sit in the gap
# between the sparse parque farms (scale_pos=0) and the high-N gate's onset
# (0.5). Suppressed by (1 - high_n_ramp) so it fades out once the high-N gate
# engages, avoiding double relief on the n80 band.
_MID_N_P0 = 0.2
_MID_N_P1 = 0.45
_MID_HOLD_FRAC_SPAN = 0.03      # a bit more exploration hold for N~=40-50 farms
_MID_RELAX_P1_SPAN = 0.04       # a bit more delay before constraints engage
_MID_RELAX0_SPAN = -0.03        # a bit lower early-exploration penalty floor
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_ramp = (_smoothstep_scalar(scale_pos, _MID_N_P0, _MID_N_P1)
                  * (1.0 - high_n_ramp))                # N~=40-50 band, fades once high-N gate takes over
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _MID_HOLD_FRAC_SPAN * mid_n_ramp)     # extra gated hold for N~=40-50 band
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              ...
              + _MID_RELAX0_SPAN * mid_n_ramp)          # extra gated lower floor for N~=40-50 band
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _MID_RELAX_P1_SPAN * mid_n_ramp)      # extra gated delay for N~=40-50 band
