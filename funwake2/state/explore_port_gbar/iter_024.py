"""Schedule: phase-couple the Adam moment ramp to the alpha (feasibility) ramp.

Diagnosis vs. the current best (density+absolute-N adaptive relax/spike/hold
on top of self-contained inverse-time product lr decay + short linear
warmup): the penalty multiplier `alpha` already ramps from a relaxed early
value to a spiked terminal value over a farm-adaptive window
[_RELAX_P0, relax_p1]. But the Adam moments (beta1/beta2) ramp on a
*separate*, only loosely related window [_BETA_P0(+scale), _BETA_P1]. This
mismatch means that when the penalty gradient term starts dominating (as
alpha ramps up), the optimizer is not yet using the more-averaged/adaptive
moments meant to stabilize it, and conversely the moments can start
smoothing well before there's much penalty signal to stabilize -- wasted
averaging during the pure-AEP exploration phase, and insufficient averaging
right as the constraint term engages.

Fix (prior-art idea: "phase-transition the Adam moments with the alpha
phase", exact-penalty/ALM literature): drive the beta1/beta2 ramp off the
*same* progress window as the alpha ramp (relax_p0 .. relax_p1), with only a
small, farm-adaptive lead-in so momentum starts smoothing slightly before
the penalty fully engages rather than exactly in lockstep or independently.
This makes the "when to start trusting averaged gradients" decision follow
the same farm-adaptive density/size signal that already governs "when to
start enforcing constraints strictly", instead of a second, uncoupled
heuristic window.

Everything else (lr decay/warmup, density- and absolute-N-adaptive
relax/spike alpha shaping) is unchanged from the current best.
"""
...
# Adam moments: native early -> mildly averaged late, phase-coupled to alpha
_B1_LO, _B1_HI = 0.1, 0.25
_B2_LO, _B2_HI = 0.2, 0.45
_BETA_LEAD = 0.12                # moments start ramping this far before relax_p0
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Phase-couple the Adam moment ramp to the SAME farm-adaptive window that
    # governs the alpha ramp (_RELAX_P0 .. relax_p1), with a small lead-in so
    # averaging starts warming up just ahead of the penalty engaging rather
    # than in lockstep with, or independently of, it.
    beta_p0 = max(_RELAX_P0 - _BETA_LEAD, 0.0)
    beta_p1 = relax_p1
