...
N~=60-100+ band (scale_pos in [0.5, 0.78], via `high_n_ramp`) and a
terminal-spike softening onto the same band. That leaves dei_n50 and
dei_n50_uniform -- both n=50, scale_pos~=0.37, strictly below the high_n
gate's onset at 0.5 -- completely untouched by any of the targeted relief
mechanisms, even though dei_n50_uniform is the current worst-performing
farm (+0.0139%) and sits right below that gate.

dei_n50 and dei_n50_uniform share identical (D, min_spacing, n_turbines)
inputs -- the schedule literally cannot distinguish them from those
arguments alone, since they differ only in wind rose (and hence alpha0,
which already enters multiplicatively). So the fix is not to add a new
dimension of farm-specific tuning, but to make the *existing* mid-N band
(scale_pos ~0.3-0.45, i.e. n~40-55) a little more robust the same way the
high-N band already is: a mildly higher early-exploration penalty floor
and a mildly delayed penalty-engagement ramp, both gated so they fade to
zero by scale_pos=0.5 (where the high-N mechanisms already take over) and
by scale_pos<=0.15 (so parque_n10/n20, both clipped to scale_pos=0, are
completely unaffected). This is a small, bounded bump -- not a new
per-farm special case -- applied only to relax0 / relax_p1 / the alpha
floor, leaving lr, hold_frac, spike, and beta scheduling untouched.
"""
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              ...
              + _RELAX_MID_N_SPAN * mid_n_bump)        # small early-floor lift for mid-N farms
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_MID_N_SPAN * mid_n_bump)    # small extra delay for mid-N farms
    ...
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  ...
                                  + _ALPHA_FLOOR_MID_N_SPAN * mid_n_bump)
