...
SECOND, independent smoothstep-gated relief band centered on the mid-N
region (scale_pos in [0.20, 0.45]) that dei_n50 and dei_n50_uniform occupy
(scale_pos ~= 0.37 for n=50 farms, per the parent's own comments). The
existing `high_n_ramp` gate only engages for scale_pos in [0.5, 0.78]
(n ~= 60-100, e.g. dei_n80_omnidir) and leaves the n=50 farms with zero
extra relief -- exactly the two weakest farms in the reported per-cell
fitness (dei_n50_uniform +0.0139%, dei_n50 +0.0277%), while parque_n10/n20
(scale_pos = 0, below the new band) and dei_n80_omnidir (already gated by
the existing high-N band) are untouched by construction (smoothstep is 0
outside its [p0, p1] window).

Mechanism, mirroring the already-validated high-N treatment but scaled
down since the mid-N farms are less extreme: a bit more exploration hold,
a further-delayed relax ramp, a slightly softened terminal spike, and a
modest alpha-floor relief -- all gated through `mid_n_ramp`, which is exactly
zero for parque (scale_pos=0) and for dei_n80 (scale_pos>=0.5, past the new
band's upper edge), so this only redistributes budget within the n~=50
farms that were previously getting the least help.
"""
...
_RELAX_P1_MID_N_SPAN = 0.03     # extra gated delay for N~=50 (dei_n50/dei_n50_uniform) band
...
# Second, independent gate for the N ~= 50 band (scale_pos in [0.20, 0.45]),
# which sits strictly below the high-N gate and is zero outside its window --
# gives dei_n50/dei_n50_uniform their own small floor relief.
_ALPHA_FLOOR_MID_N_P0 = 0.20
_ALPHA_FLOOR_MID_N_P1 = 0.45
_ALPHA_FLOOR_MID_N_SPAN = 0.6
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_ramp = _smoothstep_scalar(
        scale_pos, _ALPHA_FLOOR_MID_N_P0, _ALPHA_FLOOR_MID_N_P1)
