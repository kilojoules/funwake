"""Schedule: couple the Adam beta ramp onset to the alpha engagement ramp
(single-lever change vs. the parent).

Diagnosis vs. the parent: `relax` ramps alpha from its low early-exploration
floor (relax0) up to full strength over p in [_RELAX_P0, relax_p1] -- this
is the window where the constraint penalty actually engages. `beta1`/`beta2`
independently ramp from their low (native, 0.1/0.2) to high (averaged,
0.25/0.45) values over p in [beta_p0, _BETA_P1], with beta_p0 fixed at
~0.40-0.50 (only nudged by scale_pos), unrelated to relax_p1 which can range
~0.45-0.63 for extreme/large-N farms. So on those farms the momentum/2nd-
moment averaging that is supposed to stabilize steps against the penalty's
now-larger gradient contribution starts ramping *before* the penalty itself
has engaged, and can finish ramping well before or well after relax_p1
depending on the farm -- an uncoupled, coincidental relationship instead of
a designed one.

This directly targets the still-untried item from the design-menu survey:
"phase-transition the Adam moments with the alpha phase" (beta2 up / beta1
toward-averaged in the feasibility phase). The fix ties beta_p0 partially to
relax_p1 via a blend factor (_BETA_P0_COUPLE = 0.3, a conservative partial
coupling rather than a full override, since a full override would be a much
larger behavioral change than the single-lever adjustments that have moved
the score so far): beta_p0 = (1-c)*old_beta_p0 + c*relax_p1. This pulls the
momentum ramp to start closer to (and scale with) the point where the
penalty actually starts biting, without discarding the existing scale_pos-
based early-onset behavior for large-N farms. All other machinery (lr decay
table, warmup, relax/spike shaping, alpha denominator floor) is unchanged
from the parent.
"""
...
# NEW: partially couple the beta ramp onset to the alpha engagement ramp
# (relax_p1), so momentum/2nd-moment averaging phases in alongside the
# constraint penalty instead of on an independent, coincidental schedule.
_BETA_P0_COUPLE = 0.3
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # NEW: blend the beta ramp onset toward relax_p1 (the alpha engagement
    # point) so the Adam-moment averaging phase-transitions alongside the
    # constraint penalty instead of on an independent schedule.
    beta_p0_native = _BETA_P0 + _BETA_P0_SCALE_SPAN * scale_pos
    beta_p0 = (1.0 - _BETA_P0_COUPLE) * beta_p0_native + _BETA_P0_COUPLE * relax_p1
