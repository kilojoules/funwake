"""Schedule: builds on the +0.1959% farm-balanced parent, which already
gates extra relax/hold/relax-delay relief onto the reference-density band
(mid_gate, extremity near 0 -- the dei_n50 / dei_n50_uniform farms) via a
smoothstep on `extremity`. Per-cell fitness shows dei_n50_uniform is still
the single worst farm (+0.0139%) even though dei_n50 sits in the *same*
mid_gate band (identical size/extremity/scale_pos -- they differ only in
wind rose, which this schedule cannot see directly) and already improved to
+0.0277%. Since the mid-gate direction has moved the farm-balanced mean up
for two generations running (+0.1952% -> +0.1959%), this variant makes two
small, same-direction extensions, both still gated so nothing outside the
reference-density band moves:

1. Slightly deepen the existing mid-gate relax/spike/hold relief
   (_MID_RELAX_SPAN, _MID_SPIKE_SPAN, _MID_HOLD_FRAC_SPAN, _MID_RELAX_P1_SPAN)
   -- a modest increment on a direction already validated, not a new lever.
2. Mirror the high-N band's spike-onset delay (_SPIKE_P0_HIGH_N_SPAN) onto
   the mid-gate band via a new _SPIKE_P0_MID_SPAN: the terminal feasibility
   spike now fires a little later for reference-density farms too, giving
   the delayed constraint engagement (mid_gate-widened relax_p1) more clear
   room before the spike stacks on top of it, same mechanism as the
   high-N-band fix from two generations ago.

Everything else (packing/N-scale features, alpha floor, decay table, warmup,
beta ramp) is byte-identical to the parent.
"""
...
# Narrow relief band for farms sitting AT the density reference (extremity
# ~= 0, e.g. dei_n50 / dei_n50_uniform), which otherwise receive none of the
# size/scale-based shaping above. Deepened slightly relative to the parent
# (validated direction for two generations running) and now also delays the
# terminal spike onset, mirroring the high-N band's treatment.
_MID_GATE_P0, _MID_GATE_P1 = 0.0, 0.30
_MID_RELAX_SPAN = -0.11         # a bit more early exploration room (was -0.09)
_MID_SPIKE_SPAN = -0.40         # a bit softer terminal spike (was -0.35)
_MID_RELAX_P1_SPAN = 0.06       # slightly later constraint engagement (was 0.05)
_MID_HOLD_FRAC_SPAN = 0.040     # slightly longer exploration plateau (was 0.032)
_MID_SPIKE_P0_SPAN = 0.025      # NEW: delay spike onset for reference-density band
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # 1.0 exactly at the density reference (extremity=0), fading to 0.0 by
    # extremity=0.30 -- disjoint from the high-N band and from the
    # high-extremity parque farms.
    mid_gate = 1.0 - _smoothstep_scalar(extremity, _MID_GATE_P0, _MID_GATE_P1)
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _MID_HOLD_FRAC_SPAN * mid_gate)       # slight extra hold at the density reference
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              ...
              + _MID_RELAX_SPAN * mid_gate)            # a bit more exploration room at reference density
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _MID_SPIKE_SPAN * mid_gate)             # softer terminal spike at reference density
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _MID_RELAX_P1_SPAN * mid_gate)        # slightly later engagement at reference density
    ...
    spike_p0 = (_SPIKE_P0 + _SPIKE_P0_HIGH_N_SPAN * high_n_ramp   # delay spike onset for high-N
                + _MID_SPIKE_P0_SPAN * mid_gate)                  # and, a little, at reference density
