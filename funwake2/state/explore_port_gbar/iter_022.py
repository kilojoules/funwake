"""Schedule: strengthened absolute-N scale signal (v2) — parent's large-N
relief terms were too weak on dei_n80_omnidir specifically.

Diagnosis vs. the parent (which already added a `scale_pos` = clip(log-N
against a 30-turbine reference, 0, 1) signal gated to act only on large-N
farms): dei_n80_omnidir sits at n=80, giving scale_pos ~= 0.71 (a
substantial, non-negligible signal — this farm is NOT being starved of the
adaptive term). Yet it remains *below* native (-0.017%). That means the
large-N relief terms fire correctly but are simply too small in magnitude to
counteract the extra pairwise-spacing-constraint noise that 80 turbines
generates relative to the <=50-turbine farms the other coefficients were
tuned against.

Fix: scale up (not redesign) the four existing scale_pos-gated coefficients
by ~35-45%, keeping every other mechanism (packing-density extremity
shaping, inverse-time lr decay, warmup, native alpha coupling) identical to
the parent:
  - hold_frac scale-span 0.05 -> 0.08   (longer AEP-only exploration before
    penalty machinery engages, proportionally more for n=80 than n=50)
  - spike scale-span -0.4 -> -0.55      (softer terminal feasibility spike;
    more turbines means a big last-minute correction is more likely to
    unwind AEP gains across many interacting turbines at once)
  - relax_p1 scale-span 0.08 -> 0.11    (delay the relax ramp's engagement
    window further for large-N farms)
  - beta_p0 scale-span -0.10 -> -0.14   (earlier Adam-moment averaging onset
    to damp the noisier constraint gradient of a bigger farm)

scale_pos is 0 for all n_turbines <= 30 (parque_n10, parque_n20 unaffected),
so this only reshapes behavior for dei_n50/dei_n50_uniform (scale_pos~0.37,
mild extra relief) and dei_n80_omnidir (scale_pos~0.71, the intended
target). All bounds stay well inside safe ranges (hold_frac max ~0.47 of
total steps; beta_p0 min 0.36, still < beta_p1=0.90; spike0 stays positive).
"""
...
_RELAX_P1_SCALE_SPAN = 0.11     # further delay for large-N farms (was 0.08)
_SPIKE_BASE, _SPIKE_SPAN = 2.0, 0.55    # spike0 = base + span * size
_SPIKE_SCALE_SPAN = -0.55       # soften terminal spike for large-N farms (was -0.4)
_SPIKE_P0, _SPIKE_P1 = 0.90, 1.00

_HOLD_FRAC_BASE, _HOLD_FRAC_SPAN = 1.0 / 3.0, 0.06   # exploration plateau length
_HOLD_FRAC_SCALE_SPAN = 0.08    # extra hold for large-N farms (was 0.05)
...
_BETA_P0_SCALE_SPAN = -0.14     # earlier averaging onset for large-N farms (was -0.10)
