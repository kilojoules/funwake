...
gates extra hold/relax-delay/alpha-floor/spike relief onto the N~=60-100+ band
via `high_n_ramp` (scale_pos in [0.5, 0.78]) but leaves the N~=50 band
(scale_pos~=0.37, e.g. dei_n50 / dei_n50_uniform) governed only by the
linear `_SCALE_SPAN` terms.

Diagnosis: dei_n50_uniform is tied for the worst-performing cell alongside
dei_n80_omnidir. Both sit on the positive side of `_n_scale` (more turbines
than the density-only `_packing_size` alone implies), but dei_n50 sits below
the existing high-N gate (0.5) so it gets none of the extra hold/delay relief
that helped the n80 band. This adds a SECOND, narrower gated ramp
(`mid_n_ramp`, active over scale_pos in [0.18, 0.50], zeroed out where
`high_n_ramp` has already taken over) that gives the mid-N band a small
amount of the same medicine -- slightly longer exploration hold and a
slightly later penalty-engagement ramp -- without touching parque_n10/n20
(scale_pos = 0, ramp = 0) or double-applying on top of the existing n80
relief (the `(1 - high_n_ramp)` factor fades this ramp out exactly where the
high-N gate fades in). Every other mechanism, including the terminal-spike
high-N gating from the best-so-far variant, is unchanged.
"""
...
_RELAX_P1_MID_N_SPAN = 0.04     # extra gated delay for N~=50 band (below the high-N gate)
...
# A second, narrower gate for the N~=50 band (scale_pos in [0.18, 0.50]),
# zeroed out where the high-N gate has already taken over.
_MID_N_P0, _MID_N_P1 = 0.18, 0.50
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_ramp = (_smoothstep_scalar(scale_pos, _MID_N_P0, _MID_N_P1)
                  * (1.0 - high_n_ramp))                # N~=50 band only
