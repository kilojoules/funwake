"""Schedule: (1) restore the alpha-denominator floor span to the
proven-better 1.5 (the gen-91 parent's push to 4.0 gave a strictly worse
farm-balanced score, 0.1892% vs 0.1894%, and left dei_n80_omnidir still
negative -- so a bigger terminal-alpha cap alone doesn't fix that farm);
and (2) switch the extra-hold term for large-N farms from a LINEAR
scale_pos gate to a QUADRATIC scale_pos_sq gate (previously untried --
prior attempts only pushed hold_frac's magnitude via scale_pos and made
things worse). scale_pos_sq is much more selective: it is ~0.14 for
dei_n50 (N=50) but ~0.50 for dei_n80_omnidir (N=80), vs ~0.37/~0.71 for
the linear version. The new span is chosen so dei_n50's extra hold stays
essentially unchanged from the best-so-far script (0.0184 -> ~0.0184)
while dei_n80_omnidir's extra hold roughly doubles (0.0354 -> ~0.068),
giving that farm materially more exploration-plateau time before the lr
decay engages, without touching the small/mid-N farms (parque_n10/n20,
dei_n50/_uniform) at all beyond the same small amount they already get.
...
absolute-N scale on spike/relax_p1/beta/relax0) is unchanged from the
parent.
"""
...
# NEW: extra hold for large-N farms now gated by scale_pos**2 (quadratic,
# not linear) -- more selective to the highest-N farms (dei_n80_omnidir)
# and nearly inert for merely-large ones (dei_n50), calibrated so dei_n50's
# extra hold is unchanged from the best-so-far script while dei_n80's
# roughly doubles.
_HOLD_FRAC_SCALE_SPAN = 0.135
_DENSITY_REF = 12.5             # ~50 turbines at 2D spacing (mid-scale anchor)
_N_REF = 30.0                   # absolute-N reference (independent of density)

# Terminal alpha denominator (lr_base floor) for large-N farms: restored to
# the proven-better 1.5 (the parent's push to 4.0 regressed the
# farm-balanced score without fixing dei_n80_omnidir).
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 + _HOLD_FRAC_SCALE_SPAN * scale_pos_sq)  # extra plateau, selective to high-N
    ...
    # disproportionately for the highest-N farms relative to merely-large
    # ones, so the terminal alpha ceiling is pulled down harder exactly
    # where turbine-pair count is largest.
