"""Schedule: extend the smoothstep-gated high-N alpha relief (introduced for
the alpha *terminal floor*) to the early-exploration penalty floor, the
terminal spike, and the exploration hold length as well.

Diagnosis vs. parent (+0.191% farm-balanced, dei_n80_omnidir still by far
the weakest farm at +0.0015% vs dei_n50's +0.0277% and parque's
+0.2-0.7%): the parent already discovered that a *linear-in-scale_pos*
correction under-delivers for N~=80 farms because scale_pos itself is a
log-compressed, clipped quantity -- and fixed this for the alpha
*denominator floor* with a smoothstep band over scale_pos in [0.5, 0.85]
(exactly bracketing dei_n80_omnidir's scale_pos ~= 0.71). But `relax0`
(early-exploration penalty floor), `spike0` (terminal feasibility spike)
and `hold_frac` (exploration-plateau length) still only get the *linear*
scale_pos correction, so they suffer the identical under-delivery the
alpha-floor fix was built to solve -- dei_n80_omnidir's early alpha floor
and terminal spike are barely relaxed relative to dei_n50's, even though
its packing/absolute-N pressure is substantially higher. This version reuses
the same `high_n_ramp` smoothstep (already computed for the alpha floor) to
also (a) lower the early penalty floor, (b) soften the terminal spike
further, and (c) lengthen the exploration hold, specifically for the
N~=60-120 band -- while leaving scale_pos<=0.5 farms (dei_n50 at ~=0.37,
parque_n10/20 at ==0) byte-for-byte identical to the parent, since
high_n_ramp==0 there. All other machinery (self-contained inverse-time
product lr decay to gamma_min, short linear warmup, density-extremity
relax/spike/hold, the hold_frac-floored beta ramp) is unchanged.
"""
...
# Smoothstep-gated band, over ABSOLUTE-N scale_pos, isolating the N~=60-120
# region (dei_n80_omnidir sits at scale_pos~=0.71, mid-ramp) from both
# dei_n50 (scale_pos~=0.37, untouched) and the largest-N tail near
# scale_pos=1 (already well served by the linear terms alone).
_HIGH_N_P0 = 0.5
_HIGH_N_P1 = 0.85
_ALPHA_FLOOR_HIGH_N_SPAN = 3.0
# Same band, reused to relax the early penalty floor and terminal spike,
# and to lengthen the exploration hold, for the same N~=60-120 farms.
_RELAX_HIGH_N_SPAN = -0.10      # further lowers relax0 (more early freedom)
_SPIKE_HIGH_N_SPAN = -0.35      # further softens the terminal feasibility spike
_HOLD_HIGH_N_SPAN = 0.03        # extra exploration-hold length
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    high_n_ramp = _smoothstep_scalar(scale_pos, _HIGH_N_P0, _HIGH_N_P1)

    lr0 = _C * diam                                  # exploration lr from D
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 + _HOLD_FRAC_SCALE_SPAN * scale_pos
                 + _HOLD_HIGH_N_SPAN * high_n_ramp)     # extra hold for the N~=60-120 band
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              + _RELAX_SCALE_SPAN * scale_pos
              + _RELAX_HIGH_N_SPAN * high_n_ramp)      # extra early relief for N~=60-120 band
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              + _SPIKE_SCALE_SPAN * scale_pos
              + _SPIKE_HIGH_N_SPAN * high_n_ramp)       # extra terminal-spike relief for N~=60-120
