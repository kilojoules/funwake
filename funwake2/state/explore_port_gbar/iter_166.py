...
band (dei_n80_omnidir) via `high_n_ramp`. Per-farm fitness shows the
GLOBAL worst performer is actually dei_n50_uniform (+0.0139%), sitting right
next to dei_n50 (+0.0277%) -- same geometry (n=50, same D/min_spacing), only
the wind rose differs. Because `size`/`extremity`/`scale_pos` are built
purely from n_turbines/D/min_spacing, this pair is INDISTINGUISHABLE to the
schedule and always moves together; targeting it can't regress dei_n80
(already gated at scale_pos in [0.5, 0.78]) or the parque farms (which sit
at high extremity, since their N is well below the mid-scale density
reference `_DENSITY_REF`).

Diagnosis: dei_n50/_uniform sit at *low* extremity (near the density
reference anchor, by construction), so they get the SMALLEST relax delay,
smallest hold extension, and largest spike of any farm in the whole
schedule -- the opposite of dei_n80's treatment, even though dei_n50 is
also underperforming its peers. This adds a new, narrow gate `mid_gate`
that fires only near extremity~=0 (fading out by extremity~0.3) and dies to
zero wherever `high_n_ramp` is already active, giving this specific
near-reference band a bit more exploration hold, a bit more delayed
constraint engagement, and a softer terminal spike -- mirroring the
mechanism that already helped dei_n80, applied to the other neglected band.
Every other mechanism, constant, and farm treatment is unchanged from the
best-so-far variant.
"""
...
# New: narrow gate for near-reference-density farms (extremity ~= 0), which
# currently get the LEAST relax delay / hold extension / spike softening of
# any band -- despite dei_n50_uniform being the global worst-scoring farm.
# Fades to 0 by extremity=0.3 and is further damped wherever high_n_ramp is
