"""Schedule: builds on the current best (+0.1959% farm-balanced), which
already gives the density-reference band (extremity in [0, 0.25], via
`mid_gate`) extra exploration relief -- longer hold, softer early relax
floor, and a softened terminal spike. This variant deepens and slightly
widens that same gate.

Diagnosis: dei_n50 (+0.0277%) and dei_n50_uniform (+0.0139%) sit at
*exactly* the same packing extremity (n=50, D=240, min_spacing=2D gives
density == _DENSITY_REF, so size=extremity=0) -- the schedule cannot tell
them apart via n_turbines/D/min_spacing, only their wind rose differs, yet
dei_n50_uniform scores half as well. Since the mid-density band is already
the mechanism carved out for exactly this pair, giving it a bit more room
(deeper relax/hold relief, softer terminal spike) is the lowest-risk lever
left: it cannot regress the parque farms (extremity 0.66-1.0, well outside
the gate) or dei_n50 itself much, since dei_n50 already benefits from the
same gate and is not the worst performer. Widening the gate's upper edge
from 0.25 to 0.30 also pulls in part of dei_n80_omnidir (extremity ~=0.34
is just outside 0.30, so it's still excluded -- the widening mainly makes
the falloff for dei_n50/dei_n50_uniform less abrupt) without touching
parque_n10/n20, whose extremity (0.66, 1.0) stays far outside the gate.
Every other mechanism is unchanged from the current best.
"""
...
# size/scale-based shaping above. Widened slightly (0.25 -> 0.30) and
# deepened relative to the previous best -- still well short of parque_n10/
# n20's extremity (0.66, 1.0) and dei_n80_omnidir's (~0.34), so those are
# unaffected.
_MID_GATE_P0, _MID_GATE_P1 = 0.0, 0.30
_MID_RELAX_SPAN = -0.09         # a bit more early exploration room
_MID_SPIKE_SPAN = -0.35         # a bit softer terminal spike
_MID_RELAX_P1_SPAN = 0.05       # slightly later constraint engagement
_MID_HOLD_FRAC_SPAN = 0.032     # slightly longer exploration plateau
