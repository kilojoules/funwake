"""Schedule: extend the existing high-N gating (currently applied only to the
alpha terminal floor and the spike softening) to the two other levers that
also shape the crowded/many-pair regime -- relax_p1 (when the constraint
penalty fully engages) and hold_frac (how long pure exploration lasts) --
plus a modest further bump to the two already-gated terms.

Diagnosis vs. the parent: dei_n80_omnidir (scale_pos ~= 0.71, gate active
since threshold 0.5) is the only farm still below native (-0.0041%), while
dei_n50 (scale_pos ~= 0.37, gate inactive) is comfortably positive. The
parent already decouples the *alpha floor* and *spike* for high-N farms via
a quadratic term gated at scale_pos > 0.5, but relax_p1 (the point at which
the constraint penalty reaches full strength) and hold_frac (the length of
the pure-exploration plateau) are still only linear in scale_pos -- so any
further relief on those two levers for dei_n80 would proportionally leak
onto dei_n50 too, same coupling that capped progress before the alpha-floor
gate was introduced. Adding the analogous quadratic, threshold-gated terms
to relax_p1 and hold_frac gives dei_n80_omnidir a longer pure-exploration
window and a further-delayed penalty ramp -- letting the exploration/AEP
lever run longer before feasibility work dominates -- while leaving dei_n50
(0.37 < 0.5, gate contributes exactly zero) and both parque farms
(scale_pos == 0) bit-for-bit identical to the parent. The already-gated
alpha-floor and spike terms are bumped slightly further to compound the
relief now that dei_n80 gets more exploration time to exploit it. All other
machinery (self-contained inverse-time product lr decay to gamma_min, short
linear warmup, density-extremity relax/spike/hold, absolute-N scale on
hold/spike/relax_p1, the hold_frac-floored beta ramp) is unchanged.
"""
...
# Extra, threshold-gated quadratic delay of the penalty engagement window for
# farms well past the linear regime (scale_pos > 0.5), decoupled from dei_n50.
...
# well past the linear regime (scale_pos > 0.5), decoupled from dei_n50.
_SPIKE_HIGH_N_SPAN = 1.2
...
# Extra, threshold-gated quadratic hold extension for farms well past the
# linear regime (scale_pos > 0.5), decoupled from dei_n50.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_gap2)  # gated extra hold, high-N only
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              - _SPIKE_HIGH_N_SPAN * high_n_gap2)      # gated extra relief, high-N only
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_HIGH_N_SPAN * high_n_gap2)  # gated extra delay, high-N only
