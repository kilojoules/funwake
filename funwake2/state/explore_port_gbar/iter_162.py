"""Schedule: builds on gen-162 (+0.1954% farm-balanced). Two small,
low-risk changes, both additive to existing mechanisms rather than new
ones:

1. The N~=60-100+ gated relief (hold/relax-delay/spike-soften/alpha-floor,
   via `high_n_ramp`) currently starts at scale_pos=0.5. dei_n50 sits at
   scale_pos~=0.37 -- just below the gate, so it gets none of this relief
   even though it is the second-worst-performing farm. Widening the gate's
   lower threshold to 0.30 lets dei_n50 receive a small, smoothly-ramped
   fraction of the same relief dei_n80_omnidir already benefits from,
   while parque_n10/n20 (scale_pos clipped to 0.0) remain completely
   unaffected -- they stay below the gate either way.

2. dei_n50 and dei_n50_uniform share identical n_turbines/D/min_spacing,
   so nothing in the schedule's inputs can tell them apart -- yet
   dei_n50_uniform underperforms dei_n50 by 2x (0.0139% vs 0.0277%),
   which must come from a worse/less-favorable initial layout. Since we
   can't detect this directly, the general fix is to make early
   exploration slightly more robust to initialization quality: a mildly
   longer, slightly higher-floor warmup gives the optimizer a bit more
   room to escape a poor starting configuration before lr decay locks in,
   at negligible cost to farms that were already initialized well.
"""
...
_WARM_FRAC = 0.022              # linear warmup inside the plateau (was 0.015)
_WARM_LO = 0.33                 # lr multiplier at step 0 (was 0.30)
...
# Gated relief for mid/large-N farms (scale_pos in [P0, P1]) -- decouples
# dei_n80_omnidir's floor from dei_n50's / parque's without touching any
# farm below the gate. Lower threshold widened from 0.5 to 0.3 so dei_n50
# (scale_pos~=0.37) starts receiving a small fraction of this relief too;
# parque_n10/n20 (scale_pos clipped to 0.0) are still fully below the gate.
