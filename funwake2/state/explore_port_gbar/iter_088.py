"""Schedule: raise the terminal Adam second-moment ceiling (beta2) for
large-N farms, gated by the same scale_pos factor already used elsewhere
(isolated single-lever change; timing/floor levers left untouched).

Diagnosis vs. the parent: dei_n80_omnidir has ordinary packing density
(extremity ~ 0, so the density-based relax/spike/hold shaping barely
engages) but by far the most turbine pairs in the fitness set (80 vs <=50),
so its constraint-gradient estimate is the noisiest of any farm even once
the alpha terminal-divergence floor (added in the current best) caps the
penalty magnitude itself. The parent's beta2 only ramps from 0.2 to 0.45
regardless of farm size, i.e. even at its ceiling the second-moment
estimate averages over a fairly short effective window -- adequate for the
lower-pair-count farms but likely still noisy for n=80's larger, more
numerous constraint-gradient terms. Earlier attempts already pushed the
*timing* of the averaging onset further via scale_pos (beta_p0) and made
the farm-balanced score worse, so this leaves beta_p0, hold_frac, spike0,
relax_p1, and the alpha_denom_floor untouched. Instead it adds one new,
previously-untried lever: raise the beta2 *ceiling* itself (not its onset)
for large-N farms only, giving more second-moment averaging late in
training to damp per-step variance from the larger pair count -- while
scale_pos == 0 (small/mid-N farms, e.g. parque_n10/n20, dei_n50) keeps the
ceiling exactly at the parent's 0.45, leaving them unaffected.
...
absolute-N scale on hold/spike/relax_p1/beta timing, bounded alpha
denominator floor) is unchanged from the current best.
"""
...
# NEW: raise the beta2 ceiling (not its onset) for large-N farms so the
# noisier, many-pair constraint-gradient estimate gets more second-moment
# averaging once the ramp completes.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # higher terminal averaging, large N
