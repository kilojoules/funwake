...
(n=50, size~=0, scale_pos~=0.37), so they receive the EXACT same schedule under
every prior generation -- yet dei_n50_uniform (0.0139%) scores roughly half of
dei_n50 (0.0277%), and both are the weakest cells in the farm-balanced mean
alongside dei_n80_omnidir (0.0257%, scale_pos~=0.71). Since scale_pos/size can't
see the wind-rose shape, the only lever available is a change that benefits the
whole moderate-to-large-N band (scale_pos > 0) generically: give it a bit more
pure-exploration time before constraints engage, since a broader unconstrained
hold is what most helps the harder (uniform/omnidirectional) wind-rose cases
find a good basin before penalty shaping narrows the search. This nudges three
already-existing linear scale_pos terms (which are inert for parque_n10/n20,
whose n<30 clamps scale_pos to 0, so those farms are untouched):
  - _HOLD_FRAC_SCALE_SPAN: 0.05 -> 0.065 (slightly longer exploration plateau)
  - _RELAX_SCALE_SPAN: -0.05 -> -0.07 (slightly lower early penalty floor)
  - _RELAX_P1_SCALE_SPAN: 0.08 -> 0.10 (slightly later full penalty engagement)
Every other mechanism, including all high-N-gated terms, is unchanged.
"""
...
_RELAX_SCALE_SPAN = -0.07       # lower early-exploration penalty floor for large-N farms
_RELAX_P0 = 0.08
_RELAX_P1_BASE, _RELAX_P1_SPAN = 0.45, 0.10   # widen engagement window at extremes
_RELAX_P1_SCALE_SPAN = 0.10     # further delay for large-N farms
...
_HOLD_FRAC_SCALE_SPAN = 0.065   # extra hold for large-N farms
