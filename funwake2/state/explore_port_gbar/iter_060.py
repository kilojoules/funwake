"""Schedule: phase-lock the Adam beta ramp onset to the alpha (constraint)
ramp instead of a fixed progress fraction (isolated single-lever change).

Diagnosis vs. the best-so-far mutant: that schedule already (a) shapes alpha
via a size/extremity/scale_pos-dependent ramp that reaches full engagement
at `relax_p1` (density- and N-adaptive, e.g. later for large-N/extreme-
density farms), and (b) ramps beta1/beta2 from native (0.1/0.2) toward a
mildly-averaged pair (0.25/0.45) starting at a *fixed* progress fraction
`_BETA_P0 = 0.50` (only nudged by `scale_pos`, i.e. only for large absolute
N). These two ramps are therefore desynchronized for every farm whose
`relax_p1` (density/extremity driven) differs from 0.50 minus that small
N-nudge -- e.g. small/mid-density farms where `relax_p1` sits well above or
below 0.5, and non-large-N farms (scale_pos == 0) where beta_p0 is pinned
at exactly 0.50 regardless of how early/late alpha actually engages.
Prior-art bet #4 (phase-transition the Adam moments with the alpha phase:
beta2 up / beta1 down exactly as the feasibility/constraint phase begins)
is only half-implemented -- the phase link is missing.

This change replaces the fixed/scale_pos-only `_BETA_P0` with a value
derived directly from `relax_p1` (already computed, a Python float at trace
time from n_turbines/D/min_spacing -- no new hyperparameter surface beyond
one small lead constant): beta1/beta2 begin ramping `_BETA_LEAD` fraction
of the run *before* alpha reaches full engagement, so the optimizer's
second-moment estimate is already smoothing by the time the constraint
penalty is dominating the gradient, on every farm, not just large-N ones.
Everything else -- lr decay/warmup, relax/spike alpha shaping, the
scale_pos-gated alpha denominator floor -- is unchanged from the parent.
"""
...
# Adam moments: native early -> mildly averaged late, phase-locked to alpha
_B1_LO, _B1_HI = 0.1, 0.25
_B2_LO, _B2_HI = 0.2, 0.45
_BETA_LEAD = 0.08                # beta ramp starts this far before relax_p1
_BETA_WIDTH = 0.40               # beta ramp duration
_BETA_P0_MIN, _BETA_P0_MAX = 0.05, 0.90   # safety clip on the derived onset
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Phase-lock the Adam-moment ramp onset to the alpha ramp: begin
    # smoothing beta1/beta2 shortly BEFORE alpha reaches full engagement
    # (relax_p1), on every farm -- not just large-N ones -- so the second
    # moment estimate is already averaging by the time the constraint
    # penalty dominates the gradient.
    beta_p0 = min(max(relax_p1 - _BETA_LEAD, _BETA_P0_MIN), _BETA_P0_MAX)
    beta_p1 = min(beta_p0 + _BETA_WIDTH, 1.0)
    r = _ramp(p, beta_p0, beta_p1)
