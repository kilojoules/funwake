"""Schedule: same as parent (+0.1952% farm-balanced) but fixes a beta-ramp
squeeze on large-N farms.

Diagnosis vs. parent: for large-N / high-density farms, `_HOLD_FRAC_SCALE_SPAN`
and `_HOLD_FRAC_HIGH_N_SPAN` push `hold_frac` up, and `beta_p0` is floored at
`hold_frac` (so momentum cannot start ramping before the lr hold ends). But
`beta_p1` (where the beta ramp finishes) stays fixed at 0.90 for every farm.
On the worst-performing farm (dei_n80_omnidir, large absolute N), the later
`beta_p0` combined with a fixed `beta_p1` compresses the beta1/beta2 ramp
into a much shorter progress window than small farms get -- Adam barely
reaches its more-averaged late-training regime before the run ends, right
when the terminal alpha spike (which relies on stable, averaged gradients to
resolve tight spacing constraints) engages. This variant adds a small,
gated `beta_p1` reduction for the same high-N band the alpha floor and
hold/relax terms already gate on (scale_pos in [0.5, 0.78] via `high_n_ramp`),
giving large-N farms more room to complete the momentum ramp before the
spike phase. Small/mid farms (scale_pos ~ 0, high_n_ramp = 0) are completely
unaffected -- beta_p1 stays at 0.90 exactly as in the parent. Every other
mechanism (self-contained inverse-time product lr decay to gamma_min, short
linear warmup, density-extremity relax/spike/hold, absolute-N scale on
hold/spike/relax_p1, the alpha-floor relief, the hold_frac-floored beta
ramp start) is unchanged from the parent.
"""
...
# Give the same high-N band (that gets a later beta_p0 via hold_frac) more
# room to actually finish the ramp before the terminal spike phase begins.
_BETA_P1_HIGH_N_SPAN = -0.06
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # The same high-N band gets both a later beta_p0 (via hold_frac) and,
    # without relief, a fixed beta_p1=0.90 -- squeezing its ramp window.
    # Pull beta_p1 in slightly (gated the same way as the alpha floor) so
    # the ramp still has room to complete before the terminal spike phase,
    # without touching farms below the gate.
    beta_p1 = max(_BETA_P1 + _BETA_P1_HIGH_N_SPAN * high_n_ramp, beta_p0 + 0.05)
