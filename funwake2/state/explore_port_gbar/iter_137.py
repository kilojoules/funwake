...
extremity-gated relief channel (independent of the existing scale_pos/N
gate) that widens exploration hold, delays penalty engagement, and relaxes
the terminal alpha floor for ANY farm with unusually extreme packing
density -- not just large-absolute-N ones.

Diagnosis vs. parent: the worst cell is dei_n50_uniform (+0.0139%), which
has the SAME n_turbines/D as dei_n50 (+0.0277%) and therefore the SAME
scale_pos, so none of the parent's large-N-gated relief terms
(_HOLD_FRAC_HIGH_N_SPAN, _RELAX_P1_HIGH_N_SPAN, _ALPHA_FLOOR_HIGH_N_SPAN)
touch it at all -- its lower score has to come from a different
min_spacing/packing-density signature (a different `size`/`extremity`
value from `_packing_size`), which the parent only feeds through small
linear terms (_HOLD_FRAC_SPAN, _RELAX_P1_SPAN, _ALPHA_FLOOR_EXT_SPAN), not
the sharper gated boost that large-N farms get. This variant reuses the
same idea -- a smoothstep gate that only activates at extremes -- but keys
it off `extremity = |size|` (packing density distance from the mid-scale
reference) instead of `scale_pos` (absolute N), so it also helps
farms like dei_n50_uniform whose difficulty comes from an unusual
spacing/density ratio rather than sheer turbine count. The gate threshold
(0.55-0.85) sits above where dei_n50/parque_n20 are believed to live, so
already-good cells are left untouched (additive-only relief, never
reduces anything). Every other mechanism is unchanged from the parent.
"""
...
_RELAX_P1_EXT_GATE_SPAN = 0.05  # extra gated delay for ANY extreme-packing farm
...
_HOLD_FRAC_EXT_GATE_SPAN = 0.02 # extra gated hold for ANY extreme-packing farm
...
# Second, independent gate on packing-density EXTREMITY (not absolute N) --
# catches farms like dei_n50_uniform whose difficulty is an unusual
# spacing/density ratio rather than turbine count. Additive-only, and the
# gate only turns on above 0.55 so mid-density farms are untouched.
_EXT_GATE_P0, _EXT_GATE_P1 = 0.55, 0.85
_ALPHA_FLOOR_EXT_GATE_SPAN = 1.5
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    ext_ramp = _smoothstep_scalar(extremity, _EXT_GATE_P0, _EXT_GATE_P1)
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_EXT_GATE_SPAN * ext_ramp)   # extra gated hold for ANY packing extreme
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_EXT_GATE_SPAN * ext_ramp)    # extra gated delay for ANY packing extreme
    ...
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  ...
                                  + _ALPHA_FLOOR_EXT_GATE_SPAN * ext_ramp)
