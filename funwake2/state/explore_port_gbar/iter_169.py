...
Diagnosis: every extremity/scale-based shaping term in the parent
(`_RELAX_SPAN*size`, `_SPIKE_SCALE_SPAN*scale_pos`, `high_n_ramp`,
`_ALPHA_FLOOR_*`) is driven by `size` (packing density vs `_DENSITY_REF`)
and `scale_pos` (absolute N vs `_N_REF`). dei_n50 / dei_n50_uniform sit
almost exactly AT the density reference (n=50, min_spacing~=2D =>
density~=12.5==_DENSITY_REF => size~=0, extremity~=0) and below the
high-N gate (scale_pos~=0.37 < 0.5) -- so they receive essentially none
of the accumulated shaping and run on the raw base constants alone.
They are also the two weakest farms (+0.0277%, +0.0139%) despite 3/3
feasibility on every seed, i.e. there is unused feasibility headroom
being left on the table specifically at this "neutral" operating point
that the size/scale gates never reach.

This adds one new, narrowly-gated term: `mid_gate`, a smoothstep that is
1.0 at extremity=0 and decays to 0.0 by extremity=0.25 (so it is inert
for dei_n80_omnidir at extremity~=0.34, already handled by its own
high_n gate, and for the parque farms which sit at large |size|/large
extremity). Inside that narrow band it nudges relax0/spike0 down and
relax_p1 slightly later -- i.e. permits a bit more early exploration and
a softer terminal spike -- purely for farms sitting at the reference
density. Every other mechanism, constant, and gate is unchanged from the
parent.
"""
...
# New: narrow relief band for farms sitting AT the density reference
# (extremity ~= 0), which otherwise receive none of the size/scale-based
# shaping above. Fades out by extremity=0.25 so it does not overlap the
# high_n gate (active from scale_pos=0.5) or the already-well-tuned,
# high-extremity parque farms.
_MID_GATE_P0, _MID_GATE_P1 = 0.0, 0.25
_MID_RELAX_SPAN = -0.07         # a bit more early exploration room
_MID_SPIKE_SPAN = -0.30         # a bit softer terminal spike
_MID_RELAX_P1_SPAN = 0.04       # slightly later constraint engagement
_MID_HOLD_FRAC_SPAN = 0.025     # slightly longer exploration plateau
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # extremity=0.25 -- disjoint from the high-N band and from the
    # high-extremity parque farms.
