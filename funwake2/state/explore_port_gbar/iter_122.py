"""Schedule: decouple the mid-N (dei_n80_omnidir, scale_pos~=0.71) band's
spike/hold/relax shaping from the linear scale_pos terms that are tuned
against dei_n50 (scale_pos~=0.37) and parque_n10/20 (scale_pos==0).

Diagnosis vs. parent (+0.191% farm-balanced, dei_n80_omnidir the weakest
farm at +0.0015%, essentially flat): the parent already decouples the
*alpha floor* for the N~=60-120 band via a smoothstep-gated `high_n_ramp`
term (gated on scale_pos in [0.5, 0.85]), but `hold_frac`, `spike0`, and
`relax0` still only see the *linear* `scale_pos` term, which is shared
with dei_n50's tuning -- any change large enough to matter for N=80 also
moves N=50 (which is already positive and comparatively well-tuned), so a
single linear coefficient can't specialize for the N~=80 band without
regressing N=50. Applying the SAME smoothstep gate (`high_n_ramp`, zero
below scale_pos=0.5, ramping to 1 by scale_pos=0.85) to small additional
terms on hold_frac (more exploration time), spike0 (softer terminal
penalty spike), and relax0 (lower early-exploration floor) gives
dei_n80_omnidir extra room without perturbing dei_n50, parque_n10, or
parque_n20 at all (their high_n_ramp is exactly 0). Everything else
(inverse-time product lr decay, short linear warmup, density-extremity
relax/spike/hold, the alpha floor relief, the hold_frac-floored beta ramp)
...
# [0.5, 0.85]) -- decouples dei_n80_omnidir's tuning from dei_n50's without
# touching any farm below the gate.
_HIGH_N_P0 = 0.5
_HIGH_N_P1 = 0.85
_ALPHA_FLOOR_HIGH_N_SPAN = 3.0
# Same gate, applied to hold/spike/relax so the whole N~=60-120 band gets
# extra room without moving dei_n50 or parque_n10/20 at all.
_HOLD_FRAC_HIGH_N_SPAN = 0.025   # a bit more exploration time
_SPIKE_HIGH_N_SPAN = -0.35       # softer terminal spike, on top of the linear term
_RELAX_HIGH_N_SPAN = -0.04       # lower early floor, on top of the linear term
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_ramp)  # extra hold, N~=60-120 band only
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              ...
              + _RELAX_HIGH_N_SPAN * high_n_ramp)     # extra early-floor relief, N~=60-120 only
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              + _SPIKE_SCALE_SPAN * scale_pos
              + _SPIKE_HIGH_N_SPAN * high_n_ramp)     # extra spike softening, N~=60-120 only
