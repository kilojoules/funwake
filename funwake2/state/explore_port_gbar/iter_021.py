"""Schedule: absolute-N scale signal on top of density-extremity shaping.

Diagnosis vs. the parent (packing-density-adaptive relax/spike/hold, gated
only by a *density* proxy = n_turbines / (min_spacing/D)^2):

  dei_n80_omnidir has min_spacing/D ~= 2 (same as the mid-density anchor),
  so its packing DENSITY sits near the reference and `size`/`extremity` stay
  small -- it gets almost none of the extremity-based relief the parent
  built for extreme-density farms. But it is still the largest-N farm in the
  fitness set by a wide margin (80 vs. <=50 elsewhere), which means far more
  pairwise spacing constraints and a noisier constraint gradient even though
  the *density* looks ordinary. The parent has no signal for that.

  Fix: add a second, independent "scale" factor from the ABSOLUTE turbine
  count (log-compressed around a mid-N reference), separate from the
  density-normalized `size`. It is clipped to act ONLY on large-N farms
  (positive scale) so small/mid farms (parque_n10, parque_n20, dei_n50) see
  little to no change -- clip(scale, 0, 1) is 0 for all farms with
  n_turbines below the reference.

  For large-N farms this scale signal:
    1. extends the exploration hold a bit further (more AEP-only time before
       the constraint machinery engages -- more turbines means more
       conflicting pairwise constraints to resolve, so premature penalty
       pressure is more likely to trap the layout in a bad local feasible
       point),
    2. softens the terminal penalty spike slightly (avoid overcorrecting
       feasibility at the very end when there are many interacting
       constraints -- a big last-minute spike can undo AEP gains across many
       turbines at once),
    3. delays the relax ramp's engagement window a touch further (same
       exploration-first rationale as 1),
    4. starts the beta1/beta2 phase transition slightly earlier (more
       turbines -> noisier per-step constraint gradient -> earlier
       averaging helps stability).
