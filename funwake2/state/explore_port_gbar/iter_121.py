"""Schedule: smooth (cubic-ramped) high-N alpha-floor relief plus a small
extra terminal-alpha discount for high-N farms, replacing the previous
threshold-gated *quadratic-in-gap* term with a smoothstep-gated one.

Diagnosis vs. parent (+0.190% farm-balanced, dei_n80_omnidir still the
weakest farm): the parent's high-N alpha relief is
`_ALPHA_FLOOR_HIGH_N_SPAN * max(scale_pos-0.5,0)^2`, a C0-but-not-C1 kink at
scale_pos=0.5. Because `scale_pos = clip(n_scale,0,None)` is itself already a
clipped/log-compressed function of n_turbines, stacking a hard-thresholded
square on top produces a very flat response just above the gate (0.71-0.5=
0.21 -> 0.21^2=0.044, negligible) and only becomes meaningful very close to
scale_pos=1 (N ~= 120). dei_n80_omnidir (N=80, scale_pos ~= 0.71) sits right
in that "still mostly flat" region, so most of the intended relief never
actually reaches it. Replacing the hard square with a smoothstep ramp from
scale_pos=0.5 to 0.85 (dei_n80_omnidir's 0.71 lands mid-ramp instead of
early-flat) delivers meaningfully more floor relief exactly at N~=80 while
the ramp still ends at 0 for scale_pos<=0.5 (dei_n50, scale_pos~=0.37) and
farms at scale_pos==0 (parque_n10/20) are completely untouched -- same
decoupling guarantee as the parent, just retimed so the relief actually
activates in the N~=60-100 band instead of only very close to N~=120. All
other machinery (self-contained inverse-time product lr decay to gamma_min,
short linear warmup, density-extremity relax/spike/hold, absolute-N scale on
hold/spike/relax_p1, the hold_frac-floored beta ramp) is unchanged from the
parent.
"""
