"""Schedule: smooth (logaddexp) blend for the terminal alpha-denominator
floor, replacing the parent's hard jnp.maximum(lr_base, floor).

Diagnosis vs. parent (+0.191% farm-balanced, dei_n80_omnidir still weakest
at +0.0015%): `alpha_denom = jnp.maximum(lr_base, alpha_denom_floor)` has a
C0-but-not-C1 kink exactly at the step where lr_base's smooth inverse-time
decay crosses the (trace-time-constant) floor. For dei_n80_omnidir
(scale_pos ~= 0.71, high_n_ramp ~= 0.64) that floor sits well above
gamma_min, so lr_base crosses it partway through the decay -- right in the
region where alpha is also being ramped up by `spike`/`relax`. The abrupt
change in dAlpha/dStep right at the crossing (alpha_denom's derivative jumps
from ~lr_base's slope to ~0) injects a discontinuity into the penalty-weight
trajectory precisely while the optimizer is trying to settle into a
feasible/high-AEP layout, which plausibly costs this farm the most of the
five (it is simultaneously mid-scale-pos, so the floor is neither absent
[small N] nor high enough to dominate the whole decay [very large N] --
lr_base and floor are close in magnitude for a large fraction of the run).

Fix: replace the hard max with a smooth soft-max via logaddexp, normalized
by the floor's own scale so the sharpness parameter is dimensionless and
farm-size-independent. Away from the crossing region the soft-max is
numerically indistinguishable from the hard max (same asymptotes), so farms
whose floor is inactive throughout (small N, e.g. parque_n10/20 where
alpha_denom_floor's high-N term is exactly 0) or dominant throughout are
unaffected to first order; only the crossing region -- the dei_n80-like
mid-scale-pos band -- sees a materially different (now-smooth) trajectory.
...
absolute-N scale on hold/spike/relax_p1, the smoothstep-gated high-N floor
relief, the hold_frac-floored beta ramp) is unchanged from the parent.
"""
...
# Extra, smoothstep-gated relief for farms in the N ~= 60-120 band
# (scale_pos in [0.5, 0.85]) -- decouples dei_n80_omnidir's floor from
# dei_n50's without touching any farm below the gate, and (unlike a hard
# quadratic-in-gap term) actually delivers meaningful relief mid-ramp
# instead of only very close to scale_pos=1.
_ALPHA_FLOOR_HIGH_N_P0 = 0.5
_ALPHA_FLOOR_HIGH_N_P1 = 0.85
_ALPHA_FLOOR_HIGH_N_SPAN = 3.0

# Sharpness of the smooth (logaddexp) blend between lr_base and the alpha
# floor, in units of 1/floor -- dimensionless so it scales with the farm.
# Large enough that far from the crossing the blend matches the hard max to
# within noise; small enough to meaningfully round off the kink itself.
_ALPHA_DENOM_SHARPNESS = 6.0
...
def _smooth_floor(arr, floor_val, sharpness):
    """Smooth (logaddexp) approximation of max(arr, floor_val), normalized by
    floor_val so `sharpness` is a dimensionless, farm-size-independent knob.
    Matches the hard max away from the crossing; rounds off the kink at it."""
    k = sharpness / max(floor_val, 1e-9)
    return jnp.logaddexp(k * arr, k * floor_val) / k
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # terms + a smoothstep-gated term ramping over scale_pos in [0.5,0.85])
    # so alpha's terminal divergence is bounded more for the largest-N
    # (many-pair) farms specifically, without perturbing farms below the
    # gate (dei_n50 at scale_pos~=0.37, parque_n10/20 at scale_pos==0).
    # Blend lr_base with the floor smoothly (logaddexp) instead of a hard
    # max, so mid-scale-pos farms (e.g. dei_n80_omnidir) whose floor and
    # lr_base cross partway through the decay don't see a kinked
    # dAlpha/dStep right when the penalty is also ramping via spike/relax.
    ...
    alpha_denom = _smooth_floor(lr_base, alpha_denom_floor, _ALPHA_DENOM_SHARPNESS)
