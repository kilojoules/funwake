"""Schedule: scale terminal Adam beta2 (second-moment averaging) up with
absolute turbine count, on top of the alpha-floor-capped parent (isolated
single-lever change).

Diagnosis vs. the parent: dei_n80_omnidir (n=80, the largest-N farm in the
fitness set) is the weakest link at +0.0005% -- essentially flat -- while
every other farm sits well above +0.01%. The parent already bounds alpha's
terminal 1/lr_base divergence via scale_pos (helping dei_n80_omnidir go from
negative farm-balanced territory to break-even), and earlier attempts that
pushed hold_frac, spike0, relax_p1, or beta_p0 further via scale_pos made the
farm-balanced score *worse* (-0.021%, -0.012%), so those four levers are left
untouched here. The remaining untried degree of freedom from the prior-art
menu's "phase-transition the Adam moments" hypothesis is the *magnitude* of
beta2's terminal plateau (previously only its onset timing, beta_p0, was
scaled) -- more turbines means more simultaneous spacing-constraint gradient
terms superposed into the update each step, so a higher terminal beta2
averages that increased per-step gradient variance more aggressively for
large-N farms specifically, without touching lr, alpha, or beta1 at all.

This adds one new lever, _B2_HI_SCALE_SPAN, raising beta2's terminal value
(_B2_HI) by up to +0.15 scaled by scale_pos (the same absolute-N factor
already used elsewhere, in [0, 1], zero for n_turbines <= 30). It leaves
beta2's LOW value, beta1 entirely, and the ramp timing (beta_p0/beta_p1)
unchanged, so small/mid farms with scale_pos == 0 (e.g. parque_n10/n20) are
completely unaffected and match the parent exactly; dei_n50/n50_uniform
(scale_pos ~= 0.37) and dei_n80_omnidir (scale_pos ~= 0.71) get a
proportionally larger terminal beta2.
...
absolute-N-scaled alpha-denominator floor) is unchanged from the parent.
"""
...
_B2_HI_SCALE_SPAN = 0.15        # NEW: heavier terminal beta2 averaging for large-N farms
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos    # NEW: heavier terminal averaging, large N
