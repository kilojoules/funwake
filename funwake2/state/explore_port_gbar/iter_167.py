"""Schedule: builds on generation-167 parent (+0.1945% farm-balanced), which
added a static, N-only "mid_n_bump" gate (scale_pos in [0.15, 0.45]) meant to
help the mid-N farms (dei_n50 / dei_n50_uniform, both n=50). That gate cannot
tell dei_n50 apart from dei_n50_uniform -- they share identical n_turbines,
D, and min_spacing, so scale_pos/size are IDENTICAL for both -- yet their
scores differ substantially (+0.0215% vs +0.0128% in the parent's own
per-cell breakdown). Applying one fixed bump to both farms indiscriminately
is presumably why this patch scored *below* the best-so-far checkpoint
(+0.1945% vs +0.1959%): it likely helped one of the pair and hurt the other.

The only per-instance signal available beyond n/D/spacing is alpha0 (mean
|grad J| / D), which differs between dei_n50 and dei_n50_uniform because
their wind roses (and hence gradient statistics) differ even at matched
n/D/spacing. This variant modulates the mid-N bump's three spans by a
bounded, smooth factor of alpha0 (via jnp.tanh(jnp.log(alpha0)), traceable
and safe for any positive alpha0) instead of applying them uniformly. The
tanh keeps the modulation within +/-40% of the original span regardless of
how well the reference scale is calibrated, so this cannot blow up even if
the sign guess is wrong for one of the two farms -- it can only partially
differentiate them instead of forcing an identical response. Every other
mechanism (lr table, warmup, relax/spike shaping, high-N gate, alpha floor,
beta ramp) is byte-for-byte unchanged from the parent.
"""
...
# Extra, smoothstep-bump-gated relief for the MID-N band (scale_pos ~ 0.15
# to 0.45, i.e. n ~ 34-52) -- covers dei_n50 / dei_n50_uniform, which sit
# strictly below the high-N gate (onset 0.5) and so receive none of the
# above relief. The bump rises 0.15->0.30 and falls back to 0 by 0.45,
# so parque_n10/n20 (clipped to scale_pos=0) and the high-N band (>=0.5,
# already separately tuned) are both unaffected.
_MID_N_P0, _MID_N_P1 = 0.15, 0.30
_MID_N_P2, _MID_N_P3 = 0.30, 0.45
_RELAX_MID_N_SPAN = 0.04         # small early-floor lift for mid-N farms
_RELAX_P1_MID_N_SPAN = 0.03      # small extra engagement delay for mid-N farms
_ALPHA_FLOOR_MID_N_SPAN = 0.5    # small extra bounded-floor relief for mid-N farms
# alpha0-derived, per-instance modulation of the mid-N bump: dei_n50 and
# dei_n50_uniform share identical n/D/spacing (so identical scale_pos/size)
# but differ in wind rose and thus in alpha0 -- this is the only available
# signal that can tell them apart. Bounded via tanh(log(.)) so a poorly
# calibrated reference can only shrink the modulation toward 0, never
# invert or blow up the bump.
_MID_N_ALPHA_REF = 1.0
_MID_N_ALPHA_SPAN = 0.4
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_bump = (_smoothstep_scalar(scale_pos, _MID_N_P0, _MID_N_P1)
                  * (1.0 - _smoothstep_scalar(scale_pos, _MID_N_P2, _MID_N_P3)))

    # Per-instance (wind-rose-sensitive) modulation of the mid-N bump, built
    # from alpha0 -- the only signal that differs between farms sharing the
    # same n_turbines/D/min_spacing. Traced-safe: alpha0 stays a jnp value
    # throughout, tanh(log(.)) is bounded so this can only scale the bump by
    # roughly [0.6x, 1.4x], never invert its sign or diverge.
    alpha0_j = jnp.asarray(alpha0)
    mid_n_mod = 1.0 + _MID_N_ALPHA_SPAN * jnp.tanh(
        jnp.log(jnp.maximum(alpha0_j, 1e-12) / _MID_N_ALPHA_REF))
    mid_n_bump_j = mid_n_bump * mid_n_mod
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              ...
              + _RELAX_MID_N_SPAN * mid_n_bump_j)      # small early-floor lift for mid-N farms
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_MID_N_SPAN * mid_n_bump_j)  # small extra delay for mid-N farms
    ...
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  ...
                                  + _ALPHA_FLOOR_MID_N_SPAN * mid_n_bump_j)
