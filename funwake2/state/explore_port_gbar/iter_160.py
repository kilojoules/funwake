"""Schedule: starts from the best-so-far variant (+0.1959% farm-balanced),
which gates extra exploration hold / delayed penalty-engagement / softened,
delayed terminal spike / bounded alpha floor onto the N~=60-100+ band via
`high_n_ramp` (scale_pos in [0.5, 0.78]) -- a gate that is exactly 0 for
every other farm in the fitness set (parque_n10/20 at scale_pos==0, dei_n50
and dei_n50_uniform at scale_pos~=0.37), so those farms are numerically
untouched by anything this variant changes.

Diagnosis: dei_n80_omnidir (n=80, omnidirectional, scale_pos~=0.71) remains
the worst-performing farm. The best-so-far variant already delays and
softens the terminal alpha spike for this band, but the spike ramp itself
is still a *symmetric* smoothstep: essentially flat until spike_p0, then a
fast, back-loaded rise to full magnitude right at the end. For a many-pair
omnidirectional layout that has just come off a longer, further-delayed
exploration/relax phase, a back-loaded spike leaves very little optimizer
budget to actually resolve the growing constraint pressure before the run
ends -- the penalty ramps hardest exactly when there is least time left to
respond to it.

This variant keeps spike0 (magnitude), spike_p0 (onset) and _SPIKE_P1
(end) exactly as in the best-so-far variant -- same total spike magnitude,
same window -- but for the high-N gated band ONLY, blends the ramp *shape*
towards a front-loaded curve (1-(1-x)^2 instead of smoothstep's 3x^2-2x^3)
that reaches a given fraction of the terminal penalty earlier inside that
same window, giving the optimizer more time to adapt to the rising penalty
before the run ends, without changing where the ramp starts, where it
ends, or its final magnitude. Because the blend weight is `high_n_ramp`
(the same gate already used for every other high-N-only term), this is a
zero-effect no-op for parque_n10/20 and both dei_n50 variants, and only
changes dei_n80_omnidir's (and any similarly-scaled farm's) late-phase
spike trajectory. Every other mechanism is unchanged from the best-so-far
variant.
"""
