"""Schedule: extends the existing high-N relief mechanism (gated by
`high_n_ramp` on scale_pos in [0.5, 0.78], targeting dei_n80_omnidir) down
to a NEW mid-N band targeting dei_n50 / dei_n50_uniform.

Diagnosis: dei_n50 and dei_n50_uniform share identical n_turbines, D, and
min_spacing (same polygon, only the wind rose differs), so every shape
parameter derived from `_packing_size`/`_n_scale` is already IDENTICAL
between them -- the schedule can only distinguish them via the linear
alpha0 scaling. dei_n50_uniform is the worst-performing farm overall
(+0.0139%) and dei_n50 itself is also weak (+0.0277%), while the low-N
farms (parque, scale_pos=0) and the already-gated high-N farm (n=80,
scale_pos~0.71) both score far better. That pattern -- weak performance
concentrated exactly in the untouched scale_pos~0.3-0.5 gap between the
low-N and high-N bands -- mirrors why the high-N gate helped n=80: a bit
more exploration hold and a delayed, softened penalty engagement gives the
optimizer more room before constraints start dominating.

This adds a smooth trapezoidal `mid_n_ramp` gate over scale_pos in
[0.12, 0.60] (full effect ~ n in [45, 56], tapering to zero by n~=36 and
n~=69), and uses it to extend hold_frac, delay relax_p1, lower relax0, and
soften spike0 -- the exact same style of relief already applied to the
high-N band, just centered on the previously-unaddressed mid-N band.
parque_n10/n20 (scale_pos=0) and dei_n80_omnidir (scale_pos~=0.71, outside
this band, already covered by its own gate) are unaffected: mid_n_ramp is
exactly 0 there. Every other mechanism is unchanged from the best-so-far
variant.
"""
...
# NEW: mid-N band relief, targeting the untouched gap between the low-N
# (parque, scale_pos=0) and high-N (n=80, scale_pos>=0.5) gates -- roughly
# n in [45, 56] at full strength, tapering to zero by n~=36 and n~=69.
_MID_N_P0, _MID_N_P1 = 0.12, 0.30
_MID_N_P2, _MID_N_P3 = 0.45, 0.60
_MID_N_HOLD_SPAN = 0.03
_MID_N_RELAX_SPAN = 0.03
_MID_N_RELAX_P1_SPAN = 0.03
_MID_N_SPIKE_SPAN = 0.20
...
def _mid_n_ramp(scale_pos):
    """Trapezoidal gate: 0 below _MID_N_P0, 1 on [_MID_N_P1, _MID_N_P2],
    back to 0 above _MID_N_P3. Targets the n~=45-56 band left untouched by
    both the low-N (scale_pos=0) and high-N (scale_pos>=0.5) treatments."""
    up = _smoothstep_scalar(scale_pos, _MID_N_P0, _MID_N_P1)
    down = 1.0 - _smoothstep_scalar(scale_pos, _MID_N_P2, _MID_N_P3)
    return min(up, down)
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_ramp = _mid_n_ramp(scale_pos)
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _MID_N_HOLD_SPAN * mid_n_ramp)        # extra gated hold for N~=45-56 band
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              ...
              - _MID_N_RELAX_SPAN * mid_n_ramp)         # extra gated relief for N~=45-56 band
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              - _MID_N_SPIKE_SPAN * mid_n_ramp)         # extra gated softening for mid-N band
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _MID_N_RELAX_P1_SPAN * mid_n_ramp)    # extra gated delay for N~=45-56 band
