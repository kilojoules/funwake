"""Schedule: extends the best-so-far variant (+0.1959% farm-balanced) by
widening the gated high-N relief band. The gate (_ALPHA_FLOOR_HIGH_N_P0/P1)
was tuned around n=80 (scale_pos~=0.71) and previously started at
scale_pos=0.50, so it gave dei_n80_omnidir a longer hold, delayed relax
ramp, softened terminal spike, and raised alpha floor -- but excluded the
n=50 farms (dei_n50, dei_n50_uniform, both scale_pos~=0.368) entirely,
since 0.368 < 0.50. dei_n50_uniform is currently the single worst-scoring
farm (+0.0139%), well below dei_n50 (+0.0277%) at the same n_turbines/D/
min_spacing -- the only signal separating them is a harder wind rose
(reflected indirectly through alpha0's gradient-magnitude scale). Both are
mid-size farms that plausibly benefit from a bit of the same relief already
validated for the larger n=80 band: more exploration time before the
penalty ramp engages, and a softer/later terminal spike.

This variant lowers _ALPHA_FLOOR_HIGH_N_P0 from 0.50 to 0.32, so the
existing smoothstep gate starts admitting scale_pos values below the n=80
band and reaches ~0.055 (vs 0 before) at n=50's scale_pos=0.368 -- a small,
smooth, N-derived nudge, not a farm-specific hardcode. It leaves parque_n10
and parque_n20 (scale_pos=0, still gated to exactly zero relief) and
dei_n80_omnidir (deep in the gate, ramp value barely changes) untouched.
...
# (scale_pos in [0.32, 0.78]) -- widened from [0.5, 0.78] to also admit
# n=50 farms (scale_pos~=0.368), which previously got none of this relief.
