"""Schedule: extend the smoothstep-gated high-N relief mechanism (proven on
the alpha floor in the parent) to the terminal alpha SPIKE as well, and widen
the alpha-floor relief band so it reaches further into the N~=50-90 range
where dei_n80_omnidir (scale_pos~=0.71) actually sits.

Diagnosis vs. parent (+0.190% farm-balanced, dei_n80_omnidir still the only
farm below native at -0.004%): the parent's alpha-floor relief ramps over
scale_pos in [0.5, 0.85], so dei_n80_omnidir (scale_pos~=0.71) only reaches
~64% of full relief -- close, but not enough to flip it positive. Two
changes, both scoped by the *same* smoothstep gate so farms at or below the
gate (dei_n50 at scale_pos~=0.37, parque_n10/20 at scale_pos==0) are
untouched:

1. Widen the alpha-floor relief ramp to [0.45, 0.80] (from [0.5, 0.85]) so
   dei_n80_omnidir sits at ~85% of full relief instead of ~64%, without
   moving the ramp's *start* early enough to touch dei_n50.
2. Add a matching, independently-scaled smoothstep-gated relief term on the
   terminal alpha SPIKE (previously only linearly softened via
   _SPIKE_SCALE_SPAN): the same crowded/many-pair farms that need a lower
   alpha floor also over-shoot on the terminal feasibility spike, so give
   them a bit less spike too, using the identical high_n_ramp gate.

All other machinery (self-contained inverse-time product lr decay to
gamma_min, short linear warmup, density-extremity relax/spike/hold, absolute-
N scale on hold/spike/relax_p1, the hold_frac-floored beta ramp) is
...
# [0.45, 0.80]) -- widened from the parent's [0.5, 0.85] so dei_n80_omnidir
# (scale_pos~=0.71) lands near the top of the ramp (~85% relief) instead of
# mid-ramp (~64%), while still leaving dei_n50 (scale_pos~=0.37) and
# parque_n10/20 (scale_pos==0) fully below the gate, untouched.
