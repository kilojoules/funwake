"""Schedule: same as parent (+0.1952% farm-balanced) but adds a SECOND,
narrower smoothstep-gated relief band (analogous to the existing
N~=60-100+ "high_n" gate) that targets the mid-N band around n~=50 --
the band that currently contains the worst-performing farm.

Diagnosis vs. parent: dei_n50_uniform (n=50, scale_pos~=0.368) is the
worst-performing farm (+0.0139%), even below dei_n80_omnidir
(+0.0223%, already relieved by the existing high_n gate at
scale_pos in [0.5, 0.78]). dei_n50 and dei_n50_uniform share identical
n_turbines/D/min_spacing, so every *static* (trace-time) schedule
feature -- hold_frac, decay table, beta timing -- is IDENTICAL between
them; the only thing that differs at runtime is alpha0 (the realized
mean|grad J|/D), which cannot drive static scheduling decisions (it's a
traced value inside the jit loop). What CAN differ between the two n=50
farms is nothing in this schedule at all today for hold/relax timing --
so dei_n50_uniform gets exactly the same exploration budget as its
easier sibling, and evidently that's not enough for its harder
(uniform-wind-rose-driven) constraint landscape.

Since scale_pos for n=50 (~0.368) sits BELOW the existing high_n gate's
P0=0.5, that band gets none of the relief that helped dei_n80_omnidir.
This variant adds a second gate, _ALPHA_FLOOR_MID_N_P0/P1 = 0.20/0.42,
covering exactly this n~=50 band (smoothstep saturates near scale_pos
~=0.42, well below the high_n gate's P0=0.5) with its own, smaller
hold_frac / relax_p1 / alpha-floor spans -- giving n~=50 farms modestly
longer exploration and later, more relaxed constraint engagement before
the terminal spike, mirroring the mechanism that already worked for
n~=80. To keep it from double-relieving the n~=80 band (where
scale_pos briefly re-enters after saturating), the mid-band ramp is
multiplied by (1 - high_n_ramp), fading it out exactly where the
existing high_n gate turns on, so dei_n80_omnidir's already-tuned
behavior stays essentially untouched. parque_n10/n20 (scale_pos <= 0)
and dei_n80 (scale_pos ~=0.708, deep in the high_n band) are
essentially unaffected; dei_n50 and dei_n50_uniform (scale_pos ~=0.368)
get the new relief. Every other mechanism (self-contained inverse-time
product lr decay to gamma_min, short linear warmup, density-extremity
relax/spike/hold, absolute-N scale on hold/spike/relax_p1, the
existing high-N alpha-floor relief, the hold_frac-floored beta ramp) is
unchanged.
"""
...
_RELAX_P1_MID_N_SPAN = 0.03     # extra gated delay for N~=50 band
...
# Second, narrower gate for the N ~= 50 band (scale_pos in [0.20, 0.42]),
# faded out by (1 - high_n_ramp) so it does not double-relieve N~=80.
_ALPHA_FLOOR_MID_N_P0 = 0.20
_ALPHA_FLOOR_MID_N_P1 = 0.42
_ALPHA_FLOOR_MID_N_SPAN = 1.2
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # narrower N~=50 gate, faded out where the high_n gate takes over so
    # the two bands don't double-relieve N~=80.
    mid_n_ramp = _smoothstep_scalar(
        scale_pos, _ALPHA_FLOOR_MID_N_P0, _ALPHA_FLOOR_MID_N_P1) * (1.0 - high_n_ramp)
    ...
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  ...
                                  + _ALPHA_FLOOR_MID_N_SPAN * mid_n_ramp)
