"""Schedule: add a threshold-gated quadratic extension to hold_frac and a
matching quadratic softening of the terminal spike, both gated on the same
high_n_gap = max(scale_pos - 0.5, 0) used by the existing alpha-floor boost,
targeting dei_n80_omnidir specifically without touching dei_n50 (scale_pos
~=0.37, gate=0) or the parque farms (scale_pos==0, gate=0).

Diagnosis vs. the current best: the parent already decouples dei_n80's
terminal alpha *denominator* from dei_n50's via a quadratic gate on
high_n_gap, and that closed most of the gap (native-relative score moved
from clearly negative to only -0.0025%, i.e. essentially flat). But two
other levers still couple the two farms linearly through scale_pos alone:
hold_frac (exploration-plateau length) and spike0 (terminal penalty
multiplier). For an 80-turbine, high-pair-count farm, a longer pure-
exploration plateau before lr starts decaying, PLUS a further-softened
terminal spike (so alpha's late-stage multiplier doesn't overshoot on a
farm with many more spacing-constraint pairs than dei_n50), are both
mechanisms the prior-art survey identifies (delayed engagement / bounded
terminal correction) that the parent has not yet applied beyond alpha's
denominator. Reusing the same high_n_gap gate keeps every other farm's
trajectory bit-for-bit identical -- only dei_n80_omnidir (high_n_gap ~=
0.208) sees nonzero contributions from _HOLD_HIGH_N_SPAN and
_SPIKE_HIGH_N_SPAN; dei_n50 and the parque farms are provably unaffected
since high_n_gap == 0 for all of them.
"""
...
# Threshold-gated (same gate) extra hold-time and spike-softening for very
# large-N farms specifically, isolated from dei_n50/parque via high_n_gap.
_HOLD_HIGH_N_SPAN = 0.05
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_HIGH_N_SPAN * high_n_gap_sq)    # extra hold for very-large-N farms only
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGH_N_SPAN * high_n_gap_sq)     # extra softening for very-large-N farms
