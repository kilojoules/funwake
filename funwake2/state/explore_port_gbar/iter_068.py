"""Schedule: decouple beta2's ramp onset from beta1's (isolated single-lever
change) so the second Adam moment begins tracking gradient-magnitude
variance earlier than momentum builds up.

Diagnosis vs. the parent: the parent ramps beta1 (0.1->0.25) and beta2
(0.2->0.45) on the *same* smoothstep window [beta_p0, _BETA_P1], i.e. both
moments transition from native's near-raw-SGD regime to a mildly-averaged
regime simultaneously. But these two moments serve different jobs: beta1
controls momentum/inertia (useful to keep low while still exploring),
while beta2 controls the adaptive per-coordinate step-size normalization
(useful to raise sooner, so the denominator sqrt(v)+eps starts absorbing
the increasingly large constraint-gradient variance introduced by relax/
spike as alpha engages -- rather than waiting until relax/spike are
already mid-ramp). The frontier survey's beta2 row explicitly flags
"ramp up ... tames early adaptive-lr variance and absorbs the ~alpha
constraint-curvature conditioning" as a distinct, untried hypothesis from
the beta1 row's momentum framing -- the parent currently conflates them
into one shared ramp. This change adds exactly one new lever: a small,
constant lead time (_BETA2_LEAD) by which beta2's ramp onset precedes
beta1's, on every farm equally (no per-farm scale term, since the
hypothesis is about moment dynamics, not farm geometry). beta1's
schedule, and every other lever (lr decay table, warmup, relax/spike/
hold, alpha's bounded denominator), is byte-for-byte unchanged from the
parent, so any farm-balanced movement isolates this one change.
"""
...
# NEW: beta2 leads beta1's ramp by a fixed amount of run-progress, so the
# adaptive second-moment estimate starts absorbing constraint-gradient
# variance before momentum (beta1) builds up -- a decoupled, untried lever.
_BETA2_LEAD = 0.15
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    beta2_p0 = max(beta_p0 - _BETA2_LEAD, 0.0)              # beta2 leads beta1's ramp onset
    r1 = _ramp(p, beta_p0, _BETA_P1)
    r2 = _ramp(p, beta2_p0, _BETA_P1)
