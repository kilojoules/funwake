...
band via `high_n_ramp` (scale_pos in [0.5, 0.78]). The worst-performing farm
in the current parent is actually dei_n50_uniform (n=50, scale_pos~=0.368),
which sits BELOW that gate and gets none of its relief. This variant adds a
second, LOCALIZED gate -- a Gaussian bump centered at scale_pos~=0.37 -- that
gives the n~=50 band a little extra exploration hold, a slightly delayed
constraint (relax) engagement, and a softened terminal spike, exactly
mirroring the mechanism already validated for the high-N band. The bump is
narrow enough (width 0.15) that it decays to ~0 by scale_pos=0 (parque_n10/20,
untouched) and by scale_pos~=0.71 (dei_n80_omnidir, untouched, already
served by its own high-N gate). Every other mechanism is unchanged from the
best-so-far variant.
"""
...
# Localized bump gate for the N~=50 band (scale_pos~=0.37), which sits below
# the high-N gate and currently gets no dedicated relief despite being the
# worst-performing band (dei_n50 / dei_n50_uniform). Gaussian so it decays to
# ~0 at scale_pos=0 (parque_n10/n20) and at scale_pos~=0.71 (dei_n80_omnidir,
# already served separately by the high-N gate).
_MIDN_CENTER = 0.37
_MIDN_WIDTH = 0.15
_MIDN_HOLD_SPAN = 0.02
_MIDN_RELAX_P1_SPAN = 0.05
_MIDN_SPIKE_SPAN = -0.12
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_bump = math.exp(-((scale_pos - _MIDN_CENTER) / _MIDN_WIDTH) ** 2)
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _MIDN_HOLD_SPAN * mid_n_bump)          # extra gated hold for N~=50 band
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _MIDN_SPIKE_SPAN * mid_n_bump)         # extra gated softening for N~=50 band
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _MIDN_RELAX_P1_SPAN * mid_n_bump)     # extra gated delay for N~=50 band
