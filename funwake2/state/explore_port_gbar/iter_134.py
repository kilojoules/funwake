"""Schedule: same as the previous best (+0.191% farm-balanced) but with the
smoothstep-gated high-N alpha-floor relief window narrowed further (0.78 ->
0.72) so it delivers materially more relief right at dei_n80_omnidir's
scale_pos (~=0.708), which the prior 0.5-0.78 window still left at only
~84% of the relief ceiling.

Diagnosis vs. parent: the parent already narrowed the ramp from 0.5-0.85 to
0.5-0.78 to fix "too flat near the gate" for dei_n80_omnidir, and that
change measurably helped (span was also bumped 3.0 -> 3.5 in the same
step). But dei_n80_omnidir sits at scale_pos ~= 0.708, which is still only
~84% up the 0.5-0.78 ramp (smoothstep((0.708-0.5)/0.28) ~= 0.84) -- a
meaningful fraction of intended relief was still being left on the table
for exactly the farm this mechanism targets. Narrowing the upper edge to
0.72 puts dei_n80_omnidir at ~99% of the ramp (smoothstep((0.708-
0.5)/0.22) ~= 0.99), i.e. essentially full relief, while:
  - the lower edge (0.5) is unchanged, so scale_pos<=0.5 (dei_n50 ~=0.368,
    parque_n10/20==0) still gets EXACTLY zero relief from this term --
    those farms' alpha_denom_floor is untouched;
  - farms at scale_pos>=0.72 already saturated under the old 0.78 edge and
    continue to saturate to the same relief ceiling now.
A small increase in the relief span (3.5 -> 4.0) adds a bit more absolute
magnitude on top of the retiming, continuing the trend from the prior
generation (3.0 -> 3.5) that both timing AND magnitude were under-
delivering at N~=80. Every other mechanism (self-contained inverse-time
...
hold_frac-floored beta ramp) is unchanged from the parent.
"""
...
# Extra, smoothstep-gated relief for farms in the N ~= 60-90 band
# (scale_pos in [0.5, 0.72]) -- decouples dei_n80_omnidir's floor from
# dei_n50's without touching any farm below the gate, and (narrower than
# the parent's 0.5-0.78 ramp) delivers essentially full relief right at
# dei_n80_omnidir's scale_pos ~= 0.708 instead of leaving it at ~84%.
_ALPHA_FLOOR_HIGH_N_P0 = 0.5
_ALPHA_FLOOR_HIGH_N_P1 = 0.72
_ALPHA_FLOOR_HIGH_N_SPAN = 4.0
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # terms + a smoothstep-gated term ramping over scale_pos in [0.5,0.72])
