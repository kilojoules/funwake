...
N~=60-100+ band (`high_n_ramp`, scale_pos in [0.5, 0.78]) and a narrower
N~=50 band (`mid_n_bump`, scale_pos in [0.15, 0.55]).

Diagnosis: across recent runs the N=50 pair (dei_n50 +0.0501%, dei_n50_uniform
+0.0151%) remains the weakest link -- dei_n50_uniform is the worst farm
overall. Since dei_n50 and dei_n50_uniform are bit-for-bit identical in
n_turbines/D/min_spacing (only their wind rose differs, which schedule_fn has
no static access to), the schedule cannot distinguish them; the only lever
left is to give the *shared* N~=50 band proportionally more of the same
relief mechanisms that already worked for the N~=60-100+ band, since that
band is the current bottleneck for the farm-balanced mean. This scales up
the existing mid-N bump's magnitudes (hold, spike-softening, relax delay,
alpha-floor relief) to sit closer to the high-N gate's magnitudes, without
touching the low-N (parque) or high-N (n80) behavior, both of which are
already well-tuned and score highest of the five farms.
"""
...
# onset at 0.5 -- gives the N=50 band (currently the worst-scoring pair, and
# un-served by the high-N gate) a bigger, still-independent slice of the same
# relief mechanisms that already helped n80 (magnitudes moved closer to,
# though still below, the high-N gate's).
_MID_N_P0, _MID_N_P1 = 0.15, 0.55
_MID_N_HOLD_SPAN = 0.025
_MID_N_RELAX_P1_SPAN = 0.045
_MID_N_SPIKE_SPAN = -0.15
_MID_N_ALPHA_FLOOR_SPAN = 2.0
