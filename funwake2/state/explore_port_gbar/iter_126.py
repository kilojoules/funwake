"""Schedule: extend the smoothstep-gated high-N relief (already applied to the
alpha-floor) to the terminal spike and early-relax penalty shaping as well.

Diagnosis vs. parent (+0.1911% farm-balanced, dei_n80_omnidir still nearly
flat at +0.0015% despite N=80 landing mid the alpha-floor's smoothstep gate
[scale_pos~=0.71, high_n_ramp~=0.65]): the parent only decouples the alpha
*floor* for the N~=60-120 band -- but `spike0` (terminal feasibility spike
magnitude) and `relax0` (early penalty floor) still scale linearly in
`scale_pos` across the whole range, exactly like `dei_n50` (scale_pos~=0.37)
and `parque_n10/20` (scale_pos==0). So even though the alpha *denominator*
gets extra relief for dei_n80_omnidir, the penalty *multipliers* (relax,
spike) it's multiplied against are not correspondingly relaxed for that same
band, leaving the net penalty at N~=80 still close to what a linear-only
model would give and starving the farm of the late-stage AEP-optimizing
freedom the floor relief was meant to unlock. This variant reuses the exact
same `high_n_ramp` smoothstep gate (scale_pos in [0.5, 0.85], zero outside
it) to (a) further soften the terminal spike and (b) further lower the early
relax floor, specifically in that band -- without touching dei_n50
(scale_pos~=0.37, ramp==0) or parque_n10/20 (scale_pos==0, ramp==0). All
other machinery is unchanged from the parent.
"""
...
# Same gate reused to further relax the penalty *multipliers* (not just the
# alpha denominator) for the N~=60-120 band specifically.
_SPIKE_HIGH_N_SPAN = -0.5       # extra terminal-spike softening in the gate band
_RELAX_HIGH_N_SPAN = -0.08      # extra early-relax lowering in the gate band
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              ...
              + _RELAX_HIGH_N_SPAN * high_n_ramp)      # extra early-floor relief in N~=60-120 band
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              + _SPIKE_SCALE_SPAN * scale_pos
              + _SPIKE_HIGH_N_SPAN * high_n_ramp)      # extra terminal-spike softening in that band
    relax0 = max(relax0, 0.05)                        # keep a nonzero feasibility floor
    spike0 = max(spike0, 0.0)                         # never let the spike go negative
