"""Schedule: lengthen the linear lr-warmup for large-N farms via a
scale_pos-gated span on warm_frac (isolated single-lever change).

Diagnosis vs. the parent: dei_n80_omnidir (80 turbines -> scale_pos>0, far
more turbine pairs than any other fitness farm) sits flat (-0.001%) while
every other farm gains. The parent already pushed hold_frac, spike0,
relax_p1, beta_p0, and the alpha_denom_floor via scale_pos to compensate,
with rapidly diminishing (alpha_floor) or negative (hold/spike/relax/beta)
returns -- so the remaining gap is unlikely to be more alpha/beta shaping.
A more numerous set of turbine pairs means the constraint gradient at each
step is a noisier, higher-variance sum (more terms averaged), which the
fixed _WARM_FRAC=0.015 linear warmup (identical in *step count* for every
farm) does not account for: large-N farms reach full exploration lr just
as fast as small-N farms despite noisier early gradients, risking early
bad moves that the later penalty/beta shaping can only partially undo.

This change adds ONE new lever -- scale the warmup length itself by
scale_pos (0 for small/mid-N farms, matching the parent exactly; positive
only for large-N farms like dei_n80_omnidir) -- so large-N farms get a
longer, gentler ramp to full exploration lr before the existing hold/decay
schedule takes over. n_warm is still capped well inside n_hold so it never
encroaches on the decay phase. All other machinery (lr decay table,
density/extremity relax-spike shaping, absolute-N-gated hold/spike/relax_p1/
alpha_denom_floor/beta_p0) is unchanged from the parent.
"""
...
_WARM_FRAC_SCALE_SPAN = 0.02    # NEW: longer warmup for large-N (noisier-gradient) farms
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    n_warm = max(min(int(n_total * warm_frac), n_hold), 1)
