"""Schedule: extend the existing smoothstep-gated N~=60-120 relief (currently
applied only to the alpha_denom floor) to ALSO ease the terminal alpha spike
and the early relax floor over the same band, since dei_n80_omnidir
(scale_pos ~= 0.708, squarely inside the [0.5, 0.85] gate) is still the sole
farm feasible-but-below-native despite the alpha-floor relief.

Diagnosis vs. parent (+0.190% farm-balanced, dei_n80_omnidir at -0.004%):
the parent already smoothstep-relieves the alpha_denom floor for
scale_pos in [0.5, 0.85], which is exactly dei_n80_omnidir's band -- but
`spike0` and `relax0` only get a *linear* `_SCALE_SPAN * scale_pos` term
applied uniformly across all scale_pos, so a farm sitting mid-gate still
gets the terminal alpha SPIKE softened by the same fixed linear amount as a
farm at scale_pos=1.0 or 0.5, and its early-exploration floor (`relax0`)
lowered by the same fixed amount too. If the alpha_denom floor relief alone
was insufficient to fix dei_n80_omnidir, the two remaining shaping terms
(spike0, relax0) are the natural next levers to retime onto the same
[0.5, 0.85] band -- reusing the identical `high_n_ramp` gate already
computed for alpha_denom, so parque_n10/20 (scale_pos==0) and dei_n50
(scale_pos ~= 0.37, below the gate) are completely unaffected, exactly like
the existing alpha-floor relief. Both new terms are small (span 0.35 and
0.06 respectively) and directionally consistent with the existing
_SCALE_SPAN terms: less terminal spike, lower early floor, specifically in
the N~=60-120 band where dei_n80_omnidir lives. All other machinery
...
# [0.5, 0.85]) -- decouples dei_n80_omnidir's floor from dei_n50's without
# touching any farm below the gate.
_HIGH_N_P0 = 0.5
_HIGH_N_P1 = 0.85
_ALPHA_FLOOR_HIGH_N_SPAN = 3.0
# Same gate, reused to also ease the terminal spike and early relax floor
# specifically for the N~=60-120 band (new in this variant).
_SPIKE_HIGH_N_SPAN = 0.35       # extra spike0 reduction in the gated band
_RELAX_HIGH_N_SPAN = 0.06       # extra relax0 reduction (lower floor) in the gated band
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              ...
              - _RELAX_HIGH_N_SPAN * high_n_ramp)     # extra floor relief in the N~=60-120 band
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              + _SPIKE_SCALE_SPAN * scale_pos
              - _SPIKE_HIGH_N_SPAN * high_n_ramp)     # extra spike relief in the N~=60-120 band
