"""Schedule: extend the threshold-gated high-N relief (currently only applied
to the terminal alpha-denominator floor) to the relax_p1 engagement delay as
well, using the SAME smooth quadratic gate at scale_pos > 0.5.

Diagnosis vs. the current best: dei_n80_omnidir (scale_pos ~= 0.71) is the
only farm still barely above baseline (+0.0005%) while dei_n50 (scale_pos
~= 0.37, below the gate) and both parque farms (scale_pos == 0) are solidly
positive. The current best already decouples the *terminal* alpha floor for
scale_pos > 0.5 farms via a quadratic high_n_gap term, but relax_p1 (how long
the alpha ramp stays delayed before engaging at all) is still governed only
by the *linear* extremity/scale_pos terms shared with dei_n50 -- so
dei_n80_omnidir's alpha still starts ramping up at the same relative point in
training as dei_n50's, even though it needs more pure-exploration time before
the constraint penalty engages. Adding the same high_n_gap^2 term (scaled
down) to relax_p1 gives dei_n80_omnidir extra ramp delay while leaving
dei_n50 (below the 0.5 gate) and both parque farms (gate inactive, gap == 0)
completely untouched -- a strict extension of the existing decoupling
mechanism, not a new lever. All other machinery (self-contained inverse-time
product lr decay to gamma_min, short linear warmup, density-extremity
relax/spike/hold, absolute-N scale on hold/spike, the hold_frac-floored beta
ramp, the quadratic high-N alpha-floor relief) is unchanged from the parent.
"""
...
_RELAX_P1_HIGH_N_SPAN = 0.10    # extra gated delay for scale_pos beyond the 0.5 threshold
...
# (and now its ramp-engagement delay) from dei_n50's without touching any
# farm below the gate.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_HIGH_N_SPAN * high_n_gap2)  # extra gated delay past the 0.5 threshold
    relax_p1 = min(relax_p1, _SPIKE_P0 - 1e-3)          # never let engagement collide with the spike window
