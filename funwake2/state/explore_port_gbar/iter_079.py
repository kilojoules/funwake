"""Schedule: raise beta2's terminal value for large-N farms via a
scale_pos-gated additive boost on _B2_HI (isolated single-lever change on
top of the current best-so-far alpha-floor schedule).

Diagnosis vs. the current best: dei_n80_omnidir (80 turbines, by far the
most turbine pairs in the fitness set) is essentially flat (+0.0005%)
while every other farm gains meaningfully. dei_n50 and dei_n50_uniform
(the other two farms with n_turbines >= 50, hence scale_pos > 0) are also
the next-weakest results (+0.03%, +0.01%). These are exactly the three
farms where scale_pos > 0; the two parque farms (n=10, n=20) have
scale_pos clipped to 0 and are already strong (+0.22%, +0.68%). That
correlation points at gradient-noise/conditioning on many-pair farms as
the shared bottleneck, not the alpha-floor mechanism itself (which
already targets these same farms and helped, but left dei_n80 flat).

Earlier attempts already pushed hold_frac, spike0, relax_p1, and the
beta *timing* onset (beta_p0) further via scale_pos and made the
farm-balanced score worse -- so this change leaves those four levers
completely untouched and does not touch beta_p0 again. Instead it adds
one new, previously-untried lever: raise the terminal *magnitude* of
beta2 (not its onset) for large-N farms only, via a scale_pos-gated
additive term on _B2_HI. Higher terminal beta2 more strongly averages
the second-moment estimate late in optimization, which under AMSGrad-
style reasoning should damp the extra gradient-noise/curvature variance
that comes from many-pair spacing constraints on crowded layouts,
without changing when the beta ramp starts or how alpha/lr/hold behave.
For small/mid-N farms (scale_pos == 0, e.g. parque_n10/n20) beta2's
terminal value is unchanged, so this leaves those farms' schedules
exactly as in the current best.
...
absolute-N scale on hold/spike/relax_p1/beta timing/relax0, and the
alpha-denominator floor for large-N farms) is unchanged from the current
...
_B2_HI_SCALE_SPAN = 0.15        # NEW: extra terminal beta2 averaging for large-N farms
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # NEW: extra terminal beta2 averaging for large-N (many-pair) farms only,
    # to damp the additional gradient-noise/curvature variance from more
    # spacing-constraint pairs -- distinct from beta_p0 (timing), which was
    # already tuned and left alone here.
