...
Farm-balanced score is a simple mean over farms, dominated numerically by
the small-N `parque_n10`/`parque_n20` cells (0.21%, 0.70%) rather than the
dei cells (all <0.03%). parque_n20 already does very well; parque_n10 is
the visibly weaker of the two "good" farms. Both have `scale_pos == 0`
under the existing `_n_scale` gate (N_REF=30), so they currently receive
*identical* hold/relax/spike shaping to a mid-size farm -- no mechanism
distinguishes "few turbines, few spacing conflicts" from the default.

Hypothesis: a farm with very few turbines needs much less of its budget
spent on the broad exploration/hold phase (fewer local optima to search,
fewer pairwise spacing conflicts to negotiate) and benefits from shifting
that budget into the decay/fine-convergence phase instead, plus slightly
earlier constraint engagement. This adds a new, symmetric-to-`high_n_ramp`
gate on very *small* absolute N (smoothstepped over `-n_scale` in
[0.3, 0.8], i.e. N below roughly 30/4^0.3 ~= 19 down to 30/4^0.8 ~= 8) that
shortens `hold_frac` and slightly advances `relax_p1` for small-N farms.
At N=20 the gate is ~0 (untouched); at N=10 it is close to saturated. This
leaves dei_n50 (N=50), dei_n80_omnidir (N=80) and parque_n20 (N=20)
essentially unchanged, and only reshapes parque_n10 (N=10). All other
mechanisms are unchanged from the parent.
"""
...
_RELAX_P1_LOW_N_SPAN = -0.05    # advance engagement for very small-N farms
...
_HOLD_FRAC_LOW_N_SPAN = -0.05   # shorten hold for very small-N farms
...
# Gate for very small absolute N (symmetric-in-spirit to the high-N gate
# above, but on -n_scale so it only activates well below N_REF).
_LOW_N_P0, _LOW_N_P1 = 0.3, 0.8
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    n_scale = _n_scale(n_turbines)
    scale_pos = max(n_scale, 0.0)                       # 0 unless large absolute N
    high_n_ramp = _smoothstep_scalar(
        scale_pos, _ALPHA_FLOOR_HIGH_N_P0, _ALPHA_FLOOR_HIGH_N_P1)
    low_n_ramp = _smoothstep_scalar(
        max(-n_scale, 0.0), _LOW_N_P0, _LOW_N_P1)        # 0 unless small absolute N
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_LOW_N_SPAN * low_n_ramp)   # shorter hold for very small-N farms
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_LOW_N_SPAN * low_n_ramp)    # advance engagement for very small-N farms
