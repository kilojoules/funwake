"""Schedule: bound the terminal alpha divergence for large-N farms via a
scale_pos-gated floor on the lr_base denominator (isolated single-lever change).

Diagnosis vs. the parent: alpha = alpha0*diam*relax*spike / lr_base, and
lr_base's own floor is gamma_min (the decay table asymptotes there), so as
lr_base -> gamma_min at the end of the run, alpha necessarily grows to its
maximum (native's own diverging 1/lr coupling, which the parent already
delays and dampens via relax/spike but never actually bounds). For
dei_n80_omnidir -- ordinary packing density but far more turbine pairs than
any other farm in the fitness set (80 vs <=50), and consequently the
noisiest/most numerous constraint-gradient terms -- this terminal alpha
blow-up appears to fully re-engage the same constraint-dominated behavior
native exhibits, erasing the parent's gains and leaving the farm flat
(-0.0002%) instead of ahead. Earlier attempts already pushed hold_frac,
spike0, relax_p1, and beta_p0 further via scale_pos and made the
farm-balanced score worse (-0.021%, -0.012%), so this change leaves those
four levers untouched. Instead it adds one new, previously-untried lever:
raise the denominator floor under alpha (from the near-zero 1e-30 guard to
gamma_min * (1 + k*scale_pos)) so that, for large-N farms only, alpha's
terminal value is capped well before lr_base fully bottoms out --
preventing over-constraint late in a crowded layout while leaving small/
mid-N farms (scale_pos == 0, e.g. parque_n10/n20) completely unaffected
since their floor stays at gamma_min, matching the parent exactly.
...
# NEW: bound the terminal alpha denominator (lr_base floor) for large-N farms
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # NEW: cap the effective denominator at gamma_min*(1+k*scale_pos) instead
