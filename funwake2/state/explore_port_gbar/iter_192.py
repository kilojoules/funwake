"""Schedule: builds on the best-so-far farm-balanced variant (+0.2007%,
which itself layered a narrow "mid-N" bump for the dei_n50 band on top of
the earlier high-N gate for dei_n80_omnidir). All prior mechanisms are kept
bit-for-bit unchanged (hold length, warmup, relax/spike/alpha-floor shaping,
mid-N bump, base beta1/beta2 ramp) to avoid disturbing the already-tuned
per-farm behavior.

New addition (untried in prior generations per the design-menu survey,
§4/§7.5): a *phase-transition* of the Adam moments specifically gated on the
terminal feasibility-spike window (the same `spike` ramp that already
tightens alpha late in the run), not just on overall progress `p`. During
that window the penalty gradient direction becomes dominant and noisy as
alpha spikes; a small extra beta2 increase further damps the resulting
adaptive-lr variance, while a small extra beta1 decrease reduces momentum
overshoot past the now-tighter feasible region -- letting the terminal spike
do its job with less oscillation, independent of and in addition to the
farm-size-shaped alpha/relax terms. The deltas are small and additive so a
farm where the spike phase never really engages (short spike ramp) or is
gated off is unaffected.
"""
...
# NEW: extra moment phase-transition gated on the terminal feasibility spike
# window itself (not just overall progress) -- damp variance harder / cut
# momentum overshoot harder exactly while alpha is spiking.
_B2_SPIKE_SPAN = 0.08
_B1_SPIKE_SPAN = -0.04
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    spike_phase = _ramp(p, spike_p0, _SPIKE_P1)         # 0 -> 1 across the terminal window
    spike = 1.0 + spike0 * spike_phase
    ...
    # Extra moment phase-transition tied to the terminal alpha spike window
    # itself: damp adaptive-lr variance harder (beta2 up) and cut momentum
    # overshoot (beta1 down) exactly while the constraint penalty is
    # spiking, on top of (not instead of) the existing progress-based ramp.
    beta1 = _B1_LO + (_B1_HI - _B1_LO) * r + _B1_SPIKE_SPAN * spike_phase
    beta2 = _B2_LO + (_B2_HI - _B2_LO) * r + _B2_SPIKE_SPAN * spike_phase
    beta1 = jnp.clip(beta1, 0.02, 0.98)
    beta2 = jnp.clip(beta2, 0.02, 0.999)
