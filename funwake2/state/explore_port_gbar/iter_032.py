"""Schedule: sharpen the large-N alpha-floor gating from linear scale_pos
to scale_pos**2, recalibrated to match the prior linear floor at N=50 while
amplifying it further at N=80 (isolated single-lever change).

Diagnosis vs. the parent (which already added a scale_pos-gated floor under
alpha's lr_base denominator): scale_pos is a log-compressed ABSOLUTE-N
factor, and *both* dei_n50/dei_n50_uniform (N=50, scale_pos ~= 0.368) and
dei_n80_omnidir (N=80, scale_pos ~= 0.707) trigger it under the parent's
linear coefficient. That means any attempt to push the floor higher for the
worst-performing farm (dei_n80_omnidir, flat at +0.0005%) also pushes it up
for dei_n50/dei_n50_uniform, which are already comfortably positive
(+0.0326%/+0.0139%) under the *current* floor strength -- so a uniform
linear increase risks trading n50 gains for n80 gains rather than adding
a real lever.

Squaring scale_pos before applying the coefficient concentrates the
increase where it is needed: scale_pos**2 is only ~0.135 at N=50 (barely
above its current linear contribution) but ~0.5 at N=80 (more than double).
The new coefficient is chosen so the floor multiplier at N=50 is
essentially unchanged from the parent (1.542 vs. 1.552 previously) --
preserving the proven n50 behavior almost exactly -- while at N=80 the
floor multiplier rises from 2.061 to 3.0, capping alpha's terminal
divergence substantially harder for the one farm with far more turbine
pairs than any other in the fitness set. Farms below the N_REF=30
threshold (parque_n10/n20, scale_pos == 0) are completely unaffected,
matching the parent exactly.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0) is unchanged from the
parent.
"""
...
# NEW: gate on scale_pos**2 (not scale_pos) with a recalibrated coefficient
# so the floor at N=50 stays ~unchanged while the floor at N=80 is pushed
# substantially higher -- concentrating the extra capping on the farm with
# the most turbine pairs instead of spreading it uniformly across all
# large-N farms.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Cap the effective denominator at gamma_min*(1+k*scale_pos**2) instead
    ...
    # quadratic gating concentrates the extra cap on the largest-N farm
    # instead of spreading it uniformly across every farm above N_REF.
