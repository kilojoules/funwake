"""Schedule: extend the existing scale_pos>0.5 quadratic gate (previously
used only for the alpha terminal floor) to two more mechanisms that were
still purely LINEAR in scale_pos in the parent -- the terminal alpha spike
and the exploration hold length -- so the single farm still below native
(dei_n80_omnidir, scale_pos ~= 0.71) gets proportionally more relief on all
three levers while dei_n50 (scale_pos ~= 0.37, below the 0.5 gate) and the
parque farms (scale_pos == 0) see byte-for-byte identical behavior to the
parent.

Diagnosis vs. the current best: the parent already isolated the alpha-floor
lever with a quadratic gate at scale_pos > 0.5 (N > 60) so dei_n50/parque are
untouched while dei_n80_omnidir gets extra terminal-alpha relief -- but that
one lever wasn't enough to close the gap (dei_n80_omnidir still -0.0025%).
The parent's terminal SPIKE and exploration HOLD length still only scale
linearly with scale_pos (_SPIKE_SCALE_SPAN, _HOLD_FRAC_SCALE_SPAN), so their
relief at dei_n80_omnidir's scale_pos (~0.71) is capped by what's safe to
apply at scale_pos values close to the 0.5 gate boundary for smaller farms.
Since dei_n50 is safely positive and the alpha-floor gate proved that a
threshold-gated quadratic term can move a single large-N farm without
disturbing anything below the gate, the same trick is applied to: (1) the
spike amplitude (extra negative quadratic term softens dei_n80_omnidir's
terminal feasibility spike further, since a farm with more turbine pairs
needs a gentler final penalty ramp to avoid overshoot-driven layout churn),
(2) the exploration hold fraction (extra positive quadratic term gives
dei_n80_omnidir more pure-exploration time before lr starts decaying, since
more pairs need more steps to find a good packing before commitment), and
(3) the alpha floor span is nudged up slightly (6.0 -> 8.0) since the
previous magnitude only got the metric to -0.0025% (nearly neutral) --
extrapolating the same direction a bit further is the natural next step.
All three additions multiply by high_n_gap**2 = max(scale_pos-0.5,0)**2,
which is exactly 0 for dei_n50 (0.368 < 0.5) and parque_n10/20
(scale_pos == 0), so those farms' schedules are unchanged from the parent.
...
absolute-N scale on hold/spike/relax_p1, the hold_frac-floored beta ramp)
...
# Extra, threshold-gated quadratic softening of the terminal spike for farms
# well past the linear regime (scale_pos > 0.5) -- mirrors the alpha-floor
# gate below; decouples dei_n80_omnidir's spike from dei_n50's.
...
# Extra, threshold-gated quadratic hold-length boost for farms well past the
# linear regime -- more pure-exploration time for high-N farms specifically.
_HOLD_FRAC_HIGH_N_SPAN = 0.04
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_gap_sq)  # extra hold for the largest-N farms
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGH_N_SPAN * high_n_gap_sq)     # extra softening for the largest-N farms
    ...
    # crowded, many-pair farms that most need unaveraged gradient steps there.
