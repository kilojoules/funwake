...
which already gates hold/relax-delay relief and a softened terminal alpha
spike onto the N~=60-100+ band via `high_n_ramp`. This variant tests the
one frontier hypothesis the prior generations left untried: (4) phase-
transition the Adam moments *with* the alpha phase, rather than on an
independently-tuned progress window.

Diagnosis: in the parent, `beta_p0` (where beta1/beta2 start ramping up
toward their late-run values) was driven by a fixed constant (_BETA_P0)
shaped only by `scale_pos`, decoupled from `relax_p1` -- the point at which
the alpha penalty itself finishes ramping to full strength. For farms where
`relax_p1` is pushed later (high extremity / large-N / gated high-N band),
the old beta window could start averaging gradients *before* the penalty
had fully engaged, or engage momentum out of sync with when ALM-like
constraint pressure actually appears. The mechanism this variant tests:
momentum growth should track the *actual*, per-farm alpha engagement
point (relax_p1), not a separately-tuned constant -- so beta1/beta2 begin
climbing exactly when the constraint penalty finishes ramping in, acting
as an implicit augmented-Lagrangian multiplier once alpha is live. This
keeps every farm's *existing* relax/spike/hold shaping untouched and only
resynchronizes the beta ramp's start point to `relax_p1`, still floored by
`hold_frac` (never average during pure exploration) and still nudged
earlier for large-N via the existing `_BETA_P0_SCALE_SPAN` term.
"""
...
# Adam moments: native early -> mildly averaged late. The ramp START is now
# synchronized to relax_p1 (the per-farm point where alpha finishes engaging)
# rather than an independently-tuned constant -- momentum builds once the
# penalty is live, acting like an implicit ALM multiplier.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Synchronize the beta (momentum) ramp's onset to relax_p1 -- the point
    # where THIS farm's alpha penalty finishes engaging -- instead of an
    # independently-tuned constant. This makes momentum growth track actual
    # constraint pressure per-farm (implicit ALM multiplier), while still
    # never starting before the lr hold phase ends, and still nudging
    # earlier for large-N farms via the existing scale term.
    beta_p0 = max(relax_p1 + _BETA_P0_SCALE_SPAN * scale_pos, hold_frac)
