"""Schedule: same as best (+0.192% farm-balanced) but pushes the smoothstep-
gated high-N alpha-floor relief closer to full delivery at dei_n80_omnidir's
actual scale_pos (~0.71), which prior iterations already targeted but left
short of saturation.

Diagnosis: with the parent's ramp (P0=0.5, P1=0.78), dei_n80_omnidir sits at
smoothstep((0.71-0.5)/0.28) ~= 0.84 of the relief ramp -- an improvement over
the original 0.5-0.85 ramp (~0.64) but still ~16% short of the ceiling the
mechanism is designed to deliver. Narrowing P1 further, from 0.78 to 0.73,
moves dei_n80_omnidir to smoothstep((0.71-0.5)/0.23) ~= 0.95 of the ramp --
close to saturation -- while:
  - scale_pos<=0.5 (dei_n50 ~=0.37, parque_n10/20==0) is untouched: the
    ramp's lower edge and the zero-relief guarantee below the gate for
    small/mid-N farms is unchanged;
  - scale_pos>=0.73 still saturates to the same relief ceiling as before
    (farms already at or beyond the old 0.78 edge see no change beyond the
    slightly-earlier saturation point).
A small bump to the relief span (3.5 -> 3.8) adds proportionally more
magnitude at that near-saturated point, mirroring the prior successful
3.0 -> 3.5 bump. The relax engagement delay for large-N farms
(_RELAX_P1_SCALE_SPAN) is nudged up slightly (0.08 -> 0.09) so the
constraint-penalty ramp-in timing stays consistent with the now-larger
alpha-floor relief instead of engaging penalty strength slightly ahead of
where the floor relief actually kicks in. Every other mechanism (self-
contained inverse-time product lr decay to gamma_min, short linear warmup,
density-extremity relax/spike/hold, absolute-N scale on hold/spike/relax_p1,
...
_RELAX_P1_SCALE_SPAN = 0.09     # further delay for large-N farms (was 0.08)
...
# (scale_pos in [0.5, 0.73]) -- narrowed further from the prior 0.5-0.78 ramp
# so dei_n80_omnidir (scale_pos ~= 0.71) lands near-saturated (~95% of the
# ramp) instead of mid-to-upper-ramp (~84%), without moving the lower gate.
_ALPHA_FLOOR_HIGH_N_P0 = 0.5
_ALPHA_FLOOR_HIGH_N_P1 = 0.73
_ALPHA_FLOOR_HIGH_N_SPAN = 3.8
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # terms + a smoothstep-gated term ramping over scale_pos in [0.5,0.73])
