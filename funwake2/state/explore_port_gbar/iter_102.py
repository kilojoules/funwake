"""Schedule: add a quadratic, threshold-gated alpha-floor boost for farms
well above the linear scale_pos regime (N notably larger than the ~60-turbine
mark where scale_pos crosses 0.5), so dei_n80_omnidir (scale_pos ~= 0.71) gets
substantially more terminal-alpha relief while dei_n50 (scale_pos ~= 0.37,
below the 0.5 gate) and the parque farms (scale_pos == 0, gate inactive) are
left completely untouched.

Diagnosis vs. the current best: alpha_denom_floor = g_min * (1 +
ALPHA_FLOOR_SCALE_SPAN*scale_pos + ALPHA_FLOOR_EXT_SPAN*extremity) is purely
linear in scale_pos, so raising ALPHA_FLOOR_SCALE_SPAN to relieve
dei_n80_omnidir (scale_pos ~= 0.71) proportionally raises the floor for
dei_n50 (scale_pos ~= 0.37) too -- exactly the coupling that has capped
progress on this lever for several generations (0.189-0.190% plateau).
Since dei_n50 is already solidly positive (+0.01-0.03%) and dei_n80_omnidir
is the only farm still below native, the fix is to decouple them: add a
quadratic term gated at scale_pos > 0.5 (N > 30*sqrt(4) = 60 turbines) so
only farms well past dei_n50's scale sit on the quadratic ramp.
dei_n50 (0.37 < 0.5) sees the gated term contribute exactly zero -- its
alpha path is bit-for-bit identical to the parent. dei_n80_omnidir
(0.71 > 0.5) gets max(0.71-0.5,0)^2 * 6.0 ~= 0.26 extra floor multiplier on
top of the existing linear term, meaningfully capping its terminal alpha
further without perturbing any other farm in the fitness set (parque_n10/20
have scale_pos == 0, also unaffected). All other machinery (self-contained
