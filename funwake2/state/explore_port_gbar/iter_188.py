"""Schedule: builds on the current parent (+0.1959% farm-balanced), which
already gates extra hold/relax-delay/alpha-floor/spike relief onto the
N~=60-100+ band via `high_n_ramp` (scale_pos in [0.5, 0.78]), plus a
separate near-reference-density `mid_gate` for extremity~=0 farms.

Diagnosis: the two weakest cells are dei_n50 (+0.0277%) and, worse,
dei_n50_uniform (+0.0139%) -- both n=50 (scale_pos ~= 0.368). That
scale_pos sits in a dead zone: below the high_n_ramp gate's onset
(previously 0.5), so dei_n50-scale farms get NONE of the extra hold /
delayed-engagement / bounded-alpha-floor / softened-spike relief that
dei_n80 (scale_pos ~= 0.708) already enjoys, only the small continuous
linear _SCALE_SPAN terms. Widening high_n_ramp's onset from 0.5 to 0.3
lets scale_pos=0.368 farms draw a small but nonzero share of that same,
already-validated relief mechanism (smoothstep, so still 0 at
scale_pos<=0.3 -- parque_n10/20 at scale_pos=0 are completely unaffected)
while mildly *increasing* the relief dei_n80 already gets (its ramp value
rises from ~0.84 to ~0.94 at the same P1=0.78), which should not hurt a
farm that mechanism was designed for.

dei_n50_uniform, being near-reference density (extremity ~= 0), is the
only farm in the parent that also draws on `mid_gate`. Since it remains
the single worst cell even with that gate active, its magnitude was
evidently too small relative to the effect needed; the three mid_gate
spans (hold, relax delay, spike softening) are nudged up modestly. Every
other mechanism, constant, and farm-specific behavior is unchanged from
the parent.
"""
...
# Gated relief for the mid/large-N band -- widened from [0.5, 0.78] to
# [0.3, 0.78] so n=50-scale farms (scale_pos ~= 0.37, previously in the
# dead zone below the old onset) also draw a share of this relief; the
# smoothstep is still exactly 0 at scale_pos <= 0.3, so parque_n10/20
# (scale_pos = 0) are untouched, and dei_n80 (scale_pos ~= 0.71) gets a
# mildly larger, not smaller, ramp value at the same P1.
_ALPHA_FLOOR_HIGH_N_P0 = 0.3
_ALPHA_FLOOR_HIGH_N_P1 = 0.78
_ALPHA_FLOOR_HIGH_N_SPAN = 3.5

# Narrow gate for near-reference-density farms (extremity ~= 0), which
# get the least relax delay / hold extension / spike softening of any
# band despite dei_n50_uniform being the weakest cell. Fades to 0 by
# extremity=0.3 and is damped wherever high_n_ramp is already active, so
# dei_n80/parque are unaffected. Magnitudes nudged up slightly from the
# parent since dei_n50_uniform remained the worst cell even with this
# gate active.
_MID_GATE_P0, _MID_GATE_P1 = 0.0, 0.30
_MID_HOLD_FRAC_SPAN = 0.03       # small extra exploration hold
_MID_RELAX_P1_SPAN = 0.07        # small extra delay of constraint engagement
_MID_SPIKE_SPAN = -0.30          # small softening of the terminal spike
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # extremity=0.3, and is damped wherever the high-N band is already
