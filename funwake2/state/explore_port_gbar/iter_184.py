...
helped dei_n80_omnidir but left a gap: dei_n50 and dei_n50_uniform sit at
scale_pos~=0.37 (log(50/30)/log4), just below the gate's onset at p0=0.5,
so they get NONE of that relief even though they are the two weakest
farms in the current fitness table (+0.0277% and +0.0139%, both below
dei_n80_omnidir's +0.0257%). Since dei_n50 and dei_n50_uniform share
identical n_turbines/D/min_spacing (only their wind rose differs, which
the schedule cannot see), any fix must come from a schedule shape change
at scale_pos~=0.37, not from farm-specific tuning.

This variant adds a second, independent gate: a triangular bump centered
on scale_pos=0.37 (peaking exactly at the dei_n50 band) that fades back
to zero by scale_pos=0.55 -- i.e. *before* the existing high-N gate's
onset at 0.5 has ramped up much -- so the two gates stay decoupled and
dei_n80_omnidir's already-tuned behavior is left almost untouched. The
mid-N bump gives dei_n50/dei_n50_uniform a small extra exploration hold,
a delayed constraint-engagement window, and a slightly raised terminal
alpha floor -- the same *directions* of relief that helped n80, applied
at roughly a third of the magnitude since this is an untested band.
parque_n10/n20 sit at scale_pos=0 (clipped) and are unaffected. Every
other mechanism is unchanged from the best-so-far variant.
"""
...
_RELAX_P1_MID_N_SPAN = 0.03     # extra gated delay for N~=50 band (dei_n50)
...
_HOLD_FRAC_MID_N_SPAN = 0.012   # extra gated hold for N~=50 band (dei_n50)
...
# Second, independent gate: a triangular bump peaking at scale_pos~=0.37
# (the dei_n50 / dei_n50_uniform band) and fading to zero by 0.55, i.e.
# before the high-N gate above has ramped up much -- keeps the two bands
# decoupled so dei_n80_omnidir's already-tuned behavior is left alone.
_MID_N_LO, _MID_N_PEAK, _MID_N_HI = 0.15, 0.37, 0.55
...
def _bump_scalar(x, lo, mid, hi):
    """Python-float (trace-time) triangular bump: 0 at/outside [lo, hi],
    1 at x == mid, linear on either side."""
    if x <= lo or x >= hi:
        return 0.0
    if x <= mid:
        return (x - lo) / max(mid - lo, 1e-12)
    return (hi - x) / max(hi - mid, 1e-12)
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_ramp = _bump_scalar(scale_pos, _MID_N_LO, _MID_N_PEAK, _MID_N_HI)
