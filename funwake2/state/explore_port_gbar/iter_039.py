"""Schedule: sharpen the terminal alpha-denominator floor's large-N gating by
using scale_pos**2 (instead of scale_pos linearly) with a compensating larger
coefficient, so the floor rises disproportionately more for very-large-N
farms (dei_n80_omnidir, scale_pos=0.71) than for merely-large ones
(dei_n50/dei_n50_uniform, scale_pos=0.37) -- an isolated single-lever change
on top of the current best.

Diagnosis vs. the current best: alpha_denom_floor = gamma_min*(1 + 1.5*
scale_pos) grows *linearly* in scale_pos, so the ratio of floor-boost between
dei_n80_omnidir (scale_pos=0.708) and dei_n50 (scale_pos=0.368) is only
~1.9x -- barely more capping for the 80-turbine farm than the 50-turbine one,
even though dei_n80_omnidir has far more turbine pairs (~80*79/2 vs 50*49/2,
~2.6x) and is the farm still sitting flat at +0.0005% despite the floor
already being present. Squaring scale_pos before applying the coefficient
(and raising the coefficient from 1.5 to 4.0 so dei_n50's floor stays almost
unchanged at ~1.54*gamma_min, matching the prior ~1.55*gamma_min) pushes
dei_n80_omnidir's floor up to ~3.0*gamma_min (vs. ~2.06*gamma_min before) --
i.e. a substantially lower terminal alpha ceiling specifically for the
highest-N farm, while leaving dei_n50/dei_n50_uniform's floor essentially
untouched and parque_n10/n20 (scale_pos=0, unaffected either way) exactly as
before. All other machinery (lr decay table, warmup, density-extremity
relax/spike/hold, beta ramp) is unchanged from the parent.
"""
