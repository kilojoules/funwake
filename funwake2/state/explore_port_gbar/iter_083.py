"""Schedule: raise the terminal alpha-denominator floor for large-N farms
(single-lever change vs. the parent).

Diagnosis vs. the parent: the parent already bounds alpha's terminal
denominator at gamma_min * (1 + 1.5 * scale_pos) so alpha cannot fully
re-diverge to 1/gamma_min on large-N (many-pair) farms such as
dei_n80_omnidir (scale_pos ~= 0.71) and dei_n50 / dei_n50_uniform
(scale_pos ~= 0.37). That fix stopped the farm from regressing but left it
essentially flat (+0.0005%) -- i.e. alpha is still ending up high enough
late in the run to choke off AEP gains relative to the strong parque_n10/
n20 farms (scale_pos == 0, fully unaffected by this lever either way).
Earlier attempts pushing hold_frac, spike0, relax_p1, and beta_p0 further
via scale_pos already made the farm-balanced score worse, so those four
levers stay untouched here. This change instead pushes the ALREADY-ADDED,
previously-untested-at-other-values alpha_denom_floor lever further --
from _ALPHA_FLOOR_SCALE_SPAN = 1.5 to 2.4 -- so large-N farms end the run
with a noticeably lower terminal alpha ceiling (more AEP-seeking headroom
in the final phase), while small/mid-N farms (scale_pos == 0, e.g.
parque_n10/n20) keep floor == gamma_min and are completely unaffected,
exactly matching the parent's behavior for those cells.
...
# Raised from 1.5 -> 2.4: the parent's flat dei_n80_omnidir result
# (scale_pos ~= 0.71) suggests the terminal alpha ceiling is still too
# high there; small/mid-N farms (scale_pos == 0) are untouched either way.
_ALPHA_FLOOR_SCALE_SPAN = 2.4
