"""Schedule: extend the smoothstep-gated high-N relief (proven on the alpha
floor) to also softly delay relax_p1 and soften the terminal spike, plus a
small extra hold, all gated by the SAME high_n_ramp used for the alpha
floor.

Diagnosis vs. parent (+0.1911% farm-balanced, dei_n80_omnidir still weakest
at +0.0015%): the previous generation discovered that a hard-thresholded
(clip(scale_pos-0.5,0))^2 term barely engages at scale_pos~=0.71 (N=80) and
replaced it with a smoothstep-gated `high_n_ramp` -- but only for the alpha
*floor*. The other N-dependent knobs (relax_p1's engagement delay, spike0's
terminal softening, hold_frac's plateau length) still use a purely *linear*
term in scale_pos, so they under-react in exactly the same N~60-120 band
that motivated the floor fix. Since alpha = f(relax, spike) / alpha_denom,
a higher floor alone still gets fought by an early-engaging, sharp spike;
giving dei_n80_omnidir a bit more delay before full penalty engagement and a
softer terminal spike (mirroring the floor's gating) should let the extra
floor relief actually translate into more exploration late in the run there,
without touching farms below the gate (dei_n50 at scale_pos~=0.37,
parque_n10/20 at scale_pos==0 keep high_n_ramp==0 exactly as before). All
...
_RELAX_P1_HIGHN_SPAN = 0.05     # extra smoothstep-gated delay in the N~60-120 band
_SPIKE_BASE, _SPIKE_SPAN = 2.0, 0.55    # spike0 = base + span * size
_SPIKE_SCALE_SPAN = -0.4        # soften terminal spike for large-N farms
_SPIKE_HIGHN_SPAN = -0.3        # extra smoothstep-gated softening in the N~60-120 band
...
_HOLD_FRAC_HIGHN_SPAN = 0.03    # extra smoothstep-gated hold in the N~60-120 band
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGHN_SPAN * high_n_ramp)  # extra hold in the N~60-120 band
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGHN_SPAN * high_n_ramp)       # extra softened spike in N~60-120 band
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                + _RELAX_P1_SCALE_SPAN * scale_pos
                + _RELAX_P1_HIGHN_SPAN * high_n_ramp)   # extra delay in N~60-120 band
    ...
    # while _HOLD_FRAC_SPAN/_HOLD_FRAC_SCALE_SPAN/_HOLD_FRAC_HIGHN_SPAN push
