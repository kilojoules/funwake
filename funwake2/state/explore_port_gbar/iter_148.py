"""Schedule: builds on the best-so-far variant (+0.1952% farm-balanced),
which already gates extra hold/relax-delay relief onto the N~=60-100+ band
via `high_n_ramp` (scale_pos in [0.5, 0.78]). This variant extends the SAME
gate to the terminal alpha spike: it delays the spike's onset slightly and
softens its magnitude a bit further for that band only, leaving every farm
below the gate (dei_n50 at scale_pos~=0.37, parque_n10/20 at 0.0) untouched.

Diagnosis: dei_n80_omnidir (n=80, omnidirectional, scale_pos~=0.71) is the
worst-performing farm across recent generations. The best-so-far variant
already gave this band a longer exploration hold, a further-delayed penalty
engagement, and a bounded alpha floor -- but the terminal feasibility spike
(spike0, ramped over p in [0.90, 1.00]) still fires on the same schedule as
every other large-N farm. An omnidirectional many-pair layout needs more of
its late budget actually converging under a moderate, stable penalty rather
than being hit with a fast late alpha spike on top of an already-delayed
constraint engagement -- stacking a full-strength spike right after a long
delay compresses the constraint-resolution window instead of widening it.
This nudges spike0 down and its onset (spike_p0) slightly later, only in the
gated high-N band, on top of the existing linear _SPIKE_SCALE_SPAN term.
