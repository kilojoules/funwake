"""Schedule: raise the early-phase beta2 floor for large-N farms via a
scale_pos-gated additive term (isolated single-lever change on top of the
current best parent).

Diagnosis vs. the parent: beta2 (Adam second-moment decay) starts at a fixed
_B2_LO = 0.2 for every farm and only ramps up to _B2_HI = 0.45 once progress
crosses beta_p0 (mid-to-late run). That is fine for small/mid-N farms, where
each step's summed gradient (objective + constraint, over relatively few
turbine pairs) is already fairly stable. For dei_n80_omnidir -- the
farm with by far the most turbine pairs (80 turbines) AND an omnidirectional
wind rose (many more wind-direction terms summed into the AEP gradient per
step) -- the per-step gradient sum is the noisiest in the whole fitness set
from step 0 onward, not just late. A low beta2 early means Adam's adaptive
step-size denominator is estimated from a very short, noisy window, which
lets early exploration steps overreact to gradient noise on exactly the farm
that already has the weakest margin (parent: +0.0005%). Earlier attempts
already pushed hold_frac, spike0, relax_p1, beta_p0, and the alpha-denominator
floor via scale_pos; this instead adds one new, previously-untried lever:
lift the beta2 *floor itself* (the early-phase value, before the beta_p0..
beta_p1 ramp starts) for large-N farms only, via the same scale_pos gate
already used elsewhere. Small/mid-N farms (scale_pos == 0, e.g.
parque_n10/n20, dei_n50*) get beta2_lo == _B2_LO exactly, matching the
parent unchanged; only farms with n_turbines meaningfully above the
_N_REF=30 anchor get extra early gradient-variance averaging.
...
absolute-N scale on hold/spike/relax_p1/beta_p0, the alpha-denominator
...
# NEW: raise the early-phase beta2 floor for large-N farms -- their per-step
# gradient sums (more pairs, and here also more wind directions) are noisier
# from step 0, so a slightly higher beta2 floor denoises the Adam variance
# estimate during exploration without touching small/mid-N farms.
_B2_LO_SCALE_SPAN = 0.10
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_lo = _B2_LO + _B2_LO_SCALE_SPAN * scale_pos          # noisier gradients -> higher floor
    beta1 = _B1_LO + (_B1_HI - _B1_LO) * r
    beta2 = b2_lo + (_B2_HI - b2_lo) * r
