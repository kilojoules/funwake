"""Schedule: beta2 ceiling gains an independent large-N term (isolated single-lever change).

Diagnosis vs. the parent (which already scales hold_frac, spike, relax_p1,
relax0, and beta_p0 onset by an absolute-N `scale_pos` signal, gated to act
only on large-N farms): dei_n80_omnidir is still flat/slightly-below native
(-0.0002%) while every other farm improved. dei_n80_omnidir has ordinary
packing DENSITY (min_spacing/D ~= 2, same as the mid-density anchor) but is
the largest-N farm in the fitness set (80 vs. <=50 elsewhere) -- with 80
turbines the pairwise-spacing constraint count grows quadratically, so the
per-step constraint gradient (grad_constraint, and hence the alpha-weighted
combined gradient) is a noisier, higher-variance signal than for the
smaller farms, even at matched density.

The parent already moves the beta1/beta2 ramp's ONSET earlier for large N
(via `_BETA_P0_SCALE_SPAN`), but the ramp's ENDPOINT/ceiling (`_B2_HI`,
governing how much the second-moment estimate is averaged once the ramp
completes) is still the fixed generic value for every farm. A noisier
constraint gradient benefits from a higher beta2 ceiling (heavier
second-moment averaging => smoother adaptive step sizes => less thrash from
constraint-gradient noise) once optimization has moved into the
feasibility-shaping phase. Recent attempts already pushed the OTHER
large-N-gated knobs further and that made the farm-balanced score worse
(-0.021%, -0.012%) -- so this change deliberately leaves hold/spike/
relax_p1/relax0/beta_p0 untouched and adds exactly one new, previously
untried lever: an independent scale_pos term on `_B2_HI`, gated (like all
scale_pos terms) to be zero for any farm at or below the absolute-N
reference, so parque_n10, parque_n20, dei_n50, dei_n50_uniform are
unaffected and only dei_n80_omnidir (and any future large-N farm) gets
heavier late-stage second-moment averaging.
...
absolute-N scale on hold/spike/relax_p1/relax0/beta_p0) is unchanged from
the parent.
"""
...
_B2_HI_SCALE_SPAN = 0.15        # NEW: heavier late 2nd-moment averaging for large-N farms
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # heavier late averaging, large N
