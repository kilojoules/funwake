"""Schedule: widen and strengthen the threshold-gated quadratic alpha-floor
boost so dei_n80_omnidir (scale_pos ~= 0.71) gets substantially more
terminal-alpha relief, while dei_n50 (scale_pos ~= 0.37) stays exactly
untouched and the parque farms (scale_pos == 0) remain unaffected.

Diagnosis vs. the current best: the prior quadratic gate engaged only past
scale_pos > 0.5 with span 6.0, giving dei_n80_omnidir (scale_pos ~= 0.71,
gap ~= 0.21) just +0.26 extra floor multiplier -- not quite enough to flip
its score positive (still -0.0025%). dei_n50 sits at scale_pos ~= 0.37,
comfortably below even a lowered 0.42 threshold, so it never touches the
gated term regardless of how the gate is tuned; there is no coupling risk
in moving the threshold down toward (but still above) 0.37. Lowering the
threshold to 0.42 and raising the span to 7.5 roughly 2.4x's the extra
terminal-alpha relief for dei_n80_omnidir (gap 0.71-0.42=0.29,
0.29^2*7.5 ~= 0.62 vs. the prior 0.26) while leaving dei_n50's and both
parque farms' alpha paths bit-for-bit identical to the parent (their
scale_pos are 0.37, 0, and 0 respectively -- all below 0.42, so
high_n_gap == 0 for them exactly as before, just with the gate value
changed). All other machinery (self-contained inverse-time product lr
...
# regime (scale_pos > 0.42, i.e. N > 30*4^0.42 ~= 51 turbines) -- decouples
# dei_n80_omnidir's floor from dei_n50's (scale_pos ~= 0.37, below the gate)
# without touching any farm below the gate.
_ALPHA_FLOOR_HIGH_N_THRESH = 0.42
_ALPHA_FLOOR_HIGH_N_SPAN = 7.5
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # terms + a threshold-gated quadratic term for scale_pos beyond the
    # high-N gate) so alpha's terminal divergence is bounded more for the
    # largest-N (many-pair) farms specifically, without perturbing farms
    # below the gate.
