"""Schedule: strengthen the terminal alpha-denominator floor (single isolated
lever change vs. the current best parent).

Diagnosis: the parent already caps alpha's terminal 1/lr_base blow-up for
large-N farms via `alpha_denom_floor = gamma_min * (1 + _ALPHA_FLOOR_SCALE_SPAN
* scale_pos)`, which took dei_n80_omnidir (80 turbines, the largest N in the
fitness set, scale_pos ~= 0.71) from -0.0002% to +0.0005% -- a real but very
thin improvement, meaning the cap is still too loose: lr_base is allowed to
fall to ~2.06x gamma_min before the floor engages, so alpha still climbs most
of the way to its old diverging value late in the run on this many-pair farm.
dei_n50 (scale_pos ~= 0.37, +0.033%) and dei_n50_uniform (scale_pos ~= 0.37,
+0.014%) sit on the same mechanism at a smaller scale_pos, and are also the
next-weakest farms after n80 -- consistent with the floor being under-strength
across the board, not just at the n80 extreme.

Earlier attempts that pushed hold_frac, spike0, relax_p1, or beta_p0 further
via scale_pos regressed the farm-balanced score (-0.021%, -0.012%), so this
change leaves those four levers, and everything else, untouched. It only
raises `_ALPHA_FLOOR_SCALE_SPAN` from 1.5 to 3.0: the floor now reaches
~3.1x gamma_min at n80's scale_pos (vs ~2.06x before) and ~2.1x at n50's
scale_pos (vs ~1.55x before), giving alpha materially more headroom before
its terminal value is capped on every large-N farm, while small/mid-N farms
(scale_pos == 0, e.g. parque_n10/n20) are exactly unaffected since their
floor stays pinned at gamma_min, matching the parent bit-for-bit.
"""
...
# Raised from 1.5 -> 3.0 (isolated change; see module docstring).
