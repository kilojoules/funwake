"""Schedule: concentrate the alpha-denominator floor on the LARGEST-N farm only,
via a quadratic (not linear) gate on scale_pos (isolated single-lever change).

Diagnosis vs. the current best: the best-so-far already added
`alpha_denom_floor = gamma_min * (1 + 1.5*scale_pos)` to cap alpha's terminal
1/lr_base divergence for large-N farms, which flipped dei_n80_omnidir from
-0.0002% to +0.0005% -- but that farm is still by far the WORST of the five
(the next-worst, dei_n50_uniform, is 28x better at +0.0139%), so the floor is
still too weak specifically where n_turbines is largest. Simply raising the
linear coefficient _ALPHA_FLOOR_SCALE_SPAN further was already tried (per the
prompt's "-0.021%, -0.012%" regressions from pushing shared scale_pos-gated
levers harder) and is risky here too, because scale_pos is fairly close
between dei_n50/dei_n50_uniform (n=50 -> scale_pos ~= 0.368) and
dei_n80_omnidir (n=80 -> scale_pos ~= 0.708): a linear bump big enough to move
n=80 also meaningfully shifts n=50, which is *already* the strongest farm
(+0.0326%/+0.0139%) and should not be disturbed.

The fix: square scale_pos before applying the floor coefficient
(scale_pos**2), which is a convex, monotone reshaping of the SAME existing
signal (no new inputs, still bounded in [0,1], still 0 for every farm with
n_turbines <= n_ref so parque_n10/n20 are provably unaffected). Because
0.708**2 ~= 0.501 versus 0.368**2 ~= 0.135, a single coefficient can be
chosen (4.0, versus the old 1.5) so that the n=80 floor rises from ~2.06*g_min
to ~3.0*g_min (meaningfully more capping) while the n=50 floor stays almost
exactly where it was (~1.54*g_min vs ~1.55*g_min before) -- i.e. it pushes on
dei_n80_omnidir specifically without perturbing the farms that are already
performing well. All other machinery (lr decay table, warmup, density-based
relax/spike/hold, beta ramp) is byte-for-byte unchanged from the parent.
"""
...
# NEW: gate on scale_pos**2 (not scale_pos) so the effect concentrates on the
# single largest-N farm rather than also perturbing mid-N farms.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Cap the effective denominator at gamma_min*(1+k*scale_pos^2) instead of
    # letting it ride lr_base all the way to gamma_min, so alpha's terminal
    # divergence is bounded on large-N (many-pair) farms only -- and squaring
    # scale_pos concentrates the cap on the single largest-N farm rather than
    # also shifting mid-N farms that are already performing well.
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos * scale_pos)
