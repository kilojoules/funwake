...
extra hold/relax-delay/spike-softening/alpha-floor relief onto the narrow
N~=36-59 "mid-N" band (peaking at N=50, via `mid_n_bump`) to help the n=50
farms that sit below the separate N~=60-100+ `high_n_ramp` gate.

Diagnosis: within that mid-N band, dei_n50 (+0.0501%) and dei_n50_uniform
(+0.0151%, currently the worst farm) have IDENTICAL n_turbines/D/min_spacing
-- so `size`, `extremity`, `scale_pos` and hence every existing relief term
are bit-for-bit identical between them. Only their wind rose (and therefore
alpha0, the measured gradient scale) differs, and the schedule has no lever
that reacts to that. Rather than guess at a new, uncalibrated alpha0-based
discriminator (risky without eval feedback), this simply gives the already
mechanistically-validated mid-N relief (longer hold, later constraint
engagement, softer terminal spike, higher alpha floor) more headroom, since
it is currently under-dosed for the worst farm in its own band. The
mid_n_bump gate itself is unchanged (still zero outside n~=36-59), so
parque_n10/n20 and dei_n80_omnidir are untouched -- only the dei_n50 /
dei_n50_uniform pair gets the stronger dose, which should lift both (and
especially the worse of the two) without regressing anything else.
"""
...
# near N~=50 (scale_pos~=0.37-0.55), fades back to zero before the high-N
# gate's onset at 0.5-0.709 -- gives the n=50 band (currently un-served by
# the high-N gate) its own slice of the same relief mechanisms. Doses raised
# relative to the +0.2007% parent to address dei_n50_uniform, the current
# worst farm, which shares identical size/scale_pos with dei_n50 (its only
# difference is wind rose / alpha0), so more headroom on the shared mid-N
# relief is the only available lever without a new, uncalibrated signal.
_MID_N_P0, _MID_N_P1 = 0.15, 0.55
_MID_N_HOLD_SPAN = 0.022
_MID_N_RELAX_P1_SPAN = 0.045
_MID_N_SPIKE_SPAN = -0.13
_MID_N_ALPHA_FLOOR_SPAN = 1.9
