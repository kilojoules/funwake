"""Schedule: extend the existing scale_pos>0.5 (N>60) quadratic gate to three
more levers -- hold_frac, terminal spike, and beta ramp onset -- so that
dei_n80_omnidir (the only fitness-set farm past the gate, scale_pos ~= 0.71)
gets more exploration time, a further-damped terminal penalty spike, and a
later momentum ramp, while dei_n50 (scale_pos ~= 0.37, below the gate) and
both parque farms (scale_pos == 0) are bit-for-bit unaffected -- their
high_n_gap is exactly 0, so every new term evaluates to 0 for them.

Diagnosis vs. the parent: the parent already isolates the alpha-floor lever
with a quadratic high-N gate, but leaves hold_frac, spike0, and beta_p0 on
purely linear scale_pos terms shared with dei_n50. Since dei_n50 is already
solidly positive, those linear coefficients cannot be pushed further without
risking dei_n50 -- exactly the coupling the alpha-floor gate was built to
avoid, just not yet applied to the other three levers. Mirroring the same
gate (threshold 0.5, i.e. N > 60) onto hold_frac (+0.05 quadratic => longer
exploration plateau before lr/alpha engage), spike0 (-0.35 quadratic =>
softer terminal feasibility spike, since dei_n80_omnidir's many spacing
pairs make an aggressive terminal alpha spike more likely to cost AEP than
buy feasibility margin it doesn't need -- it's already 3/3 feasible), and
beta_p0 (-0.05 quadratic => later momentum-averaging onset, keeping
unaveraged gradient steps longer on the highest-pair-count farm) gives
dei_n80_omnidir more targeted relief on every axis without perturbing any
other farm in the fitness set. The beta_p0 >= hold_frac safety clamp from
the parent is preserved and re-derived from the (now higher) gated
hold_frac. All other machinery is unchanged.
"""
...
# Threshold-gated (scale_pos > 0.5, i.e. N > 60) quadratic relief, applied
# identically to four levers below so only the largest-N farms are touched
# and every farm below the gate is bit-for-bit identical to the parent.
_HIGH_N_THRESH = 0.5
_ALPHA_FLOOR_HIGH_N_SPAN = 6.0
_HOLD_FRAC_HIGH_N_SPAN = 0.05
_SPIKE_HIGH_N_SPAN = -0.35
_BETA_P0_HIGH_N_SPAN = -0.05
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    high_n_gap = max(scale_pos - _HIGH_N_THRESH, 0.0)
    high_n_quad = high_n_gap * high_n_gap               # 0 below the gate, quadratic above
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_quad)  # extra hold past the N>60 gate
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGH_N_SPAN * high_n_quad)       # further softened past the N>60 gate
    ...
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  ...
                                  + _ALPHA_FLOOR_HIGH_N_SPAN * high_n_quad)
    ...
    # large-N/extreme-density farms, the scale/high-N terms pull beta_p0
    # down while hold_frac's own scale/high-N terms push it up, and the two
    # can cross so momentum starts increasing during pure exploration
    ...
    beta_p0 = max(_BETA_P0 + _BETA_P0_SCALE_SPAN * scale_pos
                  + _BETA_P0_HIGH_N_SPAN * high_n_quad, hold_frac)
