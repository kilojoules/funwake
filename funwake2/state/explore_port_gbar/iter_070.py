...
farms (single-lever change on top of the parent's already-introduced
scale_pos-gated floor).

Diagnosis vs. the parent: the parent's ALPHA_FLOOR_SCALE_SPAN=1.5 caps
alpha's terminal denominator at gamma_min*(1+1.5*scale_pos), which for
dei_n80_omnidir (n_turbines=80, scale_pos = log(80/30)/log(4) ~ 0.71) works
out to roughly gamma_min*2.06 -- a real but still fairly small departure
from the fully-diverging native denominator (gamma_min*1.0). Given that
dei_n80_omnidir is the one farm sitting essentially flat (+0.0005%) while
every other farm (including the other DEI cases, which have lower
scale_pos and are therefore barely touched by this lever) shows a clear
gain, the natural next probe is to push exactly this lever further rather
than touching hold_frac/spike0/relax_p1/beta_p0 again (those were already
tried and made the farm-balanced score worse per the search log). Raising
ALPHA_FLOOR_SCALE_SPAN from 1.5 to 3.0 roughly doubles the terminal alpha
cap for dei_n80_omnidir (denominator -> gamma_min*3.13) while leaving
small/mid-N farms (parque_n10/n20, scale_pos == 0) completely unaffected,
since their floor stays exactly at gamma_min, matching the parent exactly.
This keeps the change isolated to the one lever most directly implicated
by the diagnosis, on the one farm most in need of it.
...
# Raised from the prior 1.5 -> 3.0 to push this further for dei_n80_omnidir,
# the one farm that was still essentially flat under the smaller cap.
