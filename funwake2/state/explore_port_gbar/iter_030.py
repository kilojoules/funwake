"""Schedule: raise the terminal beta2 ceiling for large-N farms via a
scale_pos-gated span on _B2_HI (isolated single-lever change vs. the parent).

Diagnosis vs. the parent: the parent already gates hold_frac, spike0,
relax_p1, alpha's denominator floor, and beta_p0 (the *onset* of the
beta1/beta2 ramp) on scale_pos, and prior attempts pushing those four levers
further made the farm-balanced score worse (-0.021%, -0.012%). Those levers
all change *when* the schedule reacts to large N; none change how strongly
the second-moment (beta2) estimate smooths gradient noise once the ramp
completes. dei_n80_omnidir has 80*79/2 = 3160 turbine-pair constraint terms
vs. dei_n50's 1225 -- almost 3x the pairwise gradient noise -- yet the
parent's terminal beta2 ceiling (_B2_HI = 0.45) is identical for every farm
regardless of N. With the terminal alpha spike/floor already engaged for
this farm (scale_pos ~= 0.71), a noisier constraint-gradient estimate at
that same moment plausibly caps how much of the parent's gains can show
through, leaving the farm nearly flat (+0.0005%) despite every timing lever
already being tuned in its favor.

This change adds one new, previously-untried lever, gated purely on
scale_pos and left at its parent value (0) for scale_pos == 0: raise the
asymptotic beta2 ceiling for large-N farms only, so the Adam second-moment
estimate averages over more history precisely when there are more
(noisier) pairwise constraint-gradient terms to smooth. Small/mid-N farms
(parque_n10/n20, dei_n50*) keep scale_pos == 0 and are completely
unaffected -- their beta2 ceiling matches the parent exactly.
...
absolute-N scale on hold/spike/relax_p1/beta onset/relax0, alpha denominator
...
# NEW: raise the terminal beta2 ceiling for large-N farms only, so the
# second-moment estimate smooths more of the (noisier, more numerous)
# pairwise constraint-gradient terms once the ramp completes.
