...
gradient-stiffness signal derived purely from the already-supplied alpha0
(mean|grad J|/D), gamma_min and D -- no new problem inputs, no farm-specific
constants.

Diagnosis: dei_n50 and dei_n50_uniform share identical N, D and min_spacing,
so every existing shaping term (size, extremity, scale_pos, high_n_ramp) is
IDENTICAL between them -- yet dei_n50_uniform scores half of dei_n50
(0.0139% vs 0.0277%) and is the worst farm overall. The only signal that
differs between these two problems is alpha0 itself (their wind roses
differ: a near-uniform rose yields smaller, more direction-cancelled mean
gradients than a peaked rose). The current alpha_denom_floor is a pure
function of (D, min_spacing, n_turbines) and ignores alpha0 entirely, so a
farm with intrinsically weak gradients gets the SAME late-stage alpha
denominator floor as a farm with strong gradients -- capping its terminal
alpha proportionally lower and leaving it under-enforced exactly when it
needs feasibility resolution most.

Fix: compute a bounded, log-compressed "stiffness" ratio from
alpha0*D/gamma_min (all already-available scalars) and use it, gated to only
act on soft-gradient farms (stiffness below the reference), to shave the
alpha_denom_floor down -- giving those farms a proportionally larger
terminal alpha without touching farms whose stiffness is at or above the
reference (dei_n50, parque_n10/20 are unaffected unless their own alpha0 is
unusually small). Everything else is unchanged from the parent.
"""
...
# Gradient-stiffness relief: alpha0*D/gamma_min compresses/bounds a
# dimensionless "how strong are the raw gradients relative to the
# convergence tolerance" signal. Farms below the reference (soft/averaged
# gradients, e.g. near-uniform wind roses) get their alpha_denom_floor
# shaved down proportionally, so their capped terminal alpha is not
# suppressed just because their raw alpha0 is small.
_STIFF_REF = 50.0
_STIFF_FLOOR_FRAC = 0.6         # max fractional shave of the base floor
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Gradient-stiffness signal: alpha0 is traced, so this must stay in jnp
    # (no float()/int() on it). stiff < 0 means this problem's raw gradients
    # are weak relative to gamma_min and the reference -- give it relief on
    # the alpha_denom_floor so its capped terminal alpha isn't suppressed
    # purely because its alpha0 happens to be small (e.g. near-uniform wind
    # roses that direction-cancel gradients, vs peaked roses at equal N/D).
    stiff_ratio = jnp.maximum(alpha0 * diam / max(g_min, 1e-12), 1e-9)
    stiff = jnp.clip(jnp.log(stiff_ratio / _STIFF_REF) / math.log(4.0), -1.0, 1.0)
    stiff_relief = jnp.clip(-stiff, 0.0, 1.0)         # only soft-gradient farms get relief
    ...
    alpha_denom_floor_base = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                       + _ALPHA_FLOOR_EXT_SPAN * extremity
                                       + _ALPHA_FLOOR_HIGH_N_SPAN * high_n_ramp)
    alpha_denom_floor = alpha_denom_floor_base * (1.0 - _STIFF_FLOOR_FRAC * stiff_relief)
