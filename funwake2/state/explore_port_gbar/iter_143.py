"""Schedule: same as parent (+0.1952% farm-balanced) with a small, global
softening of the early-exploration/constraint balance.

Diagnosis: dei_n50 and dei_n50_uniform share identical n_turbines, D, and
min_spacing, so every size/scale-derived shaping term (hold_frac, relax,
spike, alpha floor, beta ramp) is already IDENTICAL between them -- the
architecture already correctly separates wind-driven magnitude (via alpha0)
from geometry-driven shape. Recent generations that tried to add new
gated/farm-identity features on top of this plateaued or regressed (down to
+0.007%), suggesting the N/density-gated knobs are near a local optimum and
further splitting hairs there is not the productive direction.

Instead this variant takes the one direction that has consistently paid off
across the lineage (native's monotonic 1/lr coupling -> bounded plateau ->
delayed ramp -> softened large-N spike): push global exploration/constraint
balance a bit further along that same axis, uniformly across all farms
(not just gated for large N):
  - _RELAX_BASE 0.45 -> 0.42: slightly lower early-exploration penalty
    floor, giving a bit more unconstrained movement before the penalty
    ramp engages, on every farm equally (small enough that spacing
    feasibility by the end is unaffected -- the terminal alpha floor and
    spike are untouched in magnitude of effect, just shifted slightly
    later/softer).
  - _HOLD_FRAC_BASE 1/3 -> 0.34: a small extra sliver of pure-exploration
    plateau before any lr decay/constraint ramp begins.
  - _SPIKE_BASE 2.0 -> 1.85: softens the terminal feasibility-restoration
    spike globally, continuing the same direction the large-N-only
    _SPIKE_SCALE_SPAN=-0.4 term already uses, but as a small universal
    baseline shift rather than a new gated axis.

All N/density gating, the alpha-floor relief, the warmup, the decay-table
construction, and the beta ramp are byte-for-byte unchanged from the
parent -- this is a pure, low-risk magnitude tweak on three existing
constants, not a new mechanism.
"""
...
_RELAX_BASE, _RELAX_SPAN = 0.42, 0.11   # relax0 = base + span * size in [-1,1]
...
_SPIKE_BASE, _SPIKE_SPAN = 1.85, 0.55    # spike0 = base + span * size
_SPIKE_SCALE_SPAN = -0.4        # soften terminal spike for large-N farms
_SPIKE_P0, _SPIKE_P1 = 0.90, 1.00

_HOLD_FRAC_BASE, _HOLD_FRAC_SPAN = 0.34, 0.06   # exploration plateau length
