"""Schedule: small, targeted extension of the best-so-far mid-N variant
(+0.2007% farm-balanced). That variant added a narrow "mid-N" bump (parabola
in scale_pos, peaking near dei_n50's N~=50) giving the N=50 band a slice of
the hold/relax-delay/spike-softening/alpha-floor relief that the separate
high-N gate (N~=60-100+, e.g. dei_n80_omnidir) already had.

Diagnosis carried over: dei_n50 and dei_n50_uniform are geometrically
IDENTICAL (same n_turbines/D/min_spacing), so every size/scale_pos/
mid_n_bump-derived term is bit-for-bit identical between them -- the
schedule fundamentally cannot distinguish the two wind roses. What it CAN
do is give the whole mid-N band (which currently supplies the two lowest
per-farm scores, dei_n50 +0.0501% and dei_n50_uniform +0.0151%, the worst)
a bit more of the relief that measurably helped the high-N band previously:
slightly more feasibility-floor headroom, a slightly later constraint
engagement point, and a slightly wider bump window so the transition into
the high-N gate (which starts ramping at scale_pos=0.5) is smoother. Also
adds a small, independently-gated delay to the Adam-averaging onset
(beta_p0) for the mid-N band, mirroring the existing scale-based delay
mechanism, on the hypothesis that a few more unaveraged (low-beta) gradient
steps during the constraint-engagement window helps exactly the
feasibility-constrained farms that benefit least from the current mid-N
bump. All new terms are gated through `mid_n_bump`, which is zero outside
scale_pos in [0.15, 0.58] (widened slightly from 0.55), so parque and n80
...
# near N~=50 (scale_pos~=0.37), fades back to zero just before the high-N
# gate's onset at 0.5 -- gives the n=50 band its own smaller, independent
# slice of the same relief mechanisms. Window widened slightly (0.55->0.58)
# for a smoother handoff into the high-N gate.
_MID_N_P0, _MID_N_P1 = 0.15, 0.58
_MID_N_HOLD_SPAN = 0.015
_MID_N_RELAX_P1_SPAN = 0.045    # was 0.03: a bit more engagement delay
_MID_N_SPIKE_SPAN = -0.10
_MID_N_ALPHA_FLOOR_SPAN = 1.8   # was 1.5: a bit more feasibility-floor headroom
_MID_N_BETA_P0_SPAN = 0.04      # new: small extra delay to Adam-averaging onset
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # unaveraged gradient steps there. The mid-N band gets a small extra
    # delay of its own (_MID_N_BETA_P0_SPAN), independent of the scale-based
    # term, on the hypothesis that a few more unaveraged gradient steps
    # during constraint engagement helps the feasibility-constrained n50
