"""Schedule: concentrate the terminal-alpha denominator floor on the
LARGEST farms via scale_pos**2 instead of linear scale_pos (isolated
single-lever change vs. the parent).

Diagnosis vs. the parent: the parent's alpha_denom_floor =
gamma_min * (1 + 1.5*scale_pos) grows LINEARLY with the absolute-N scale
factor. That means dei_n50/dei_n50_uniform (n=50, scale_pos ~= 0.37) and
dei_n80_omnidir (n=80, scale_pos ~= 0.71) get bounded by very similar
multipliers (~1.55x vs ~2.06x) even though dei_n80_omnidir -- with far
more turbine pairs and (per the parent's own docstring) "ordinary packing
density but far more turbine pairs than any other farm" -- is the one
that stays flat (+0.0005%, the worst-farm bottleneck for the
farm-balanced score), while dei_n50/dei_n50_uniform are already
comfortably positive (+0.03%/+0.01%) and don't need as much bounding.

This change replaces the linear scale_pos term with scale_pos**2 and
raises the coefficient (1.5 -> 3.0) so the SAME denominator floor:
  - n=80 (scale_pos~=0.71, scale_pos**2~=0.50): factor 1+3.0*0.50 = 2.50
    (STRONGER cap than parent's 2.06 -- more terminal-alpha relief for
    the bottleneck farm)
  - n=50 (scale_pos~=0.37, scale_pos**2~=0.135): factor 1+3.0*0.135 = 1.40
    (WEAKER cap than parent's 1.55 -- lets alpha ride closer to native's
    coupling, preserving what already works for n50)
  - n<=~30 (scale_pos clipped to 0): factor stays exactly 1.0, i.e.
    zero effect vs. the parent -- parque_n10/n20 are completely
    unaffected, matching the parent bit-for-bit.

No other lever is touched: hold_frac, spike0, relax0, relax_p1, and
beta_p0 scale-pos gating are all left exactly as in the parent (earlier
attempts pushing those further already regressed the farm-balanced
score). All other machinery (self-contained inverse-time product lr
decay to gamma_min, short linear warmup, density-extremity
relax/spike/hold) is unchanged from the parent.
"""
...
# Quadratic in scale_pos (not linear) so the cap concentrates on the
# LARGEST farms (n=80) rather than spreading evenly across n=50 and n=80.
_ALPHA_FLOOR_SCALE_SPAN = 3.0
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Cap the effective denominator at gamma_min*(1+k*scale_pos**2) --
    # QUADRATIC in scale_pos so the bound concentrates on the largest
    # farms (n=80) while barely touching mid-large farms (n=50) that
    # already score positively under the parent's linear version.
