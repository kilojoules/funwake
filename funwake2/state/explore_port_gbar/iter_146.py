"""Schedule: same as parent (+0.1952% farm-balanced) but widens the
existing smoothstep-gated high-N relief window so it starts ramping at
moderate N instead of only kicking in above N~=60.

Diagnosis vs. parent: with dei_n80_omnidir's floor now decoupled via the
gated relief, the new worst farm is dei_n50_uniform (+0.0139%). Note that
dei_n50 and dei_n50_uniform share IDENTICAL D/min_spacing/n_turbines
(same physical farm, different wind rose only), so they trace to the
exact same size/extremity/scale_pos/high_n_ramp values -- the schedule
cannot distinguish them from these inputs alone. The only generalizable
lever is to change how much relief farms at n_turbines~=50 (scale_pos
~=0.37) receive from the same mechanism that already helps n=80
(scale_pos~=0.71): more exploration time before constraints fully engage,
and a lower terminal alpha floor, for farms with more turbines to
pack/route regardless of wind-rose shape (more turbines -> more competing
spacing constraints across whatever directions are active).

Rather than a hard gate starting at scale_pos=0.5 (which gives n=50 farms
literally zero of this relief), this variant lowers the gate onset from
0.5 to 0.30 so the SAME already-tuned smoothstep ramp turns on gradually
by absolute farm size: n=50 (scale_pos~=0.37) now gets a small fraction
(~5%) of the relief, n=80 (scale_pos~=0.71) is essentially unchanged
(already deep in the ramp), and parque_n10/n20 (negative/clipped
scale_pos) are completely unaffected since the ramp still floors at 0
below the gate. This is a strictly more general version of the existing
mechanism, not a farm-specific hack -- it only changes WHERE the ramp
starts, not what it does. Every other mechanism (self-contained
inverse-time product lr decay to gamma_min, short linear warmup, density-
extremity relax/spike/hold, absolute-N scale on hold/spike/relax_p1, the
alpha-floor relief, the hold_frac-floored beta ramp) is unchanged.
"""
...
_RELAX_P1_HIGH_N_SPAN = 0.05    # extra gated delay for N-relief band
...
_HOLD_FRAC_HIGH_N_SPAN = 0.02   # extra gated hold for N-relief band
...
# Smoothstep-gated relief for larger-N farms, now widened to start
# ramping at moderate N (scale_pos ~=0.30, i.e. n_turbines~=36) instead of
# only above scale_pos=0.5 (n_turbines~=60) -- decouples the floor/hold/
# delay reliefs from a hard N threshold, extending them gradually to
# mid-size farms (e.g. n=50) without touching small farms (parque_n10/20,
# whose scale_pos is <=0 and clipped to 0 regardless of the gate).
_ALPHA_FLOOR_HIGH_N_P0 = 0.30
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_ramp)  # extra gated hold for large-N band
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_HIGH_N_SPAN * high_n_ramp)  # extra gated delay for large-N band
