"""Schedule: extend the terminal alpha-denominator floor to react to packing
*extremity* (density-based), not just absolute-N `scale_pos` (single added
term, everything else identical to the parent).

Diagnosis vs. the parent: the parent already bounds alpha's terminal 1/lr_base
blow-up via `alpha_denom_floor = gamma_min * (1 + _ALPHA_FLOOR_SCALE_SPAN *
scale_pos)`, which fixed dei_n80_omnidir (large absolute N) but is *blind* to
farms that are geometrically crowded (high spacing-normalized density, large
`extremity`) without having a large absolute turbine count -- exactly the
kind of farm this schedule must still generalize to per the task brief. Such
a farm would hit the same constraint-dominated terminal-alpha regime that
hurt dei_n80_omnidir, but `scale_pos` alone would not catch it (e.g. a dense
20-turbine farm below `_N_REF`). Earlier attempts already found that pushing
hold_frac / spike0 / relax_p1 / beta_p0 further via scale_pos regresses the
farm-balanced score, so those four levers, and the existing scale_pos term,
are left untouched. This adds exactly one new, small, additive term to the
alpha floor -- keyed on `extremity` (the packing-density factor already
computed and already used for relax/spike/hold) -- so the terminal alpha cap
now engages for *either* large-N farms or geometrically extreme (very dense
or very sparse) farms, without changing behavior for farms that are neither
(extremity ~ 0 and scale_pos == 0, e.g. mid-density small farms), and without
touching lr, warmup, relax, spike, hold, or beta schedules at all.
"""
...
# divergence is capped instead of fully re-engaging native's constraint-
# dominated tail, for either large-N OR geometrically-extreme farms.
_ALPHA_FLOOR_SCALE_SPAN = 1.5
_ALPHA_FLOOR_EXTREME_SPAN = 0.5   # NEW: react to packing-density extremity too
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Cap the effective denominator at gamma_min*(1 + scale term) instead of
    # letting it ride lr_base all the way to gamma_min, so alpha's terminal
    # divergence is bounded on large-N AND geometrically-extreme farms.
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  + _ALPHA_FLOOR_EXTREME_SPAN * extremity)
