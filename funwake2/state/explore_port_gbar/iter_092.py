"""Schedule: further raise the terminal-alpha denominator floor for large-N
farms (single-lever tweak on the prior winning change).

The previous winning edit added `alpha_denom_floor = gamma_min * (1 +
_ALPHA_FLOOR_SCALE_SPAN * scale_pos)` to cap alpha's terminal 1/lr_base blow-up
on large-N (many-pair) farms, lifting dei_n80_omnidir from -0.0002% to
+0.0005% and the farm-balanced score from +0.137% to +0.189% without moving
the small/mid-N farms (scale_pos == 0 there, so alpha_denom_floor == gamma_min,
identical to the parent). dei_n80_omnidir is still the weakest feasible farm
by a wide margin (+0.0005% vs +0.014-0.68% elsewhere), and it is exactly the
farm this lever targets (n=80, scale_pos ~ 0.71) -- so the natural next probe
is to push the SAME lever further rather than touch the other, already-tried-
and-worse levers (hold_frac, spike0, relax_p1, beta_p0). Raising
_ALPHA_FLOOR_SCALE_SPAN from 1.5 to 2.2 raises the alpha floor for large-N
farms further above gamma_min, capping the terminal constraint-penalty
divergence earlier/harder on crowded layouts, while leaving scale_pos == 0
farms (parque_n10/n20, dei_n50*) completely untouched (their floor is still
exactly gamma_min, matching the parent bit-for-bit on those farms).

Everything else (inverse-time product lr decay to gamma_min, short linear
warmup, density-extremity relax/spike/hold, absolute-N scale on hold/spike/
relax_p1/beta) is unchanged from the parent.
"""
...
# constraint-dominated tail on crowded (many-pair) layouts. Raised from 1.5:
# dei_n80_omnidir (scale_pos ~ 0.71) is still the weakest feasible farm, so
# push this single, already-validated lever further before touching others.
