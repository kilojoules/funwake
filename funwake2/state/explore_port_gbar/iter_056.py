"""Schedule: add a quadratic scale_pos term to the alpha-denominator floor
(isolated single-lever change on top of the parent's linear floor).

Diagnosis vs. the parent: the parent already bounds alpha's terminal
denominator via alpha_denom_floor = gamma_min * (1 + _ALPHA_FLOOR_SCALE_SPAN
* scale_pos), which fixed most of dei_n80_omnidir's regression (from
negative to +0.0005%) but left it far behind every other farm in the
fitness set (dei_n50: +0.0326%, parque_n20: +0.68%) -- i.e. the *linear*
floor growth in scale_pos is still too weak at n_turbines=80 (scale_pos ~=
0.71) relative to n_turbines=50 (scale_pos ~= 0.37), where the parent is
already doing fine. Since scale_pos is a log-compressed, bounded factor
that only activates for above-reference absolute-N farms and is exactly 0
for parque_n10/n20 (leaving them byte-for-byte unaffected), adding a
*quadratic* term in scale_pos widens the floor specifically at the
high-scale_pos tail (n80) much more than at the mid-scale_pos region (n50),
without touching the small/mid farms at all. This directly targets the
worst-performing farm in the set while leaving the four farms the parent
already handles well untouched to first order.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0, the linear alpha
floor term itself) is unchanged from the parent.
"""
...
# alpha-denominator floor (caps terminal alpha divergence on large-N farms):
# linear term (parent, unchanged) plus a NEW quadratic term that grows the
# floor faster at the high-scale_pos tail (e.g. n=80) than at mid scale_pos
# (e.g. n=50), where the parent already scores well.
_ALPHA_FLOOR_SCALE_SPAN = 1.5
_ALPHA_FLOOR_SCALE_SPAN2 = 0.8
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # divergence is bounded on large-N (many-pair) farms only.  The
    # quadratic term (NEW) widens this floor faster at the high-scale_pos
    # tail (e.g. n=80) than at mid scale_pos (e.g. n=50), where the linear
    # term alone already performs well.
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  + _ALPHA_FLOOR_SCALE_SPAN2 * scale_pos * scale_pos)
