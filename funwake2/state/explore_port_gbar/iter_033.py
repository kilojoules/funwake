"""Schedule: smooth the alpha-denominator floor for large-N farms (replace the
hard jnp.maximum kink with a temperature-scaled log-sum-exp soft-max),
isolated single-lever change on top of the current best.

Diagnosis vs. the parent: alpha = alpha0*diam*relax*spike / alpha_denom, where
alpha_denom = max(lr_base, gamma_min*(1 + K*scale_pos)). For large-N farms
(scale_pos > 0, e.g. dei_n80_omnidir with 80 turbines / many spacing pairs)
lr_base crosses the floor partway through the decay, and jnp.maximum
introduces a genuine kink (non-differentiable point) in alpha's trajectory
exactly where the constraint-gradient noise from many turbine pairs is
already worst. dei_n80_omnidir is the weakest farm in the balanced set
(+0.0005%, essentially flat), consistent with this crossover perturbing the
optimizer on the hardest, most crowded layout. This change replaces the hard
max with a temperature-scaled log-sum-exp soft-max:

    alpha_denom = tau * logaddexp(lr_base/tau, floor/tau),  tau = 0.1*gamma_min*scale_pos

so the floor engages smoothly instead of abruptly. Crucially, tau is itself
gated by scale_pos: for small/mid-N farms (scale_pos == 0, e.g. parque_n10,
parque_n20, dei_n50*) tau -> 0 and the soft-max collapses back to the exact
hard max -- and since lr_base >= gamma_min == floor for those farms at every
step, alpha_denom == lr_base exactly, identical to the parent's behavior
(these farms are provably untouched, bit-for-bit). Only scale_pos > 0 farms
see the smoothing, and only near the crossover region.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0, the alpha floor level
itself) is unchanged from the parent -- this is a single isolated lever.
"""
...
# NEW: smoothing temperature for the alpha-denominator floor, scaled by
# scale_pos so small/mid-N farms reduce to the exact hard max.
_ALPHA_FLOOR_SMOOTH = 0.1
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # The effective denominator is capped at gamma_min*(1+k*scale_pos) instead
    ...
    # NEW: engage that cap via a smooth (temperature-scaled log-sum-exp)
    # soft-max instead of jnp.maximum, removing the kink at the crossover.
    # tau -> 0 when scale_pos == 0, so small/mid-N farms get the exact hard
    # max (bit-identical to before); only large-N farms see the smoothing.
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos)
    tau = _ALPHA_FLOOR_SMOOTH * g_min * scale_pos + 1e-9
    alpha_denom = tau * jnp.logaddexp(lr_base / tau, alpha_denom_floor / tau)
