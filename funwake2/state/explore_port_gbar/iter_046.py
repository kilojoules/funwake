"""Schedule: raise the late-run beta2 ceiling for large-N farms, scaled by
scale_pos (isolated single-lever change on top of the current best parent).

Diagnosis vs. the parent: the parent's beta1/beta2 phase-transition ramps
from (0.1, 0.2) to a *fixed* (0.25, 0.45) regardless of farm size, timed
earlier for large-N via beta_p0's scale_pos term. But beta2 controls how
aggressively the Adam second-moment estimate averages gradient-magnitude
noise, and pairwise spacing-constraint gradients grow noisier as O(n^2) with
turbine count -- a fixed 0.45 ceiling under-averages that noise for the
farms that need it most (dei_n50/dei_n50_uniform at n=50, dei_n80_omnidir
at n=80, all with scale_pos > 0), leaving the late-run update direction
jittery exactly when the terminal alpha spike is trying to lock in
feasibility. This is the weakest cell in the current fitness set
(dei_n80_omnidir at +0.0005%), consistent with under-averaged noise eating
the gains the alpha/spike shaping already fought for.

This patch adds one new, previously-untried lever: scale beta2's *ceiling*
(not its onset -- beta_p0 is left untouched) up from 0.45 toward 0.60 as
scale_pos grows, via a fresh _B2_HI_SCALE_SPAN term. Farms with scale_pos==0
(e.g. parque_n10/n20, well below the n=30 absolute-N reference) are
completely unaffected -- beta2 still tops out at exactly 0.45, matching the
parent bit-for-bit. beta1's ceiling, and every lr/alpha/hold/relax/spike
lever, is unchanged from the parent.
"""
...
_B2_HI_SCALE_SPAN = 0.15         # NEW: raise beta2's late-run ceiling for large-N farms
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # NEW: higher late-run ceiling for large N
