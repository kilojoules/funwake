"""Schedule: extend the alpha-denominator floor to also react to packing
density extremity for large-N farms (product-gated second lever), on top of
the existing scale_pos-only floor.

Diagnosis vs. the current best: alpha's terminal denominator floor is
g_min * (1 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos), gated purely on absolute
turbine count. dei_n80_omnidir is not just large-N (80 turbines, the largest
in the fitness set) but also the highest-density farm among the dei_* group
(same D=240m and spacing rule as dei_n50, but more turbines packed into the
same polygon), so its `_packing_size` / extremity should be elevated too.
The current floor already partially compensates for the large-N side but
leaves the extra density-driven noise from that farm's crowded pairwise
constraint terms unaddressed, which likely explains why it is still stuck at
+0.0005% (near flat vs. baseline) even after the scale_pos-only fix. Earlier
attempts that pushed extremity into unrelated levers (hold_frac, spike0,
relax_p1, beta_p0) regressed the farm-balanced score, so this instead adds
one new, narrowly-scoped, PRODUCT-gated term: `scale_pos * extremity`
multiplying a small extra coefficient in the alpha_denom_floor only. Because
it is gated on scale_pos (zero for parque_n10/parque_n20, whose absolute N
is below `_N_REF`), those farms see exactly the same alpha_denom_floor as
before -- only farms that are BOTH large-N and high/low density-extreme
(dei_n50, dei_n50_uniform, dei_n80_omnidir) get the extra bounding, with
dei_n80_omnidir (highest N * highest extremity in that group) affected most.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0, the base
scale_pos-only alpha_denom_floor term) is unchanged from the parent.
"""
...
# NEW: extra floor, product-gated on (large-N AND density-extreme), targeting
# farms like dei_n80_omnidir that are both high-N and high-density within
# the large-N group. Zero for scale_pos == 0 farms (parque_n10/n20).
_ALPHA_FLOOR_EXTREMITY_SPAN = 1.0
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Cap the effective denominator at gamma_min*(1 + k*scale_pos +
    # k2*scale_pos*extremity) instead of letting it ride lr_base all the way
    # to gamma_min, so alpha's terminal divergence is bounded on large-N
    # (many-pair) farms only, with an extra product-gated bound for those
    # large-N farms that are also density-extreme (e.g. dei_n80_omnidir).
    alpha_denom_floor = g_min * (1.0
                                  + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  + _ALPHA_FLOOR_EXTREMITY_SPAN * scale_pos * extremity)
