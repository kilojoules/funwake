"""Schedule: same as parent (+0.1952% farm-balanced) but adds a
band-pass "mid-N" relief gate targeting dei_n50 / dei_n50_uniform.

Diagnosis vs. parent: dei_n50 (+0.0277%) and especially dei_n50_uniform
(+0.0139%, the current worst farm) sit at a density (n=50,
spacing_ratio~=2D -> density=12.5) that exactly equals _DENSITY_REF, so
`size` = 0 and `extremity` = 0 for them -- every density-based relief
term (relax0, spike0, relax_p1, hold_frac) evaluates to its unmodified
base value. Their absolute-N scale_pos (~0.37) is also below the
existing high-N gate's onset (0.5), so the alpha-floor/hold/relax_p1
relief that already helped dei_n80_omnidir (scale_pos~=0.71, gated
ramp~=0.84) gives them nothing either. Both dei_n50 variants therefore
get zero adaptive relief from any existing mechanism, despite being the
two lowest/near-lowest scorers.

Rather than widening the existing high-N gate (which would also change
dei_n80_omnidir's already-tuned relief and risks regressing it), this
adds an independent, band-pass "bump" gate that ramps 0 -> 1 -> 0 across
scale_pos in [0.15, 0.40, 0.65]. It is ~0 for parque_n10/n20
(scale_pos=0, below the band) and exactly 0 for dei_n80_omnidir
(scale_pos~=0.71, past the band, fully faded), but peaks near 1 at
dei_n50's scale_pos~=0.37 -- giving it (and dei_n50_uniform, same N/D)
a modest, isolated boost to alpha-floor relief, exploration hold, and
penalty-engagement delay, mirroring (at reduced magnitude) the mechanism
that already helped the n80 band. Every other mechanism is unchanged.
"""
...
_RELAX_P1_MIDN_SPAN = 0.04      # extra gated delay for the N~=50-ish band
...
_HOLD_FRAC_MIDN_SPAN = 0.015    # extra gated hold for the N~=50-ish band
...
# Independent, BAND-PASS gated relief for the N ~= 50-ish band
# (scale_pos in [0.15, 0.65], peaking near 0.40) -- targets dei_n50 /
# dei_n50_uniform, which sit exactly at the density reference (size=0,
# extremity=0) AND below the high-N gate's onset, so they currently get
# zero adaptive relief from any other mechanism. Fully faded by the time
# scale_pos reaches the high-N band, so dei_n80_omnidir is untouched.
_MIDN_P0, _MIDN_MID, _MIDN_P1 = 0.15, 0.40, 0.65
_ALPHA_FLOOR_MIDN_SPAN = 1.5
...
def _bump_scalar(x, p0, mid, p1):
    """Python-float (trace-time) band-pass bump: 0 at p0 and p1, ~1 near mid."""
    return _smoothstep_scalar(x, p0, mid) * (1.0 - _smoothstep_scalar(x, mid, p1))
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    midn_ramp = _bump_scalar(scale_pos, _MIDN_P0, _MIDN_MID, _MIDN_P1)
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_MIDN_SPAN * midn_ramp)      # extra gated hold for N~=50-ish band
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_MIDN_SPAN * midn_ramp)       # extra gated delay for N~=50-ish band
    ...
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  ...
                                  + _ALPHA_FLOOR_MIDN_SPAN * midn_ramp)
