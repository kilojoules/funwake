...
which already gates extra hold/relax-delay/alpha-floor/spike relief onto the
N~=60-100+ band (scale_pos in [0.5, 0.78]) via `high_n_ramp`. That gate
starts too late to touch n=50 farms (scale_pos~=0.368), which sit in the
gap between "no relief" (parque_n10/n20, scale_pos==0) and the existing
high-N band. dei_n50 and dei_n50_uniform are two of the three worst-scoring
cells (+0.0277%, +0.0139%) despite sharing the same n_turbines/D/min_spacing
(hence identical size/scale_pos/schedule) as each other -- their gap is
wind-rose-driven (via alpha0), not something this input-conditioned
schedule can separate, but both should benefit from the same kind of
large-N relief that already helped dei_n80_omnidir.

This adds a *bump* gate -- rising over scale_pos in [0.15, 0.37], falling
back to 0 by scale_pos=0.50 -- that peaks almost exactly at n=50's
scale_pos (~0.368) and is exactly 0 at parque_n10/n20 (scale_pos==0) and at
dei_n80_omnidir (scale_pos~=0.71, past the falling edge, and already
covered by the separate high_n_ramp gate). It reuses the same four
mechanisms (extra hold, delayed relax engagement, delayed+softened
terminal spike, eased alpha floor) at smaller magnitude than the high-N
spans since this is an untested band. No other farm's schedule changes.
"""
...
_RELAX_P1_MID_N_SPAN = 0.025    # small extra gated delay for N~=50 band
...
_SPIKE_MID_N_SPAN = -0.08       # small extra gated softening for N~=50 band
...
_HOLD_FRAC_MID_N_SPAN = 0.010   # small extra gated hold for N~=50 band
...
# dei_n50's without touching any farm below the gate (dei_n50 at
# scale_pos~=0.37, parque_n10/20 at scale_pos==0).
_ALPHA_FLOOR_HIGH_N_P0 = 0.5
_ALPHA_FLOOR_HIGH_N_P1 = 0.78
_ALPHA_FLOOR_HIGH_N_SPAN = 3.5
# Separate bump gate isolating the N~=50 band: rises over [0.15, 0.37],
# falls back to 0 by 0.50, so it peaks near dei_n50/dei_n50_uniform's
# scale_pos (~0.368) and is exactly 0 for parque_n10/n20 (scale_pos==0)
# and for dei_n80_omnidir (scale_pos~=0.71, past the falling edge).
_MID_N_P0, _MID_N_MID, _MID_N_P1 = 0.15, 0.37, 0.50
_ALPHA_FLOOR_MID_N_SPAN = 0.8
...
def _mid_n_bump(scale_pos):
    """Triangular gate peaking at scale_pos~=0.37 (n~=50), 0 elsewhere."""
    rise = _smoothstep_scalar(scale_pos, _MID_N_P0, _MID_N_MID)
    fall = _smoothstep_scalar(scale_pos, _MID_N_MID, _MID_N_P1)
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_ramp = _mid_n_bump(scale_pos)                 # isolated to N~=50 band
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_MID_N_SPAN * mid_n_ramp)        # extra gated softening for N~=50 band
