...
# alpha0-derived "gradient intensity" signal: a farm-specific, traceable
# feature (built from the already-supplied alpha0, D, gamma_min) that can
# distinguish farms sharing IDENTICAL n_turbines/D/min_spacing but different
# wind roses (e.g. dei_n50 vs dei_n50_uniform) -- those are indistinguishable
# to `size`/`scale_pos` but have different mean|grad J| and therefore
# different alpha0. Squashed via tanh(log(.)/3) so its magnitude is bounded
# to (-1, 1) regardless of the raw scale of alpha0 across farms.
_A0_RELAX_SPAN = -0.04
_A0_SPIKE_SPAN = -0.15
_A0_FLOOR_SPAN = 0.8
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Farm-specific "gradient intensity" feature from alpha0 (traced), D and
    # gamma_min alone -- this is what lets the schedule tell apart farms that
    # are otherwise identical in n_turbines/D/min_spacing (e.g. a directional
    # vs. a uniform wind rose on the same layout), since those differ only in
    # mean|grad J| and hence in alpha0. Bounded via tanh(log(.)/3) so it stays
    # well-conditioned regardless of alpha0's raw scale.
    a0n = jnp.asarray(alpha0) * diam / max(g_min, 1e-9)
    a0_sig = jnp.tanh(jnp.log(jnp.maximum(a0n, 1e-9)) / 3.0)

    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              + _RELAX_SCALE_SPAN * scale_pos          # lower early floor for large-N farms
              + _A0_RELAX_SPAN * a0_sig)                # gradient-intensity-aware trim
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _A0_SPIKE_SPAN * a0_sig)                # gradient-intensity-aware softening
