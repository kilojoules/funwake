"""Schedule: extends the best-so-far variant (+0.1959% farm-balanced) with a
new, independently-gated "mid-N band" relief term (scale_pos in [0.15, 0.42])
that targets dei_n50 / dei_n50_uniform specifically.

Diagnosis: dei_n50 and dei_n50_uniform share identical n_turbines/D/min_spacing
(scale_pos ~= 0.37), so every existing size/scale_pos-gated term treats them
identically -- yet dei_n50_uniform is the worst-performing farm in the suite
(+0.0139%) while dei_n50 itself is comfortably mid-pack (+0.0277%). The
existing high-N gate (scale_pos in [0.5, 0.78]) was built for dei_n80_omnidir
and does not reach this lower band at all, so dei_n50/dei_n50_uniform get none
of that relief even though they clearly benefit from the same *style* of
adjustment (more hold, later constraint engagement, gentler terminal spike,
looser alpha floor). Because both farms sit at the identical trace-time
scale_pos, any relief here helps both together -- there is no tradeoff between
them, only a (small, deliberately bounded) risk to dei_n80 (partial overlap at
the top of the new band) and none to the parque farms (scale_pos == 0, gate
inactive). Coefficients are set to roughly half of the analogous high-N spans
to keep the mutation conservative. Every other mechanism is unchanged from the
best-so-far variant.
"""
...
_RELAX_P1_MID_N_SPAN = 0.03     # extra gated delay for N~=40-55 band (dei_n50*)
...
_SPIKE_MID_N_SPAN = -0.08       # extra gated softening for N~=40-55 band (dei_n50*)
...
_HOLD_FRAC_MID_N_SPAN = 0.015   # extra gated hold for N~=40-55 band (dei_n50*)
...
# NEW: independent gate for the N ~= 40-55 band (scale_pos in [0.15, 0.42]),
# which covers dei_n50 / dei_n50_uniform without reaching the low-N parque
# farms (scale_pos == 0, gate inactive) or materially double-counting on top
# of the high-N gate (disjoint except a small overlap near scale_pos ~= 0.42-0.5,
# where both ramps are already close to saturated/zero respectively).
_MID_N_P0 = 0.15
_MID_N_P1 = 0.42
_ALPHA_FLOOR_MID_N_SPAN = 1.5
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_MID_N_SPAN * mid_n_ramp)   # extra gated hold for N~=40-55 band
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_MID_N_SPAN * mid_n_ramp)    # extra gated delay for N~=40-55 band
