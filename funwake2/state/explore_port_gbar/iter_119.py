"""Schedule: extend the existing scale_pos>0.5 gate (currently only bounding
the alpha-floor) to three more levers -- hold_frac, relax_p1, and spike0 --
so the whole exploration/constraint-engagement profile shifts for large-N
farms, not just the terminal alpha ceiling.

Diagnosis vs. the current best: dei_n80_omnidir (scale_pos ~= 0.706, above
the 0.5 gate) is still barely below native (-0.0025%) even after the
quadratic alpha-floor relief, while dei_n50 (scale_pos ~= 0.368, below the
gate) is solidly positive and the parque farms (scale_pos == 0) are
strongly positive. This means the remaining shortfall for dei_n80_omnidir is
not purely a terminal-alpha problem -- the exploration phase itself (how
long the plateau lasts, how late the constraint penalty starts ramping, how
strong the terminal spike is) is still tuned mid-density-farm-first and only
partially adapted for high absolute N. Since `high_n_gap = max(scale_pos -
0.5, 0)` already exists and is (by construction) exactly zero for dei_n50 and
the parque farms, reusing it to gate three more quadratic correction terms
extends relief to dei_n80_omnidir (and by the same adaptive mechanism, to
the held-out 74-turbine ROWP test farm, whose scale_pos ~= 0.65 also clears
the gate) while leaving every farm below the gate bit-for-bit identical to
the parent:

  * hold_frac gets an extra quadratic bump (_HOLD_HIGH_N_SPAN) -- more pure
    unconstrained exploration time for crowded, many-pair layouts before any
    decay/constraint engagement starts.
  * relax_p1 gets an extra quadratic delay (_RELAX_P1_HIGH_N_SPAN) -- the
    constraint penalty ramp is pushed even later into the run for high-N
    farms specifically.
  * spike0 gets an extra quadratic reduction (_SPIKE_HIGH_N_SPAN) -- the
    terminal feasibility spike is softened further, on top of the existing
    linear _SPIKE_SCALE_SPAN term, so high-N farms are not over-penalized
    right at the end when they have the least time to recover AEP.

All three reuse the same `high_n_gap` used by the alpha-floor term, so they
activate together and stay at exactly zero below the gate -- no new
threshold, no risk of perturbing dei_n50/parque_n10/parque_n20. Everything
else (self-contained inverse-time product lr decay to gamma_min, short
linear warmup, density-extremity relax/spike/hold, absolute-N scale on
hold/spike/relax_p1, the hold_frac-floored beta ramp, the existing alpha
floor) is unchanged from the parent.
"""
...
# (scale_pos > 0.5, i.e. N > 60), shared by all four high-N correction terms
# below so they activate together and stay exactly zero under the gate.
_ALPHA_FLOOR_HIGH_N_THRESH = 0.5
_ALPHA_FLOOR_HIGH_N_SPAN = 6.0
# Extra high-N-gated corrections to the exploration/engagement profile
# (new in this generation) -- all zero below the scale_pos 0.5 gate.
_HOLD_HIGH_N_SPAN = 0.05         # longer plateau, high-N only
_RELAX_P1_HIGH_N_SPAN = 0.10     # later constraint-ramp start, high-N only
_SPIKE_HIGH_N_SPAN = -0.6        # softer terminal spike, high-N only
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    high_n_gap2 = high_n_gap * high_n_gap

    lr0 = _C * diam                                  # exploration lr from D
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 + _HOLD_FRAC_SCALE_SPAN * scale_pos
                 + _HOLD_HIGH_N_SPAN * high_n_gap2)     # extra plateau, high-N only
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGH_N_SPAN * high_n_gap2)      # extra softened spike, high-N only
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                + _RELAX_P1_SCALE_SPAN * scale_pos
                + _RELAX_P1_HIGH_N_SPAN * high_n_gap2)  # extra-delayed ramp, high-N only
    ...
    # while _HOLD_FRAC_SPAN/_HOLD_FRAC_SCALE_SPAN/_HOLD_HIGH_N_SPAN push
