"""Schedule: raise the late-run beta2 (second-moment decay) ceiling for
large-N farms via a scale_pos-gated span, leaving everything else
identical to the parent (isolated single-lever change).

Diagnosis vs. the parent: dei_n80_omnidir (80 turbines) has by far the
most turbine pairs of any farm in the fitness set, so its spacing-
constraint gradient is summed over many more noisy pairwise terms than
dei_n50 (50) or parque_n10/n20 (10/20) -- yet beta2's late-run ceiling
(_B2_HI = 0.45) is currently the SAME for every farm regardless of how
many pairwise terms are being averaged into the adaptive second moment.
A fixed beta2 ceiling under-smooths the noisier per-step gradient
estimate on the largest farm, which plausibly explains why dei_n80_omnidir
sits nearly flat (+0.0005%) while the other farms comfortably beat
baseline. This change adds one new, previously-untried lever: scale the
beta2 ceiling upward with scale_pos (the same absolute-N indicator
already used elsewhere) so large-N farms get more late-run averaging of
their noisier constraint-gradient estimate, while leaving small/mid-N
farms (scale_pos == 0, e.g. parque_n10/n20, where scale_pos clips to 0)
completely unaffected -- beta2 there matches the parent exactly. dei_n50
(scale_pos ~0.37) gets a small nudge; dei_n80_omnidir (scale_pos ~0.71)
gets the largest nudge. beta1, alpha, lr, hold_frac, relax, spike and the
alpha-denominator floor are all untouched from the parent.
...
absolute-N scale on hold/spike/relax_p1/beta_p0/relax0, alpha denominator
floor for large-N) is unchanged from the parent.
"""
...
# NEW: raise the late-run beta2 ceiling further for large-N farms, whose
# spacing-constraint gradient sums over many more noisy pairwise terms
# and benefits from stronger second-moment smoothing.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # more late-run smoothing, large N
