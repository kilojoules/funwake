...
Diagnosis vs. the parent (which shapes hold_frac, spike, relax_p1, and
beta_p0 purely from packing DENSITY via `size`/`extremity`): dei_n80_omnidir
is feasible but sits BELOW native (-0.054%), while every other farm is
positive. dei_n80_omnidir has ordinary packing DENSITY (min_spacing/D ~ 2,
same as the mid-density anchor) but is the largest-N farm in the fitness
set by a wide margin (80 vs. <=50 elsewhere) -- far more pairwise spacing
constraints, hence a noisier constraint gradient, even though density alone
looks unremarkable and so the density-based `size` signal barely reacts.

`relax0` -- the FLOOR penalty multiplier applied during early exploration,
before the relax ramp even engages -- is wired only to the density-based
`size` signal, not to absolute turbine count. For a large-N, ordinary-
density farm this floor stays at the generic mid-density value, so early
optimization still carries a touch more constraint pressure per
turbine-pair than a small/mid farm at the same density would need.

This is a single, isolated addition: an absolute-N factor `scale_pos`
(log-compressed, bounded, and gated to zero at/below an absolute-N
reference) that lowers `relax0`'s floor only for large-N farms, leaving
parque_n10, parque_n20, dei_n50, dei_n50_uniform untouched. All other
machinery (self-contained inverse-time product lr decay to gamma_min,
short linear warmup, density-extremity relax/spike/hold, betas) is
unchanged from the parent -- deliberately not touching hold/spike/relax_p1/
beta with this same signal, since combining multiple scale_pos levers at
once previously regressed the farm-balanced score.
"""
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = _HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity   # longer plateau at extremes
