"""Schedule: builds on the best-so-far variant (+0.1959% farm-balanced),
which already gates hold/relax-delay/alpha-floor/spike relief onto the
N~=60-100+ band via `high_n_ramp`, and shapes a bounded, delayed, late
terminal alpha spike via `spike0`/`spike_p0`/`_SPIKE_P1`. Every one of the
recent generations has plateaued at ~+0.17-0.196%, all via further tuning
of that same spike/relax/hold machinery -- suggesting that lever is close
to saturated for this farm set.

This variant instead introduces a mechanism the parent does not yet use:
phase-transitioning beta2 WITH the alpha spike phase (prior-art guidance
section 4/10 -- "ramp beta2 to tame early adaptive-lr variance and absorb
the ~alpha constraint-curvature conditioning", and "phase-transition Adam
moments with the alpha phase"). When the terminal spike ramps alpha up by
`spike0` (which itself is per-farm: bigger for small/sparse farms via
`size`, damped for large-N farms via `scale_pos`/`high_n_ramp`), the
penalty-gradient curvature entering Adam's second moment jumps sharply at
exactly the same time. The existing beta2 ramp (tied to `p` in
[beta_p0, _BETA_P1]) is already saturated well before the spike window
(_SPIKE_P0..1.00) in most cases, so Adam's adaptive scaling sees that
curvature jump with only its plateaued beta2 -- undershooting on farms
whose spike is proportionally large (small/uniform layouts, size>0) and
doing basically nothing extra on farms whose spike is already softened
(large-N, high_n_ramp>0), which is exactly backwards: a bigger spike
needs MORE extra smoothing, not the plateaued value all farms already get.

Fix: add a second, independent beta2 (and small beta1) contribution that
ramps over the SAME window as the alpha spike (`spike_p0` to `_SPIKE_P1`,
via `_ramp`, reusing the spike ramp shape) and whose magnitude is
proportional to spike0 itself (normalized against the base magnitude
_SPIKE_BASE) -- so farms with a stronger terminal alpha spike get
correspondingly more Adam-moment damping right when that spike engages,
while farms with an already-softened spike (large-N band) get a small,
consistent boost rather than none. Both new terms are additive, bounded
(clipped below 1), and reduce to ~0 extra contribution when spike0 is
near its base value at size=0 (mid-density, mid-N), so they don't disturb
the already-tuned mid-range farms much. Everything else is byte-identical
to the parent.
"""
...
# NEW: phase-transition beta1/beta2 WITH the terminal alpha-spike window,
# proportional to that farm's spike strength (normalized to the base spike
# magnitude) so a bigger spike gets more damping, not the same flat ramp.
_B2_SPIKE_SPAN = 0.12
_B1_SPIKE_SPAN = 0.04
_BETA_SPIKE_CAP = 0.60          # never push beta2 past this via the spike term alone
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    spike_ramp = _ramp(p, spike_p0, _SPIKE_P1)
    ...
    # Phase-transition the moments WITH the alpha spike itself: a farm whose
    # terminal spike is proportionally stronger (spike0 large relative to the
    # base magnitude) sees more curvature entering the penalty gradient right
    # as spike_ramp engages, so give it correspondingly more extra damping
    # there -- reduces to ~0 extra contribution at spike0 == _SPIKE_BASE.
    spike_strength = max(spike0 / _SPIKE_BASE, 0.0)
    beta2 = jnp.minimum(beta2 + _B2_SPIKE_SPAN * spike_ramp * spike_strength, _BETA_SPIKE_CAP)
    beta1 = jnp.minimum(beta1 + _B1_SPIKE_SPAN * spike_ramp * spike_strength, _B1_HI + _B1_SPIKE_SPAN)
