"""Schedule: builds on best-so-far farm-balanced +0.1959% variant. dei_n50 and
dei_n50_uniform share IDENTICAL (D, min_spacing, n_turbines) inputs, so the
schedule cannot distinguish them via any input signal -- yet dei_n50_uniform
(omnidirectional wind rose) scores far worse (+0.0139%) than dei_n50
(+0.0277%). Since we can't gate on wind-rose type, the only lever is to make
the schedule more robust for the *harder*, more isotropic search landscape
that a uniform/omnidirectional rose implies, without meaningfully hurting the
already-strong parque farms or the already-decent dei_n50/dei_n80_omnidir.

Three small, ungated (farm-independent) changes:
  1. Slightly longer, gentler warmup (_WARM_FRAC 0.015->0.02) -- more low-lr
     steps at the very start to build a better-conditioned initial spread
     before the full exploration lr kicks in; harder omnidirectional
     landscapes benefit more from not committing early, easy ones barely
     notice a few extra steps.
  2. Slightly longer base exploration hold (_HOLD_FRAC_BASE 1/3 -> 0.35) --
     a bit more time at full exploration lr before decay begins, giving
     harder/more isotropic layouts more chances to escape a bad basin.
  3. Slightly softer base terminal spike (_SPIKE_BASE 2.0 -> 1.85) -- less
     violent late constraint enforcement uniformly, so farms that need more
     of their late budget to keep resolving structure (rather than being
     yanked into feasibility) get a bit more room; the existing
     size/scale/high-N terms still shape this per-farm on top.

All gated (high-N / extremity / scale) terms are left untouched from the
best-so-far variant since they already help dei_n80_omnidir specifically.
"""
...
_WARM_FRAC = 0.02                # linear warmup inside the plateau (slightly longer)
...
_SPIKE_BASE, _SPIKE_SPAN = 1.85, 0.55   # spike0 = base + span * size (base softened)
...
_HOLD_FRAC_BASE, _HOLD_FRAC_SPAN = 0.35, 0.06   # exploration plateau length (base extended)
