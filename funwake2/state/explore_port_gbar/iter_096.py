"""Schedule: decouple beta1 from the shared beta1/beta2 ramp so it DECAYS
(Demon-style) through the alpha feasibility phase instead of increasing in
lockstep with beta2 (isolated single-lever change vs. the best-so-far
parent).

Diagnosis vs. the parent: beta1 and beta2 currently share one ramp
`r = _ramp(p, beta_p0, _BETA_P1)` and both increase from their LO to HI
bounds together as the run enters the alpha-engaged feasibility phase
(relax/spike ramping alpha up over the same window). That means momentum
(beta1) goes UP exactly when the constraint penalty is also ramping up and
alpha starts pulling the layout toward feasibility -- i.e. the optimizer
gets MORE inertia right when it most needs to respond quickly to the newly
dominant constraint gradient. The prior-art survey's phase-transition bet
(#4: "beta2 up / beta1 down in the feasibility phase") is untried in the
best-so-far code, which only ever ramps both moments the same direction.
This change keeps beta2's ramp-up exactly as-is (still tames adaptive-lr
variance as alpha engages) but flips beta1 to ramp DOWN over the same
window: beta1 starts at its previous "late" value (more momentum during
pure AEP-exploration, before alpha matters) and decays toward a lower
floor as r -> 1, so the optimizer becomes more responsive to the fresh
constraint-gradient direction exactly when alpha is largest. All bounds,
onsets (beta_p0/_BETA_P1, including the large-N earlier-onset shift), and
every other lever (lr decay/warmup, relax/spike/hold, alpha_denom_floor)
are byte-for-byte unchanged from the parent -- this isolates the beta1
direction flip as the only tested change.
"""
...
# Adam moments: beta2 ramps native -> mildly averaged late (unchanged);
# beta1 now DECAYS over the same window (Demon-style) instead of ramping up
# in lockstep, so momentum drops exactly as alpha's feasibility pull grows.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # beta2 still ramps UP (native -> more-averaged) exactly as the parent.
    beta2 = _B2_LO + (_B2_HI - _B2_LO) * r
    # beta1 now ramps DOWN over the same window: high momentum during pure
    # AEP exploration (r=0), decaying toward the low-momentum floor as the
    # alpha-driven feasibility phase takes over (r=1) -- letting the
    # optimizer react quickly to the now-dominant constraint gradient
    # instead of carrying stale exploration momentum through it.
    beta1 = _B1_HI - (_B1_HI - _B1_LO) * r
