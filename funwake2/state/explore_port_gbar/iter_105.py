"""Schedule: add threshold-gated quadratic (high-N) relief on the
*timing* levers -- hold_frac, relax_p1 (penalty engagement delay), spike0
(terminal spike), and beta_p0 (momentum-ramp onset) -- instead of pushing
the already-plateaued alpha_denom_floor lever further.

Diagnosis vs. the current best: for six straight generations the
farm-balanced score has sat at +0.190% while dei_n80_omnidir stayed just
below native (-0.0025%). All of the recent search pressure went through
ALPHA_FLOOR_HIGH_N_SPAN, which only bounds the *terminal alpha magnitude*
via alpha_denom_floor -- it does nothing to change *when* the constraint
penalty starts dominating or how long dei_n80_omnidir gets to explore
before that ramp begins. Since alpha_denom_floor is saturated, the next
lever is timing: dei_n80_omnidir (scale_pos ~= 0.71, n=80) is the only
farm in the fitness set with scale_pos > 0.5 (dei_n50 sits at ~0.37,
parque_n10/20 and dei_n50_uniform all sit at 0 after the max(...,0)
clip), so a quadratic term gated on high_n_gap = max(scale_pos-0.5, 0)
touches dei_n80_omnidir alone and leaves every other farm's trajectory
bit-for-bit identical to the parent.

Concretely, for dei_n80_omnidir this generation:
  - hold_frac gets a small additional boost (longer pure-exploration
    plateau before lr starts decaying),
  - relax_p1 (the progress fraction where the constraint penalty finishes
    ramping to full strength) is pushed further out (penalty engages
    later, giving more time near the boundary/spacing limits to settle),
  - spike0 (the terminal feasibility-restoration multiplier) is softened
    further (less violent last-mile correction on a farm that already has
    a bounded alpha floor -- avoid overshooting into a worse local
    optimum), and
  - beta_p0 (Adam-moment averaging onset) is delayed further, keeping
    unaveraged gradient steps active longer for this many-pair farm.

All four are small, independently-tunable additions on top of the
existing linear scale_pos terms, and all vanish for scale_pos <= 0.5, so
dei_n50, dei_n50_uniform, parque_n10_omnidir and parque_n20 are
...
# NEW: same high-N gate, applied to the *timing* levers instead of alpha
# magnitude -- longer hold, later penalty engagement, softer terminal spike,
# later beta ramp, all specifically for scale_pos > 0.5 farms.
_HOLD_FRAC_HIGH_N_SPAN = 0.05
_RELAX_P1_HIGH_N_SPAN = 0.06
_SPIKE_HIGH_N_SPAN = -0.35
_BETA_P0_HIGH_N_SPAN = -0.08
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGH_N_SPAN * high_n_gap_sq)    # extra-softened spike, high-N only
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                + _RELAX_P1_SCALE_SPAN * scale_pos
                + _RELAX_P1_HIGH_N_SPAN * high_n_gap_sq)   # extra delay, high-N only
    ...
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  ...
                                  + _ALPHA_FLOOR_HIGH_N_SPAN * high_n_gap_sq)
    ...
    # large-N/extreme-density farms, _BETA_P0_SCALE_SPAN (and now the
    # high-N-gated term) pull beta_p0 down while _HOLD_FRAC_SPAN/
    # _HOLD_FRAC_SCALE_SPAN/_HOLD_FRAC_HIGH_N_SPAN push hold_frac up, and
    # the two can cross so momentum starts increasing during pure
    # exploration (before lr has decayed at all) on exactly the crowded,
    # many-pair farms that most need unaveraged gradient steps there.
    beta_p0 = max(_BETA_P0 + _BETA_P0_SCALE_SPAN * scale_pos
                  + _BETA_P0_HIGH_N_SPAN * high_n_gap_sq, hold_frac)
