...
which already gates extra hold/relax-delay/spike relief onto the N~=60-100+
band via `high_n_ramp` (scale_pos in [0.5, 0.78]) and a bounded alpha floor.

Diagnosis: among the current cell scores, dei_n50_uniform (+0.0139%) is the
worst farm, and it sits at the SAME n_turbines/D/min_spacing as dei_n50
(+0.0277%) -- so every size-/scale_pos-based knob in the parent treats them
identically. The only per-farm signal that differs between a directionally
concentrated wind rose (dei_n50) and a broad/uniform one (dei_n50_uniform) is
the supplied `alpha0 = mean|grad J|/D`: a uniform rose spreads the AEP
objective's push across many directions, so the *mean* gradient magnitude
that drives early exploration is more diffuse/weaker than a farm with a
dominant wind direction, even at identical turbine count and packing. A
weaker, more diffuse objective gradient gives the optimizer less signal to
resolve spacing conflicts through the objective alone during the
exploration hold -- by the same logic already used for large-N farms (more
freedom before the penalty engages), such farms should get a bit more early
alpha relief and a slightly delayed penalty engagement window.

This adds one new, bounded, traceable signal `grad_size` from alpha0 (via
jnp, never float()/int()'d) that nudges `relax0` down and `relax_p1` up for
farms with a below-reference alpha0*D/gamma_min ratio -- mirroring the
existing `scale_pos`-gated large-N relief but keyed off the wind-rose/
gradient character instead of turbine count. It is small-span and additive
only on top of the untouched parent mechanics, and is clipped so it cannot
push relax0/relax_p1 out of a safe range even at full saturation. Every
other mechanism (hold_frac, spike, alpha floor, beta ramp) is unchanged.
"""
...
_RELAX_GRAD_SPAN = 0.04         # lower early floor further for weak/diffuse-gradient (e.g. uniform-rose) farms
_RELAX_P0 = 0.08
...
_RELAX_P1_GRAD_SPAN = 0.03      # further delay for weak/diffuse-gradient farms
...
_GRAD_REF = 1.0                 # alpha0*D/gamma_min reference (order-1 anchor; log-compressed & clipped)
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Traceable, bounded wind-rose/gradient-diversity signal from alpha0
    # (never float()/int()'d -- alpha0 is a traced value): ratio of the
    # supplied mean-gradient scale to the tolerance scale, log-compressed
    # and clipped to [-1, 1] just like `size`/`scale_pos` above.
    grad_ratio = jnp.asarray(alpha0) * diam / max(g_min, 1e-12)
    grad_log = jnp.log(jnp.maximum(grad_ratio, 1e-9) / _GRAD_REF) / math.log(4.0)
    grad_size = jnp.clip(grad_log, -1.0, 1.0)

    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              + _RELAX_SCALE_SPAN * scale_pos          # lower early floor for large-N farms
              + _RELAX_GRAD_SPAN * grad_size)          # ...and for weak/diffuse-gradient farms
    relax0 = jnp.clip(relax0, 0.05, 0.95)
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                - _RELAX_P1_GRAD_SPAN * grad_size)      # further delay for weak-gradient farms
    relax_p1 = jnp.clip(relax_p1, _RELAX_P0 + 0.05, 0.9)
