"""Schedule: extend the warmup window itself with packing extremity (isolated
single-lever change vs. the +0.189% parent).

Diagnosis vs. the parent: the parent already scales `hold_frac` (exploration
plateau length), `relax0/relax_p1` (penalty floor/onset), `spike0` (terminal
spike), and `beta_p0` (moment-averaging onset) with either `extremity`
(density distance from the mid-scale reference) or `scale_pos` (large
absolute N) -- but `_WARM_FRAC`/`_WARM_LO` (the very first, linear lr
warmup) is still a flat constant for every farm. For farms at packing-density
extremes (either far sparser or far denser than the ~50-turbine/2D reference
that _DENSITY_REF encodes) the initial grid placement sits further from a
locally-good configuration, so a fixed 1.5%-of-run linear warmup either
completes too quickly for a dense/crowded farm (letting alpha's own ramp,
which is itself density-scaled, engage before lr has really reached its
exploration plateau) or overshoots-then-corrects unnecessarily for a sparse
farm that never needed the extra ramp room. This leaves one new, previously
untried lever: make the warmup fraction itself grow with `extremity`, mirroring
the same density-extremity signal already driving hold_frac/relax/spike,
while leaving mid-density farms (extremity ~ 0, e.g. dei_n50/parque_n20)
exactly at the parent's flat 1.5% warmup.
...
hold/spike/relax_p1/beta/alpha-denom-floor) is unchanged from the parent.
"""
...
_WARM_FRAC_BASE, _WARM_FRAC_SPAN = 0.015, 0.02   # NEW: warmup lengthens at density extremes
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    warm_frac = _WARM_FRAC_BASE + _WARM_FRAC_SPAN * extremity   # NEW: longer warmup at extremes
