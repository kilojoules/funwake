"""Schedule: builds on the current-best variant (+0.1959% farm-balanced),
which already gates extra hold/relax-delay relief and terminal-spike
softening onto the N~=60-100+ band (high_n_ramp) and separately gives
near-reference-density farms (mid_gate, extremity~=0) extra hold, delayed
relax engagement, and a softened spike.

Gap: mid_gate currently touches hold_frac, relax_p1, and spike0 -- but NOT
alpha_denom_floor, even though the high-N band gets exactly that floor
relief (_ALPHA_FLOOR_HIGH_N_SPAN). dei_n50 and dei_n50_uniform sit at
extremity~=0 (density ~= _DENSITY_REF exactly) and are two of the
worst-scoring farms; they are NOT in the high-N band (n=50 -> scale_pos
~=0.37, below the 0.5 gate start) so they get none of the alpha-floor
relief that dei_n80_omnidir enjoys. This adds a small, mid_gate-scaled term
to alpha_denom_floor, mirroring the existing high-N treatment, so these
mid-density farms' terminal alpha divergence is capped a bit more instead
of fully re-engaging the native 1/lr_base coupling. Magnitude is kept
modest (well below the high-N span) since this is an untested mechanism;
parque_n10/parque_n20 sit at extremity>>0.3 so mid_gate~=0 there and this
term has essentially no effect on them. Every other mechanism is
...
# Small analogous relief for near-reference-density farms (mid_gate),
# which currently get NO alpha-floor relief despite dei_n50/dei_n50_uniform
# being among the worst-scoring farms. Kept modest and mutually exclusive
# with the high-N band via mid_gate's own (1 - high_n_ramp) damping.
_MID_ALPHA_FLOOR_SPAN = 1.0

# Narrow gate for near-reference-density farms (extremity ~= 0), which get
# the LEAST relax delay / hold extension / spike softening of any band --
# despite dei_n50_uniform being the global worst-scoring farm. Fades to 0
# by extremity=0.3 and is further damped wherever high_n_ramp is already
# active, so dei_n80/parque are unaffected.
_MID_GATE_P0, _MID_GATE_P1 = 0.0, 0.30
_MID_HOLD_FRAC_SPAN = 0.02       # small extra exploration hold
_MID_RELAX_P1_SPAN = 0.05        # small extra delay of constraint engagement
_MID_SPIKE_SPAN = -0.25          # small softening of the terminal spike
