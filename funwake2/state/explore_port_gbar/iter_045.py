"""Schedule: raise the terminal alpha-denominator floor's large-N gain
(isolated single-lever tweak on the newest, not-yet-tuned lever).

Diagnosis vs. the current best: the parent already caps alpha's terminal
denominator at `gamma_min * (1 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos)`
instead of letting it ride `lr_base` all the way down to `gamma_min`, which
fixed dei_n80_omnidir from a regression (-0.0002%) to roughly flat
(+0.0005%) -- but "roughly flat" means the floor is still barely biting for
that farm: with `_ALPHA_FLOOR_SCALE_SPAN = 1.5` and n=80 giving
`scale_pos = n_scale(80) = log(80/30)/log(4) ~= 0.708`, the floor sits at
only `gamma_min * (1 + 1.5*0.708) ~= gamma_min * 2.06` -- a small multiple
of an already-tiny tolerance, so alpha still climbs close to its native,
mostly-unbounded terminal value on this many-pair farm. Unlike the earlier
failed attempts (further pushing hold_frac, spike0, relax_p1, beta_p0),
`_ALPHA_FLOOR_SCALE_SPAN` itself has not yet been tuned since its
introduction, so it is the natural next single lever to move. Raising it
(1.5 -> 3.0) roughly doubles the terminal floor for large-N farms (e.g.
~3.1x gamma_min for n=80), giving the late-stage optimizer materially more
room to keep improving AEP once spacing feasibility is already met, while
leaving every small/mid-N farm (scale_pos == 0: parque_n10, parque_n20,
dei_n50*) completely untouched, since their floor stays pinned at
gamma_min, identical to the parent.
...
# Raised from 1.5 -> 3.0: the prior value barely moved the floor on
# dei_n80_omnidir (only ~2x gamma_min), leaving that farm essentially flat.
