"""Schedule: relax0 gains an independent large-N term (isolated single-lever change).

Diagnosis vs. the parent (which already scales hold_frac, spike, relax_p1,
and beta_p0 by an absolute-N `scale_pos` signal, gated to act only on
large-N farms): dei_n80_omnidir is still feasible but sits slightly BELOW
native (-0.017%), while every other farm improved. dei_n80_omnidir has
ordinary packing DENSITY (min_spacing/D ~= 2, same as the mid-density
anchor) but is the largest-N farm in the fitness set by a wide margin (80
vs. <=50 elsewhere) -- far more pairwise spacing constraints, hence a
noisier constraint gradient, even though density alone looks unremarkable.

The parent's `scale_pos` signal already reaches this farm (hold/spike/
relax_p1/beta all get some correction), but `relax0` -- the FLOOR penalty
multiplier applied during early exploration, before the relax ramp even
engages -- is wired only to the density-based `size` signal, not to
`scale_pos`. For a large-N, ordinary-density farm this floor stays at the
generic mid-density value, so early optimization still carries a touch more
constraint pressure per turbine-pair than a small/mid farm at the same
density would need. Recent attempts already pushed the OTHER large-N knobs
(hold/spike/relax_p1/beta) further and that made the farm-balanced score
worse (-0.021%, -0.012%) -- so this change deliberately leaves those levers
untouched and adds exactly one new, previously-untried one: give `relax0`
its own independent scale_pos term, gated (like all scale_pos terms) to be
zero for any farm at or below the absolute-N reference, so parque_n10,
parque_n20, dei_n50, dei_n50_uniform are unaffected and only dei_n80_omnidir
(and any future large-N farm) sees a lower early-exploration penalty floor.
...
absolute-N scale on hold/spike/relax_p1/beta) is unchanged from the parent.
"""
...
_RELAX_SCALE_SPAN = -0.05       # NEW: lower early-exploration penalty floor for large-N farms
_RELAX_P0 = 0.08
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              + _RELAX_SCALE_SPAN * scale_pos)        # NEW: lower early floor for large-N farms
