...
gates extra hold/relax-delay/spike-softening/alpha-floor relief onto a
high-N band via `high_n_ramp = smoothstep(scale_pos, 0.5, 0.78)`. That band
currently only reaches dei_n80_omnidir (scale_pos ~= 0.71); dei_n50 and
dei_n50_uniform sit at scale_pos ~= 0.368 and get zero relief.

Diagnosis: dei_n50_uniform is the worst-performing farm (+0.0139%), worse
than dei_n50 (+0.0277%) despite IDENTICAL n_turbines/D/min_spacing -- the
only schedule-visible difference between the two is alpha0 (the omnidirectional
wind rose changes the gradient statistics). This mirrors exactly why
dei_n80_omnidir needed the high-N relief: an omnidirectional/uniform wind
rose creates spacing conflicts across many more heading sectors, so premature
constraint engagement compresses the resolution window the same way it does
for the n=80 farm. The fix is not to add a farm-specific branch (forbidden
and impossible anyway, since dei_n50 and dei_n50_uniform are geometrically
identical) -- it is to widen the SAME size-driven gate so it starts ramping
in earlier, giving n~50 farms partial access to the delayed-engagement /
longer-hold / softened-spike / raised-alpha-floor relief that n~80 farms
already get. The gate is widened from [0.5, 0.78] to [0.25, 0.6] in
`scale_pos` units:
  - n=50 (scale_pos~=0.368) now gets high_n_ramp~=0.26 (was 0.0)
  - n=80 (scale_pos~=0.707) still saturates at high_n_ramp=1.0 (unchanged,
    since 0.707 > 0.6), so dei_n80_omnidir's tuning is untouched
  - parque_n10/n20 (scale_pos clipped to 0) still get high_n_ramp=0.0
    (unchanged, since their scale_pos is <= 0 well below the new P0)
Every other mechanism is unchanged from the best-so-far variant.
"""
...
_RELAX_P1_HIGH_N_SPAN = 0.05    # extra gated delay for mid/high-N (omnidir-heavy) farms
_SPIKE_BASE, _SPIKE_SPAN = 2.0, 0.55    # spike0 = base + span * size
_SPIKE_SCALE_SPAN = -0.4        # soften terminal spike for large-N farms
_SPIKE_HIGH_N_SPAN = -0.15      # extra gated softening for mid/high-N band
...
_HOLD_FRAC_HIGH_N_SPAN = 0.02   # extra gated hold for mid/high-N farms
...
# Extra, smoothstep-gated relief for farms in the N ~= 50-80+ band
# (scale_pos in [0.25, 0.6]) -- widened from [0.5, 0.78] so dei_n50 /
# dei_n50_uniform (scale_pos ~= 0.368) get partial relief too, while
# dei_n80_omnidir (scale_pos ~= 0.71, already saturating past 0.6) and
# parque_n10/n20 (scale_pos clipped to 0) are unaffected.
_ALPHA_FLOOR_HIGH_N_P0 = 0.25
_ALPHA_FLOOR_HIGH_N_P1 = 0.60
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_ramp)  # extra gated hold for mid/high-N band
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGH_N_SPAN * high_n_ramp)     # extra gated softening for mid/high-N band
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                + _RELAX_P1_SCALE_SPAN * scale_pos      # further delay for large-N farms
                + _RELAX_P1_HIGH_N_SPAN * high_n_ramp)  # extra gated delay for mid/high-N band
    relax = relax0 + (1.0 - relax0) * _ramp(p, _RELAX_P0, relax_p1)
    spike_p0 = _SPIKE_P0 + _SPIKE_P0_HIGH_N_SPAN * high_n_ramp  # delay spike onset for mid/high-N
