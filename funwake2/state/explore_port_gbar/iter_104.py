"""Schedule: extend the proven high-N quadratic-gate pattern (already used for
the alpha-floor) to three more levers -- hold_frac, spike0, and relax_p1 --
so dei_n80_omnidir (scale_pos ~= 0.71, N=80, omnidirectional wind rose) gets
a longer exploration/feasibility-delay phase and a softer terminal spike,
while dei_n50 (scale_pos ~= 0.37, below the 0.5 gate), parque_n20 (scale_pos
== 0) and parque_n10 (scale_pos == 0) are bit-for-bit unaffected -- the gate
term is exactly zero for all of them.

Diagnosis vs. the current best: the alpha_denom_floor quadratic gate
(high_n_gap**2, gated at scale_pos>0.5) was the one lever already decoupled
from dei_n50, and it got dei_n80_omnidir to -0.0025% (from further below).
But hold_frac, spike0, and relax_p1 are still purely LINEAR in scale_pos --
so pushing them further to help dei_n80_omnidir (N=80, scale_pos~=0.71)
proportionally over-relaxes dei_n50 (N=50, scale_pos~=0.37), which is why
those levers plateaued. Mirroring the same quadratic, threshold-gated
high_n_gap**2 boost onto hold_frac (longer pure-exploration plateau before
lr/alpha engage), spike0 (further-softened terminal feasibility spike, since
dei_n80_omnidir's many-pair geometry makes a hard terminal push more likely
to destabilize than help), and relax_p1 (later full-penalty engagement)
gives dei_n80_omnidir the same kind of surgical extra relief the alpha floor
already validated, with the same decoupling guarantee: high_n_gap is exactly
0 for every other farm in the fitness set, so their trajectories are
unchanged. beta_p0's existing max(..., hold_frac) floor means it
automatically tracks the extended hold_frac too, so momentum still can't
start ramping before exploration ends on this farm.
"""
...
# Same quadratic gate, applied to hold_frac / spike0 / relax_p1 -- extends
# dei_n80_omnidir's exploration+feasibility-delay phase and softens its
# terminal spike further, with zero effect below the scale_pos>0.5 gate.
_HOLD_FRAC_HIGH_N_SPAN = 0.05
_SPIKE_HIGH_N_SPAN = -0.35
_RELAX_P1_HIGH_N_SPAN = 0.07
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_gap2)   # extra hold, gated at N>60 only
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGH_N_SPAN * high_n_gap2)      # further-softened spike, gated at N>60
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                + _RELAX_P1_SCALE_SPAN * scale_pos
                + _RELAX_P1_HIGH_N_SPAN * high_n_gap2)  # further-delayed engagement, gated at N>60
    ...
    # there. The max(...) floor auto-tracks the extended hold_frac too.
