"""Schedule: add two quadratic, threshold-gated relief terms (spike softening
and delayed penalty engagement) for farms well above the linear scale_pos
regime, mirroring the alpha-floor gate that already decoupled dei_n80_omnidir
from dei_n50 on the previous lever.

Diagnosis vs. the current best: dei_n80_omnidir (scale_pos ~= 0.71, gap over
the 0.5 gate ~= 0.21) is the only farm still below native (-0.0025%) while
every other farm (dei_n50 scale_pos ~= 0.37, parque_n10/n20 scale_pos == 0)
is solidly positive. The prior generations already spent the *alpha_denom*
lever's headroom pushing dei_n80_omnidir up from further negative to
essentially flat; the two remaining untouched levers in the same
constraint-pressure family are (1) how hard the terminal feasibility spike
pushes alpha up right at the end, and (2) how early the penalty ramp
(relax_p1) starts biting. Both are still purely *linear* in scale_pos on the
parent, so any relief sized for dei_n80_omnidir necessarily leaks onto
dei_n50 too. Gating a small *additional* quadratic term at
scale_pos > 0.5 -- exactly as done for alpha_denom_floor -- lets us soften
the terminal spike and delay the penalty ramp specifically for
dei_n80_omnidir (high_n_gap ~= 0.21, so the quadratic contributes
~=0.21^2 = 0.044 of its span) while leaving dei_n50 (below the gate, gap=0)
and the parque farms (scale_pos == 0) bit-for-bit identical to the parent.
This gives dei_n80_omnidir a bit more room to hold AEP-improving moves late
in the run instead of being pulled back onto the boundary/spacing manifold
as hard, without touching any farm that is already comfortably positive.
"""
...
# Extra, threshold-gated quadratic delay of penalty engagement for farms
# well past the linear regime (scale_pos > 0.5, i.e. N > 60) -- decouples
# dei_n80_omnidir's relax ramp start from dei_n50's.
_RELAX_P1_HIGH_N_THRESH = 0.5
_RELAX_P1_HIGH_N_SPAN = 0.05
...
# Extra, threshold-gated quadratic softening of the terminal spike for
# farms well past the linear regime -- same gate/idea as the alpha-floor
# quadratic, applied to the spike lever instead.
_SPIKE_HIGH_N_THRESH = 0.5
_SPIKE_HIGH_N_SPAN = 0.3
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Same gate as high_n_gap (thresholds are equal); kept as a separate name
    # so the relax/spike relief terms read independently of the alpha-floor
    # term even though today they share one threshold.
    relax_high_n_gap = max(scale_pos - _RELAX_P1_HIGH_N_THRESH, 0.0)
    spike_high_n_gap = max(scale_pos - _SPIKE_HIGH_N_THRESH, 0.0)
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              - _SPIKE_HIGH_N_SPAN * spike_high_n_gap * spike_high_n_gap)  # extra terminal-spike relief, high-N only
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                + _RELAX_P1_SCALE_SPAN * scale_pos
                + _RELAX_P1_HIGH_N_SPAN * relax_high_n_gap * relax_high_n_gap)  # further-delayed engagement, high-N only
