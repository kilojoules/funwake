"""Schedule: extends the current-best variant's `mid_gate` mechanism (which
already gives near-reference-density farms — extremity ~= 0 — a small extra
hold/relax-delay/spike-soften boost) further, because dei_n50_uniform is now
the worst-scoring farm of all five (+0.0139%), clearly worse than dei_n50
(+0.0277%) and dei_n80_omnidir (+0.0257%), and sits squarely in that band.

Changes, all gated by the SAME `mid_gate` (near extremity=0, damped by
high_n_ramp so the N~=60-100+ band and parque farms are unaffected):
  - widen the gate a bit (P1: 0.30 -> 0.35) so dei_n50 (moderate, nonzero
    extremity) also picks up a partial, smaller share of the relief instead
    of none, since it is also underperforming;
  - strengthen the existing hold/relax-delay/spike-soften spans modestly
    (roughly +40-50%) to give the exploration/constraint-engagement window a
    real widening rather than a token nudge;
  - add a new, matching mid-gated delay to the beta (Adam-moment) ramp
    start, mirroring the existing beta_p0 vs hold_frac coupling: previously
    hold_frac grew (via _MID_HOLD_FRAC_SPAN) without beta_p0 following, so
    on this band the beta ramp could start creeping up before the extended
    exploration hold actually ends. Coupling them keeps unaveraged gradient
    steps available through the whole widened hold phase.

Everything else (alpha floor, high-N gating, lr table, warmup) is unchanged
from the current best so dei_n80_omnidir / parque_n10 / parque_n20, which
are not near-reference-density, see no behavior change.
"""
...
# already active, so dei_n80/parque are unaffected. Widened slightly (P1
# 0.30 -> 0.35) so dei_n50 (moderate nonzero extremity, also underperforming)
# picks up a smaller share of the same relief instead of none.
_MID_GATE_P0, _MID_GATE_P1 = 0.0, 0.35
_MID_HOLD_FRAC_SPAN = 0.03       # extra exploration hold (was 0.02)
_MID_RELAX_P1_SPAN = 0.07        # extra delay of constraint engagement (was 0.05)
_MID_SPIKE_SPAN = -0.32          # softening of the terminal spike (was -0.25)
