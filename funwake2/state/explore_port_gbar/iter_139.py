"""Schedule: parent (+0.1952% farm-balanced) plus a gated "mid-N" relief band.

Diagnosis: dei_n50_uniform (n=50, scale_pos ~=0.37, size ~=0 -- squarely at the
density reference so `extremity` ~=0 too) is now the worst-performing farm
(+0.0139%), well below its twin dei_n50 (+0.0277%, identical n_turbines/D/
min_spacing/shaping, different wind rose). Since schedule_fn has no direct
wind-rose input, the two farms are computed identically -- the only lever
left is making the *shared* schedule for this n~50, mid-density regime more
robust, which should lift both. The existing high_n_ramp gate protects
scale_pos in [0.5, 0.78] (dei_n80_omnidir); nothing analogous protects the
0.25-0.55 band where dei_n50/dei_n50_uniform sit (scale_pos ~=0.37), so they
only get the small linear *_SCALE_SPAN terms shared with every large-N farm.
This variant adds a second, independently-gated ramp (mid_n_ramp, zero
outside [0.25, 0.55], so parque_n10/20 at scale_pos=0 and dei_n80_omnidir's
already-tuned band are essentially untouched) giving a small extra
exploration hold, delayed penalty engagement, and slightly higher terminal
Adam averaging specifically in that band. Every other mechanism (self-
contained inverse-time product lr decay to gamma_min, short linear warmup,
density-extremity relax/spike/hold, absolute-N scale terms, the alpha-floor
relief incl. its high-N gate, the hold_frac-floored beta ramp) is unchanged.
"""
...
_RELAX_P1_MID_N_SPAN = 0.03     # extra gated delay for N~=50 (mid-N) band
...
_HOLD_FRAC_MID_N_SPAN = 0.012   # extra gated hold for N~=50 (mid-N) band
_DENSITY_REF = 12.5             # ~50 turbines at 2D spacing (mid-scale anchor)
_N_REF = 30.0                   # absolute-N reference (independent of density)

# Gated ramp covering the N~=50 band (scale_pos ~= 0.37), disjoint-ish from
# the existing high-N gate at [0.5, 0.78] and zero at scale_pos=0.
_MID_N_P0, _MID_N_P1 = 0.25, 0.55
...
_B1_MID_N_SPAN = 0.03           # slightly more terminal averaging for N~=50 band
_B2_MID_N_SPAN = 0.05
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_ramp = _smoothstep_scalar(scale_pos, _MID_N_P0, _MID_N_P1) * (1.0 - high_n_ramp)
    ...
    b1_hi = _B1_HI + _B1_MID_N_SPAN * mid_n_ramp
    b2_hi = _B2_HI + _B2_MID_N_SPAN * mid_n_ramp
