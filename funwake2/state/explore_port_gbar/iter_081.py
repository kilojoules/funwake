"""Schedule: lengthen and soften the exploration warmup for large-N farms via
a scale_pos-gated bump to warm_frac/warm_lo (isolated single-lever change).

Diagnosis vs. the current best (which already bounds the terminal alpha
denominator for large-N farms): dei_n80_omnidir sits at +0.0005% while the
mid/small farms sit at +0.02% to +0.68%. Earlier attempts pushed hold_frac,
spike0, relax_p1, and beta_p0 further via scale_pos and made the
farm-balanced score worse -- those late-run/mid-run levers are exhausted.
The warmup itself, however, is still farm-agnostic (_WARM_FRAC=0.015,
_WARM_LO=0.30 for every farm), even though a que crowded layout (80
turbines, most turbine pairs of any farm in the set) has far more
spacing-conflict gradients firing from step 0 than a sparse one. A short,
low-floor warmup lets those early constraint gradients combine with a
near-full-strength lr before alpha has ramped past its (already-lowered,
scale_pos-gated) early floor -- i.e. the crowded farm's first few hundred
steps are the least-conditioned part of the whole run. This change adds one
new, previously-untried lever: stretch n_warm and raise the warm-start lr
floor (warm_lo) for large-N farms only, via the existing scale_pos factor,
so turbines settle into a locally-feasible arrangement before the lr reaches
full strength. scale_pos == 0 for small/mid farms (parque_n10/n20,
dei_n50*), so their warmup is byte-for-byte identical to the current best;
only dei_n80_omnidir (and any future large-N farm) sees a longer, gentler
ramp-in.
...
hold/spike/relax_p1/beta/relax0, the scale_pos-gated alpha denominator
...
# NEW: for large-N (many-pair) farms, stretch and soften the warmup so early
# constraint gradients don't fire at near-full lr before alpha engages.
_WARM_FRAC_SCALE_SPAN = 0.02
_WARM_LO_SCALE_SPAN = 0.18
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    warm_lo = _WARM_LO + _WARM_LO_SCALE_SPAN * scale_pos          # gentler start for large-N farms
