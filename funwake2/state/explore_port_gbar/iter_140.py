"""Schedule: same as parent (+0.1952% farm-balanced) but adds an isolated
"mid-N bump" gate targeting n_turbines around ~50 (scale_pos in [0.15, 0.65],
peaking on [0.35, 0.45]) -- the exact regime occupied by dei_n50 and
dei_n50_uniform, which are now the two worst-performing farms (+0.0277% and
+0.0139%, the latter being the farm-balanced bottleneck).

Diagnosis: dei_n50 and dei_n50_uniform share IDENTICAL n_turbines, D, and
min_spacing (only their wind rose differs, which schedule_fn cannot see),
so `size`/`extremity`/`scale_pos` are identical for both -- any schedule
change affects them together. Their scale_pos (~0.37) sits below the
existing `_ALPHA_FLOOR_HIGH_N`/`_HOLD_FRAC_HIGH_N` gate (which only turns on
in [0.5, 0.78] for the N~=60-100+ band, e.g. dei_n80_omnidir), so this n=50
regime got none of the extra exploration relief that previously lifted
dei_n80_omnidir off the floor. This variant adds a smoothstep BUMP function
(up-ramp on [0.15, 0.35], down-ramp on [0.45, 0.65]) that is ~1 exactly in
the n~=50 band and ~0 both for small farms (parque_n10/20, scale_pos=0) and
for the already-tuned high-N band (dei_n80_omnidir, scale_pos~=0.71) --
verified numerically so this cannot regress those farms' shaping. Within
the bump, it grants a modest extra exploration hold, a further-delayed
penalty engagement, and a small extra alpha-floor relief, mirroring the
mechanism that previously helped the N~=60-100+ band. Every other mechanism
...
hold/spike/relax_p1, the high-N alpha-floor relief, the hold_frac-floored
beta ramp) is unchanged.
"""
...
_RELAX_P1_MIDN_SPAN = 0.04      # extra gated delay for N~=50 band
...
_HOLD_FRAC_MIDN_SPAN = 0.025    # extra gated hold for N~=50 band
...
# Isolated bump gate for the N~=50 band: up-ramp on [0.15, 0.35], down-ramp
# on [0.45, 0.65] -- zero at scale_pos=0 (small farms) AND zero once past
# ~0.45 (so it does not overlap the N~=60-100+ high_n_ramp gate above).
_MIDN_P0, _MIDN_P1 = 0.15, 0.35
_MIDN_P2, _MIDN_P3 = 0.45, 0.65
_ALPHA_FLOOR_MIDN_SPAN = 1.5    # small extra alpha-floor relief inside the bump
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    midn_up = _smoothstep_scalar(scale_pos, _MIDN_P0, _MIDN_P1)
    midn_down = _smoothstep_scalar(scale_pos, _MIDN_P2, _MIDN_P3)
    midn_ramp = midn_up * (1.0 - midn_down)             # isolated bump, ~0 outside N~=50 band
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_MIDN_SPAN * midn_ramp)     # extra gated hold for N~=50 band
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_MIDN_SPAN * midn_ramp)      # extra gated delay for N~=50 band
