"""Schedule: floor the Adam-momentum ramp onset at the lr hold length so
beta1/beta2 never start averaging before the lr decay phase begins
(isolated single-lever change on top of the alpha-floor parent).

Diagnosis vs. the parent: beta_p0 (where beta1/beta2 begin ramping from
their exploration-phase values toward their late-phase values) is set by
`_BETA_P0 + _BETA_P0_SCALE_SPAN * scale_pos`, i.e. it *decreases* toward
0.40 for large absolute-N farms (scale_pos -> 1). Meanwhile hold_frac (the
length of the lr exploration plateau, in the SAME progress units) *increases*
toward ~0.44 for large-N/extreme-density farms via
`_HOLD_FRAC_SPAN*extremity + _HOLD_FRAC_SCALE_SPAN*scale_pos`. These two
levers were tuned independently and can cross: for the largest-N farm in the
fitness set (dei_n80_omnidir, the current worst performer at +0.0005%),
beta_p0 can fall below hold_frac, meaning beta1/beta2 start ramping toward
higher momentum/averaging *while lr is still flat in the exploration
plateau* -- i.e. before any decay has begun. That smooths/averages gradients
during pure exploration on exactly the farm that most needs unaveraged,
noisy steps to escape the crowded (80-turbine) packing configuration.

Fix: clamp beta_p0 to never precede the end of the hold phase --
`beta_p0 = max(_BETA_P0 + _BETA_P0_SCALE_SPAN*scale_pos, hold_frac)`. Both
terms are plain Python floats at trace time (derived from n_turbines/D/
min_spacing, not from `step`), so this is a compile-time max, not a traced
branch. For small/mid-N farms (parque_n10/n20, dei_n50*) beta_p0 was already
>= hold_frac (0.40-0.50 vs 0.333), so the clamp is a no-op there and those
farms are byte-for-byte unaffected -- this change only tightens the ordering
invariant for the large-N/extreme-density regime where it was previously
violated or near-violated.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0, and the alpha
denominator floor from the previous generation) is unchanged from the
parent.
"""
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # NEW: never let the beta ramp start before the lr hold phase ends --
    # for large-N/extreme-density farms, _BETA_P0_SCALE_SPAN pulls beta_p0
    # down while _HOLD_FRAC_SPAN/_HOLD_FRAC_SCALE_SPAN push hold_frac up,
    # and the two can cross so momentum starts increasing during pure
