"""Schedule: extends the existing N~=60-100+ gate (`high_n_ramp`, driven by
`_ALPHA_FLOOR_HIGH_N_P0/P1`) to also give a small amount of relief to the
N~=50 band (scale_pos~=0.37), which was previously excluded entirely
(gate started at scale_pos=0.5).

Diagnosis: dei_n50 and dei_n50_uniform share IDENTICAL structural inputs
(D, min_spacing, n_turbines) and therefore get the exact same schedule from
this farm-blind function -- yet dei_n50_uniform is the worst-scoring farm
(+0.0139%) while dei_n80_omnidir, sitting in the gated high-N band, does
much better (+0.0257%) despite being a harder (larger, omnidirectional)
problem. That band already receives extra exploration hold, delayed penalty
engagement, a softened terminal spike, and a raised alpha floor. The n=50
farms (scale_pos~=0.37) sit just below the old gate's onset (p0=0.5) and get
none of that relief, even though they are much closer to the gated band than
to the small parque farms (scale_pos=0).

This nudges the gate's onset earlier (0.5 -> 0.30) so n=50 farms pick up a
small (~4%) fraction of the same relief, while simultaneously nudging the
gate's saturation point later (0.78 -> 0.85) so dei_n80_omnidir's existing
ramp value at its scale_pos (~0.71) is left essentially unchanged (0.836 vs
0.834) -- i.e. this does not re-tune what already works for the n=80 farm,
it only extends the ramp's low end to stop excluding n=50 farms outright.
Every other mechanism, and every other constant, is unchanged from the
best-so-far variant.
"""
...
# (scale_pos in [0.30, 0.85]) -- extended leftward from the original
# [0.5, 0.78] gate so N~=50 farms (scale_pos~=0.37) pick up a small share
# of this relief instead of getting none, while the rightward shift to 0.85
# keeps the N~=80 band's (scale_pos~=0.71) ramp value ~unchanged.
