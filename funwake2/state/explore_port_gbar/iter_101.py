"""Schedule: merge the two independent, previously-validated fixes that are
currently split across separate branches of the search.

The CURRENT PARENT (gen 101) added a safeguard so beta1/beta2 never start
ramping before the lr hold phase ends: beta_p0 = max(BASE + SCALE_SPAN *
scale_pos, hold_frac). This matters because BETA_P0_SCALE_SPAN is negative
(earlier averaging for large-N farms) while HOLD_FRAC_SCALE_SPAN is positive
(longer hold for large-N farms) -- without the max(), those two can cross on
exactly the crowded, many-pair farms (e.g. dei_n80_omnidir) that most need
unaveraged gradient steps throughout the whole exploration plateau, so
momentum would start increasing mid-hold on those farms specifically.

Separately, the best-scoring variant found so far (+0.1896% farm-balanced)
widened the terminal alpha denominator floor -- raising
ALPHA_FLOOR_SCALE_SPAN from 1.5 to 2.0 and adding a new additive extremity
term (0.3 * extremity) -- which gave dei_n80_omnidir (the flattest, most
turbine-pair-dense farm in the fitness set) more terminal-alpha relief
without disturbing the already-good small/mid farms (parque_n10/n20,
dei_n50*). But that variant was built from an ancestor that PREDATES the
beta_p0 safeguard, so it never carries the safeguard fix.

These two changes touch disjoint lines (alpha_denom_floor vs. beta_p0) and
were each independently validated on non-overlapping ancestors. This change
simply takes the current parent (with the beta safeguard) and applies the
already-confirmed alpha floor widening on top of it, so both fixes are
active simultaneously instead of split across branches. No other machinery
(inverse-time product lr decay to gamma_min, short linear warmup,
density/extremity relax/spike/hold, absolute-N scale gating) is changed.
"""
...
# layouts. Widened per the best-so-far branch: larger scale_pos coefficient
# (more relief for large-N farms like dei_n80_omnidir) plus a small
# extremity term (relief for farms whose packing density departs from the
# reference even at moderate N).
