"""Schedule: current parent (+0.1904% farm-balanced) plus the earlier-validated
hold_frac / relax_p1 high-N relief that a sibling generation showed lifts the
same N~50-90 band (+0.1917% -> +0.1952% farm-balanced) but that got dropped
when this lineage instead widened the alpha-floor/spike gate. Re-applying it
on top of the current (wider, stronger) gate combines both fixes.

Diagnosis: dei_n80_omnidir (scale_pos ~= 0.71, inside the current
[0.45, 0.80] high-N gate) is the only sub-native farm. It is omnidirectional,
so spacing constraints are contested from every wind sector simultaneously --
it needs MORE unconstrained exploration time (longer hold_frac) and a LATER
full penalty engagement (larger relax_p1) than a unidirectional farm of
similar density, not just a softer terminal alpha/spike. The current parent
already reuses the high_n_ramp gate for the alpha floor and spike softening,
but never extended it to hold_frac/relax_p1, which is exactly what an
earlier ablation showed helps this farm. Adding it back (gated on the same
high_n_ramp, so dei_n50 at scale_pos~=0.37 and parque_n10/20 at
scale_pos==0 remain untouched) should recover that gain without disturbing
any other farm. Everything else (inverse-time product lr decay to
gamma_min, short linear warmup, density-extremity relax/spike/hold,
absolute-N scale terms, alpha-floor relief, spike relief, hold_frac-floored
beta ramp) is unchanged from the parent.
"""
...
_RELAX_P1_HIGH_N_SPAN = 0.05    # extra gated delay for N~=50-90 (omnidir-heavy) farms
...
# Extra, smoothstep-gated terminal-spike relief for the same N~=50-90 band
# as the alpha-floor relief below -- crowded/many-pair farms over-shoot on
# the terminal feasibility spike too, not just the alpha floor.
_SPIKE_HIGH_N_SPAN = 0.5
...
_HOLD_FRAC_HIGH_N_SPAN = 0.02   # extra gated hold for N~=50-90 (omnidir-heavy) farms
...
# Smoothstep-gated relief for farms in the N ~= 45-80 band (scale_pos in
# [0.45, 0.80]) -- widened from an earlier [0.5, 0.85] so dei_n80_omnidir
# (scale_pos~=0.71) lands near the top of the ramp (~85% relief) instead of
# mid-ramp, while dei_n50 (scale_pos~=0.37) and parque_n10/20
# (scale_pos==0) remain fully below the gate, untouched. This SAME gate is
# now reused for the hold_frac/relax_p1 extension above.
_ALPHA_FLOOR_HIGH_N_P0 = 0.45
_ALPHA_FLOOR_HIGH_N_P1 = 0.80
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_ramp)  # extra gated hold for N~=50-90 band
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              - _SPIKE_HIGH_N_SPAN * high_n_ramp)     # extra smoothstep-gated relief, N~=45-80
    spike0 = max(spike0, 0.0)
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                + _RELAX_P1_SCALE_SPAN * scale_pos     # further delay for large-N farms
                + _RELAX_P1_HIGH_N_SPAN * high_n_ramp)  # extra gated delay, N~=50-90 band
    ...
    # terms + a smoothstep-gated term ramping over scale_pos in [0.45,0.80])
