"""Schedule: extend the terminal alpha-denominator floor to also react to
packing-density extremity, not just absolute-N (isolated single-lever add
on top of the current best).

Diagnosis vs. current best: the current best's `_ALPHA_FLOOR_SCALE_SPAN`
lever caps alpha's terminal 1/lr_base blow-up using `scale_pos`
(log-compressed ABSOLUTE turbine count only). dei_n80_omnidir is the
worst-performing farm in the fitness set (+0.0005%, essentially flat) and
is unusual on *both* axes at once: large absolute N (80) *and* elevated
packing density relative to the 50-turbine/2D reference (`_DENSITY_REF`).
Because the floor lever only reads `scale_pos`, a farm that is large-N but
only moderately dense gets the full floor-relief already tuned for the
best score, while a farm that is *additionally* dense gets no extra
floor-relief even though its per-step constraint-gradient term is noisier
(more close turbine pairs) and the terminal alpha divergence should be
capped more aggressively there too. This adds one new, previously-untried
term, `_ALPHA_FLOOR_EXTREMITY_SPAN * extremity`, to the same floor
expression the current best already validated -- it only engages when a
farm is *also* far from the density reference (extremity > 0, i.e.
dei_n80_omnidir and the small parque farms which sit at the opposite
extreme), while leaving farms near the density reference (extremity ~ 0,
where the current best's floor sizing already works) unchanged. Small
magnitude (0.5, a third of the validated 1.5 scale_pos span) keeps this a
conservative nudge rather than a re-run of the previously-failed
hold_frac/spike0/relax_p1/beta_p0 pushes.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0, the beta_p0 vs
hold_frac ordering fix) is unchanged from the parent/current-best.
"""
...
# NEW: additionally cap it for farms far from the density reference (either
# direction) -- catches large-N farms that are *also* unusually dense,
# where the constraint-gradient term is noisiest.
_ALPHA_FLOOR_EXTREMITY_SPAN = 0.5
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Cap the effective denominator at a floor that grows with BOTH absolute
    # N (scale_pos) and density extremity, so alpha's terminal divergence is
    # bounded more aggressively for farms that are large-N *and* unusually
    # dense (e.g. dei_n80_omnidir), while small/mid-N, mid-density farms
    # (scale_pos == 0, extremity ~= 0) keep the floor at gamma_min exactly.
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  + _ALPHA_FLOOR_EXTREMITY_SPAN * extremity)
