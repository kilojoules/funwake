"""Schedule: lengthen warmup and raise terminal beta2 for large-N farms,
scaled by scale_pos (isolated two-lever change, both previously untried).

Diagnosis vs. the parent: dei_n50 (+0.033%), dei_n50_uniform (+0.014%), and
especially dei_n80_omnidir (+0.0005%, essentially flat) are the three
weakest farms in the fitness set, and all three have moderate-to-large
absolute turbine counts (50, 50, 80) where scale_pos = max(_n_scale, 0) is
strictly positive (~0.37 for the n50 farms, ~0.71 for n80). More turbines
means more simultaneous pairwise spacing-constraint gradient terms summed
into grad_constraint each step, which is a noisier, higher-variance signal
than on the small farms (parque_n10/n20, where scale_pos == 0 and the
parent already scores well). Two mechanisms in the parent are well-suited
to damping exactly this kind of early/ongoing gradient noise but were never
made to depend on scale_pos:

  1. Warmup length (_WARM_FRAC): a longer linear warmup gives the Adam
     moment estimates more early steps to average out per-step noise from
     many simultaneous pairwise constraint terms before lr reaches full
     exploration scale, instead of committing to a possibly noisy early
     direction as quickly as on a 10-20 turbine farm.
  2. Terminal beta2 (_B2_HI): a higher late-phase second-moment decay
     smooths the adaptive per-coordinate step-size denominator more
     aggressively against the larger, noisier gradient magnitudes produced
     by many-pair constraint terms on large-N farms, reducing overshoot
     that plain low beta2 would otherwise amplify.

Both levers scale by scale_pos (0 for small/mid farms, so parque_n10/n20
and any farm with n_turbines <= _N_REF are completely unaffected -- exactly
matching the parent) and are additive, small-magnitude, and independent of
the four levers (hold_frac, spike0, relax_p1, beta_p0) that earlier attempts
already pushed further via scale_pos and found to hurt the farm-balanced
score. All other machinery (self-contained inverse-time product lr decay to
gamma_min, density-extremity relax/spike/hold, alpha denominator floor) is
...
_WARM_FRAC_SCALE_SPAN = 0.01    # NEW: longer warmup for large-N farms (noisier early gradients)
...
_B2_HI_SCALE_SPAN = 0.10        # NEW: extra terminal beta2 for large-N farms (smooth noisier grads)
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    warm_frac = _WARM_FRAC + _WARM_FRAC_SCALE_SPAN * scale_pos   # longer warmup for large-N farms
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # more terminal smoothing, large N
