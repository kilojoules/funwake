"""Schedule: restore the validated linear (not squared) alpha-denominator
floor scaling for large-N farms, and add one new, previously-untried lever:
delay the terminal-spike ONSET (not just its magnitude) for large-N farms.

Diagnosis vs. the parent: the parent's own farm-balanced score (+0.1892%)
is actually a small regression from the best-known configuration
(+0.1894%), which used a *linear* `scale_pos` gate (span 1.5) on the alpha
denominator floor. The parent's switch to a *squared* `scale_pos**2` gate
with a larger span (4.0) raises the floor more aggressively at high
scale_pos, further shrinking alpha late in the run -- and this made
dei_n80_omnidir (the farm with the largest scale_pos, ~0.71) *worse*
(-0.001% vs -0.0002% under the linear/1.5 version), not better, despite
leaving mid-N farms (dei_n50*, scale_pos ~0.37) roughly unaffected. So this
edit reverts that lever back to the validated linear/1.5 form.

On top of that restored baseline, this adds one genuinely new, untried
lever for dei_n80_omnidir specifically: previous attempts already pushed
hold_frac, spike0 *magnitude*, relax_p1, and beta_p0 via scale_pos (and
made things worse), but none has touched the terminal-spike's *onset*
(_SPIKE_P0). Delaying when the terminal feasibility spike engages -- not
just softening its peak height -- gives large-N farms (more turbine pairs,
noisier constraint gradients) more of the exploration phase before the
late-stage penalty ramp begins, without touching small/mid-N farms
(scale_pos == 0 for parque_n10/n20, so spike_p0 stays exactly 0.90 there,
matching the parent bit-for-bit).
...
absolute-N scale on hold/spike/relax_p1/beta/relax0) is unchanged.
"""
...
_SPIKE_P0_SCALE_SPAN = 0.04     # NEW: delay spike onset further for large-N farms
...
# the validated linear scale_pos gate (not scale_pos**2), which scored
# better farm-balanced than the steeper squared gate.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    spike_p0 = _SPIKE_P0 + _SPIKE_P0_SCALE_SPAN * scale_pos  # NEW: delay spike onset for large N
    ...
    # squared gate): bounds alpha's terminal divergence on large-N (many-
    # pair) farms only, leaving small/mid-N farms (scale_pos == 0) exactly
    # matching native's lr_base floor of gamma_min.
