"""Schedule: phase-couple the Adam moment ramp to the alpha engagement point
(isolated single-lever change vs. the current best).

Diagnosis vs. the best-so-far: alpha's penalty ramp fully engages at
relax_p1 (progress ~0.45-0.60 depending on density extremity and absolute-N
scale_pos), while beta1/beta2's own ramp begins independently at a
near-constant beta_p0 (~0.50, shifted only by scale_pos) that was tuned in
isolation from relax_p1. The prior-art "phase-transition the Adam moments
with the alpha phase" hypothesis (beta2 up / beta1 down as the run enters
its feasibility-enforcing phase) was therefore only approximately realized
-- on farms where extremity or scale_pos pushes relax_p1 well past 0.50
(e.g. dei_n80_omnidir, which is both far from the density reference and has
large absolute N), the moments were already ramping to their averaged,
constraint-absorbing values *before* alpha actually tightened, over-damping
exploration during a still-loose-penalty window; on farms where relax_p1
sits below 0.50 the reverse happened, leaving alpha's ramp under-supported
by momentum. This change replaces the independently-tuned beta_p0 with
beta_p0 = relax_p1 - 0.05 (still nudged earlier by scale_pos for large-N
farms), so the moment phase-transition tracks the penalty phase-transition
directly, per farm, via the same extremity/scale_pos-adaptive relax_p1
already computed for alpha -- rather than reintroducing a second,
uncoupled tuning constant. beta_p1 stays fixed at 0.90 (moments finish
ramping by 90% progress regardless of farm).
...
absolute-N scale on hold/spike/relax_p1, and the alpha-denominator floor
for large-N farms) is unchanged from the parent.
"""
...
# Adam moments: native early -> mildly averaged late, phase-transition
# coupled to the alpha engagement point (relax_p1) instead of an
# independently-tuned constant.
_B1_LO, _B1_HI = 0.1, 0.25
_B2_LO, _B2_HI = 0.2, 0.45
_BETA_P0_OFFSET = 0.05          # moments begin ramping this far before relax_p1
_BETA_P1 = 0.90
_BETA_P0_SCALE_SPAN = -0.10     # earlier averaging onset for large-N farms
_BETA_P0_MIN, _BETA_P0_MAX_MARGIN = 0.05, 0.05
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # phase-transition the moments together WITH the penalty ramp: begin
    # averaging shortly before alpha reaches full engagement (relax_p1),
    # per farm, instead of an independently-tuned constant onset.
    beta_p0 = min(max(relax_p1 - _BETA_P0_OFFSET + _BETA_P0_SCALE_SPAN * scale_pos,
                       _BETA_P0_MIN),
                  _BETA_P1 - _BETA_P0_MAX_MARGIN)
