"""Schedule: further relax the large-N alpha-denominator floor (single-lever
tune of the previously-added, isolated `_ALPHA_FLOOR_SCALE_SPAN` gate).

Diagnosis vs. the parent: the parent's `alpha_denom_floor = gamma_min * (1 +
_ALPHA_FLOOR_SCALE_SPAN * scale_pos)` lever was the first change that moved
dei_n80_omnidir off its near-zero/negative score (native's terminal
alpha = alpha0*diam*relax*spike / lr_base blows up as lr_base -> gamma_min,
and dei_n80_omnidir -- 80 turbines, far more constraint-gradient pairs than
any other farm in the set -- is the one farm where that re-engagement fully
erases the parent's exploration gains). But the fix only lifted that farm to
+0.0005%, essentially flat, while the four *previously existing* levers
(hold_frac, spike0, relax_p1, beta_p0) had already been pushed further via
scale_pos in earlier iterations and each made the farm-balanced score worse
(-0.021%, -0.012%) -- i.e. this family of large-N farms is sensitive to
over-adjustment on those axes. This edit leaves all four of those levers
untouched and instead nudges the one lever that *did* help
(`_ALPHA_FLOOR_SCALE_SPAN`: 1.5 -> 2.2), giving the terminal alpha a looser
cap specifically for scale_pos>0 farms (n_turbines > 30: dei_n50,
dei_n50_uniform, dei_n80_omnidir), while leaving scale_pos==0 farms
(parque_n10/n20, both already scoring well at +0.22%/+0.68%) completely
unaffected since their floor stays pinned at gamma_min, byte-for-byte
matching the parent.
...
# Tuned up from 1.5 -> 2.2: the prior value only barely lifted the worst
# farm (dei_n80_omnidir) off flat; this is the one lever (of the five
# large-N gates) that has not yet regressed when pushed further.
