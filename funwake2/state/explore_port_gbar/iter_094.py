"""Schedule: revert the alpha terminal-floor scale span from 2.4 back to 1.5
(isolated single-lever change vs. the immediate parent).

Diagnosis vs. the parent: the parent raised _ALPHA_FLOOR_SCALE_SPAN from
1.5 -> 2.4, reasoning that dei_n80_omnidir's flat result (scale_pos ~= 0.71)
meant the terminal alpha ceiling was still too high there. But the measured
outcome of that change was a farm-balanced regression (+0.1893% vs the
prior +0.1894%) -- raising the floor further did not help dei_n80_omnidir
enough to outweigh the small cost it imposed elsewhere among the large-N-
scaled farms. Since 1.5 is the only other value of this lever that has
actually been scored, and it scored strictly better on the farm-balanced
mean while keeping all farms feasible, the evidence-backed move is to
revert this one lever rather than push it further in a direction already
shown to underperform. All other machinery (self-contained inverse-time
...
# Reverted 2.4 -> 1.5: pushing this further regressed the farm-balanced
# score (+0.1893% vs +0.1894%); 1.5 is the best-measured value of this
# lever so far.
