"""Schedule: sharpen the large-N alpha-floor lever with a quadratic (rather than
linear) scale_pos exponent, recalibrated so mid-N farms are left almost
untouched while extreme-N farms get a materially higher floor.

Diagnosis vs. the current best: the previous change bounded the terminal
alpha denominator via `gamma_min * (1 + 1.5*scale_pos)`, which is LINEAR in
scale_pos. But scale_pos is itself already log-compressed, so a farm like
dei_n80_omnidir (n=80, scale_pos~0.71) gets barely more floor-boost than a
farm like dei_n50 (n=50, scale_pos~0.37) -- roughly 2x scale_pos but the same
linear coefficient applies to both, so dei_n80_omnidir's terminal alpha still
mostly re-diverges (leaving it flat at +0.0005%) while dei_n50 is already
comfortably positive and doesn't need more relief.

Squaring scale_pos before applying the coefficient (with the coefficient
rescaled so the *linear* farms, i.e. dei_n50/dei_n50_uniform at scale_pos
~0.37, see almost the same floor as before) concentrates the extra
denominator headroom on the highest-scale_pos farm (dei_n80_omnidir) while
leaving mid-N farms and the completely-unaffected small farms
(parque_n10/n20, scale_pos == 0) essentially where the parent already left
them. This is a single, isolated recalibration of the alpha-floor lever
only -- hold_frac, spike0, relax_p1, and beta_p0 are untouched, matching the
guidance that pushing those directly regressed the farm-balanced score.
...
# alpha denominator floor: now quadratic in scale_pos (recalibrated so the
# ~0.37 scale_pos of dei_n50/dei_n50_uniform sees roughly the same floor as
# the prior linear coefficient of 1.5, while the ~0.71 scale_pos of
# dei_n80_omnidir sees a materially larger floor).
_ALPHA_FLOOR_SCALE_SPAN = 4.0
_ALPHA_FLOOR_POWER = 2.0
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Cap the effective denominator at a scale_pos-gated floor instead of
    # letting it ride lr_base all the way to gamma_min, so alpha's terminal
    # divergence is bounded on large-N (many-pair) farms only. The floor now
    # scales with scale_pos**2 (recalibrated coefficient) so the boost is
    # concentrated on the highest-N farms rather than spread linearly.
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * (scale_pos ** _ALPHA_FLOOR_POWER))
