"""Schedule: builds on the +0.2007% farm-balanced parent, which already gates
extra hold/relax-delay/spike-softening/alpha-floor relief onto the N~=60-100+
band via `high_n_ramp` and a separate N~=50 mid-band bump via `mid_n_bump`.

Diagnosis: dei_n50 and dei_n50_uniform share IDENTICAL n_turbines/D/min_spacing
(same scale_pos, size, extremity), so every Python-float-derived shaping term
(hold_frac, relax0, spike0, relax_p1, alpha_denom_floor) is bit-for-bit
identical between them -- the schedule literally cannot tell them apart via
those signals, yet dei_n50_uniform is the worst cell (+0.0151%) while dei_n50
scores +0.0501%. The one per-farm signal that DOES differ between them is
`alpha0` itself (mean|grad J|/D, a function of the wind rose/turbine coupling,
not just farm geometry) -- a uniform wind rose spreads gradient mass more
evenly across turbines than a directional one, typically yielding a smaller
alpha0. `alpha0` is a traced value, so it cannot feed hold_frac/table sizing
(no int()/float() allowed), but it CAN modulate the already-traced relax/spike
arrays multiplicatively. This adds a small, bounded, log-tanh "gradient-scale"
signal from alpha0 (relative to gamma_min/D, the natural terminal-alpha
reference) that nudges relax/spike -- giving low-alpha0 farms (like the
uniform-rose case) a bit more early feasibility slack and a softer terminal
spike, without touching lr, hold length, or the beta schedule, and without
altering the N-based mechanisms that already tuned parque/n80/mid-n50.
"""
...
# onset at 0.5 -- gives the n=50 band (currently un-served by the high-N
# gate) a smaller, independent slice of the same relief mechanisms.
_MID_N_P0, _MID_N_P1 = 0.15, 0.55
_MID_N_HOLD_SPAN = 0.015
_MID_N_RELAX_P1_SPAN = 0.03
_MID_N_SPIKE_SPAN = -0.10
_MID_N_ALPHA_FLOOR_SPAN = 1.5

# alpha0-derived (traced) gradient-scale signal: distinguishes farms that
# share identical N/D/spacing but differ in wind-rose-driven gradient
# magnitude (e.g. dei_n50 vs dei_n50_uniform). Small, bounded spans so a
# mistuned sign cannot destabilize feasibility -- only nudges relax/spike.
_ALPHA0_LOG_SCALE = 8.0
_ALPHA0_RELAX_SPAN = 0.06       # low alpha0 (relative to gamma_min/D) -> more early slack
_ALPHA0_SPIKE_SPAN = -0.06      # low alpha0 -> softer terminal spike
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _MID_N_HOLD_SPAN * mid_n_bump)         # small extra hold for N~=50 band
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _MID_N_SPIKE_SPAN * mid_n_bump)        # small extra softening for N~=50 band
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _MID_N_RELAX_P1_SPAN * mid_n_bump)    # small extra delay for N~=50 band
    ...
    # Traced, alpha0-derived gradient-scale signal: alpha0*D/gamma_min is the
    # natural terminal-alpha reference (what alpha_denom's floor compares
    # against). Farms sharing identical N/D/spacing but a lower alpha0 (e.g.
    # a more uniform wind rose spreading gradient mass more evenly) get a
    # smooth, bounded nudge toward more early slack and a softer spike; a
    # higher alpha0 nudges the other way. tanh keeps it bounded even if
    # alpha0 is unexpectedly large/small on an unseen farm.
    alpha0_t = jnp.asarray(alpha0)
    g0_ratio = jnp.log(jnp.maximum(alpha0_t * diam / g_min, 1e-6))
    alpha0_sig = jnp.tanh(g0_ratio / _ALPHA0_LOG_SCALE)
    relax = relax * (1.0 - _ALPHA0_RELAX_SPAN * alpha0_sig)
    spike = spike * (1.0 - _ALPHA0_SPIKE_SPAN * alpha0_sig)
