"""Schedule: quadratic-in-scale_pos alpha-denominator floor (isolated
single-lever refinement of the previous linear floor lever).

Diagnosis vs. the "best so far" parent: that parent already bounds the
terminal alpha divergence via `alpha_denom_floor = gamma_min * (1 +
_ALPHA_FLOOR_SCALE_SPAN * scale_pos)`, which helped the farm-balanced mean
overall but left dei_n80_omnidir essentially flat (-0.0002%) -- it still
re-engages enough of native's constraint-dominated tail on the highest-N,
most turbine-pair-dense farm in the set. Meanwhile dei_n50 (scale_pos~0.37,
well below n80's ~0.71) is already comfortably positive and doesn't need as
strong a floor -- earlier attempts that pushed *other* large-N levers
(hold_frac, spike0, relax_p1, beta_p0) further via scale_pos regressed the
mean, suggesting those levers are already well-tuned and any additional
push should be concentrated specifically on the farms with the most
turbine pairs rather than spread linearly across all "large-N" farms.

This change replaces the *linear* scale_pos term in the floor lever with a
*quadratic* one (scale_pos**2) and retunes the coefficient upward. Squaring
concentrates the correction on genuinely large-N farms (n80: 0.71**2=0.50)
while proportionally easing it on moderate-N farms (n50: 0.37**2=0.13) --
so dei_n80_omnidir gets a stronger terminal-alpha cap than before while
dei_n50 gets a gentler one, better matching the "far more turbine pairs"
diagnosis (pair count grows faster than N) without disturbing the small/
mid farms (parque_n10/n20, whose scale_pos is already 0).
...
# NEW: quadratic in scale_pos, concentrating the correction on the
# highest-N farms (dei_n80_omnidir) rather than spreading it linearly.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Cap the effective denominator so alpha's terminal divergence is
    # bounded on large-N (many-pair) farms only, with the correction
    # concentrated on the highest-N farms via a quadratic scale_pos term.
