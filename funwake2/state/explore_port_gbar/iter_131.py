"""Schedule: extend the smoothstep-gated high-N relief (currently only
applied to the alpha-floor) to the relax_p1 delay, spike softening, and
hold_frac -- the three other places the parent already scales linearly with
`scale_pos` but not with the same 60-120-turbine-band gate that fixed the
alpha floor.

Diagnosis vs. parent (+0.191% farm-balanced, dei_n80_omnidir still the
weakest farm at +0.0015%, effectively flat): dei_n80_omnidir sits at
scale_pos ~= 0.71 (n_turbines=80, log-compressed against _N_REF=30), i.e.
mid-way through the `_ALPHA_FLOOR_HIGH_N_P0..P1` = [0.5, 0.85] gate that the
alpha-floor relief already uses. That gate was previously found to matter a
lot (it's what took the score from ~+0.18% to +0.191% by fixing a C0 kink
that left dei_n80 under-relieved). But relax_p1 (when full penalty engages),
spike0 (terminal feasibility-spike height) and hold_frac (exploration
plateau length) still only scale *linearly* in scale_pos, so dei_n80 gets a
much smaller share of "large-N" easing on those three knobs than a farm at
scale_pos=1 (N~=120) would, even though it's the one farm we can see is
still stuck near zero.

Fix: reuse the exact same `high_n_ramp = smoothstep(scale_pos, 0.5, 0.85)`
gate (already computed for the alpha floor) to add extra, bounded relief to
relax_p1 (delay full penalty a bit further), spike0 (soften the terminal
spike a bit more) and hold_frac (hold the exploration plateau a bit longer)
specifically in the N~=60-120 band. Because high_n_ramp==0 for scale_pos<=0.5
(dei_n50 at ~0.37, parque_n10/20 at 0), those farms are completely
unaffected -- this only touches dei_n80_omnidir (and any future farm in the
same N band). All other machinery (self-contained inverse-time product lr
decay to gamma_min, short linear warmup, density-extremity relax/spike/hold,
the alpha-floor high-N relief, the hold_frac-floored beta ramp) is unchanged
from the parent.
"""
...
# Smoothstep-gated relief for farms in the N ~= 60-120 band (scale_pos in
# [0.5, 0.85]) -- decouples dei_n80_omnidir from dei_n50 without touching
# any farm below the gate. Reused below for relax_p1 / spike0 / hold_frac.
_HIGH_N_P0 = 0.5
_HIGH_N_P1 = 0.85
_ALPHA_FLOOR_HIGH_N_SPAN = 3.0
_RELAX_P1_HIGH_N_SPAN = 0.08     # delay full penalty engagement further for N~=60-120
_SPIKE_HIGH_N_SPAN = -0.30       # soften terminal spike further for N~=60-120
_HOLD_FRAC_HIGH_N_SPAN = 0.03    # extend exploration plateau further for N~=60-120
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_ramp)  # extra hold for N~=60-120 specifically
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGH_N_SPAN * high_n_ramp)      # extra-softened terminal spike for N~=60-120
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                + _RELAX_P1_SCALE_SPAN * scale_pos      # delay full penalty further for large N
                + _RELAX_P1_HIGH_N_SPAN * high_n_ramp)  # extra delay for N~=60-120 specifically
