"""Schedule: hard-bound the terminal alpha PLATEAU for large-N farms via an
absolute cap alpha <= k * alpha0 * D, applied additively on top of the
existing lr_base-floor lever (isolated single-lever change).

Diagnosis vs. the current best: the winning prior lever
(`alpha_denom_floor = gamma_min * (1 + k*scale_pos)`) only rescales the
*denominator* of alpha's native 1/lr_base coupling. Because gamma_min is a
metre-scale tolerance (near-zero), even a 2x-larger floor still leaves
alpha0*diam/alpha_denom enormous in absolute terms once lr_base bottoms
out near gamma_min -- the floor barely dents the terminal blow-up, which is
consistent with dei_n80_omnidir (the only fitness farm with meaningfully
positive scale_pos) remaining essentially flat (+0.0005%) instead of
gaining like the other farms. The prior-art survey's highest-value untried
bet is a genuinely BOUNDED alpha plateau (exact-penalty / ALM literature),
not merely a rescaled denominator -- so this change adds a second,
independent lever: an absolute cap `alpha <= _ALPHA_CAP_MULT * alpha0 *
diam`, gated by `scale_pos` (a static, non-traced quantity, branched on in
plain Python exactly like the existing hold_frac/spike0/relax0 gating) so
it only engages for large-N farms (scale_pos > 0, i.e. only dei_n80_omnidir
in the fitness set) and leaves every other farm's alpha trajectory
byte-for-byte identical to the current best (scale_pos == 0 there, so the
cap is skipped entirely). The existing lr_base-floor lever is left
untouched so this is a clean, additive ablation on top of it.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0, the alpha_denom
...
# NEW: absolute plateau cap on alpha itself for large-N farms -- the
# lr_base-floor lever above only rescales the denominator, which barely
# dents the terminal blow-up since gamma_min is near-zero in absolute
# terms; this bounds alpha directly to a small multiple of the native
# gradient scale (alpha0 * diam), independent of gamma_min.
_ALPHA_CAP_MULT = 6.0
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # NEW: absolute plateau cap, independent of gamma_min, for large-N
    # farms only -- the denom floor above still lets alpha diverge in
    # absolute terms since gamma_min is near-zero, so bound it directly to
    # a small multiple of the native gradient scale (alpha0 * diam).
    if scale_pos > 0.0:
        alpha_cap = alpha0 * diam * _ALPHA_CAP_MULT
