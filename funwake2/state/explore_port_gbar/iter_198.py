"""Schedule: builds on the +0.2007% farm-balanced parent, which gates extra
hold/relax-delay/spike-softening/alpha-floor relief onto both the N~=60-100+
band (`high_n_ramp`) and the narrow N~=50 band (`mid_n_bump`).

Diagnosis: dei_n50_uniform (+0.0151%) is still the clear worst farm, well
below dei_n50 (+0.0501%) despite IDENTICAL n_turbines/D/min_spacing -- the
schedule cannot see wind-rose shape, so it cannot target dei_n50_uniform
without also moving dei_n50 (both sit at the same scale_pos~=0.368, near the
mid-N bump's peak). Since any mid-N-band change shifts both farms together,
and the mid-N relief mechanisms (extra hold, delayed relax engagement,
softened spike, raised alpha floor) were already validated as net-positive
for that band, this modestly strengthens each of them (roughly +25-30%
larger magnitude) to push the pair further along the same validated
direction, without touching the high-N gate, parque, or beta terms at all.
"""
...
# Magnitudes bumped up modestly from the prior generation (0.015 -> 0.020,
# 0.03 -> 0.045, -0.10 -> -0.14, 1.5 -> 1.9) to push further along the same
# validated direction for the still-worst dei_n50 / dei_n50_uniform pair.
_MID_N_P0, _MID_N_P1 = 0.15, 0.55
_MID_N_HOLD_SPAN = 0.020
_MID_N_RELAX_P1_SPAN = 0.045
_MID_N_SPIKE_SPAN = -0.14
