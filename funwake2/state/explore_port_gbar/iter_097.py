"""Schedule: revert the terminal-alpha denominator floor to the confirmed-better
LINEAR scale_pos gating (span 1.5, not the squared/span-4.0 variant currently in
the parent), and add one new, previously-untried, isolated lever: decouple the
beta1 ceiling from beta2's for large-N farms.

Diagnosis vs. the parent: the search history shows the LINEAR scale_pos floor
(span 1.5) already scored farm-balanced +0.1894% -- strictly better than this
parent's SQUARED scale_pos**2 floor (span 4.0, +0.1892%), and the squared
version's only behavioral difference is on farms with scale_pos>0 (dei_n50,
dei_n80_omnidir), where it leaves dei_n80_omnidir at -0.001%, essentially
unchanged from the -0.0002% seen with NO floor boost at all. That flatness
across "no cap" / "linear cap" / "squared cap" indicates the alpha-denominator
lever is not what's limiting dei_n80_omnidir -- so this edit (a) reverts to
the empirically-better linear gating (recovering the known +0.1894% baseline)
and (b) does not push that same lever further.

Instead it adds a single new, different lever aimed at large-N farms: it
decouples the beta1 ceiling from beta2's during the late-run averaging ramp.
Currently beta1 and beta2 both ramp UP together (_B1_LO->_B1_HI,
_B2_LO->_B2_HI) using the same ramp r. For farms with many turbine pairs
(large N), heavier beta1 momentum late in the run smooths over many
competing, noisy constraint-gradient terms, which can blur exactly the
per-pair corrections a crowded layout needs. Design-menu bet #4 (phase-
transition the Adam moments with the alpha phase: beta2 up / beta1 down in
the feasibility phase) is untried in the search log -- earlier attempts only
ever moved beta_p0 (the ramp's *onset*), never decoupled the two ceilings.
This adds `_B1_HI_SCALE_SPAN` so beta1's ceiling is pulled down for large-N
farms only (scale_pos>0), while beta2's ceiling is untouched -- keeping
variance-stabilizing beta2 growth but giving large-N farms less momentum
smoothing (more per-step responsiveness to constraint gradients) exactly when
the feasibility ramp is most active. Small/mid-N farms (scale_pos==0, e.g.
parque_n10/n20) are exactly unaffected, matching the +0.1894%-scoring
reference precisely.
...
absolute-N scale on hold/spike/relax_p1/beta onset, linear-scale_pos alpha
floor) is unchanged from the +0.1894%-scoring reference.
"""
...
# alpha's 1/lr_base divergence is capped instead of fully re-engaging native's
# constraint-dominated tail on crowded (many-pair) layouts. Reverted to the
# empirically-better LINEAR scale_pos gating (not squared).
_ALPHA_FLOOR_SCALE_SPAN = 1.5

# Adam moments: native early -> mildly averaged late.
# NEW: beta1's ceiling is pulled down for large-N farms (less late-run
# momentum smoothing over many noisy constraint-gradient terms), while
# beta2's ceiling is untouched (variance stabilization unchanged).
_B1_LO, _B1_HI = 0.1, 0.25
_B1_HI_SCALE_SPAN = -0.10
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # divergence is bounded on large-N (many-pair) farms only -- linear
    # gating on scale_pos (not squared).
    ...
    b1_hi = _B1_HI + _B1_HI_SCALE_SPAN * scale_pos          # lower late-run momentum ceiling, large N
