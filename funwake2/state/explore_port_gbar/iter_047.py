"""Schedule: add a scale_pos-gated terminal ceiling on beta2 (isolated
single-lever change on top of the +0.1894% farm-balanced parent).

Diagnosis vs. the parent: the alpha_denom floor already bounds the terminal
constraint penalty for large-N farms (dei_n80_omnidir: 80 turbines -> far
more turbine-pair gradient terms than any other farm in the set), but that
farm still scores ~flat (+0.0005%) even though it stays feasible -- i.e. the
optimizer isn't failing on feasibility, it just isn't finding extra AEP once
alpha is relaxed there. A likely reason: with many more pairwise spacing
terms summed into grad_constraint, the *residual* per-step gradient noise
seen by Adam's second-moment estimate is larger for n80 than for n10/n20/n50
at the same beta2, so late-run updates on the crowded farm are noisier and
less effective at exploiting the now-loosened alpha. This is the "phase-
transition the Adam moments with the alpha phase" lever from the prior-art
survey (AMSGrad-monotone / ramped beta2), which the parent applies uniformly
via _B2_HI but never scales by farm size the way it already scales hold_frac,
spike0, relax_p1, beta_p0, and the alpha floor.

The change: gate the *terminal* beta2 ceiling (_B2_HI) upward by scale_pos,
the same absolute-N factor already used for four other levers, so only
large-N farms average squared-gradients more aggressively late in the run
(smoother, less noisy adaptive step) while small/mid-N farms (scale_pos == 0,
e.g. parque_n10/n20, and dei_n50 which sits well below the reference) keep
exactly the parent's beta2 schedule. Everything else -- lr decay table,
warmup, relax/spike shaping, the alpha_denom floor, beta1, beta_p0 -- is
...
# NEW: raise the terminal beta2 ceiling for large-N (many-pair) farms only,
# so second-moment averaging is heavier where per-step constraint-gradient
# noise is largest -- smoother late-run adaptive steps once alpha has been
# relaxed via the floor above.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # heavier terminal averaging, large N
