"""Schedule: extend the large-N exploration-freedom logic (already applied to
the terminal alpha floor) to the EARLY penalty floor `relax0` as well
(isolated single-lever change from the current best).

Diagnosis vs. the current best: that variant added `_ALPHA_FLOOR_SCALE_SPAN`
to cap alpha's *terminal* denominator for large-N farms, which raised the
farm-balanced score from a flat/negative dei_n80_omnidir contribution to
+0.0005%. But dei_n80_omnidir (80 turbines -- the largest N in the fitness
set, and thus the most turbine-pairs/constraint-gradient terms) is still
essentially flat versus native, meaning the schedule buys it almost nothing
even though it stays feasible throughout. The terminal-alpha fix addressed
the *end* of the run; the same large-N crowding pressure is present from
step 0, but `relax0` (the early-run penalty floor, set by `_RELAX_BASE +
_RELAX_SPAN*size + _RELAX_SCALE_SPAN*scale_pos`) only gets a small nudge
(-0.05 * scale_pos) toward more exploration freedom for large-N farms.
Earlier attempts that pushed hold_frac, spike0, relax_p1, and beta_p0
further via scale_pos made the farm-balanced score worse -- but
`_RELAX_SCALE_SPAN` itself (unlike those) was never independently pushed;
only the floor-style alpha-denominator addition was. This change nudges
just that one already-present-but-timid lever a bit further (-0.05 ->
-0.09), giving large-N farms more low-penalty room to spread turbines out
early, symmetric with the terminal floor fix, while leaving small/mid-N
farms (scale_pos == 0, e.g. parque_n10/n20, dei_n50) completely unaffected
since their relax0 is untouched.
...
absolute-N scale on hold/spike/relax_p1/beta/alpha-floor) is unchanged.
"""
...
_RELAX_SCALE_SPAN = -0.09       # lower early-exploration penalty floor for large-N farms (was -0.05)
_RELAX_P0 = 0.08
