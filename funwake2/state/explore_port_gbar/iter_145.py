"""Schedule: same as parent (+0.1952% farm-balanced) with a small,
size-independent widening of the pure-exploration phase.

Diagnosis vs. parent: dei_n50_uniform (n=50, min_spacing/D~=2 -> density
exactly at _DENSITY_REF) sits at size~=0, extremity~=0, and scale_pos is
small (n_turbines close to _N_REF), so it receives essentially NONE of the
existing density/scale conditioning -- it runs the raw base constants
(_RELAX_BASE, _RELAX_P1_BASE, _HOLD_FRAC_BASE, ...) unmodified. It is also
the worst-performing farm in the current fleet (+0.0139%, well behind the
next-worst dei_n80_omnidir which already gets extra high-N relief). Since
no existing lever conditions on this "neutral" configuration, the fix is to
mildly relax the *base* values themselves rather than another
farm-specific gate: (1) lower the early-exploration penalty floor
(_RELAX_BASE 0.45->0.40) so the optimizer explores more freely before
constraints start to bind, (2) delay full penalty engagement slightly
(_RELAX_P1_BASE 0.45->0.48), and (3) lengthen the pure-exploration lr
plateau a touch (_HOLD_FRAC_BASE 1/3->0.35). All three are tiny, additive
shifts to constants shared by every farm (not new conditioning), so
farms that already move away from base via size/scale_pos terms (parque_n20,
dei_n80_omnidir, ...) are barely touched -- only the neutral middle of the
density/scale spectrum, where dei_n50_uniform lives, shifts meaningfully.
Every other mechanism (self-contained inverse-time product lr decay to
gamma_min, short linear warmup, density-extremity relax/spike/hold,
absolute-N scale on hold/spike/relax_p1, the alpha-floor relief incl. the
gated high-N band, the hold_frac-floored beta ramp) is unchanged.
"""
...
_RELAX_BASE, _RELAX_SPAN = 0.40, 0.11   # relax0 = base + span * size in [-1,1]
_RELAX_SCALE_SPAN = -0.05       # lower early-exploration penalty floor for large-N farms
_RELAX_P0 = 0.08
_RELAX_P1_BASE, _RELAX_P1_SPAN = 0.48, 0.10   # widen engagement window at extremes
...
_HOLD_FRAC_BASE, _HOLD_FRAC_SPAN = 0.35, 0.06   # exploration plateau length
