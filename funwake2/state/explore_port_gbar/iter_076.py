"""Schedule: scale the terminal beta2 ceiling with absolute farm size
(scale_pos-gated), on top of the parent's alpha-denominator floor
(isolated single-lever addition).

Diagnosis vs. the parent: the parent already bounds alpha's terminal
denominator for large-N farms (via alpha_denom_floor), but that alone
barely moved dei_n80_omnidir (+0.0005%) and left dei_n50 / dei_n50_uniform
(scale_pos = 0.368 each, since n_turbines=50 > _N_REF=30) still near-flat
(+0.03% / +0.01%) versus the strong gains on the small/mid farms
(parque_n10/n20, scale_pos == 0, +0.22% / +0.68%). A farm with more
turbines has quadratically more pairwise spacing-constraint terms, so its
constraint gradient (and hence the alpha*grad_constraint contribution to
the combined gradient) is noisier and higher-variance late in the run,
when alpha has ramped up via `spike`. The parent's beta2 ramp (_B2_LO ->
_B2_HI) already times *when* the second-moment averaging kicks in based on
farm size (via beta_p0), but the ceiling it ramps to, _B2_HI = 0.45, is
fixed regardless of size -- so large-N farms get earlier-onset but equally
weak variance smoothing as small-N farms, right when their constraint
gradient noise is worst.

This adds one new, previously-untried lever: raise the beta2 ceiling
itself for large-N farms only (scale_pos-gated, exactly like the existing
alpha_denom_floor and beta_p0 shifts), so Adam's adaptive-lr denominator
averages over a longer effective window late in the run on crowded farms,
damping the noisy constraint-gradient variance without touching small/mid
farms (scale_pos == 0 there, so _B2_HI stays exactly at the parent's 0.45
and dei_n50/parque_n10/n20 behavior is unchanged). beta1's ceiling, and
all other machinery (lr decay table, warmup, relax/spike shaping, alpha
denominator floor, beta timing), are left untouched from the parent.
"""
...
# NEW: raise the beta2 ceiling further for large-N farms, so the
# adaptive-lr denominator averages over a longer window late in the run
# when the (many-pair) constraint gradient is noisiest.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos    # deeper late-run averaging, large N
