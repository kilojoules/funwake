...
farms (single-lever tweak on the current best) -- the prior single-lever
change (raising the floor from 1e-30 to gamma_min*(1+1.5*scale_pos)) was the
only recent edit to raise the farm-balanced score (+0.187% -> +0.189%), but
dei_n80_omnidir (n=80, scale_pos = log(80/30)/log(4) ~= 0.71) still only
gained +0.0005%, i.e. it stayed essentially flat while the smaller/mid farms
improved. That means the floor is still binding too close to gamma_min for
this farm: alpha_denom = gamma_min*(1+1.5*0.71) ~= 2.06*gamma_min, which is
still small enough that alpha's terminal value is close to native's full
constraint-dominated re-engagement on an 80-turbine (many-pair) layout.

Earlier attempts pushing hold_frac / spike0 / relax_p1 / beta_p0 further via
scale_pos made the farm-balanced score worse, so those four remain
untouched. This edit instead pushes only the newest, narrowly-scoped lever
(_ALPHA_FLOOR_SCALE_SPAN) further, from 1.5 to 3.0, doubling the terminal
alpha-denominator floor for large-N farms (dei_n80_omnidir: ~2.06*gamma_min
-> ~3.13*gamma_min), so alpha's late-run cap is meaningfully lower there.
Small/mid-N farms (scale_pos == 0, e.g. parque_n10/n20, dei_n50) are
completely unaffected since their floor stays at gamma_min, matching the
parent exactly -- this cannot regress them.
...
# Raised from 1.5 -> 3.0: the prior value left dei_n80_omnidir essentially
# flat, so the floor needed to bind harder specifically for large-N farms.
