"""Schedule: builds on the +0.2007% farm-balanced best (which already gates
extra hold/relax-delay/spike-softening/alpha-floor relief onto both the
N~=60-100+ band via `high_n_ramp` and the N~=50 band via `mid_n_bump`).

Diagnosis: dei_n50 (+0.0501%) and dei_n50_uniform (+0.0151%) share IDENTICAL
n_turbines/D/min_spacing, so every geometry-derived feature (`size`,
`extremity`, `scale_pos`, `mid_n_bump`, `high_n_ramp`) is bit-for-bit
identical between them -- the schedule structurally cannot tell them apart
even though dei_n50_uniform is now the worst farm. The one input that DOES
differ between these two cases is `alpha0` (mean|grad J|/D), since the
uniform-wind-rose farm has a different gradient-magnitude landscape than the
directional one. This adds a small, bounded, gradient-magnitude-adaptive
term (`grad_sig`, a tanh-squashed log-ratio of the native terminal penalty
scale alpha0*D/gamma_min) that nudges the relax-engagement delay and the
alpha floor -- the two already-validated, bounded levers -- by a modest
amount. Because `grad_sig` is a smooth, bounded function of a physically
meaningful dimensionless ratio (not a farm-identity lookup), it generalizes
to unseen farms and only perturbs the existing best schedule slightly: at
`grad_sig ~= 0` (the implicit reference point) all four outputs reduce
exactly to the parent's values. `_ramp` is updated to use jnp.maximum
instead of Python max so it stays traceable when p1 itself becomes a traced
value (needed now that relax_p1 depends on the traced `grad_sig`).
"""
...
# Gradient-magnitude-adaptive term: breaks the tie between farms that share
# identical n_turbines/D/min_spacing but differ in wind-rose concentration
# (e.g. dei_n50 vs dei_n50_uniform), via the one input that differs, alpha0.
# grad_sig in (-1, 1), ~0 at the "typical" native terminal-penalty scale
# (alpha0*D ~= gamma_min), positive when the raw gradient pressure is large
# relative to the tolerance scale, negative when it's small.
_GRAD_LOG_SCALE = 2.0
_GRAD_RELAX_P1_SPAN = 0.04      # small extra relax-engagement delay
_GRAD_FLOOR_SPAN = 0.15         # small extra alpha-floor bound
...
def _ramp(p, p0, p1):
    """Traceable smoothstep from 0 at p0 to 1 at p1 (p0/p1 may themselves be
    traced JAX values, so span/clamp use jnp ops, never Python max/min)."""
    span = jnp.maximum(p1 - p0, 1e-12)
    x = jnp.clip((p - p0) / span, 0.0, 1.0)
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Gradient-magnitude-adaptive signal: bounded, traced (depends on alpha0),
    # ~0 at the "typical" native terminal-penalty scale alpha0*D ~= gamma_min.
    grad_ratio = jnp.maximum(alpha0 * diam / g_min, 1e-6)
    grad_sig = jnp.tanh(jnp.log(grad_ratio) / _GRAD_LOG_SCALE)
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _GRAD_RELAX_P1_SPAN * grad_sig)       # gradient-scale-adaptive delay (traced)
    ...
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
        ...
        * (1.0 + _GRAD_FLOOR_SPAN * grad_sig)           # gradient-scale-adaptive floor bound
