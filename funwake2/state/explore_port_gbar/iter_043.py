"""Schedule: further lower the early-exploration penalty floor for large-N
farms via a stronger _RELAX_SCALE_SPAN (isolated single-lever change).

Diagnosis vs. the parent: the parent already bounds alpha's terminal
denominator for large-N farms (via alpha_denom_floor), which fixed
dei_n80_omnidir's terminal over-constraint but left it (and dei_n50,
dei_n50_uniform -- the other large-N/mid-packing-density farms) still far
behind the parque farms in score_pct. All three weak farms sit near
_packing_size == 0 (dei's spacing/D ratio puts density right at the
_DENSITY_REF anchor), so the *density*-driven relax/spike/hold terms
(which multiply by `size` or `extremity = |size|`) barely engage for them
-- they inherit close to the un-adapted _RELAX_BASE/_SPIKE_BASE defaults.
The only lever still differentiating them from the parque farms is
absolute turbine count via scale_pos (n=50/80 vs n=10/20), and of the
scale_pos-gated terms, _RELAX_SCALE_SPAN (the early-exploration penalty
floor discount for large N) is the one NOT yet in the tried-and-failed set
(hold_frac, spike0, relax_p1, beta_p0 were pushed further via scale_pos
and regressed the farm-balanced score; alpha_denom_floor via scale_pos
improved it). This change pushes _RELAX_SCALE_SPAN more negative
(-0.05 -> -0.18), giving large-N farms (dei_n50/n50_uniform/n80_omnidir)
more early AEP-exploration headroom (lower relax0 penalty floor) before
the late-run spike/hold phases re-tighten feasibility -- without touching
parque_n10/n20 at all, since their scale_pos == 0 keeps relax0 identical
to the parent.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0, bounded alpha
denominator floor) is unchanged from the parent.
"""
...
_RELAX_SCALE_SPAN = -0.18       # lower early-exploration penalty floor for large-N farms (was -0.05)
_RELAX_P0 = 0.08
