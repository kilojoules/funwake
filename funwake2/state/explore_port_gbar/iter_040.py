"""Schedule: give large-N farms a higher terminal beta2 (adaptive-scaling
smoothing) via the existing scale_pos gate, leaving lr/alpha/beta1 and every
small/mid-N farm untouched (isolated single-lever change on top of the
current best).

Diagnosis vs. the current best: the alpha-denominator floor already bounds
the terminal constraint-penalty divergence for large-N farms (dei_n80_omnidir,
scale_pos ~= 0.71), and per-farm scores show that farm is feasible on all
seeds but essentially flat (+0.0005%) versus dei_n50/dei_n50_uniform's small
but positive gains and parque's large gains -- i.e. large-N farms are not
mis-converging into infeasibility, they are under-converging in AEP. A farm
with 80 turbines has ~3.2x as many turbine-pairs (and hence noisier,
higher-variance constraint-gradient terms) as a 50-turbine farm at the same
density, so the same terminal beta2 (second-moment decay) that suffices for
dei_n50 leaves more residual gradient-noise variance unaveraged for dei_n80,
which can stall the fine-convergence phase before it fully exploits the
feasible region. This adds one new, previously-untried lever -- scaling only
the *terminal* beta2 target (_B2_HI) upward via the existing scale_pos gate
(0 for n_turbines <= ~30, e.g. parque_n10/n20, so those farms see byte-for-
byte the same schedule as the current best) -- to smooth the adaptive
second-moment estimate more aggressively for large-N farms during the same
beta ramp window, without touching lr, alpha, or beta1 (so feasibility
margins and the constraint-penalty profile are unchanged).
...
absolute-N scale on hold/spike/relax_p1/beta_p0/alpha floor) is unchanged
...
_B2_HI_SCALE_SPAN = 0.15        # NEW: extra terminal beta2 smoothing for large-N farms
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # NEW: raise the terminal beta2 target for large-N farms only -- more
    # turbine-pairs means noisier constraint-gradient terms, so a bit more
    # second-moment averaging in the same fine-convergence window helps
    # those farms converge further without changing lr, alpha, or beta1.
