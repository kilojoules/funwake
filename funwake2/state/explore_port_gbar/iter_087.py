"""Schedule: increase the alpha-denominator floor scale for large-N farms
(single-lever refinement of the accepted parent's new floor mechanism).

Diagnosis vs. the parent: the parent already introduced
`alpha_denom_floor = gamma_min * (1 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos)`
to bound alpha's terminal 1/lr_base divergence on large-N farms, and this
raised dei_n80_omnidir from -0.0002% to +0.0005% -- but it is still the
worst-performing farm by a wide margin and nearly flat, meaning the floor
is not yet high enough to meaningfully relieve the late-stage over-
constraint on that many-pair (80-turbine) layout. Every other recent
attempt that pushed hold_frac, spike0, relax_p1, or beta_p0 further via
scale_pos made the farm-balanced score worse, so those four levers are
left untouched. This change instead pushes the ONE lever that empirically
already worked in the right direction: raising `_ALPHA_FLOOR_SCALE_SPAN`
from 1.5 to 2.5, i.e. capping alpha's terminal denominator higher (further
above gamma_min) specifically for large-absolute-N farms, so alpha's
end-of-run ceiling on dei_n80_omnidir is lower and the optimizer keeps
more of its late-stage exploration budget instead of re-engaging a near-
native constraint-dominated tail. Because the floor multiplies
`scale_pos` (which is exactly 0 for parque_n10/n20 and dei_n50* -- all
at or below the N_REF=30 anchor), this floor value is unreachable for
those farms and their trajectories are bit-for-bit identical to the
parent. All other machinery (self-contained inverse-time product lr
decay to gamma_min, short linear warmup, density-extremity relax/spike/
hold, absolute-N scale on hold/spike/relax_p1/beta/relax0) is unchanged.
"""
...
# Raised from 1.5 -> 2.5: the smaller floor still left dei_n80_omnidir
# nearly flat, so push the cap higher for large-absolute-N farms only.
_ALPHA_FLOOR_SCALE_SPAN = 2.5
