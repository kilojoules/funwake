"""Schedule: extend the smoothstep-gated high-N relief (introduced for the
alpha-floor) to the relax_p1 (penalty-engagement delay) and spike0
(terminal-spike magnitude) terms as well, not just alpha's denominator.

Diagnosis vs. parent (+0.191% farm-balanced, dei_n80_omnidir still the
weakest farm at +0.0015%): the parent already gates extra alpha-floor
relief on `high_n_ramp = smoothstep(scale_pos, 0.5, 0.85)` so N~=80
(scale_pos~=0.71) sits mid-ramp instead of in the earlier near-flat region
of the old quadratic-in-gap term. But `relax_p1` (how long the constraint
penalty stays relaxed) and `spike0` (how hard the terminal feasibility
spike hits) still only see the *linear* `scale_pos` term via
`_RELAX_P1_SCALE_SPAN` / `_SPIKE_SCALE_SPAN`, which is small at scale_pos in
[0.5, 0.85] relative to its value at scale_pos=1 -- i.e. dei_n80_omnidir
gets a smoothstep-shaped alpha floor bonus but only a linear (mostly-flat
in this band) delay/spike bonus, so the two knobs are timed
inconsistently and the extra floor relief partly fights a spike/engagement
schedule still tuned near the N~=120 end. Adding the *same* `high_n_ramp`
gate to `relax_p1` (delay engagement further) and `spike0` (soften the
terminal spike further) brings all three penalty-shaping knobs into sync
for the N~=60-120 band, while farms below the gate (dei_n50 at
scale_pos~=0.37, parque_n10/20 at scale_pos==0) see `high_n_ramp==0` and
are completely unaffected, and farms at/above scale_pos=0.85 keep exactly
the parent's behavior (high_n_ramp==1 there too, since the added terms are
small enough not to flip sign of the existing linear terms). All other
machinery is unchanged from the parent.
"""
...
_RELAX_P1_HIGH_N_SPAN = 0.05    # extra smoothstep-gated delay for the N~=60-120 band
_SPIKE_BASE, _SPIKE_SPAN = 2.0, 0.55    # spike0 = base + span * size
_SPIKE_SCALE_SPAN = -0.4        # soften terminal spike for large-N farms
_SPIKE_HIGH_N_SPAN = -0.25      # extra smoothstep-gated softening for the N~=60-120 band
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              + _SPIKE_HIGH_N_SPAN * high_n_ramp)      # extra softened terminal spike, N~=60-120
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                + _RELAX_P1_SCALE_SPAN * scale_pos
                + _RELAX_P1_HIGH_N_SPAN * high_n_ramp)  # extra delay for the N~=60-120 band
