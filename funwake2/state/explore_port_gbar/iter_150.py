"""Schedule: extends the current best-so-far variant (+0.1959% farm-balanced,
worst farm dei_n50_uniform at +0.0139%). All packing/absolute-N-based shaping
(size, extremity, scale_pos, high_n_ramp) is unchanged from the parent -- but
that shaping is a function ONLY of (n_turbines, D, min_spacing), so it cannot
tell dei_n50 (directional wind, score +0.0277%) apart from dei_n50_uniform
(same n/D/spacing, uniform/omnidirectional wind, score +0.0139%): both get
byte-identical schedules today even though one badly underperforms the other.

The one signal that DOES differ between those two farms is alpha0 = mean|grad
J|/D: an omnidirectional wind rose averages the AEP gradient over many more
directions than a narrow directional rose, so for the same layout/N it tends
to produce a smaller mean|grad AEP| (directional cancellation). This adds a
small, bounded, fully traceable correction driven by alpha0 (never cast to
float/int, only used in jnp arithmetic) that softens the penalty engagement
(lower relax0, gentler terminal spike) when the gradient magnitude is weak
relative to N -- giving low-gradient (uniform-wind-like) farms a bit more
room to keep improving AEP before the constraint penalty dominates. The
correction is a smoothstep-gated multiplicative discount (max ~15-20%) so it
leaves farms with normal/large gradient magnitude (dei_n50, parque_*,
dei_n80_omnidir which already gets separate high-N relief) essentially at
their parent behavior, while only meaningfully engaging on the weak-gradient
regime the parent schedule currently cannot see at all.
"""
...
# Weak-gradient (uniform/omnidirectional-wind-like) relief, driven by alpha0.
# grad_scale = alpha0 * D / gamma_min is the gradient magnitude in units of
# the metre tolerance; om_ratio further normalizes by N so the reference is
# roughly N-invariant. Low om_ratio (weak averaged gradient relative to N,
# typical of a wide/omnidirectional wind rose) softens the penalty ramp.
_OM_REF = 0.5
_OM_WIDTH = 1.0
_RELAX_OM_SPAN = 0.15
_SPIKE_OM_SPAN = 0.20
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    n_turb = max(float(n_turbines), 1.0)
    ...
    # Weak-averaged-gradient relief: alpha0 is traced, so this stays jnp-only.
    # grad_scale is in units of gamma_min (metre tolerance); om_ratio further
    # divides by N so the reference _OM_REF is roughly farm-size invariant.
    # om_factor -> 1 when om_ratio << _OM_REF (weak gradient, e.g. wide /
    # omnidirectional wind rose), -> 0 when om_ratio >= _OM_REF (normal or
    # strong directional gradient), leaving the parent schedule untouched.
    grad_scale = alpha0 * diam / g_min
    om_ratio = grad_scale / n_turb
    om_z = (jnp.log(jnp.maximum(om_ratio, 1e-9)) - math.log(_OM_REF)) / _OM_WIDTH
    om_factor = 1.0 / (1.0 + jnp.exp(om_z))

    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              + _RELAX_SCALE_SPAN * scale_pos)        # lower early floor for large-N farms
    relax0 = relax0 * (1.0 - _RELAX_OM_SPAN * om_factor)   # further ease for weak-gradient farms
    ...
    spike0 = spike0 * (1.0 - _SPIKE_OM_SPAN * om_factor)   # gentler terminal spike, weak-gradient
