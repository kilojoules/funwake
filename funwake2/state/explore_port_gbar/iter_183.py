...
which already gates extra hold/relax-delay/spike relief onto both the
high-N band (scale_pos-gated) and the near-reference-density band
(mid_gate, extremity-gated). That variant's alpha floor relief
(`alpha_denom_floor`) is only gated on the high-N band, even though the
comment accompanying `_MID_GATE_*` explicitly notes dei_n50 was left out
of the analogous floor relief. dei_n50_uniform is now the worst-scoring
farm (+0.0139%), and as a near-reference-density, uniform-wind-rose farm
it sits squarely inside `mid_gate`'s band without picking up any alpha
floor relief -- only hold/relax/spike shaping. This adds a modest,
mid_gate-gated alpha floor term (smaller than the high-N one, since
mid_gate farms are mid-density rather than crowded) so the terminal
alpha's 1/lr_base divergence is capped a bit for this band too, giving
the optimizer more room to keep improving AEP late instead of being
pulled fully into the constraint-dominated tail. Every other mechanism,
and every farm outside `mid_gate`, is unchanged from the best-so-far
variant.
"""
...
# Smaller, mid_gate-gated relief for near-reference-density farms (e.g.
# dei_n50_uniform) -- these are not crowded like the high-N band, so the
# floor bump is modest, just enough to loosen the terminal constraint-
# dominated tail without destabilizing feasibility.
_MID_ALPHA_FLOOR_SPAN = 0.6
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    alpha_denom_floor = g_min * (1.0 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos
                                  ...
                                  + _MID_ALPHA_FLOOR_SPAN * mid_gate)  # modest relief near ref density
