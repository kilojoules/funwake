"""Schedule: extend the threshold-gated high-N decoupling (already used for
the alpha-floor) to the terminal spike and early relax floor as well, so
dei_n80_omnidir (scale_pos ~= 0.71, the current weakest farm at +0.0005%)
gets additional, farm-specific relief while dei_n50 (scale_pos ~= 0.37,
below the gate) and the parque farms (scale_pos == 0) remain bit-for-bit
unchanged.

Diagnosis vs. the current best: the alpha_denom_floor already carries a
threshold-gated quadratic term (high_n_gap**2) that decouples large-N relief
from dei_n50, and that was the last source of gain (0.187% -> 0.190%). But
spike0 and relax0 are still purely LINEAR in scale_pos -- so any further
softening of the terminal penalty spike or the early-exploration floor for
dei_n80_omnidir still drags dei_n50's spike/relax down proportionally (both
have scale_pos>0, just different magnitudes). Since dei_n50 is comfortably
positive (+0.01-0.03%) and dei_n80_omnidir is barely positive and the
farm-balanced mean is capped by the weakest farm, apply the SAME
threshold-gated quadratic (active only for scale_pos > 0.5, i.e. N > 60) to:
  - spike0: extra softening of the terminal feasibility spike for high-N
    farms (less late disruption to the AEP-improving layout there), and
  - relax0: extra lowering of the early-exploration penalty floor for
    high-N farms (more room to explore before constraints bite).
Both terms are exactly zero below the gate, so dei_n50 and both parque farms
see an identical trace to the current best. All other machinery (inverse-
time product lr decay to gamma_min, short linear warmup, density-extremity
relax/spike/hold, absolute-N scale on hold/spike/relax_p1, the gated
quadratic alpha-floor relief, hold_frac-floored beta ramp) is unchanged.
"""
...
# Threshold-gated (scale_pos > 0.5, i.e. N > 60) quadratic relief that
# decouples the largest-N farms from the linear terms above without
# touching any farm below the gate.
_HIGH_N_THRESH = 0.5
_ALPHA_FLOOR_HIGH_N_SPAN = 6.0
# Same gate applied to the terminal spike (softer late feasibility push)
# and the early relax floor (more early-exploration headroom) for the
# largest-N farms only.
_SPIKE_HIGH_N_SPAN = 1.1
_RELAX_HIGH_N_SPAN = 0.18
_RELAX_FLOOR_MIN = 0.05         # keep relax0 comfortably positive
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              ...
              - _RELAX_HIGH_N_SPAN * high_n_gap2)      # extra early headroom, high-N only
    relax0 = max(relax0, _RELAX_FLOOR_MIN)
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              + _SPIKE_SCALE_SPAN * scale_pos
              - _SPIKE_HIGH_N_SPAN * high_n_gap2)      # softer terminal spike, high-N only
