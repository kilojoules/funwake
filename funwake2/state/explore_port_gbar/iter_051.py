"""Schedule: raise the terminal Adam beta2 (second-moment decay) for large-N
farms, isolated single-lever change on top of the current best.

Diagnosis vs. the current best: of the five fitness farms, the two weakest
by a wide margin are dei_n80_omnidir (+0.0005%) and dei_n50_uniform
(+0.0139%) -- both large-turbine-count and/or omnidirectional/uniform wind
roses, where the constraint-gradient signal is noisier and more diffuse
across many simultaneously-active turbine pairs than in the strongly
directional parque farms (+0.22%, +0.68%). The current best already bounds
alpha's terminal divergence for large-N farms via a denominator floor, and
earlier attempts pushed hold_frac, spike0, relax_p1, and beta_p0 further via
scale_pos -- all regressed the farm-balanced score, so those four levers are
left untouched here.

This change adds one new, previously-untried lever: for large-N farms only
(scale_pos > 0), raise beta2's terminal asymptote (_B2_HI) via a
scale_pos-gated span, giving more second-moment (gradient-variance)
averaging in the late phase where many noisy constraint-pair gradients are
active. beta2's *onset* timing (beta_p0) and beta1 are both left exactly as
in the parent/current-best, so this only changes how strongly the adaptive
step-size denominator is smoothed once averaging has already begun --
small/mid-N farms (scale_pos == 0, e.g. parque_n10/n20, dei_n50) are
completely unaffected since their beta2 asymptote stays at the parent's
0.45.
...
absolute-N scale on hold/spike/relax_p1/beta/relax0, the alpha denominator
floor) is unchanged from the current best.
"""
...
# NEW: extra second-moment averaging strength (beyond _B2_HI) for large-N
# farms, where many simultaneously-active constraint-pair gradients make
# the adaptive-lr denominator noisier late in the run.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # stronger terminal smoothing, large N
