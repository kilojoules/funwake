"""Schedule: adds a gradient-magnitude (alpha0) signal alongside the existing
packing-density and absolute-N signals, and strengthens large-N relief.

Diagnosis vs. the current best (packing-density `size`/`extremity` +
absolute-N `scale_pos` adaptivity):

  dei_n50 and dei_n50_uniform have IDENTICAL n_turbines, D, and min_spacing --
  so `size`, `extremity`, and `scale_pos` are bit-for-bit identical between
  them, meaning the schedule currently treats them identically even though
  their real dynamics differ (uniform vs. non-uniform initial layout changes
  how much of the early AEP gradient is "free" exploration vs. immediately
  spacing-constrained). The only per-farm signal available that *can* differ
  between these two is `alpha0` (mean|grad J|/D at the current step), so we
  add a second, independent shaping term built from it: `grad_sig`, a
  log-compressed, bounded (in [-1, 1]) measure of the initial objective
  gradient relative to the exploration learning rate. This is threaded only
  through the per-step (traced) relax/spike terms -- never through the
  compile-time hold/decay horizons, since alpha0 is a traced value inside
  the jit loop.

  Separately, dei_n80_omnidir is still feasible but below native despite the
  existing `scale_pos` relief -- so the large-N softening (terminal spike,
  relax-ramp delay, hold extension, beta phase-in) is strengthened somewhat
  further, and the beta-phase-in is nudged earlier for large N (noisier
  per-step constraint gradient with many pairwise spacing terms benefits
  from earlier moment-averaging).

  All other machinery (self-contained inverse-time product lr decay to
  gamma_min, short linear warmup, density-extremity relax/spike/hold) is
...
_RELAX_P1_SCALE_SPAN = 0.13     # further delay for large-N farms
_SPIKE_BASE, _SPIKE_SPAN = 2.0, 0.55    # spike0 = base + span * size
_SPIKE_SCALE_SPAN = -0.65       # soften terminal spike for large-N farms
_SPIKE_P0, _SPIKE_P1 = 0.90, 1.00

_HOLD_FRAC_BASE, _HOLD_FRAC_SPAN = 1.0 / 3.0, 0.06   # exploration plateau length
_HOLD_FRAC_SCALE_SPAN = 0.10    # extra hold for large-N farms
_DENSITY_REF = 12.5             # ~50 turbines at 2D spacing (mid-scale anchor)
_N_REF = 30.0                   # absolute-N reference (independent of density)

# gradient-magnitude signal: differentiates farms sharing N/D/min_spacing
# (e.g. uniform vs. non-uniform initial layout) via alpha0 = mean|grad J|/D.
_GRAD_REF = 1.0                 # reference for alpha0*D/lr0 (dimensionless)
_GRAD_RELAX_SPAN = 0.05         # steeper init gradient -> a bit more early slack
_GRAD_SPIKE_SPAN = -0.15        # steeper init gradient -> softer terminal spike

# Adam moments: native early -> mildly averaged late
_B1_LO, _B1_HI = 0.1, 0.25
_B2_LO, _B2_HI = 0.2, 0.45
_BETA_P0, _BETA_P1 = 0.50, 0.90
_BETA_P0_SCALE_SPAN = -0.17     # earlier averaging onset for large-N farms
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # gradient-magnitude signal (traced, from alpha0): differentiates farms
    # that share n_turbines/D/min_spacing but have different init gradients.
    grad_ratio = jnp.asarray(alpha0) * diam / lr0
    grad_sig = jnp.clip(
        jnp.log(jnp.maximum(grad_ratio, 1e-6) / _GRAD_REF) / math.log(4.0),
        -1.0, 1.0,
    )

    relax0 = (_RELAX_BASE + _RELAX_SPAN * size
              + _GRAD_RELAX_SPAN * grad_sig)          # sparse farms -> freer early
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              + _SPIKE_SCALE_SPAN * scale_pos
              + _GRAD_SPIKE_SPAN * grad_sig)           # softened terminal spike for large N / steep grad
