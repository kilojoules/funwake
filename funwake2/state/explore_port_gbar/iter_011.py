"""Schedule: delayed alpha ramp + freer early exploration on top of farm-adaptive sizing.

Changes vs. the parent (farm-adaptive relax/spike/hold via packing-density
`size`, native inverse-time lr decay, warmup, beta phase-transition):

  1. DELAYED alpha ramp: the relax curve (native-coupling multiplier that
     rises from `relax0` toward 1.0) now starts later in training
     (_RELAX_P0: 0.08 -> 0.16, _RELAX_P1: 0.45 -> 0.55). This follows the
     prior-art "delayed penalty ramp" idea (LANCELOT / filter methods):
     give the optimizer a longer pure-AEP exploration window before the
     constraint penalty starts climbing off its relaxed floor, instead of
     starting to tighten almost immediately after warmup. The terminal
     feasibility spike (_SPIKE_P0/_SPIKE_P1, at 0.90-1.00) is UNCHANGED so
     the same late feasibility-restoration push still fully engages and
     all farms stay feasible.

  2. Slightly freer early-exploration floor: _RELAX_BASE raised from 0.45
     to 0.48. A small, uniform reduction in the average early penalty
     weight across all farm sizes (the packing-density `size` adjustment
     on top of this base is unchanged), consistent with the "bounded,
     less-aggressive early penalty beats native's aggressive coupling"
     hypothesis from the penalty-method survey.

  3. beta2 (Adam second-moment decay) starts ramping up slightly earlier
     (_BETA_P0: 0.50 -> 0.45) to begin taming adaptive-lr variance a bit
     sooner, matching the now-slightly-longer relaxed-exploration phase.
     beta1's ramp window is unchanged.

  Everything else -- the self-contained inverse-time product lr decay to
  gamma_min (log-gamma bisection, cached table), the short linear warmup,
  and the packing-density-based farm adaptation of relax0/spike0/hold_frac
...
_RELAX_BASE, _RELAX_SPAN = 0.48, 0.15   # relax0 = base + span * size in [-1,1]
_RELAX_P0, _RELAX_P1 = 0.16, 0.55       # delayed ramp: longer pure-exploration window
_SPIKE_BASE, _SPIKE_SPAN = 2.0, 0.8     # spike0 = base + span * size
_SPIKE_P0, _SPIKE_P1 = 0.90, 1.00       # terminal feasibility spike, unchanged
