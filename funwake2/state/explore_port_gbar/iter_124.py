"""Schedule: extend the smoothstep high-N gate (introduced for the alpha
floor) to also soften the terminal spike and further delay the relax ramp
in the same N~=60-100 band.

Diagnosis vs. parent (+0.191% farm-balanced, dei_n80_omnidir still weakest
at +0.0015%): the alpha-floor relief was already retimed from a hard
quadratic-in-gap term to a smoothstep gated on scale_pos in [0.5, 0.85], so
it now delivers real relief at dei_n80_omnidir's scale_pos~=0.71. But
_SPIKE_SCALE_SPAN and _RELAX_P1_SCALE_SPAN -- the other two "large-N"
knobs that shape the *terminal feasibility spike* and *penalty-engagement
delay* -- are still purely linear in scale_pos, so (exactly like the old
alpha floor) they are nearly flat just above the gate and only become
meaningful very close to scale_pos=1 (N~=120). dei_n80_omnidir sits mid-ramp
(scale_pos~=0.71, high_n_ramp~=0.65) and gets almost none of the intended
softening from those two terms. Reusing the same high_n_ramp gate to add a
further spike discount and a further relax-ramp delay lets both terms
actually engage at N~=80 while leaving dei_n50 (scale_pos~=0.37, below the
gate) and parque_n10/20 (scale_pos==0) completely untouched -- same
decoupling guarantee, just extended to the two remaining linear-in-scale_pos
knobs. All other machinery (self-contained inverse-time product lr decay to
gamma_min, short linear warmup, density-extremity relax/spike/hold, the
alpha-floor high-N relief, the hold_frac-floored beta ramp) is unchanged.
"""
...
_RELAX_P1_HIGH_N_SPAN = 0.06    # extra smoothstep-gated delay for N~=60-120 band
_SPIKE_BASE, _SPIKE_SPAN = 2.0, 0.55    # spike0 = base + span * size
_SPIKE_SCALE_SPAN = -0.4        # soften terminal spike for large-N farms
_SPIKE_HIGH_N_SPAN = -0.3       # extra smoothstep-gated softening for N~=60-120 band
...
# touching any farm below the gate. This same gate (`high_n_ramp`) is also
# reused below to soften the terminal spike and delay the relax ramp.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_HIGH_N_SPAN * high_n_ramp)  # extra delay, N~=60-120
