...
farms (single-lever bump of _ALPHA_FLOOR_SCALE_SPAN, 1.5 -> 2.5).

Diagnosis vs. the current best: alpha = alpha0*diam*relax*spike / alpha_denom,
where alpha_denom = max(lr_base, gamma_min*(1 + _ALPHA_FLOOR_SCALE_SPAN*scale_pos)).
Per-farm scale_pos is 0 for parque_n10/n20 (N=10/20, below the N_REF=30
absolute-N anchor) and positive for dei_n50/dei_n50_uniform (N=50,
scale_pos~0.368) and dei_n80_omnidir (N=80, scale_pos~0.708) -- so every
scale_pos-gated lever, including this one, is already provably inert on the
two farms carrying the most score (parque_n10 +0.22%, parque_n20 +0.68%) and
only acts on the three weak farms. Among those three, the improvement
shrinks as scale_pos grows (dei_n50 +0.033%, dei_n50_uniform +0.014%,
dei_n80_omnidir +0.0005%), i.e. the farm with the *most* turbine pairs and
the strongest existing floor multiplier (2.06x gamma_min) still sees almost
no gain -- evidence the floor is not yet loose enough to stop alpha's
terminal re-engagement of native's constraint-dominated tail on that farm.
Earlier attempts already pushed hold_frac, spike0, relax_p1, and beta_p0
further via scale_pos and made the farm-balanced score worse, so this
change again isolates the single lever that hasn't been retried:
_ALPHA_FLOOR_SCALE_SPAN 1.5 -> 2.5, raising the terminal floor to ~2.77x
gamma_min for dei_n80_omnidir (~1.92x for the N=50 farms) instead of
~2.06x/~1.55x, while leaving parque_n10/n20 (scale_pos==0, floor stays at
gamma_min) exactly as before -- matching the parent bit-for-bit there.
...
# (bumped from 1.5 -- see module docstring)
