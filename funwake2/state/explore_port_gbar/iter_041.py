...
farms via a scale_pos-gated span on _B2_HI (isolated single-lever change).

Diagnosis vs. the parent: beta2 ramps from _B2_LO=0.2 to a single fixed
_B2_HI=0.45 regardless of farm size, via the same smoothstep `r` that also
drives beta1. dei_n80_omnidir -- the largest-N farm in the fitness set, and
omnidirectional (full wind rose, so the AEP gradient at each turbine sums
contributions from many more direction bins than the other farms) -- has by
far the noisiest per-step gradient of any farm here, yet it gets the exact
same adaptive-moment smoothing ceiling as the small farms. The parent
already recognizes n_turbines-driven noise via scale_pos for hold_frac,
spike0, relax_p1, beta_p0, and the alpha_denom floor, but never lets that
same signal touch the *magnitude* of beta2's ceiling -- only the *timing* of
its ramp (beta_p0). A higher terminal beta2 gives the late-stage optimizer
more second-moment averaging exactly where gradient variance (many wind
directions x many close turbine pairs) is highest, which should stabilize
fine-grained constraint satisfaction on dei_n80_omnidir without touching the
small/mid-N farms (parque_n10/n20, scale_pos == 0) at all, since their
beta2 ceiling is unchanged from the parent. beta1 is left untouched so this
is a single isolated lever, distinct from the already-explored beta_p0
timing shift.
...
absolute-N scale on hold/spike/relax_p1/beta timing, alpha_denom floor) is
...
# NEW: raise the terminal beta2 ceiling for large-N farms, giving more
# second-moment averaging where per-step gradient noise (many wind
# directions x many turbine pairs) is highest.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # higher ceiling for large-N farms
