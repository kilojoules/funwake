"""Schedule: push the terminal alpha-denominator floor further for large-N
farms (isolated single-lever tweak on the last confirmed win).

Diagnosis vs. the parent: the parent already added `alpha_denom_floor =
gamma_min * (1 + 1.5*scale_pos)` to cap alpha's terminal 1/lr_base blow-up on
large-N farms, which is what moved the farm-balanced score from +0.177% to
+0.189%. But among the fitness farms, `dei_n80_omnidir` (n_turbines=80,
scale_pos = log(80/30)/log(4) ~= 0.71, the largest scale_pos of any fitness
farm) is still essentially flat (+0.0005%), while the other scale_pos>0
farms, `dei_n50` and `dei_n50_uniform` (n_turbines=50, scale_pos ~= 0.37),
are modestly positive. That gradient -- the farm with the largest scale_pos
is the one still failing to escape native's constraint-dominated tail --
suggests the floor is scaling in the right direction but is not yet high
enough at high scale_pos to fully decouple alpha from lr_base's terminal
gamma_min collapse on the most crowded (most turbine-pairs) farm. This
change raises _ALPHA_FLOOR_SCALE_SPAN from 1.5 to 2.2: at scale_pos~=0.71
(dei_n80_omnidir) the floor rises from ~2.06*gamma_min to ~2.56*gamma_min
(further bounding the terminal alpha spike), while at scale_pos~=0.37
(dei_n50/dei_n50_uniform) it only rises from ~1.56*gamma_min to
~1.81*gamma_min (a much gentler nudge on farms that are already feasible
and modestly positive), and small/mid-N farms with scale_pos==0
(parque_n10/n20) are completely unaffected since their floor stays at
gamma_min, matching the parent exactly. No other lever is touched.
...
# Raised from 1.5 -> 2.2: the previous value under-bounded alpha on the
# highest-scale_pos fitness farm (dei_n80_omnidir), leaving it flat.
_ALPHA_FLOOR_SCALE_SPAN = 2.2
