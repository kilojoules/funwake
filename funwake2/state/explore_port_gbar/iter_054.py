...
farms (single-lever extension of the parent's already-successful change).

Diagnosis vs. the parent: the parent introduced `alpha_denom_floor =
gamma_min * (1 + _ALPHA_FLOOR_SCALE_SPAN * scale_pos)` to cap alpha's
terminal 1/lr_base blow-up on large-N (many-pair) farms, which took
dei_n80_omnidir from a regression (-0.0002%) to roughly flat (+0.0005%)
while leaving farm-balanced at +0.1894%. That farm is still by far the
weakest in the set (dei_n50 +0.0326%, dei_n50_uniform +0.0139%,
dei_n80_omnidir +0.0005%, parque_n10 +0.22%, parque_n20 +0.68%), so it is
the highest-leverage target for further farm-balanced gains. Earlier
attempts pushing OTHER scale_pos-gated levers further (hold_frac, spike0,
relax_p1, beta_p0) made things worse, but the alpha_denom_floor lever
itself has only been tried once, at span=1.5. dei_n80_omnidir has
scale_pos ~= 0.71 (n=80 vs the N_REF=30 anchor), so at span=1.5 its floor
sits at only ~2.06*gamma_min -- still very close to gamma_min, i.e. still
close to native's near-full terminal alpha re-engagement. Raising the span
to 2.2 (~2.56*gamma_min floor for dei_n80) caps the terminal penalty more
aggressively for exactly the many-pair farms where it was over-constraining,
while leaving small/mid-N farms (parque_n10/n20, scale_pos == 0) completely
unaffected, since their floor stays at gamma_min, matching the parent
exactly. All other machinery (lr decay table, warmup, relax/spike/hold
shaping by packing size and absolute-N scale, beta ramp) is unchanged.
"""
...
# Pushed further than the parent (1.5 -> 2.2): dei_n80_omnidir was still
# only marginally improved at 1.5, and it remains the weakest farm.
