...
every size/density-derived signal in this schedule (size, scale_pos,
high_n_ramp) is IDENTICAL for both -- yet dei_n50_uniform is the current
worst-performing farm (+0.0139%) at less than half of dei_n50's score
(+0.0277%). The gated "high-N band" relief (extra hold, delayed penalty
engagement, bounded alpha floor, softened terminal spike) previously only
engaged for scale_pos in [0.5, 0.78], which is effectively just
dei_n80_omnidir (scale_pos~=0.71); dei_n50's band (scale_pos~=0.37) got
zero gated relief. This widens that gate to [0.30, 0.62] so the n~=50 band
now receives a partial (~13%) dose of the same relief already validated
for n~=80 -- while parque_n10/n20 (scale_pos=0, gate=0 either way) are
completely untouched, and dei_n80_omnidir now sits past the new upper
threshold so it gets full (rather than ~84%) gated relief. Every other
...
# Gated relief for farms in the N ~= 50-100+ band (scale_pos in
# [0.30, 0.62]) -- widened from [0.5, 0.78] so dei_n50/dei_n50_uniform
# (scale_pos~=0.37) pick up a partial dose of the same relief already
# validated for dei_n80_omnidir (scale_pos~=0.71, now past the upper
# threshold so it gets full relief instead of ~84%).
_ALPHA_FLOOR_HIGH_N_P0 = 0.30
_ALPHA_FLOOR_HIGH_N_P1 = 0.62
