"""Schedule: single, minimal, well-scoped tweak to the best-so-far variant
(+0.1959% farm-balanced). The two worst-performing farms today are
dei_n50_uniform (scale_pos~=0.37, same size features as dei_n50 but a much
lower score) and dei_n80_omnidir (scale_pos~=0.71). Both sit either just
below or inside the existing `high_n_ramp` gate
(_ALPHA_FLOOR_HIGH_N_P0=0.5 .. _ALPHA_FLOOR_HIGH_N_P1=0.78), which currently
grants extra exploration hold / delayed constraint engagement / a softened,
delayed terminal spike / a raised alpha-denominator floor -- but only once
scale_pos clears 0.5. dei_n50-scale farms (scale_pos~=0.37) get essentially
none of that relief today, even though they are large enough (50 turbines,
many spacing pairs) to plausibly benefit from the same "more exploration
before constraints bite" treatment that helps n80.

This variant lowers the gate's onset (_ALPHA_FLOOR_HIGH_N_P0: 0.5 -> 0.32),
which does two things simultaneously, from the *same* single parameter:
  1. n~=50 farms (scale_pos~=0.37) now receive a small (~3% strength)
     fraction of the high-N relief instead of zero -- a gentle nudge toward
     more exploration time / less aggressive terminal spike, which should
     help exactly the flatter, harder-to-improve landscape that
     dei_n50_uniform represents, without meaningfully disturbing dei_n50
     (already the best-performing DEI farm).
  2. n~=80 farms (scale_pos~=0.71) move further into the ramp (smoothstep
     value rises from ~0.84 to ~0.93 at that scale_pos), giving
     dei_n80_omnidir slightly MORE of the relief it already benefits from.
  parque_n10/parque_n20 have scale_pos clipped to 0 (n well below the
  reference), so they are completely unaffected by this change.

No new mechanisms, no new magic constants beyond the one threshold shift --
every other term is byte-identical to the best-so-far variant.
"""
...
_RELAX_P1_HIGH_N_SPAN = 0.05    # extra gated delay for N~=50-100+ (omnidir-heavy) farms
_SPIKE_BASE, _SPIKE_SPAN = 2.0, 0.55    # spike0 = base + span * size
_SPIKE_SCALE_SPAN = -0.4        # soften terminal spike for large-N farms
_SPIKE_HIGH_N_SPAN = -0.15      # extra gated softening for N~=50-100+ band
...
_HOLD_FRAC_HIGH_N_SPAN = 0.02   # extra gated hold for N~=50-100+ farms
...
# Extra, smoothstep-gated relief for farms in the N ~= 50-100+ band
# (scale_pos in [0.32, 0.78]) -- lowered onset from 0.5 so n~=50 farms (e.g.
# dei_n50_uniform) pick up a small fraction of this relief instead of none,
# while n~=80 farms move further into the ramp and get slightly more.
_ALPHA_FLOOR_HIGH_N_P0 = 0.32
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_HIGH_N_SPAN * high_n_ramp)  # extra gated hold for N~=50-100+ band
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_HIGH_N_SPAN * high_n_ramp)  # extra gated delay for N~=50-100+ band
