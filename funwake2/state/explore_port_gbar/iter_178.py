"""Schedule: extends the best-so-far variant's high-N gated relief pattern
(already proven on dei_n80_omnidir) to a NEW, narrower mid-N band (n~=40-65)
that covers dei_n50 / dei_n50_uniform (scale_pos~=0.37), the two weakest
farms in the current farm-balanced mean. That band currently sits below the
existing high-N gate's onset (scale_pos=0.5) and receives no relief at all.

The new gate is a triangular bump in scale_pos -- rises from 0 at ~n=40,
peaks near n=50-55, and returns to exactly 0 by n=80 -- so it leaves the
existing, already-tuned dei_n80_omnidir behavior untouched, and is 0 at
scale_pos=0 so both parque farms (n=10, n=20) are untouched too. It is only
wired into hold_frac (a bit more exploration time) and relax_p1 (a bit more
delay before constraint engagement ramps up) -- NOT into the terminal alpha
floor or the terminal spike -- to avoid disturbing the feasibility-critical
end-of-run mechanisms on a farm that is currently 3/3 feasible. Every other
mechanism is unchanged from the best-so-far variant.
"""
...
_RELAX_P1_MID_N_SPAN = 0.04     # extra gated delay for N~=40-65 (dei_n50) band
...
_HOLD_FRAC_MID_N_SPAN = 0.02    # extra gated hold for N~=40-65 (dei_n50) band
...
# Mid-N triangular gate: rises over [0.2, 0.4], falls back to 0 over
# [0.45, 0.65] -- covers n~=40-65 (dei_n50 sits at scale_pos~=0.37) while
# returning exactly to 0 by n=80 (scale_pos~=0.71), so it never overlaps the
# high-N gate's operating band and leaves dei_n80_omnidir's tuning intact.
_MID_N_RISE_P0, _MID_N_RISE_P1 = 0.2, 0.4
_MID_N_FALL_P0, _MID_N_FALL_P1 = 0.45, 0.65
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    mid_n_rise = _smoothstep_scalar(scale_pos, _MID_N_RISE_P0, _MID_N_RISE_P1)
    mid_n_fall = _smoothstep_scalar(scale_pos, _MID_N_FALL_P0, _MID_N_FALL_P1)
    mid_n_ramp = max(0.0, mid_n_rise - mid_n_fall)      # triangular bump, n~=40-65
    ...
    hold_frac = (_HOLD_FRAC_BASE + _HOLD_FRAC_SPAN * extremity
                 ...
                 + _HOLD_FRAC_MID_N_SPAN * mid_n_ramp)   # extra gated hold for N~=40-65 band
    ...
    relax_p1 = (_RELAX_P1_BASE + _RELAX_P1_SPAN * extremity
                ...
                + _RELAX_P1_MID_N_SPAN * mid_n_ramp)    # extra gated delay for N~=40-65 band
