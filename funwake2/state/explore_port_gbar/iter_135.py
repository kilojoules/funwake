"""Schedule: same as parent (+0.1917% farm-balanced) but extends the
existing smoothstep-gated high-N relief (already applied to the alpha
floor) to also gate a longer exploration hold and a further-delayed
penalty engagement for the same N~=60-100+ band.

Diagnosis vs. parent: dei_n80_omnidir (n=80, omnidirectional wind, scale_pos
~=0.71) is the worst-performing farm by a wide margin (+0.0044% vs. the next
worst at +0.0139%). The parent already narrowed and boosted the alpha-floor
relief for this band, but hold_frac and relax_p1 -- which control how long
the farm spends in pure exploration before the constraint penalty engages
-- only see a *linear* scale_pos term (_HOLD_FRAC_SCALE_SPAN,
_RELAX_P1_SCALE_SPAN) shared with every large-N farm, not the sharper,
gated boost the alpha floor got. An omnidirectional 80-turbine farm has to
resolve many more competing spacing constraints across all wind sectors
than a unidirectional one of similar size, so it disproportionately
benefits from a longer unconstrained exploration phase. This variant reuses
the SAME high_n_ramp gate (scale_pos in [0.5, 0.78], zero below the gate --
so dei_n50 at 0.37 and parque_n10/20 at 0.0 are untouched) to add a modest
extra hold_frac and relax_p1 boost specifically in that band, on top of the
existing linear terms. Every other mechanism (self-contained inverse-time
product lr decay to gamma_min, short linear warmup, density-extremity
relax/spike/hold, absolute-N scale on hold/spike/relax_p1, the alpha-floor
relief, the hold_frac-floored beta ramp) is unchanged.
"""
