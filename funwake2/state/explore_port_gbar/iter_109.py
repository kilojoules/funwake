"""Schedule: strengthen the high-N-only (scale_pos > 0.5) relief already
present in the parent, so dei_n80_omnidir crosses from slightly-negative to
positive without touching any farm below the gate.

Diagnosis vs. parent: the parent's alpha_denom_floor already carries a
threshold-gated quadratic term (_ALPHA_FLOOR_HIGH_N_SPAN * high_n_gap**2,
gated at scale_pos > 0.5) that decouples dei_n80_omnidir's terminal-alpha
relief from dei_n50's, and it moved dei_n80_omnidir close to zero
(-0.0025%) without regressing anything else -- confirming the gated-
quadratic lever is safe and still under-saturated for this farm
(scale_pos(n=80) ~= 0.71, high_n_gap ~= 0.21). Two changes, both gated the
same way (zero effect below scale_pos = 0.5, so dei_n50/parque_n10/
parque_n20 are bit-for-bit unchanged):

1. Raise _ALPHA_FLOOR_HIGH_N_SPAN 6.0 -> 9.0: pushes the terminal alpha
   denominator floor higher specifically for dei_n80_omnidir, capping its
   late-stage constraint penalty further (same mechanism that already
   worked, just under-scaled).
2. New _SPIKE_HIGH_N_SPAN quadratic term (gated on the same high_n_gap)
   trims the terminal feasibility spike0 for high-N farms only. The spike
   sits on top of an already-floor-capped alpha, so at high N its
   multiplicative effect is proportionally larger than at mid-N -- softening
   it specifically there gives a second, independent lever on the same
   symptom (excess terminal constraint pressure on many-pair layouts)
   without re-coupling to dei_n50's spike0, which stays on the untouched
   linear term.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    spike0 = (_SPIKE_BASE + _SPIKE_SPAN * size
              ...
              - _SPIKE_HIGH_N_SPAN * high_n_gap * high_n_gap)  # gated extra relief, high-N only
