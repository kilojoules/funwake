"""Schedule: phase-couple the Adam-moment ramp to the alpha engagement phase
(single-lever change vs. the parent).

Diagnosis vs. the parent: the parent already computes `relax_p1`, the
progress fraction at which the constraint penalty (`alpha`) finishes ramping
from its relaxed floor to full strength -- shaped per-farm by `extremity`
(density) and `scale_pos` (large-N). But the Adam-moment ramp (`beta_p0`,
governing when beta1/beta2 shift from native-fast (0.1/0.2) toward
more-averaged (0.25/0.45)) was scheduled independently, anchored only to
`scale_pos` via `_BETA_P0_SCALE_SPAN`, with no direct link to the alpha
phase transition itself. The prior-art design menu flags exactly this gap
("phase-transition the Adam moments with the alpha phase") as untried.
Mechanically: once alpha ramps up, the constraint gradient starts dominating
and gets noisier/stiffer (more so on the highly-anisotropic dei farms than
on the small, well-conditioned parque farms) -- added momentum smoothing
timed to coincide with, rather than independently precede/follow, that
alpha ramp should damp the transition better across differently-shaped
farms, since `relax_p1` already carries the farm's density/size shape
information alpha uses.

Concretely: `beta_p0` is now derived from `relax_p1` (minus a small lag, so
momentum starts firming up just before full penalty engages) instead of
from a separate `_BETA_P0 + _BETA_P0_SCALE_SPAN*scale_pos` formula. All
other machinery (lr decay table, warmup, relax/spike penalty shaping, the
alpha-denominator floor for large-N farms) is unchanged from the parent.
"""
...
_BETA_P1 = 0.90
# NEW: beta ramp start is phase-coupled to relax_p1 (the alpha engagement
# point) instead of scheduled independently, so momentum firms up right as
# the constraint penalty turns on rather than on an unrelated timeline.
_BETA_LAG = 0.05
_BETA_P0_MIN, _BETA_P0_MAX = 0.30, _BETA_P1 - 0.05
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # beta ramp starts just before the alpha penalty finishes engaging
    # (relax_p1), so momentum firms up in sync with the farm-shaped
    # constraint-engagement phase rather than on an independent timeline.
    beta_p0 = max(_BETA_P0_MIN, min(_BETA_P0_MAX, relax_p1 - _BETA_LAG))
