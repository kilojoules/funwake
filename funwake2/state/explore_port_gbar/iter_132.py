...
smoothstep-gated high-N alpha-floor relief re-timed to deliver materially
more relief specifically in the N~=80 band, where diagnosis shows it still
under-delivers.

Diagnosis vs. parent: the parent already replaced a hard C0 kink with a
smoothstep ramp from scale_pos=0.5 to 0.85 to fix a "too flat near the gate"
problem for dei_n80_omnidir (scale_pos ~= 0.71). But 0.71 still sits at only
~64% of that ramp (smoothstep((0.71-0.5)/0.35) ~= 0.64), so a meaningful
chunk of the intended relief is still being left on the table for exactly
the farm the parent's own fix targeted. Narrowing the ramp's upper edge from
0.85 to 0.78 moves dei_n80_omnidir to ~84% of the ramp (smoothstep((0.71-
0.5)/0.28) ~= 0.84), delivering ~30% more relief there, while:
  - scale_pos<=0.5 (dei_n50 ~=0.37, parque_n10/20==0) is untouched -- the
    ramp's lower edge and hence the zero-relief guarantee below the gate is
    unchanged;
  - scale_pos>=0.78 still saturates to the same relief ceiling as before
    (was already effectively saturated by 0.85, so nothing changes for
    farms at or beyond that absolute-N scale).
A small increase in the relief span (3.0 -> 3.5) adds a bit more absolute
magnitude on top of the retiming, consistent with the original diagnosis
that both timing AND magnitude were under-delivering at N~=80. Every other
...
# dei_n50's without touching any farm below the gate, and (narrower than the
# parent's 0.5-0.85 ramp) delivers meaningfully more relief right at
# dei_n80_omnidir's scale_pos ~= 0.71 instead of leaving it mid-ramp.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # terms + a smoothstep-gated term ramping over scale_pos in [0.5,0.78])
