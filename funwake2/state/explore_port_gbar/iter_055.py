"""Schedule: raise the Adam beta2 ceiling for large-N farms (isolated
single-lever change on top of the alpha-denominator-floor best).

Diagnosis vs. the current best: dei_n80_omnidir (80 turbines -- the largest
absolute N in the fitness set) sits at only +0.0005%, the bottleneck that
caps the farm-balanced score even though the alpha-denominator floor already
bounds its terminal constraint penalty. With more turbines comes many more
pairwise spacing-constraint terms contributing to grad_constraint, so the
per-step constraint-gradient estimate is noisier (higher variance across
turbine pairs) than on the small/mid farms (parque_n10/n20, dei_n50). The
existing beta2 ramp (0.2 -> 0.45) is shaped only by progress `p`, with
scale_pos only shifting *when* the ramp starts (beta_p0), not how far it
goes. That leaves the late-run second-moment averaging under-powered for
noisy, many-pair gradients on large-N farms specifically, causing Adam's
per-parameter step size to react to noise rather than signal right when the
alpha floor is trying to hold a tight feasibility band -- plausibly what is
eating the AEP gain on dei_n80_omnidir.

This patch adds exactly one new lever: _B2_HI_SCALE_SPAN, which raises the
beta2 ceiling (more second-moment smoothing) for large absolute-N farms only,
via the same scale_pos in [0, 1] already used for the alpha floor and hold
timing. Small/mid-N farms (scale_pos == 0, e.g. parque_n10/n20, dei_n50) get
beta2 ceiling == _B2_HI exactly, matching the current best unchanged. All
other machinery (decay table, warmup, relax/spike penalty shaping, alpha
denominator floor, beta1 and beta_p0 onset) is untouched.
"""
...
# NEW: raise the beta2 ceiling for large-N farms -- many-pair constraint
# gradients are noisier, so more second-moment smoothing is warranted late.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # more smoothing for large N
