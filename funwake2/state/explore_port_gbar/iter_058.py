"""Schedule: raise the terminal beta2 ceiling for large-N farms only (isolated
single-lever change vs. the alpha-floor parent).

Diagnosis vs. the parent: dei_n80_omnidir (n=80, ordinary packing density,
scale_pos ~ 0.7) is the worst-performing farm at +0.0005%, essentially flat,
despite the alpha-floor change that fixed its terminal over-constraint. What
that fix did not address is that an 80-turbine layout has far more pairwise
spacing-constraint terms than the other farms (dei_n50, parque_n10/n20), so
even after mean-normalization (alpha0 = mean|grad J|/D) the per-step
constraint gradient direction is noisier -- many more colliding pairwise
terms partially cancelling/interfering step to step. The parent's beta2
schedule (0.2 -> 0.45) ramps identically regardless of n_turbines, i.e. the
same amount of second-moment averaging is applied whether there are 10 pairs
or 80+. This under-smooths the noisiest, highest-pair-count farm right when
alpha is engaging hardest (late run), yielding jittery updates that erase
gains the way the flat score suggests.

This change adds ONE new lever: _B2_HI_SCALE_SPAN, which raises the late-run
beta2 ceiling (more second-moment averaging / noise damping) proportionally
to scale_pos (the absolute-N factor, already computed and already used
elsewhere for hold/spike/relax/beta_p0). For small/mid-N farms (scale_pos ==
0, e.g. parque_n10/n20, dei_n50*) beta2_hi is exactly unchanged from the
parent (0.45), so those farms are bit-for-bit unaffected. Only large-N farms
(dei_n80_omnidir) get extra late-stage smoothing. beta1's schedule, and all
lr/alpha machinery (decay table, warmup, relax/spike/hold, alpha floor), are
left untouched -- this isolates the one hypothesis under test.
"""
...
# NEW: extra late-stage beta2 (second-moment averaging) for large-N farms,
# whose many pairwise constraint terms make the per-step gradient noisier.
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos          # extra late damping, large-N farms
