"""Schedule: scale up the terminal Adam beta2 (second-moment averaging) for
large-N farms via a scale_pos-gated span on B2_HI (isolated single-lever
change, on top of the accepted alpha-denominator-floor parent).

Diagnosis vs. the parent: the parent already bounds alpha's terminal
divergence for large-N farms via `alpha_denom_floor`, which stopped the
farm-balanced score from being erased by dei_n80_omnidir but only pulled it
to +0.0005% -- essentially flat, still the worst farm by a wide margin.
dei_n80_omnidir has far more turbine pairs than any other farm in the
fitness set (80 vs <=50), so its constraint gradient (summed over all
pairs) is intrinsically noisier/higher-variance than on smaller farms, even
after alpha's magnitude is capped. The parent's beta2 ramp (0.2 -> 0.45,
gated only on progress `p` via `_ramp(p, beta_p0, _BETA_P1)`) does not
scale with this per-pair noise at all -- every farm gets the same terminal
second-moment averaging regardless of how many pairwise constraint terms
are being summed into the gradient. This change adds one new lever,
`_B2_HI_SCALE_SPAN`, that raises the terminal beta2 ceiling for large-N
farms only (scale_pos > 0, i.e. n_turbines above the N_REF=30 anchor),
giving Adam's adaptive denominator more averaging exactly where pairwise
gradient noise is largest, without touching beta1, the alpha shaping, or
the lr/hold/warmup machinery. For small/mid-N farms (scale_pos == 0, e.g.
parque_n10/n20, dei_n50*) beta2's ceiling is unchanged from the parent, so
those farms are unaffected.
...
absolute-N scale on hold/spike/relax_p1/beta_p0/relax0, and the accepted
alpha-denominator floor) is unchanged from the parent.
"""
...
# NEW: extra terminal beta2 (second-moment averaging) ceiling for large-N
# farms, to absorb the higher per-pair constraint-gradient noise that
# grows with the number of turbine pairs (~O(n_turbines^2) terms).
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    b2_hi = _B2_HI + _B2_HI_SCALE_SPAN * scale_pos     # more terminal averaging, large N
