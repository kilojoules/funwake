"""Schedule: symmetric extremity-aware plateau + softened penalty shaping.

Changes vs. the parent (packing-density-adaptive relax/spike/hold, tuned only
by the *sign* of a log-density proxy):

  Both regressed farms (dei_n80_omnidir: dense/large-N, parque_n10_omnidir:
  sparse/small-N) sit at OPPOSITE ends of the packing-density axis, yet BOTH
  score below native while the mid-density farms (dei_n50*, parque_n20) do
  well. That is the signature of an adaptation that is too aggressive (and
  wrongly *asymmetric*) at the extremes, not one that points the wrong way.

  1. hold_frac (exploration-plateau length) now scales with |size| instead of
     signed size -- BOTH extreme-density farms get a longer AEP-only
     exploration plateau before the constraint machinery engages, instead of
     dense farms exploring longer and sparse farms exploring *shorter* (which
     starved parque_n10 of exploration time).

  2. relax/spike sensitivity (_RELAX_SPAN, _SPIKE_SPAN) is softened (0.15->
     0.11, 0.8->0.55) so the penalty shaping doesn't overshoot for farms far
     from the mid-scale density anchor -- reduces the chance that an
     over-large early relax or over-large terminal spike is itself what hurts
     the extreme farms.

  3. the relax ramp's engagement window widens slightly for extreme-density
     farms (larger |size| pushes _RELAX_P1 later), giving them a bit more
     unconstrained exploration before the constraint weight ramps to full
     strength, on top of the longer hold. For |size|~0 (the farms already at
     the optimum) this collapses back to the parent's window, so mid-density
     farms are unaffected.

  All other machinery (self-contained inverse-time product lr decay to
  gamma_min, short linear warmup, beta1/beta2 phase-transition) is unchanged.
"""
