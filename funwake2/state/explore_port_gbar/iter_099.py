"""Schedule: widen the terminal alpha denominator floor (raise its scale_pos
coefficient and add a small extremity term) so crowded, many-pair farms get
more terminal-alpha relief without touching small/mid farms.

Diagnosis vs. the current best: alpha = alpha0*diam*relax*spike / alpha_denom,
with alpha_denom = max(lr_base, gamma_min*(1 + ALPHA_FLOOR_SCALE_SPAN*scale_pos)).
This floor is what let dei_n80_omnidir stop regressing relative to the parent,
but that farm (80 turbines, by far the most turbine-pairs in the fitness set)
is still nearly flat (+0.0005%) versus the other farms' +0.01% to +0.68%,
meaning its terminal alpha is still climbing close to native's diverging
1/lr_base behavior late in the run. The existing floor is linear in
scale_pos = clip(log(n/30)/log(4), 0, 1) alone; dei_n80_omnidir sits at
scale_pos ~= 0.71 (not the extreme 1.0), so it only gets partial relief from
the tuned coefficient. Rather than reshaping the many already-tuned
extremity-gated levers (hold_frac, spike0, relax_p1, beta_p0 -- earlier
attempts pushing those further made the farm-balanced score worse), this
change touches ONLY the one lever that produced the last confirmed
improvement: it raises ALPHA_FLOOR_SCALE_SPAN (1.5 -> 2.0) so alpha's floor
sits higher (alpha capped lower) for any farm with scale_pos > 0, giving
dei_n80_omnidir specifically more terminal relief, and adds a small new
*additive* extremity term (0.3 * extremity) so farms with unusually sparse
OR unusually dense packing (regardless of absolute N) also get a modest
floor lift instead of being gated on absolute N alone. Both terms are 0 for
scale_pos == 0 and extremity == 0 farms will still see a nonzero extremity
term if their packing density departs from the reference, but the
coefficient (0.3) is deliberately small relative to the scale_pos term (2.0)
so already-good farms (e.g. parque_n10/n20, dei_n50) are only lightly
perturbed, not restructured. All other machinery (self-contained inverse-
time product lr decay to gamma_min, short linear warmup, density-extremity
relax/spike/hold, absolute-N scale on hold/spike/relax_p1/beta/relax0) is
...
# layouts. Widened vs. the prior best: larger scale_pos coefficient (more
# relief for large-N farms like dei_n80_omnidir) plus a small extremity term
# (relief for farms whose packing density departs from the reference even at
# moderate N).
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # Cap the effective denominator at gamma_min*(1 + scale term + extremity
    # term) instead of letting it ride lr_base all the way to gamma_min, so
    # alpha's terminal divergence is bounded more for large-N (many-pair)
    # farms, and lightly for unusually sparse/dense farms too.
