...
# is changed from a cosine SGDR oscillation to a WSD FLAT-TOP HOLD with narrow
# SQUARE-ISH REPAIR NOTCHES — the prior-art menu's one untried lr hypothesis
# (§6/§2: "hold near c*D, then (near-)linear cool-down beats cosine/product
# decay") fused with the proven restoration-burst feasibility machinery.
#
# The key structural change is a DUTY-CYCLE INVERSION. The parent's cosine
# restarts spend ~half of each cycle below mid-lr, so much of the exploration
# budget is neither hot enough to hop basins nor cold enough to repair. Here
# the lr sits pinned at the full hot envelope for ~80% of each cycle (a
# sustained hold far hotter cumulatively than any momentary 1.65*D peak
# tried), and drops into a brief, DEEP notch (0.35*D, well below the parent's
# 0.65*D trough) for the remaining ~20%, where a synchronized alpha burst
# spends the small steps purely on constraint repair. More time at full heat,
# and colder, more surgical repair windows — same debt-repayment cadence.
#
#   lr    — 3% linear warmup (proven) -> flat-top hold with envelope decaying
#           1.5*D -> 1.1*D across three narrow sin^12 notches down to 0.35*D
#           -> exploration ends at the proven 62% -> proven straight linear
#           tail landing exactly on gamma_min at the last step.
#   alpha — proven architecture kept intact: 0.4*alpha0 exploration floor,
#           growing restoration bursts (3 -> 8 alpha0) synchronized with the
#           lr notches, logistic ramp to the bounded 6*alpha0 ALM plateau at
#           66%, and the 5/5-seed-feasible cubic-delayed geometric climb from
#           78% to the terminal 5*alpha0*D/gamma_min spike.
#   betas — proven transitions: beta2 0.2 -> 0.9 at cool-down; beta1 0.1 with
#           a dip to 0.05 inside each repair notch (momentum must not carry
...
_HI0 = 1.5            # initial hold level, in units of D — sustained, not peak
