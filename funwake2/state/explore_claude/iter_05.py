...
_CONST_FRAC = 0.44     # hold the peak lr slightly longer (parent held 0.40 of steps)
_SPIKE_START = 0.93    # terminal feasibility spike starts a bit earlier (parent: 0.94)
_SPIKE_WIDTH = 0.015
_SPIKE_GAIN = 4.0      # stronger terminal alpha ramp (parent: 3.0) to cover the hotter exploration
_BISECT_LOWER = 0.0
_BISECT_UPPER = 0.1
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    # the parent's so the hotter, longer exploration phase still finishes
    # strictly feasible.
    frac = step / total
