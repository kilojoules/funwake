...
_CONST_FRAC = 0.45     # hold the peak lr longer still (parent held 0.40; grandparent 1/3)
_SPIKE_START = 0.93    # terminal feasibility spike over the last ~7% of steps (parent: 0.94)
_SPIKE_WIDTH = 0.015
_SPIKE_GAIN = 4.0      # alpha multiplier ramps smoothly to ~(1 + gain) at the end (parent: 3.0)
_BISECT_LOWER = 0.0
_BISECT_UPPER = 0.1
...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr = jnp.maximum(lr, float(gamma_min))             # never undershoot the tolerance

    # native coupling recovered via /D: alpha0 = mean|grad J|/D  ==>
    #   alpha = alpha0 * D / lr = mean|grad J| / lr
    # During the (longer, higher) peak hold this is a bounded alpha plateau;
    # it only grows as lr decays, i.e. the ramp is effectively delayed.
    ...
    # gamma_min — negligible AEP cost. Started earlier and made stronger than
    # the parent's to cover the more aggressive exploration phase, so the
    # strict-feasibility finish is preserved.
    frac = step / total
