...
already gates extra hold/relax-delay/spike-softening/alpha-floor relief onto
the N~=60-100+ band via `high_n_ramp` (scale_pos in [0.5, 0.78]).

Diagnosis: the mean is currently limited as much by the N=50 band (dei_n50 at
+0.0277%, dei_n50_uniform at only +0.0139% -- the worst farm) as by n80. Both
n50 variants share IDENTICAL n_turbines/D/min_spacing, so `size`, `extremity`
and `scale_pos` are bit-for-bit identical between them -- the schedule cannot
tell them apart, but at scale_pos~=0.368 they sit just *below* the existing
high-N gate's onset (P0=0.5), so the n=50 band currently gets ZERO of the
hold/delay/spike/floor relief that helped n80. This adds a SEPARATE, narrow
"mid-N" bump -- a smooth parabola in scale_pos, zero at parque's low N and
zero again by the time the existing high-N gate takes over at n80 -- so it
only touches the N~=35-55 band. It reuses the same qualitatively-validated
mechanisms (extra hold, delayed relax engagement, softened terminal spike,
higher alpha floor) at smaller, independent magnitudes, without altering the
already-tuned parque or n80 behavior at all.
"""
