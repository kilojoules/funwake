...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_scale = (209.0 / 240.0) * D
    lr_peak = 1.12 * lr_scale

    warm = jnp.clip(t / 0.085, 0.0, 1.0)
    warm_s = warm * warm * (3.0 - 2.0 * warm)

    shoulder = jnp.clip((t - 0.16) / 0.18, 0.0, 1.0)
    shoulder_s = shoulder * shoulder * (3.0 - 2.0 * shoulder)

    cool = jnp.clip((t - 0.43) / 0.49, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = lr_peak * (0.69 + 0.31 * warm_s) * (1.0 + 0.045 * shoulder_s) * (1.0 - 0.957 * cool_s)
    lr_body = jnp.maximum(lr_body, 2.65 * gamma_min)
    ...
    beta1 = 0.205 - 0.118 * delay_s - 0.045 * terminal_s
