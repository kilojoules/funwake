...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    warm = jnp.clip(t / 0.11, 0.0, 1.0)
    warm_s = warm * warm * (3.0 - 2.0 * warm)

    cool = jnp.clip((t - 0.485) / 0.435, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = lr_peak * (0.665 + 0.335 * warm_s) * (1.0 - 0.948 * cool_s)
    lr_body = jnp.maximum(lr_body, 2.80 * gamma_min)
