...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    cool = jnp.clip((t - 0.435) / 0.485, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    cruise = 1.0 + 0.035 * warm_s * (1.0 - cool_s)
    lr_body = lr_peak * (0.68 + 0.32 * warm_s) * cruise * (1.0 - 0.955 * cool_s)
    ...
    alpha_late = alpha0 * D / jnp.maximum(2.30 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.05 + 2.85 * delay_s)
    alpha_main = jnp.minimum(alpha_base, alpha_plateau)

    alpha_spike = alpha_late * (1.0 + 1.30 * terminal_s)
    alpha = (1.0 - terminal_s) * alpha_main + terminal_s * alpha_spike

    beta1 = 0.21 - 0.12 * delay_s - 0.045 * terminal_s
    beta2 = 0.145 + 0.20 * delay_s + 0.115 * terminal_s
