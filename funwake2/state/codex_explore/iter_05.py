...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_peak = 1.10 * lr_scale

    warm = jnp.clip(t / 0.09, 0.0, 1.0)
    warm_s = warm * warm * (3.0 - 2.0 * warm)

    cool = jnp.clip((t - 0.40) / 0.52, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = lr_peak * (0.70 + 0.30 * warm_s) * (1.0 - 0.955 * cool_s)
    ...
    alpha_plateau = alpha0 * D / jnp.maximum(0.40 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(2.35 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.08 + 2.95 * delay_s)
    ...
    beta1 = 0.20 - 0.11 * delay_s - 0.045 * terminal_s
    beta2 = 0.15 + 0.19 * delay_s + 0.11 * terminal_s
