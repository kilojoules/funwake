...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    warm = jnp.clip(t / 0.080, 0.0, 1.0)
    warm_s = warm * warm * (3.0 - 2.0 * warm)

    cool = jnp.clip((t - 0.535) / 0.385, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = lr_peak * (0.650 + 0.350 * warm_s) * (1.0 - 0.959 * cool_s)
    lr_body = jnp.maximum(lr_body, 3.08 * gamma_min)
    ...
    delay = jnp.clip((t - 0.465) / 0.275, 0.0, 1.0)
    delay_s = delay * delay * (3.0 - 2.0 * delay)

    alpha_plateau = alpha0 * D / jnp.maximum(0.358 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(1.98 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.025 + 3.14 * delay_s)
    ...
    beta1 = 0.226 - 0.132 * delay_s - 0.060 * terminal_s
