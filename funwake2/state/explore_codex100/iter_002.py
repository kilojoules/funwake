...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_scale = (216.0 / 240.0) * D
    lr_peak = 1.17 * lr_scale

    warm = jnp.clip(t / 0.115, 0.0, 1.0)
    warm_s = warm * warm * (3.0 - 2.0 * warm)

    shelf = jnp.clip((t - 0.18) / 0.24, 0.0, 1.0)
    shelf_s = shelf * shelf * (3.0 - 2.0 * shelf)

    cool = jnp.clip((t - 0.485) / 0.43, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = lr_peak * (0.66 + 0.34 * warm_s) * (1.0 + 0.035 * (1.0 - shelf_s)) * (1.0 - 0.947 * cool_s)
    ...
    delay = jnp.clip((t - 0.45) / 0.29, 0.0, 1.0)
    ...
    alpha_base = alpha0 * (1.04 + 3.22 * delay_s)
    alpha_main = jnp.minimum(alpha_base, alpha_plateau)

    alpha_spike = alpha_late * (1.0 + 1.62 * terminal_s)
    alpha = (1.0 - terminal_s) * alpha_main + terminal_s * alpha_spike

    beta1 = 0.215 - 0.124 * delay_s - 0.055 * terminal_s
    beta2 = 0.135 + 0.218 * delay_s + 0.122 * terminal_s
