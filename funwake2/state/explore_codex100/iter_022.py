...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    shoulder = jnp.clip((t - 0.16) / 0.25, 0.0, 1.0)
    shoulder_s = shoulder * shoulder * (3.0 - 2.0 * shoulder)

    cool = jnp.clip((t - 0.49) / 0.425, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_hold = lr_peak * (0.665 + 0.335 * warm_s) * (1.0 + 0.035 * (1.0 - shoulder_s))
    lr_body = lr_hold * (1.0 - 0.954 * cool_s)
    ...
    delay = jnp.clip((t - 0.455) / 0.285, 0.0, 1.0)
    ...
    alpha_base = alpha0 * (1.03 + 3.18 * delay_s)
    ...
    beta1 = 0.215 - 0.123 * delay_s - 0.056 * terminal_s
