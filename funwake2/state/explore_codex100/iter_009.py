...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_peak = 1.228 * lr_scale

    warm = jnp.clip(t / 0.105, 0.0, 1.0)
    warm_s = warm * warm * (3.0 - 2.0 * warm)

    shoulder = jnp.clip((t - 0.190) / 0.305, 0.0, 1.0)
    shoulder_s = shoulder * shoulder * (3.0 - 2.0 * shoulder)

    cool = jnp.clip((t - 0.535) / 0.385, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = (
        lr_peak
        * (0.655 + 0.345 * warm_s)
        * (1.0 + 0.054 * shoulder_s)
        * (1.0 - 0.960 * cool_s)
    )
    lr_body = jnp.maximum(lr_body, 3.18 * gamma_min)

    terminal = jnp.clip((t - 0.920) / 0.080, 0.0, 1.0)
    terminal_s = terminal * terminal * (3.0 - 2.0 * terminal)
    lr = (1.0 - terminal_s) * lr_body + terminal_s * gamma_min
    lr = jnp.maximum(lr, gamma_min)

    delay = jnp.clip((t - 0.475) / 0.270, 0.0, 1.0)
    delay_s = delay * delay * (3.0 - 2.0 * delay)

    alpha_plateau = alpha0 * D / jnp.maximum(0.372 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(1.96 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.025 + 3.22 * delay_s)
    alpha_main = jnp.minimum(alpha_base, alpha_plateau)

    alpha_spike = alpha_late * (1.0 + 1.78 * terminal_s)
    alpha = (1.0 - terminal_s) * alpha_main + terminal_s * alpha_spike

    beta1 = 0.222 - 0.130 * delay_s - 0.058 * terminal_s
    beta2 = 0.132 + 0.232 * delay_s + 0.132 * terminal_s
