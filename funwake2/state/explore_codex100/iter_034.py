...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_body = lr_peak * (0.665 + 0.335 * warm_s) * (1.0 - 0.949 * cool_s)
    lr_body = jnp.maximum(lr_body, 2.92 * gamma_min)
    ...
    delay = jnp.clip((t - 0.445) / 0.292, 0.0, 1.0)
    delay_s = delay * delay * (3.0 - 2.0 * delay)

    alpha_plateau = alpha0 * D / jnp.maximum(0.372 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(2.10 * gamma_min, 1e-30)
    ...
    beta1 = 0.212 - 0.122 * delay_s - 0.055 * terminal_s
    beta2 = 0.138 + 0.218 * delay_s + 0.120 * terminal_s
