...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    cool = jnp.clip((t - 0.515) / 0.400, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    shoulder = jnp.clip((t - 0.215) / 0.260, 0.0, 1.0)
    shoulder_s = shoulder * shoulder * (3.0 - 2.0 * shoulder)

    lr_body = lr_peak * (0.650 + 0.350 * warm_s) * (1.0 + 0.045 * shoulder_s) * (1.0 - 0.958 * cool_s)
    ...
    beta1 = 0.218 - 0.128 * delay_s - 0.056 * terminal_s
    beta2 = 0.135 + 0.225 * delay_s + 0.125 * terminal_s
