...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_peak = 1.165 * lr_scale

    warm = jnp.clip(t / 0.105, 0.0, 1.0)
    warm_s = warm * warm * (3.0 - 2.0 * warm)

    cool = jnp.clip((t - 0.475) / 0.445, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = lr_peak * (0.70 + 0.30 * warm_s) * (1.0 - 0.950 * cool_s)
    ...
    delay = jnp.clip((t - 0.445) / 0.290, 0.0, 1.0)
    delay_s = delay * delay * (3.0 - 2.0 * delay)

    alpha_plateau = alpha0 * D / jnp.maximum(0.390 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(2.16 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.05 + 3.16 * delay_s)
    ...
    beta1 = 0.212 - 0.120 * delay_s - 0.055 * terminal_s
    beta2 = 0.140 + 0.212 * delay_s + 0.120 * terminal_s
