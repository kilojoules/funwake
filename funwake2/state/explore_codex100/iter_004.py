...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_body = lr_peak * (0.67 + 0.33 * warm_s) * (1.0 - 0.953 * cool_s)
    ...
    delay = jnp.clip((t - 0.44) / 0.31, 0.0, 1.0)
    delay_s = delay * delay * (3.0 - 2.0 * delay)

    alpha_plateau = alpha0 * D / jnp.maximum(0.38 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(2.12 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.045 + 3.20 * delay_s)
    ...
    beta1 = 0.202 - 0.112 * delay_s - 0.052 * terminal_s
    beta2 = 0.146 + 0.212 * delay_s + 0.118 * terminal_s
