...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_peak = 1.14 * lr_scale

    warm = jnp.clip(t / 0.10, 0.0, 1.0)
    warm_s = warm * warm * (3.0 - 2.0 * warm)

    cool = jnp.clip((t - 0.455) / 0.465, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = lr_peak * (0.68 + 0.32 * warm_s) * (1.0 - 0.952 * cool_s)
    ...
    delay = jnp.clip((t - 0.43) / 0.305, 0.0, 1.0)
    delay_s = delay * delay * (3.0 - 2.0 * delay)

    alpha_plateau = alpha0 * D / jnp.maximum(0.385 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(2.22 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.06 + 3.10 * delay_s)
    alpha_main = jnp.minimum(alpha_base, alpha_plateau)

    alpha_spike = alpha_late * (1.0 + 1.55 * terminal_s)
    alpha = (1.0 - terminal_s) * alpha_main + terminal_s * alpha_spike

    beta1 = 0.205 - 0.115 * delay_s - 0.052 * terminal_s
