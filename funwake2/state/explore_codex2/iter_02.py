...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    sustain = 1.0 + 0.045 * jnp.exp(-0.5 * ((t - 0.25) / 0.16) ** 2)

    cool = jnp.clip((t - 0.44) / 0.48, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = lr_peak * (0.68 + 0.32 * warm_s) * sustain * (1.0 - 0.958 * cool_s)
    ...
    alpha_plateau = alpha0 * D / jnp.maximum(0.395 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(2.25 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.06 + 3.05 * delay_s)
    alpha_main = jnp.minimum(alpha_base, alpha_plateau)

    alpha_spike = alpha_late * (1.0 + 1.45 * terminal_s)
