...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_peak = 1.16 * lr_scale

    warm = jnp.clip(t / 0.08, 0.0, 1.0)
    warm_s = warm * warm * (3.0 - 2.0 * warm)

    cool = jnp.clip((t - 0.44) / 0.48, 0.0, 1.0)
    cool_s = cool * cool * (3.0 - 2.0 * cool)

    lr_body = lr_peak * (0.69 + 0.31 * warm_s) * (1.0 - 0.958 * cool_s)
    lr_body = jnp.maximum(lr_body, 2.8 * gamma_min)
    ...
    alpha_plateau = alpha0 * D / jnp.maximum(0.39 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(2.25 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.06 + 3.02 * delay_s)
    ...
    beta1 = 0.205 - 0.115 * delay_s - 0.050 * terminal_s
