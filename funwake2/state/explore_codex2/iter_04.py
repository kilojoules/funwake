...
def schedule_fn(step, total_steps, D, min_spacing, n_turbines, gamma_min, alpha0):
    ...
    lr_body = lr_peak * (0.68 + 0.32 * warm_s) * (1.0 - 0.957 * cool_s)
    ...
    delay = jnp.clip((t - 0.425) / 0.315, 0.0, 1.0)
    delay_s = delay * delay * (3.0 - 2.0 * delay)

    alpha_plateau = alpha0 * D / jnp.maximum(0.395 * lr_scale, 1e-30)
    alpha_late = alpha0 * D / jnp.maximum(2.20 * gamma_min, 1e-30)

    alpha_base = alpha0 * (1.04 + 3.02 * delay_s)
    ...
    beta1 = 0.215 - 0.125 * delay_s - 0.050 * terminal_s
    beta2 = 0.145 + 0.195 * delay_s + 0.120 * terminal_s
