...
the best of a converged multistart, a discovered schedule beats native by $+0.074\%$
...
interface, a hard feasibility gate, held-out selection, an information firewall, and
...
balances both across farms of different rotor diameter. Feasibility itself
...
The value of the search is less the marginal AEP than the \emph{diversity of deployable
schedule shapes} it surfaces. \Cref{fig:schedules} plots four schedules evaluated at the
held-out ROWP scales: the native reference (constant learning rate then a
compounding-product decay, penalty coupled as $\alpha\propto1/\eta$, fixed TopFarm
moment coefficients $\beta_1{=}0.1,\beta_2{=}0.2$); a portfolio-discovered schedule
(it21) that ramps quickly to the exploration scale, holds, then decays smoothly while
\emph{raising} the adaptive coefficient $\beta_2$ from $0.2$ to $0.77$ and easing the
momentum $\beta_1$ down into the tail; and two seeded ancestors that mark the extremes
the interface admits --- a dual-bump learning rate (it192) and a cyclic warm-restart
schedule (it118) that repeatedly re-heats the learning rate and both moment
coefficients. All four decay the learning rate toward the tolerance $\gm$ and restore
the penalty near the end to drive the final iterate onto the feasible set, but they
reach it by very different routes. That distinct, hand-inscrutable shapes all attain
comparable feasible AEP is itself a finding: the objective is forgiving in schedule
space once the scale-aware exploration and the terminal feasibility restoration are
right.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig_search.pdf}
\caption{Schedule search on the training farm for two mutation engines (Claude, left;
Antigravity/Gemini~3, right). Each point is a candidate schedule proposed by the LLM
and scored by the fixed skeleton; filled points are feasible on every seed, hollow
points are infeasible; the solid line is the best feasible fitness so far; the dashed
line is the native reference ($0\%$); the star marks the best (deployed) schedule. The
best-so-far improves only $\approx\!0.05$--$0.09\%$ over the native reference, and the
infeasible $+0.7\%$ outliers (in the Claude panel) are non-deployable layouts discarded
...
beats the native best-of-$K$ by $+3.2$\,GWh --- $+0.074\%$ of the native best, with a
...
bootstrap band; at the calibrated budget $K=1024$ it is $+3.2$\,GWh ($+0.074\%$) and the
...
executed here; we report the held-out selection farm, and treat the pre-registered test
...
generalization figures in particular are four-seed estimates and individually noisy.
...
converged multistart --- where a discovered schedule beats native by $+0.074\%$ on a
