...
the schedules the search discovers (Section~\ref{sec:discovered}), and the
reproducibility and cost-control machinery (Section~\ref{sec:repro}).
...
Each point is one candidate the LLM proposed and ran through the fixed skeleton; filled
points are feasible on every seed, hollow points are infeasible, and the solid line is
the best feasible fitness so far. Most candidates land within a few hundredths of a
percent of the native reference, the best-so-far improves by only
$\approx\!0.05$--$0.09\%$ over hundreds of attempts, and the infeasible outliers near
$+0.7\%$ are exactly the non-deployable layouts the feasibility gate
(\cref{sec:fitness}) discards --- a reminder that raw AEP off the feasible set is not a
fitness signal. The picture is a faithful near-null: the interface admits broad
...
infeasible $+0.7\%$ outliers are non-deployable layouts discarded by the feasibility
gate.}
\label{fig:search}
\end{figure}
