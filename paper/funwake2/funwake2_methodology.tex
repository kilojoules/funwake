...
\begin{abstract}
\noindent
We present a framework for the automated discovery of \emph{deployment-candidate}
learning-rate and constraint-penalty schedules for gradient-based wind-farm layout
optimization using large language models (LLMs). Candidate schedules are expressed in a
...
$\approx\!0.05$--$0.09\%$ on the training cells. At the deployment-relevant metric, the
best of a converged multistart, a discovered schedule beats native by a comparable
$+0.075\%$ on one held-out farm firewalled from the search, with a bootstrap $95\%$
interval that excludes zero \emph{conditional on the selected schedule and that farm}. The
value added is the statistical robustness of the best-of-$K$ gain on that held-out farm,
not a larger or uniform effect: the gain is small and farm-specific, is computed on our
reimplemented skeleton, and rests on a schedule selected as a search argmax, so we report
it as a deployment \emph{candidate} and define a pre-registered test to confirm or refute
it.
\end{abstract}
...
stochastic gradient descent (SGD) optimizer \citep{quick2022sgd} adapts the Adam
...
paradigm for optimizer-schedule design, and add three ingredients aimed at making the
discovered artifact a \emph{deployment candidate} rather than merely high-scoring on one farm:
...
(Sections~\ref{sec:provenance},~\ref{sec:firewall},~\ref{sec:repro}) --- full lineage, a behavioral
...
TopFarm SGD driver \citep{quick2022sgd} and inclusion/exclusion-zone handling
...
class of solver with an LLM and to evaluate the discovery as a deployment candidate across
...
engine for testing. The two searches shown below (\cref{fig:search}) use the Claude and Gemini engines; the
deployed schedule comes from a separate codex-engine portfolio run, not plotted
(\cref{sec:discovered}). We adopt two eval-saving mechanisms from ShinkaEvolve
...
learning rate, $\eta_{\text{peak}}=c_0 D$ with $c_0=0.8333$ ($=200/240$) calibrated once
...
elites are archived and every candidate is logged with full provenance. Training
selection, the held-out out-of-sample check, the pre-registered paired test, and
stochastic re-validation sit outside the loop.}
\label{fig:loop}
\end{figure}
...
($D=198$\,m, single polygon), held out from the search as an out-of-sample check; and the Parque Ficticio
...
$D{=}240$\,m) and ROWP (held out as an out-of-sample check, $D{=}198$\,m) on a shared scale, and the
...
across turbine counts and roses, selects on the training farms and holds ROWP out as an out-of-sample check, and reserves a
...
$420\pm214$ at $M=1024$ against $\approx\!1070$ at the deep pool --- so it is the deep
...
\Cref{fig:search} shows a schedule search on a single training cell (DEI-50) for two mutation engines
...
two searches shown are single-cell (DEI-50) engine runs that illustrate the search dynamics and the
diversity of shapes; the deployed schedule it21 comes from a separate farm-balanced
portfolio search of $\sim\!120$ attempts over a five-cell subset of the training suite
(the DEI single-polygon rose cell (DEI-50), the DEI omnidirectional and unidirectional
cells, and the two Parque cells --- five cells; this run does not cover the full training
suite of \cref{tab:cells}, in particular the high-count DEI-120 and the feasibility-only
Parque cells) --- the elite that maximizes the farm-balanced training fitness of
\cref{eq:agg} (\cref{sec:selection}). The
...
\caption{Schedule search on a single training cell (DEI-50) for two of the mutation engines
...
these panels illustrate search dynamics, not the deployment decision. These single-cell
scores are normalized to their own DEI-50 reference and are not directly comparable
cell-for-cell to the quad-norm portfolio numbers of \cref{tab:cells,tab:generalization};
because it21 maximizes farm-balanced fitness rather than DEI-50 in particular, its DEI-50
entry ($+0.015\%$, \cref{tab:generalization}) is naturally below these single-cell bests.
The best-so-far
...
the portfolio-discovered schedule it21, $99.7\%$ feasible). The median advantage is
positive across the whole budget range; at $K=1024$ its best-of-$K$ layout beats the
native best-of-$K$ by $+3.2$\,GWh --- $+0.075\%$ of the native best, with a bootstrap
$95\%$ confidence interval of $[+1.3,+4.9]$\,GWh that excludes zero (the discovered
schedule wins in every one of $4000$ bootstrap replicates; at small $K$ the interval is
wide and includes zero). This interval is \emph{conditional}: it quantifies best-of-$K$
sampling variability within the two ROWP pools for the \emph{already-selected} it21, and
does \emph{not} propagate the $\sim\!120$-attempt selection or cross-farm variability
(\cref{sec:limitations}). The advantage is a distributional shift, not a lucky tail: the
discovered schedule's ROWP pool mean exceeds native's by $+0.065\%$ and its pool maximum
by $+2.95$\,GWh. Because ROWP was firewalled
...
ROWP best-of-$K$ result above as the one deeply-sampled signal (deep sampling buys
statistical cleanliness on that farm, not generalizability across farms). The search itself is near-null in
mean terms (\cref{fig:search}); the deployable value comes from selecting the best of a
converged multistart, where the discovered schedule's small but consistent distributional
edge yields a robust best-of-$K$ gain.

\begin{table}[t]
\centering
\small
\caption{Generalization of the discovered schedule across the five scored portfolio cells
plus the held-out ROWP cell (percentage change in mean AEP over the native reference, four
paired seeds per cell, both schedules feasible on every seed; the high-count DEI-120
training cell of \cref{tab:cells} was not in this portfolio run and is omitted here).
Gains are small and positive on the offshore
single-polygon cells; the two multi-zone Parque cells are high-variance and mixed (a loss
on Parque-20, the largest gain in the set on Parque-10). Per-cell
...
percentile band); the discovered schedule's median sits above the native reference across
budgets. \emph{Right:} the best-of-$K$ advantage (discovered $-$ native) with a $95\%$
bootstrap band; the median advantage is positive throughout, but the band excludes zero
only near the calibrated budget $K=1024$, where the advantage is $+3.2$\,GWh
($+0.075\%$). The interval reflects best-of-$K$ sampling variability only, conditional on
the selected schedule and the ROWP pools.}
...
\bibitem[Quick et al.(2022)]{quick2022sgd}
