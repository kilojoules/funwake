...
$\approx\!0.05$--$0.09\%$ on the training farm --- but at the deployment-relevant metric,
...
($K^\star_5\approx730$, from the same shuffle-saturation analysis), and covers the
held-out first percentile at the $0.1\%$
...
$M\!\approx\!4000$ runs (DEI budgets resolved on a $2048$-run sub-pool, ROWP on the full
$4096$-run pool); ``spread'' is the relative AEP range $(\max-\min)/\max$ over the pool;
...
fitness signal (the Antigravity engine's largest infeasible score is only $+0.05\%$). The picture is a faithful near-null: the interface admits broad
exploration, but the scale-aware native reference is already close to the best a
schedule achieves on this farm.
...
matrix: the DEI and ROWP single-polygon cells improve by small, consistent margins (about
$+0.02$ to $+0.05\%$), while the two multi-zone Parque cells are high-variance and
\emph{mixed} --- a $-0.10\%$ loss on the $20$-turbine cell and a $+0.35\%$ gain on the
...
effect needs of order $25$ seeds to resolve at $80\%$ power. We therefore read the table
...
schedule is the argmax of a finite ($\sim\!10^2$-attempt) search; the held-out evaluation
